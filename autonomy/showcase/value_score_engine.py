...
def audit_log(event, details=None):
    AUDIT_LOG_PATH = os.path.join(os.path.dirname(__file__), 'value_score_audit_log.json')
    log_entry = {
...
def compute_value_score(metadata: dict, outline: list, owner_override: int = None) -> int:
    """
    ...
    if owner_override is not None:
        ...
        score += 10
        score = min(score, 100)
        audit_log('COMPUTE_VALUE_SCORE', {'metadata': metadata, 'outline_len': len(outline), 'score': score})
...
def save_value_score(vault_path: str, value_score: int):
    ...
    audit_log('SAVE_VALUE_SCORE', {'vault_path': vault_path, 'value_score': value_score})
