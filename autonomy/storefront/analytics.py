...
def storefront_analytics(data):
    ...
    funnel = {
        ...
        'click_through_rate': (data.get('clicks', 0) / data.get('page_views', 1)) * 100 if data.get('page_views', 0) else 0,
        'conversion_rate': (data.get('conversions', 0) / data.get('clicks', 1)) * 100 if data.get('clicks', 0) else 0
    }
    return funnel
