...
PRICE_LOG = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../analytics/pricing_optimizer_enhanced_log.jsonl'))
...
def suggest_price(vault_id, sales, refunds, competitors, min_price=5, max_price=99):
    ...
    avg_competitor = sum(competitor_prices)/len(competitor_prices) if competitor_prices else min_price
