...
RISK_LOG = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../analytics/refund_risk_early_warning_log.jsonl'))
...
def flag_refund_risk(vault_id, sales, refunds):
    ...
    risk = 'high' if rate > 0.25 and refund_count >= 3 else 'medium' if rate > 0.1 else 'low'
    entry = {
