...
def buyer_journey_funnel(data):
    ...
    funnel = {
        ...
        'view_to_cart_rate': (data.get('add_to_cart', 0) / data.get('views', 1)) * 100 if data.get('views', 0) else 0,
        'cart_to_purchase_rate': (data.get('purchases', 0) / data.get('add_to_cart', 1)) * 100 if data.get('add_to_cart', 0) else 0
    }
    return funnel
