...
def log_ai_output(entry):
    ...
    c.execute('''INSERT INTO ai_logs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
        entry['timestamp'], entry['vault_id'], entry['niche'], entry['ai_output'], entry['ai_version'], entry['latency'], int(entry['success']), entry['language'], entry['customer_id'], int(entry['preview_required']), json.dumps(entry['spell_errors'])
    ))
    conn.commit()
    conn.close()
...
def preview_required_entries():
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT timestamp, vault_id, ai_output, spell_errors FROM ai_logs WHERE preview_required=1 ORDER BY timestamp DESC")
    rows = c.fetchall()
    conn.close()
    for row in rows:
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == 'tail':
            ...
            if not anomalies:
                ...
                for a in anomalies:
                    print(f"[ANOMALY] idx={a['idx']} sim={a['similarity']:.2f} output={a['output'][:60]}...")
