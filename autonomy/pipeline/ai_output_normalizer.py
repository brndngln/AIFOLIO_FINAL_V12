...
NORMALIZER_LOG = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../analytics/ai_output_normalizer_log.jsonl'))
os.makedirs(os.path.dirname(NORMALIZER_LOG), exist_ok=True)

BRAND_GUIDELINES = [
    'AIFOLIO', 'reliable', 'trusted', 'compliant', 'ethical', 'professional', 'non-sentient', 'human preview'
]
LEGAL_SAFE_PHRASES = [
    'for informational purposes', 'not legal advice', 'compliant with applicable law', 'no personal data'
]
TONE_WHITELIST = ['professional', 'friendly', 'neutral', 'authoritative']

# --- Output Normalizer ---
def normalize_output(text, brand_terms=BRAND_GUIDELINES, legal_phrases=LEGAL_SAFE_PHRASES, tone='professional', reading_level=10):
    # Check tone
    ...
    # Compose result
    flags = []
    if tone_flag: flags.append('tone')
    if reading_flag: flags.append('reading_level')
    if brand_flag: flags.append('brand_guidelines')
    if legal_flag: flags.append('legal_safe')
    if spell_flag: flags.append('spelling')
    result = {
