...
GATEKEEPER_LOG = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../analytics/ai_quality_gatekeeper_log.jsonl'))
...
def score_output(text):
    ...
    block = any(score < 7 for score in [grammar_score, style_score, readability_score, legal_score]) or bool(legal_flags)
    result = {
