...
NICHE_LOG = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../analytics/niche_performance_log.jsonl'))
...
def aggregate_niche_stats():
    ...
    for niche, group in df.groupby('niche'):
        ...
        avg_quality = group['quality_score'].dropna().mean() if 'quality_score' in group else None
        stats[niche] = {
