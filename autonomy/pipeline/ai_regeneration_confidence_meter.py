...
CONFIDENCE_LOG = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../analytics/ai_regeneration_confidence_log.jsonl'))
...
def confidence_score(text, history=None):
    ...
    # Compute score
    score = 10
    if not consistent: score -= 2
    if not grammar: score -= 2
    if legal_risk: score -= 4
