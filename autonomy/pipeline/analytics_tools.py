...
DASHBOARD_IMG_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../docs/images'))
os.makedirs(DASHBOARD_IMG_DIR, exist_ok=True)

# --- Analytics Log Schema ---
LOG_FIELDS = [
    'vault_id', 'niche', 'product_name', 'ai_prompt_version', 'ai_style', 'ai_generation_success',
    'ai_latency', 'ai_version', 'sale_datetime', 'sale_amount', 'refund', 'refund_reason', 'language',
    'country', 'customer_type', 'delivery_success', 'funnel_stage', 'visit_id'
]
...
def generate_sample_analytics_log(path=None, n=1000):
    ...
    for i in range(n):
        ...
        sale_datetime = (now - datetime.timedelta(days=random.randint(0,30), hours=random.randint(0,23), minutes=random.randint(0,59))).isoformat()
        sale_amount = round(random.uniform(9.99, 99.99), 2)
        refund = random.choices([True, False], weights=[0.08, 0.92])[0]
        refund_reason = random.choice(['', 'Product mismatch', 'Price dissatisfaction', 'Description unclear']) if refund else ''
...
def ai_generation_stats(log):
    ...
    return {'success': successes, 'fail': fails, 'avg_latency': avg_latency, 'by_version': dict(by_version)}
...
def sales_performance_over_time(log, freq='D'):
    ...
    if freq == 'W':
        return df.groupby(df['sale_datetime'].dt.isocalendar().week)['sale_amount'].sum().to_dict()
    if freq == 'H':
...
# All analytics are passive, read-only, and audit-logged. No AI can self-modify or act on this data.
