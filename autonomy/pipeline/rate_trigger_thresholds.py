...
THRESHOLD_CONFIG_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), 'listeners/config/thresholds.json'))
THRESHOLD_ALERT_LOG = os.path.abspath(os.path.join(os.path.dirname(__file__), '../analytics/threshold_alert_log.json'))
...
def check_event_rate(event_type, event_count, error_count):
    ...
    if event_count > thresholds["max_events"] or error_count > thresholds["max_errors"]:
        ...
        with open(THRESHOLD_ALERT_LOG, 'a') as f:
            f.write(json.dumps({"event_type": event_type, "event_count": event_count, "error_count": error_count, "alert": True}) + '\n')
    return alert
