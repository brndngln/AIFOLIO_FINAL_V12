...
>>>>>>> omni_repair_backup_20250704_1335
    ...
    if api_key:
        ...
        if resp.status_code == 202:
            ...
            logging.error(f"[AIFOLIO] Failed to send preview bundle: {resp.status_code} {resp.text}")
            raise RuntimeError(f"SendGrid error: {resp.status_code} {resp.text}")
    else:
        logging.warning(f"[AIFOLIO] Preview bundle sent to {buyer_email}. (stub, no SENDGRID_API_KEY)")
