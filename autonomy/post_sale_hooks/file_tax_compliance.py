...
class file_tax_compliance:
    ...
    def trigger(sale_data):
        ...
        """
        start = time.time()
        log_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../analytics/tax_compliance.json'))
        api_url = os.environ.get('TAX_API_URL')
        try:
            if api_url:
                resp = requests.post(api_url, json=sale_data, timeout=10)
                if resp.status_code == 200:
                    logging.info(f"[AIFOLIO] Tax compliance API success for sale: {sale_data}.")
                else:
                    logging.error(f"[AIFOLIO] Tax API error: {resp.status_code} {resp.text}")
                ...
                logging.info(f"[AIFOLIO] Tax compliance logged for sale: {sale_data} (no TAX_API_URL)")
        except Exception as e:
            logging.error(f"[AIFOLIO] Exception in tax compliance: {e}")
            raise
        elapsed = time.time() - start
        if elapsed > 2.0:
            logging.warning(f"[AIFOLIO][PERF] file_tax_compliance.trigger took {elapsed:.2f}s")
