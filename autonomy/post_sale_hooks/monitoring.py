...
POST_SALE_HOOKS_LOG = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../analytics/post_sale_hooks.log")
)
        ...
        for line in f:
            if "[ERROR]" in line:
                ...
                try:
                    ...
                    context = (
                        json.loads(context_part.strip().replace("'", '"'))
                        if "{" in context_part
                        else {}
                    )
                    failed.append(
...
def send_alert(message, service="slack"):
    ...
    try:
        ...
        if resp.status_code != 200:
            print(
                f"[ALERT-{service.upper()}] Failed to send alert: {resp.status_code} {resp.text}"
            )
    except Exception as e:
