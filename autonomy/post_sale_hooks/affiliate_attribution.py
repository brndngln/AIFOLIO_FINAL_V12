...
def affiliate_attribution(buyer_email, referral_data):
    ...
    """
    start = time.time()
    log_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../analytics/affiliate_attribution.json'))
    ...
    if api_url:
        try:
            ...
            if resp.status_code == 200:
                ...
                logging.error(f"[AIFOLIO] Affiliate API error: {resp.status_code} {resp.text}")
                raise RuntimeError(f"Affiliate API error: {resp.status_code} {resp.text}")
        except Exception as e:
            logging.error(f"[AIFOLIO] Exception posting to affiliate API: {e}")
            raise
    else:
        logging.warning(f"[AIFOLIO] Affiliate attribution for {buyer_email} | Referral: {referral_data}. (stub, no AFFILIATE_API_URL)")
