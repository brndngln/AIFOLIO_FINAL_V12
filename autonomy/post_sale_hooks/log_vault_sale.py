...
def log_vault_sale(order_id, vault_id, metadata):
    ...
    """
    log_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../analytics/vault_sales.json'))
    ...
    try:
        ...
        logging.info(f"[AIFOLIO] Vault sale logged: order {order_id}, vault {vault_id}.")
        if api_url:
            resp = requests.post(api_url, json=entry, timeout=10)
            if resp.status_code == 200:
                logging.info(f"[AIFOLIO] Analytics API notified for order {order_id}.")
            else:
                logging.error(f"[AIFOLIO] Analytics API error: {resp.status_code} {resp.text}")
