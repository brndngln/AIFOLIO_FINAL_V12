...
def send_confirmation_email(buyer_email, vault_name):
    ...
    if api_key:
        ...
        if resp.status_code == 202:
            logging.info(f"[AIFOLIO] Confirmation email sent to {buyer_email} for vault '{vault_name}'.")
        else:
            logging.error(f"[AIFOLIO] Failed to send confirmation email: {resp.status_code} {resp.text}")
            raise RuntimeError(f"SendGrid error: {resp.status_code} {resp.text}")
    else:
        logging.warning(f"[AIFOLIO] Confirmation email sent to {buyer_email} for vault '{vault_name}'. (stub, no SENDGRID_API_KEY)")
