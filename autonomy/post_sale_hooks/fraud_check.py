...
def fraud_check(buyer_data, sale_metadata):
    ...
    """
    start = time.time()
    log_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../analytics/fraud_checks.json'))
    result = {'buyer_data': buyer_data, 'sale_metadata': sale_metadata, 'fraud_flag': False}
    ...
    try:
        if api_url:
            ...
            if resp.status_code == 200:
                logging.info(f"[AIFOLIO] Fraud API check success for buyer: {buyer_data}.")
            else:
                logging.error(f"[AIFOLIO] Fraud API error: {resp.status_code} {resp.text}")
        ...
        if result['fraud_flag']:
            logging.warning(f"[AIFOLIO] FRAUD FLAGGED for buyer {buyer_data}: {sale_metadata}")
        else:
