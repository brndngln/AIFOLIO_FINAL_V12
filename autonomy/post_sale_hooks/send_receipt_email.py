    ...
    try:
        if api_key:
            ...
            if resp.status_code == 202:
                logging.info(f"[AIFOLIO] Receipt email sent to {user_email} for order {order_id}.")
            else:
                logging.error(f"[AIFOLIO] Failed to send receipt email: {resp.status_code} {resp.text}")
        else:
            ...
            print(f"[AIFOLIO] Receipt email logged for {user_email} (no SENDGRID_API_KEY)")
