...
def detect_income_mismatch(api_income, bank_income, user_profile):
    ...
    for ai in api_income:
        match = next((b for b in bank_income if b['date'] == ai['date'] and b['amount'] == ai['amount']), None)
        if not match:
            flags.append({
...
def log_flags(flags, user_id):
    entry = f"{datetime.now()}|USER:{user_id}|INCOME_MISMATCH_FLAGS:{json.dumps(flags)}\n"
...
if __name__ == '__main__':
    ...
    flags = run_income_mismatch_alert(api_income, bank_income, user_profile, user_id='demo')
