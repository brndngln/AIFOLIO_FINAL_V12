...
def load_audit_log():
    log_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../audit_trail.log'))
    ...
    with open(log_path) as f:
        for line in f:
            try:
                ...
                time = parts[0]
                event = next((p.split(':',1)[1] for p in parts if p.startswith('EVENT:')), None)
                user = next((p.split(':',1)[1] for p in parts if p.startswith('USER:')), 'N/A')
                details = next((p.split(':',1)[1] for p in parts if p.startswith('DETAILS:')), '{}')
                if not event:
                    ...
                    if any('FLAGS:' in p for p in parts):
                        ...
                        event = 'Other'
                entries.append({'time': time, 'event': event, 'user': user, 'details': details})
            except Exception:
                continue
...
def load_alerts():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT level, message, created_at FROM alerts ORDER BY created_at DESC LIMIT 100;")
    ...
    if not alerts:
        try:
            ...
            if splunk_url and splunk_token:
                resp = ext_requests.get(splunk_url, headers={'Authorization': f'Bearer {splunk_token}'}, timeout=2)
                if resp.ok:
                    for alert in resp.json().get('alerts', []):
                        alerts.append({'level': alert.get('level','INFO'), 'message': alert.get('message',''), 'time': alert.get('time','')})
        except Exception:
            pass
    # Try ElasticSearch
    if not alerts:
        try:
            ...
            if es_url and es_token:
                resp = ext_requests.get(es_url, headers={'Authorization': f'Bearer {es_token}'}, timeout=2)
                if resp.ok:
                    for alert in resp.json().get('alerts', []):
                        alerts.append({'level': alert.get('level','INFO'), 'message': alert.get('message',''), 'time': alert.get('time','')})
        except Exception:
    ...
    if not alerts:
        try:
            ...
            if cw_url and cw_token:
                resp = ext_requests.get(cw_url, headers={'Authorization': f'Bearer {cw_token}'}, timeout=2)
                if resp.ok:
                    for alert in resp.json().get('alerts', []):
                        alerts.append({'level': alert.get('level','INFO'), 'message': alert.get('message',''), 'time': alert.get('time','')})
        except Exception:
    ...
    if not alerts:
        try:
            ...
            if az_url and az_token:
                resp = ext_requests.get(az_url, headers={'Authorization': f'Bearer {az_token}'}, timeout=2)
                if resp.ok:
                    for alert in resp.json().get('alerts', []):
                        alerts.append({'level': alert.get('level','INFO'), 'message': alert.get('message',''), 'time': alert.get('time','')})
        except Exception:
    ...
    if not alerts:
        try:
            ...
            if qradar_url and qradar_token:
                resp = ext_requests.get(qradar_url, headers={'Authorization': f'Bearer {qradar_token}'}, timeout=2)
                if resp.ok:
                    for alert in resp.json().get('alerts', []):
                        alerts.append({'level': alert.get('level','INFO'), 'message': alert.get('message',''), 'time': alert.get('time','')})
        except Exception:
    ...
    if not alerts:
        try:
            ...
            if chronicle_url and chronicle_token:
                resp = ext_requests.get(chronicle_url, headers={'Authorization': f'Bearer {chronicle_token}'}, timeout=2)
                if resp.ok:
                    for alert in resp.json().get('alerts', []):
                        alerts.append({'level': alert.get('level','INFO'), 'message': alert.get('message',''), 'time': alert.get('time','')})
        except Exception:
            ...
            with open(alerts_path) as f:
                for line in f:
                    try:
                        ...
                        continue
    return alerts[-100:] if alerts else [
        {'level': 'INFO', 'message': 'No active alerts.'}
    ]
...
def api_compliance_charts():
    ...
    for i, e in enumerate(entries[-50:]):
        risk_points.append({'x': i, 'y': min(1.0, 0.1 + 0.02 * event_counts.get(e['event'], 1))})
    return jsonify({
        'eventBreakdown': event_counts,
        'riskOverTime': risk_points
    })
...
def api_user_activity():
    ...
    activity = [
        {'user': 'alice', 'actions': 32, 'last_active': '2025-06-07T10:57:00', 'ip': '8.8.8.8', 'device': 'MacBook Pro'},
        {'user': 'bob', 'actions': 19, 'last_active': '2025-06-07T10:55:00', 'ip': '1.1.1.1', 'device': 'iPhone 12'},
        {'user': 'carol', 'actions': 28, 'last_active': '2025-06-07T10:52:00', 'ip': '4.4.4.4', 'device': 'Windows 10 PC'},
    ]
...
def api_custom_anomaly_detection():
    ...
    anomalies = [
        {'type': 'Unusual Login', 'user': 'bob', 'time': '2025-06-07T11:10:00', 'severity': 'high'},
        {'type': 'Multiple Failed Logins', 'user': 'carol', 'time': '2025-06-07T10:59:00', 'severity': 'medium'},
    ]
    return jsonify({'anomalies': anomalies, 'checked_at': datetime.utcnow().isoformat()})
...
def api_regulatory_news():
    ...
    news = [
        {'headline': 'IRS Updates 2025 Filing Rules', 'url': 'https://www.irs.gov/news/2025-update'},
        {'headline': 'New GDPR Fines Announced', 'url': 'https://www.europa.eu/news/gdpr-fines'},
        {'headline': 'FedRAMP Revamps Baseline Controls', 'url': 'https://www.fedramp.gov/news/baseline-update'},
    ]
...
def api_external_compliance_feeds():
    ...
    feeds = [
        {'feed': 'PCI DSS Announcements', 'url': 'https://www.pcisecuritystandards.org/news_events/'},
        {'feed': 'ISO 27001 Updates', 'url': 'https://www.iso.org/isoiec-27001-information-security.html'},
...
def api_advanced_reporting():
    ...
    return jsonify({'summary': summary, 'trends': trends, 'outliers': outliers, 'checked_at': datetime.utcnow().isoformat()})
...
def api_regulatory_api_status():
    ...
    try:
        ...
        if irs_url and irs_token:
            resp = ext_requests.get(irs_url, headers={'Authorization': f'Bearer {irs_token}'}, timeout=3)
            if resp.ok:
                return jsonify({'status': resp.json().get('status','Compliant'), 'source': 'IRS', 'checked_at': datetime.utcnow().isoformat()})
    except Exception:
        pass
    return jsonify({'status': 'Unavailable', 'source': 'IRS', 'checked_at': datetime.utcnow().isoformat()})
