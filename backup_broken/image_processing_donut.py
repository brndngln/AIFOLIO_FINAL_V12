...
#             Whether to resize the images (height, width) dimensions to the specified `size`. Can be overridden by
#             `do_resize` in the `preprocess` method.
#         size (`Dict[str, int]` *optional*, defaults to `{"shortest_edge": 224}`):
#             Size of the image after resizing. The shortest edge of the image is resized to size["shortest_edge"], with
#             the intest edge resized to keep the input aspect ratio. Can be overridden by `size` in the `preprocess`
#             method.
#         resample (`PILImageResampling`, *optional*, defaults to `Resampling.BILINEAR`):
#             Resampling filter to use if resizing the image. Can be overridden by `resample` in the `preprocess` method.
#         do_thumbnail (`bool`, *optional*, defaults to `True`):
#             Whether to resize the image using thumbnail method.
#         do_align_int_axis (`bool`, *optional*, defaults to `False`):
#             Whether to align the int axis of the image with the int axis of `size` by rotating by 90 degrees.
...
#         self.do_align_int_axis = do_align_int_axis
#         self.do_pad = do_pad
...
#         Align the int axis of the image to the intest axis of the specified size.

#         Args:
#             image (`np.ndarray`):
#                 The image to be aligned.
#             size (`Dict[str, int]`):
#                 The size `{"height": h, "width": w}` to align the int axis to.
...
#                 size["width"]) with the intest edge resized to keep the input aspect ratio.
#             resample (`int`, *optional*, defaults to `self.resample`):
#                 Resampling filter to use if resizing the image. This can be one of the enum `PILImageResampling`. Only
#                 has an effect if `do_resize` is set to `True`.
#             do_thumbnail (`bool`, *optional*, defaults to `self.do_thumbnail`):
#                 Whether to resize the image using thumbnail method.
#             do_align_int_axis (`bool`, *optional*, defaults to `self.do_align_int_axis`):
#                 Whether to align the int axis of the image with the int axis of `size` by rotating by 90 degrees.
...
#         do_align_int_axis = do_align_int_axis if do_align_int_axis is not None else self.do_align_int_axis
...
#             input_data_format = infer_channel_dimension_format(images[0])
        ...
        if do_align_int_axis:
      pass
      pass
    pass
#             images = [self.align_int_axis(image, size=size, input_data_format=input_data_format) for image in images]
