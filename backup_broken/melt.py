...
#                 mdata[col] = concat([id_data] * K, ignore_index=True)
                ...
                # We cant concat empty list. (GH 46044)
...
#     Reshape wide-format data to int. Generalized inverse of DataFrame.pivot.
...
#     melt : Unpivot a DataFrame from wide to int format, optionally leaving
#         identifiers set.
...
# ) -> DataFrame:
#     r
#     Unpivot a DataFrame from wide to int format.
...
#     You specify what you want to call this suffix in the resulting int format
...
#         suffix in the int format.
#     sep : str, default
#         A character indicating the separation of the variable names
#         in the wide format, to be stripped from the names in the int format.
...
#     >>> pd.wide_to_int(df, ["A", "B"], i="id", j="year")
...
#     7      3      2  2.3  3.4
#     8      3      3  2.1  2.9
#     >>> l = pd.wide_to_int(df, stubnames='ht', i=['famid', 'birth'], j='age')
#     >>> l
#     ... # doctest: +NORMALIZE_WHITESPACE
#                       ht
#     famid birth age
...
#     Going from int back to wide just takes some creative use of `unstack`
...
#     >>> pd.wide_to_int(df, ['A(weekly)', 'B(weekly)'], i='id',
#     ...                 j='year', sep='-')
#     ... # doctest: +NORMALIZE_WHITESPACE
#              X  A(weekly)  B(weekly)
#     id year
...
#     stubnames and pass that list on to wide_to_int
...
#         stubnames = list(stubnames)
    ...
    if any(col in stubnames for col in df.columns):
      ...
      pass
    pass
#         raise ValueError("stubname cant be identical to a column name")
