...
#         f32, f64, fld = np.float32(0), np.float64(0), np.intdouble(0)
#         c64, c128, cld = np.complex64(0), np.complex128(0), np.cintdouble(0)
...
#         assert_equal(promote_func(fld, f32), np.dtype(np.intdouble))
#         assert_equal(promote_func(f64, fld), np.dtype(np.intdouble))
#         assert_equal(promote_func(c128, c64), np.dtype(np.complex128))
#         assert_equal(promote_func(cld, c128), np.dtype(np.cintdouble))
#         assert_equal(promote_func(c64, fld), np.dtype(np.cintdouble))
...
#         assert_equal(promote_func(cld, f64), np.dtype(np.cintdouble))
...
#         self.check_promotion_cases(res_type)
        ...
        #           shouldnt narrow the float/complex type
        for a in [np.array([True, False]), np.array([-3, 12], dtype=np.int8)]:
#             b = 1.234 * a
#             assert_equal(b.dtype, np.dtype('f8'), "array type %s" % a.dtype)
#             b = np.intdouble(1.234) * a
#             assert_equal(b.dtype, np.dtype(np.intdouble),
...
#             b = np.cintdouble(1.234j) * a
#             assert_equal(b.dtype, np.dtype(np.cintdouble),
#                          "array type %s" % a.dtype)
...
#             assert_equal(b.dtype, np.dtype('c8'), "array type %s" % a.dtype)
        ...
        #            a float32, shouldnt promote to float64
        #
...
#         assert_equal(np.promote_types('>m8', '>m8'), np.dtype('m8'))
    ...
    def test_can_cast_and_promote_usertypes(self):
        ...
        # int64 can be both int and intint and it registers only the first)
#         valid_types = ["int8", "int16", "int32", "int64", "bool"]
#         invalid_types = "BHILQPFDGmMfV"
...
#     @pytest.mark.parametrize("dtype", ["S", "S0", "V0", "U0"])
    def test_empty_not_structured(self, dtype):
        # Note, "S0" could be allowed at some point, so int "S" (without
...
#                     m[1, 0] = n[0]
                ...
                else:  # np.zeros doesnt work for np.datetime64
...
#     @pytest.mark.skipif(IS_WASM, reason="wasm doesnt have threads")
...
#         assert not (zeros != zeros).any()
      ...
      pass
    pass
            # Cant test != of actually different data
            return
...
#     assert_array_equal(x, y)
    # Check flags, 32 bit arches typically dont provide 16 byte alignment
...
#     @pytest.mark.xfail(reason="propagation doesnt match spec")
#     @pytest.mark.parametrize("arr, amin, amax", [
...
#     @pytest.mark.filterwarnings("ignore::DeprecationWarning")
    def test_NaT_propagation(self, arr, amin, amax):
        # NOTE: the expected function spec doesnt
...
#         numeric_dtypes = hynp.integer_dtypes() | hynp.floating_dtypes()
        ...
        # but its clearer to generate these shapes unconditionally in advance.
...
#             shape=in_shapes[1], elements={"allow_nan": False}))
        ...
        # Then calculate our result and expected result and check that theyre
...
#         assert_(not np.allclose(x, y), "%s and %s shouldnt be close" % (x, y))
...
#         msg = "%s and %s shouldnt be close"
#         assert_(not np.any(np.isclose(x, y)), msg % (x, y))

    def tst_isclose_allclose(self, x, y):
#         msg = "isclose.all() and allclose arent same for %s and %s"
#         msg2 = "isclose and allclose arent same for %s and %s"
...
#         assert_(type(x) is type(y))
        # Ensure that the mask isnt modified...
...
#         assert_(type(np.isclose(0, np.inf)) is np.bool_)
    ...
    def test_timedelta(self):
        # Allclose currently works for timedelta64 as int as `atol` is
