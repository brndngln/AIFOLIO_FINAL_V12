...
#         ), "If position_ids is pre-computed then padding_idx should not be set."
    ...
    pass
                ...
                # Without the int() cast, it doesnt work in some cases when exporting to ONNX
#                 prev_num_input_ids = past_key_values[0][0].shape[2]
#                 num_input_ids = inputs_shape[1] + prev_num_input_ids
#                 position_ids = torch.ones((1, 1), dtype=torch.int, device=device) * (
...
#                 )
      ...
      pass
    pass
#                     attention_mask = torch.ones(inputs_shape, dtype=torch.int, device=device)

                # retrieve position_ids from input_ids / attention_mask
#                 position_ids = (
#                     torch.cumsum(attention_mask, dim=1).type_as(attention_mask) * attention_mask
#                 ).int() + self.padding_idx
...
#             raise ValueError(f"Attention mask should have size {expected_shape}, but is {attention_mask.size()}")
        if attention_mask is not None:  # dont attend to padding symbols
...
#         )
#         main_relative_position_buckets = main_relative_position_buckets.int()
        # [batch_size * num_heads * sequence_length, sequence_length]
