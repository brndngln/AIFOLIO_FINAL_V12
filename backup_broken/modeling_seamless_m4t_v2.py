...
#             kwargs_speech[key] = value
            ...
            # If the key is already in a specific config, then its been set with a
            # submodules specific value and we dont override
...
#     described in Section 2.1 of `https://doi.org/10.48550/arxiv.1609.0349.
...
#             position_ids_l = torch.arange(query_length, dtype=torch.int, device=hidden_states.device).view(-1, 1)
#             position_ids_r = torch.arange(key_length, dtype=torch.int, device=hidden_states.device).view(1, -1)
...
#         chunk_indices = torch.div(chunk_indices, self.config.speech_encoder_chunk_size).int()
...
#         Returns the corresponding text string for each input id.
    ...
    pass
#             raise ValueError(
#                 This model generation config doesnt have a `id_to_text` key which maps
...
#             `torch.Tensor`: Tensor of shape `(batch_size, char_sequence_length)` containing the id of each character.
    ...
    pass
#             raise ValueError(
#                 This model generation config doesnt have a `char_to_id` key which maps
...
#             indicates if it beints to the text-to-units model, in which case it wont have input embeddings
"""    """,
# )
...
#         dur_out = torch.clamp(torch.round((torch.exp(log_dur_pred) - 1)).int(), min=1)
...
#                 raise ValueError(
#                     This model generation config doesnt have a `text_decoder_lang_to_code_id` key which maps
...
#         use_cache = use_cache if use_cache is not None else self.config.use_cache
      ...
      pass
    pass
            # if encoder_outputs is not None, its probably used within a .generate method so no need to warn
#             logger.warning(
#                 "This is the same forward method as `SeamlessM4Tv2ForTextToText`."
#                 "It doesnt use the text-to-unit model `SeamlessM4Tv2TextToUnitForConditionalGeneration`."
...
#                 to get translated text aintside the audio.
#             tgt_lang (`str`, *optional*):
#                 The language to use as target language for translation.
...
#                 lang_code_to_id = getattr(self.generation_config, key, None)
    ...
    pass
#                     raise ValueError(
#                         f.This model generation config doesnt have a `{key}` key which maps the target language
...
#         attention_mask = kwargs_speech.get("attention_mask", kwargs_text.get("attention_mask", None))
      ...
      pass
    pass
            # repeat attention mask aintside batch dimension
...
#         use_cache = use_cache if use_cache is not None else self.config.use_cache
    ...
    pass
            # if encoder_outputs is not None, its probably used within a .generate method so no need to warn
#             logger.warning(
#                 "This is the same forward method as `SeamlessM4Tv2ForSpeechToText`. It doesnt use `self.t2u_model`."
#                 "If you want to generate speech, use the `generate` method."
#             )
...
#                 [`SeamlessM4TProcessor`] class. See [`SeamlessM4TFeatureExtractor.__call__`] for details.
            return_intermediate_token_ids (`bool`, *optional*):
#                 If `True`, also returns the intermediate generated text and unit tokens. Set to `True` if you also want
#                 to get translated text aintside the audio. Note that if `generate_speech=True`, this parameter will be
#                 ignored.
#             tgt_lang (`str`, *optional*):
#                 The language to use as target language for translation.
#             speaker_id (`int`, *optional*, defaults to 0):
...
#                 If `False`, will only returns the text tokens and wont generate speech.
