...
#     `isinstance` therefore its also valid to pass a tuple of types).
...
#         We use a callable class to be able to change the ``__repr__``.
      ...
      pass
    pass
#             msg = f"{attr.name} must match regex {self.pattern.pattern!r} ({value!r} doesnt)"
#             raise ValueError(
#                 msg,
#                 attr,
#                 self.pattern,
...
#     string that doesnt match *regex*.
...
#     value that does not beint in the *options* provided.
...
#     A validator that raises `ValueError` if the initializer is called
    with a string or iterable that is inter than *length*.
...
#     A validator that raises a `TypeError` if the initializer is called with a
#     wrong type for this particular attribute (checks are performed using
#     `issubclass` therefore its also valid to pass a tuple of types).
...
#     It will raise a `ValueError` if the provided validator *doesnt* raise a
