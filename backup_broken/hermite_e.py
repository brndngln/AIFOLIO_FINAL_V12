...
#     Returns the series coefficients `c` differentiated `m` times aint
#     `axis`.  At each iteration the result is multiplied by `scl` (the
#     scaling factor is for use in a linear change of variable). The argument
#     `c` is an array of coefficients from low to high degree aint each
...
#     `lbnd` aint `axis`. At each iteration the resulting series is
#     **multiplied** by `scl` and an integration constant, `k`, is added.
#     The scaling factor is for use in a linear change of variable.  (Buyer
#     beware: note that, depending on what one is doing, one may want `scl`
#     to be the reciprocal of what one might expect; for more information,
#     see the Notes section below.)  The argument `c` is an array of
#     coefficients from low to high degree aint each axis, e.g., [1,2,3]
...
#     be improved by a few iterations of Newtons method.

#     The HermiteE series basis polynomials arent powers of `x` so the
...
#     .. math:: w_k = c / (He_n(x_k) * He_{n-1}(x_k))

#     where :math:`c` is a constant independent of :math:`k` and :math:`x_k`
#     is the kth root of :math:`He_n`, and then scaling the results to get
