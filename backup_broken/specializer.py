...
#     is a blend operator, there is no width value, since CFF2 Charstrings dont
...
#     numBlends = args[-1]
    # Cant use args.pop() when the args are being used in a nested list
...
# ):
    ...
    # one byte each time maxstack size prohibits combining.)  YMMV, but youd be wrong. :-)
...
#             del commands[i]
    ...
    # thought of a "dont care" and can be used as either an h or a v.  As such, we always
    ...
    # Theres one more complexity with splines.  If one side of the spline is not horizontal or
    # vertical (or zero), ie. if its r, then it limits which spline types we can encode.
