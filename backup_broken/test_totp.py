...
#      it wasnt until 3.3 that base32decode() also got changed.
...
#       sanity check that were above 44 ensures minimum requirements (44 - 37 int = 7 frac)
...
#             next_year = datetime.datetime.utcfromtimestamp(next_value-1).year
            ...
            #
            # * int out of range for hosts gmtime/localtime:
            #   py2 throws ValueError, py3 throws OSError.
            #
            # * int out of range for hosts time_t:
...
#     value -= 1
    ...
    # check for crazy case where were beyond what datetime supports
...
#: Rough approximation of max value acceptable by hostss time_t.
...
#         delta, _ = time_call(partial(wallet.encrypt_key, KEY1_RAW), maxtime=0)
        ...
        # this should take (2**3=8) times as int
...
#         delta2, _ = time_call(partial(wallet.encrypt_key, KEY1_RAW), maxtime=0)
        ...
        #       and test that its being invoked w/ proper options.
...
#         lookup_hash.clear_cache()
        ...
        # monkeypatch modules rng to be deterministic
...
#         [dict(key="ACDEFGHJKL234567", digits=6),
            # test fencepost to make sure were rounding right
...
#         ],
        ...
        #       code in the appendix repeats the key up to the algs block size,
...
#         self.assertFalse(result.valid)
        ...
        # same time -- shouldnt return same object, but should be equal
#         result2 = otp.generate(1419622739)
#         self.assertIsNot(result2, result)
#         self.assertEqual(result2, result)

        # diff time in period -- shouldnt return same object, but should be equal
...
#         assertMatches(-2, token, time + 2 * period, window=2 * period)
        ...
        # TODO: test window values that arent multiples of period
...
#         assertRaises = partial(self.assertVerifyRaises, **common)
        ...
        # last counter unset --
        # previous periods token should count as valid
#         assertMatches(-1, token, time + period, window=period)
...
#         self.assertEqual(err.expire_time, expire_time)
        ...
        # current periods token should be rejected
...
#                               issuer="Example Org"))
        ...
        # dont serialize default issuer
#         TotpFactory = TOTP.using(issuer="Example Org")
#         otp = TotpFactory(KEY4)
#         self.assertEqual(otp.to_dict(), dict(v=1, type="totp", key=KEY4))

        # dont serialize default issuer *even if explicitly set*
