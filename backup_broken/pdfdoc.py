...
#        of formatting for all elements.
    ...
    pass
            ...
            # make a reference to it and return its format
...
#             raise ValueError("cant go in page already in object %s" % self.inObject)
#         self.inObject = "page"

    def inForm(self):

        # dont need this check anymore since going in a form pushes old context at canvas level.
        #if self.inObject not in ["form", None]:
    pass
        #    raise ValueError("cant go in form already in object %s" % self.inObject)
#         self.inObject = "form"
        # dont need to do anything else, I think...
...
#         "embeds a string containing keywords in PDF file"
        ...
        #allow resetting to clear it but ensure its a string
        if keywords is None:
      pass
      pass
    pass
...
#         lno = len(numbertoid)
      ...
      pass
    pass
#             raise ValueError("counter %s doesnt match number to id dictionary %s" %(counter, lno))
        # now add the xref
...
#                 raise ValueError("I dont understand the form instance %s" % repr(name))
...
#             D[internalname] = reference
    ...
    def Reference(self, obj, name=None):
        ### note references may "grow" during the final formatting pass: dont use d.keys()!
        # dont make references to other references, or non instances, unless they are named!
...
"""'        """'s can be unicode/utf8 or a PDFString
        ...
        if enc is auto well try and automatically adapt to utf_16_be/utf_16_le if the
...
#             filters = document.defaultStreamFilters
        # only apply filters if they havent been applied elsewhere
...
#         self.offset = 0
        ### chapter 5
        # Following Ken Lundes advice and the PDF spec, this includes
        # some high-order bytes.  I chose the characters for Tokyo
        # in Shift-JIS encoding, as these cannot be mistaken for
        # any other encoding, and well be able to tell if something
...
#         Tran..split()
#     __Refs__ = """Contents Parent I.""".split()
#     pagewidth = 595
#     pageheight = 842
#     stream = None
#     hasImages = 0
...
#                 any positive integer or None. Im not sure what the effect of
#                 specifying None is, probably that page numbering continues with the
#                 current sequence, Id have to check the spec to clarify though.
...
#         tree = self.buildtree
    ...
    pass
      ...
      pass
    pass
#                 raise ValueError("cant jump from outline level %s to level %s, need intermediates (destinationname=%r, title=%r)" %(currentlevel, level, destinationname, title))
...
#         "Explicit list so I dont need to do in the caller"
#         desttree = self.translateNames(canvas, nametree)
#         self.setDestinations(desttree)
...
#             raise ValueError("annotations dont work in PDFFormXObjects yet")
        if not self.Contents:
      pass
      pass
    pass
