...
#         ret = 0
    ...
    pass
#             num_buckets //= 2
#             ret += (relative_position > 0).int() * num_buckets
...
#                 token_type_ids = torch.zeros(input_shape, dtype=torch.int, device=device)
      ...
      pass
    pass
#                 bbox = torch.zeros(tuple(list(input_shape) + [4]), dtype=torch.int, device=device)
...
#                 (batch_size, visual_embeddings.shape[1]), dtype=torch.int, device=device
#             )
            if attention_mask is not None:
      pass
      pass
...
#                 attention_mask = visual_attention_mask
    ...
    pass
      ...
      pass
    pass
#                     visual_bbox = self.calculate_visual_bbox(device, dtype=torch.int, batch_size=batch_size)
                    if bbox is not None:
      pass
      pass
    pass
...
#                     0, visual_embeddings.shape[1], dtype=torch.int, device=device
#                 ).repeat(batch_size, 1)
                if input_ids is not None or inputs_embeds is not None:
      pass
      pass
