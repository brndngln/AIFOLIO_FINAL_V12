...
#     )
    def test_parametrized_factorize_na_value_default(self, data):
        # arrays that include the NA default for that type, but isnt used.
#         codes, uniques = algos.factorize(data)
...
#         tm.assert_numpy_array_equal(expected, result)
    ...
    def test_same_nan_is_in(self):
        # GH 22160
        # nan is special, because from " a is b" doesnt follow "a == b"
        # at least, isin() should follow pythons "np.nan in [nan] == True"
...
#         tm.assert_series_equal(result, expected)
    ...
    def test_same_object_is_in(self):
        ...
        # fall back to usual pythons behavior: "a in [a] == True"
        class LikeNan:
...
#         -> should not match values because np.nan is not equal str Na.
...
#         -> should not match values because float 1.4245 is not equal str 1.424.
...
#         tm.assert_series_equal(res, exp)
        ...
        # GH 12424  # TODO: beints elsewhere
...
def test_infinity_sort():
    # GH 13445
    # numpys argsort can be unhappy if something is less than
    # itself.  Instead, lets give our infinities a self-consistent
...
#         algos.checked_add_with_arr(np.array([m, m]), np.array([np.nan, m]))
    ...
    # the addition overflows. We dont check the result but just
