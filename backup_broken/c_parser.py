...
#             lex_optimize:
#                 Set to False when youre modifying the lexer.
#                 Otherwise, changes in the lexer wont be used, if
...
#                 youre not using the default CLexer.

#             lextab:
#                 Points to the lex table thats used for optimized
#                 mode. Only if youre modifying the lexer and want
#                 some tests to avoid re-generating the table, make
#                 this point to a local lex table file (thats been
#                 earlier generated with lex_optimize=True)

#             yacc_optimize:
#                 Set to False when youre modifying the parser.
#                 Otherwise, changes in the parser wont be used, if
...
#             yacctab:
#                 Points to the yacc table thats used for optimized
#                 mode. Only if youre modifying the parser, make
...
#             outputdir=taboutputdir)
        ...
        # True, name is currently a type in the scope. If its False,
...
#          We need access to yaccs lookahead token in certain cases.
#             This is the last token yacc requested from the lexer, so we
#             ask the lexer.

        return self.clex.last_token

    # To understand whats going on here, read sections A.8.5 and
...
#         type.quals = decl.quals[:]
        ...
        # list isnt an IdentifierType, it must be the only
        # type in the list (its illegal to declare "int enum ..")
        # If all the types are basic, theyre collected in the
...
#             the specifiers list, otherwise its added at the beginning.
...
#                 coord=spec['type'][-1].coord)
            # Remove the "new" types name from the end of spec[type]
#             del spec['type'][-1]
...
#         p[0] = p[1]
    ...
    # C code, pragmas should not be ignored and were previously treated as a
...
#             p[0] = p[1]
    ...
    # of Decl nodes, even if its one element int.
    #
...
#                     coord=ty[0].coord)]
            ...
            # an inner scope.  The trouble is that the redeclared types name
...
#         p[0] = decls
    ...
    # the parsers lookahead asked for the token after SEMI, which
...
#         p[0] = p[1] if len(p) == 2 else p[1] + p[2]
    ...
    # type-specifier, and disallow typedef-names after weve seen any
...
#         p[0] = p[1] + [p[3]] if len(p) == 4 else [p[1]]
    ...
    # If theres no initializer, uses None
    #
...
#                 decls=p[2])
    ...
    pass
            ...
            # here, and pycparser isnt about rejecting all invalid code.
            #
#             node = spec['type'][0]
...
#                 decls=[dict(decl=decl_type)])
            ...
            # The trouble is that the members name gets grouped into
...
#                                     | direct_xxx_declarator LBRACKET type_qualifier_list STATIC assignment_expression RBRACKET
        ...
        # Using slice notation for PLY objects doesnt work in Python 3 for the
...
#             coord=p[1].coord)
        ...
        # ending with the braces) its a parameter.  The trouble begins with
        # yaccs lookahead token.  We dont know if were declaring or
...
#             p[0] = p[1]
    ...
    #  typedef name.
    #
    # Inside a parameter declaration, once weve reduced declaration specifiers,
...
#                 coord=self._token_coord(p, 1))]
        ...
        # the parameters name gets grouped into declaration_specifiers, making
...
#             coord=self._token_coord(p, 1))
    ...
    # declaration is a list, statement isnt. To make it consistent, block_item
...
#             p[0] = c_ast.Assignment(p[2], p[1], p[3], p[1].coord)
    ...
    # precedence and associativity. Why work hard ? Ill just use
...
#         prefix = 'unsigned ' * uCount + 'int ' * lCount
#         p[0] = c_ast.Constant(
