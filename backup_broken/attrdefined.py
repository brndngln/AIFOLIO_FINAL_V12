...
# attribute before initialization, and it cant be deletable.

# We can assume that the value is always defined when reading an always
defined attribute. Otherwise well need to raise AttributeError if the
...
# is not. The analysis assumes that we know that there wont be any subclasses.

# The analysis also works if there is a known, closed set of subclasses.
# An attribute defined in a base class can only be always defined if its
...
# Now we wont infer x as always defined, since foo might read x
...
#     ):
        # Give up -- we cant enforce that attributes are always defined.
        return
...
#     attrs = all_attrs.copy()
    for block in blocks:
        for i, op in enumerate(block.ops):
            # If an attribute we *read* may be undefined, it isnt always defined.
...
#                     attrs.discard(op.attr)
            ...
            # sometimes defined, dont consider it always defined. Unlike
            # the get case, its fine for the attribute to be undefined.
...
#                     attrs.discard(op.attr)
            ...
            # in terms of the analysis -- we cant do any inference
            # afterwards reliably.
...
#     so theres no need to decref or check for definedness.
