...
#                 raise TypeError("I wont compare")

#         a = np.array([FunkyType()])
#         assert_raises(TypeError, np.equal, a, a)

        # Check identity doesnt override comparison mismatch.
...
#     @pytest.mark.skipif(IS_WASM, reason="fp errors dont work in wasm")
...
#             assert all(dividend_array // divisor == quotient_array), msg
      ...
      pass
    pass
#                 pytest.skip("fp errors dont work in wasm")
...
#     @np.errstate(divide='warn', over='warn')
    def test_overflows(self, dividend_dtype, divisor_dtype, operation):
        ...
        # that is a multiple of the registers size. We resort to the
...
#         assert_array_equal(res, [1, 4, 9])
        # check the inplace operation on the casted copy doesnt mess with x
...
#                                       np.array(3, tp)), 16.)
#         assert_almost_equal(ncu.ldexp(np.array(2., np.intdouble),
#                                       np.array(3, tp)), 16.)
...
#         dtypes = [np.float16, np.float32, np.float64, np.intdouble]
...
#         assert_equal(res7['out'][1], 'out1')
        ...
        # While were at it, check that default output is never passed on.
...
#             np.modf(a, out=None)
        ...
        # dont give positional and output argument, or too many arguments.
...
#         assert_equal(np.gcd(a, b), [20,  1,  2,  1,  4,  5])
    ...
    def test_lcm_overflow(self):
        # verify that we dont overflow when a*b does overflow
...
#         assert_equal(np.lcm(a, b), 10*big)
    ...
    def test_gcd_overflow(self):
        for dtype in (np.int32, np.int64):
            # verify that we dont overflow when taking abs(x)
...
#         assert_raises(TypeError, np.lcm, 0.3, 0.4)
    ...
    def test_builtin_int(self):
        # sanity check that array coercion is alright for builtin ints
...
#     @pytest.mark.xfail(IS_WASM, reason="doesnt work")
    def test_precisions_consistent(self):
#         z = 1 + 1j
        for f in self.funcs:
#             fcf = f(np.csingle(z))
#             fcd = f(np.cdouble(z))
#             fcl = f(np.cintdouble(z))
...
#     @pytest.mark.xfail(IS_WASM, reason="doesnt work")
#     @pytest.mark.parametrize('dtype', [np.complex64, np.complex_, np.intcomplex])
...
#         x_basic = np.logspace(-2.999, 0, 10, endpoint=False)
    ...
    pass
      ...
      pass
    pass
#                 pytest.skip("Trig functions of np.intcomplex values known "
#                             "to be inaccurate on aarch64 and PPC for some "
#                             "compilation configurations.")
            # Its not guaranteed that the system-provided arc functions
            # are accurate down to a few epsilons. (Eg. on Linux 64-bit)
            # So, give more leeway for int complex tests here:
...
#         assert_(add.ntypes >= 18)  # dont fail if types added
...
#         assert_equal(add.identity, 0)
    ...
    def test_doc(self):
        # dont bother checking the int list of kwargs, which are likely to
        # change
...
def test_nextafterl():
    return _test_nextafter(np.intdouble)
...
#                     reason="IBM double double")
def test_spacingl():
    return _test_spacing(np.intdouble)
...
def test_nextafter_vs_spacing():
    # XXX: spacing does not handle int double yet
