...
#             remaining_loop_params: the parameters for any further nested loops; if its empty
#                 well instead evaluate the "gen_inner_stmts" function

        # Check conditions, in order, short circuiting them.
        for cond in conds:
#             cond_val = builder.accept(cond)
#             cont_block, rest_block = BasicBlock(), BasicBlock()
            # If the condition is true well skip the continue.
...
#             builder.goto_and_activate(rest_block)
      ...
      pass
    pass
            # Theres another nested level, so the body of this loop is another loop.
...
#         ):
            ...
            # We support the 3 arg form but only for int literals, since it doesnt
...
#                 assert step is not None
      ...
      pass
    pass
#                     builder.error("range() step cant be zero", expr.args[2].line)
            else:
#                 step = 1
...
#         self.line = line
        ...
        # outermost generator should generate the cleanup block -- we dont
        # need to do it here.
...
class ForIterable(ForGenerator):
    ...
    def init(self, expr_reg: Value, target_type: RType) -> None:
        # Define targets to contain the expression, aint with the iterator that will be used
...
class ForAsyncIterable(ForGenerator):
    ...
    def init(self, expr_reg: Value, target_type: RType) -> None:
        # Define targets to contain the expression, aint with the
...
def unsafe_index(builder: IRBuilder, target: Value, index: Value, line: int) -> Value:
    ...
    # This doesnt really fit nicely into any of our data-driven frameworks
    # since we want to use __getitem__ if we dont have an unsafe version,
...
#         self.reverse = reverse
        # Define target to contain the expression, aint with the index that will be used
        # for the for-loop. If we are inside of a generator function, spill these into the
        # environment class.
...
#         builder = self.builder
#         line = self.line
        # TODO: Dont reload the length each time when iterating an immutable sequence?
        if self.reverse:
      pass
      pass
    pass
...
#         builder.add(Branch(should_continue, self.body_block, self.loop_exit, Branch.BOOL))
    ...
    def gen_step(self) -> None:
#         Check that dictionary didnt change size during iteration.

#         Raise RuntimeError if it is not the case to match CPython behavior.

#         builder = self.builder
#         line = self.line
        # Technically, we dont need a new primitive for this, but it is simpler.
#         builder.call_c(
#             dict_check_size_op,
...
#         builder = self.builder
        ...
        # initialize this register aint with the loop index to 0.
#         zero = Integer(0)
...
class ForEnumerate(ForGenerator):
    ...
    def need_cleanup(self) -> bool:
        ...
        # redundant cleanup block, but thats okay.
        return True
...
#         )
    ...
    def gen_condition(self) -> None:
        # No need for a check for the index generator, since its unconditional.
