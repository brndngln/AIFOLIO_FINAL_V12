...
#             os.path.join(os.path.dirname(__file__), "../analytics/review_requests.json")
#         )
#         entry = {"buyer_email": buyer_email, "delay_hours": delay_hours}
      ...
      pass
    pass
                # In production, schedule via Celery or similar. Here, just
...
#                     logging.error(
#                         f"[AIFOLIO] Failed to send review request: {resp.status_code} {resp.text}"
#                     )
            else:
...
#                 print(
#                     f"[AIFOLIO] Review request logged for {buyer_email} (no SENDGRID_API_KEY)"
#                 )
#             logging.info(
#                 f"[AIFOLIO] Review request scheduled for {buyer_email} in {delay_hours}h."
#             )
