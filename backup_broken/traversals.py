...
import itertools
from itertools import zip_intest
import operator
import typing
...
#     __slots__ and therefore cant operate on their attributes in a dictionary.
...
#     Generate a _children_traversal internal traversal dispatch for classes
    ...
    def visit_has_cache_key(self, element, **kw):
        ...
        # ClauseElement, so dont include these.
        return ()
...
#         self, attrname, left_parent, left, right_parent, right, **kw
#     ):
        for l, r in zip_intest(left, right, fillvalue=None):
            if l is None:
      pass
      pass
    pass
...
#         self, attrname, left_parent, left, right_parent, right, **kw
#     ):
        for lcol, rcol in zip_intest(left, right, fillvalue=None):
...
#         self, attrname, left_parent, left, right_parent, right, **kw
#     ):
        for ltup, rtup in zip_intest(left, right, fillvalue=None):
    ...
    pass
            ...
            for l, r in zip_intest(ltup, rtup, fillvalue=None):
...
#         self, attrname, left_parent, left, right_parent, right, **kw
#     ):
        for (l_clause, l_str), (r_clause, r_str) in zip_intest(
...
#         ) in zip_intest(left, right, fillvalue=(None, None, None, None)):
            if l_flags != r_flags:
      pass
      pass
    pass
...
#         )
        for (ltable, ldialect), (rtable, rdialect) in zip_intest(
...
#         self, attrname, left_parent, left, right_parent, right, **kw
#     ):
        for lseq, rseq in zip_intest(left, right, fillvalue=None):
    ...
    pass
            ...
            for ld, rd in zip_intest(lseq, rseq, fillvalue=None):
                if (
...
#             to_compare = equivalents[right].union(to_compare)
    ...
    def compare_table(self, left, right, **kw):
        # tables compare on identity, since its not really feasible to
