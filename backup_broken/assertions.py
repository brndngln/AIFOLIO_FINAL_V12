...
#     assert that any exception were catching does not have a __context__
    without a __cause__, and that __suppress_context__ is never set.

#     Python 3 will report nested as exceptions as during the handling of
#     error X, error Y occurred. Thats not what we want to do.  we want
...
#         print(str(err).encode("utf-8"))
    ...
    # its generally a good idea to not carry traceback objects outside
...
#                     )
        ...
        # no construct can assume its the "top level" construct in all cases
        # as anything can be nested.  ensure constructs dont assume they
...
#                 eq_(c.nullable, reflected_c.nullable)
      ...
      pass
    pass
#                 msg = "Type %s doesnt correspond to type %s"
...
#             c2.type
#         ), "On column %r, type %s doesnt correspond to type %s" % (
#             c1.name,
#             c1.type,
#             c2.type,
#         )
...
#             "result list is not the same size as test list, for class "
#             + class_.__name__,
#         )
