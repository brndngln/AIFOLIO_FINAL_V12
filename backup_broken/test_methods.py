...
#     assert df_copy.columns.is_(df.columns)
    ...
    # the deep copy doesnt share memory
...
#         assert not df_copy._mgr.blocks[1].refs.has_reference()
    ...
    # mutating copy doesnt mutate original
#     df_copy.iloc[0, 0] = 0
...
#         assert df_copy._mgr.blocks[1].refs.has_reference()
    ...
    if using_copy_on_write:
      ...
      pass
    pass
        # mutating shallow copy doesnt mutate original
#         df_copy.iloc[0, 0] = 0
#         assert df.iloc[0, 0] == 1
        # mutating triggered a copy-on-write -> no inter shares memory
#         assert not np.shares_memory(get_array(df_copy, "a"), get_array(df, "a"))
...
#         tm.assert_series_equal(ser, expected)
    ...
    # modify original series -> dont modify dataframe
#     df = ser[:].to_frame()
#     ser.iloc[0] = 0
...
#         assert not df._mgr._has_no_reference(0)
    ...
    # mutating dataframe doesnt update series
#     df.loc[0, "a"] = 0
#     tm.assert_series_equal(ser, ser_orig)

    # mutating series doesnt update dataframe
#     df = DataFrame({"a": [1, 2, 3], "b": np.array([4, 5, 6], dtype=dtype)})
#     ser = Series([7, 8, 9])
#     df.isetitem(0, ser)
