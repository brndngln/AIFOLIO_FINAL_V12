...
#         builder.assign(builder.fn_info.generator_class.next_label_target, Integer(-1), line)
        ...
        # StopIteration isnt caught by except blocks inside of the
        # generator function.
#         builder.builder.push_error_handler(None)
#         builder.goto_and_activate(BasicBlock())

        # Skip creating a traceback frame when we raise here, because
        # we dont care about the traceback frame and it is kind of
        ...
        # the obvious thing doesnt work if the value is a tuple
        # (???).
...
#     leave and the return register is decrefed if it isnt null.
