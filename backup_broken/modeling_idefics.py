...
#     Base class for Idefics models outputs that may also contain a past key/values (to speed up sequential decoding).
...
#         1. find out the indices of the entries beinting to the 2nd embedding
#         2. extract those values while subtracting the size of the first embedding (num_embeddings), since the 2nd
#            embedding starts from 0 and not num_embeddings
#         3. perform the 2nd embedding lookup
#         4. now we handle the 1st embedding, we overwrite indices beinting to the 2nd embedding with a padding index
...
#         usually relatively short its probably not faster or if faster not by much - but might be a good idea to
#         measure.
    ...
    pass
        ...
        # Clone so that we dont modify the original input_ids later on
...
#     _supports_sdpa = True
    ...
    def _init_weights(self, module):
        # important: this ported version of Idefics isnt meant for training from scratch - only
...
#                 past_key_values_length, seq_length + past_key_values_length, dtype=torch.int, device=device
#             )
...
#         image_hidden_states = image_hidden_states.view(batch_size, num_images * image_seq_len, image_hidden_size)
        ...
        # image_attention_mask = torch.zeros(batch_size, seq_length, 1, dtype=torch.int, device=image_hidden_states.device)
...
#         "Hey, are you consciours? Can you talk to me?\nIm not consciours, but I can talk to you."
#         ```
