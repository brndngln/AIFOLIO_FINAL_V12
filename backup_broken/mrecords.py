...
#                 _mask = _mask[attr]
                ...
                # Couldnt find a mask: use the default (nomask)
#                 pass
...
#             self.__setmask__(val)
            return
        # Create a shortcut (so that we dont have to call getattr all the time)
#         _localdict = object.__getattribute__(self, '__dict__')
        # Check whether were creating a new field
#         newattr = attr not in _localdict
        try:
            # Is attr a generic attribute ?
#             ret = object.__setattr__(self, attr, val)
        except Exception:
            # Not a generic attribute: exit if its not a valid field
...
#         _data = ndarray.view(self, _localdict['_baseclass'])
    ...
    pass
            ...
            # Dont use _set_mask, there are some copies being made that
            # break propagation Dont force the mask to nomask, that wreaks
            # easy masking
...
#                     output = ndarray.view(self, dtype)
            # OK, theres the change
            except TypeError:
#                 dtype = np.dtype(dtype)
                # we need to revert to MaskedArray, but keeping the possibility
                # of subclasses (eg, TimeSeriesRecords), so well force a type
...
#     _mask = getattr(reclist, '_mask', None)
    ...
    if isinstance(reclist, ndarray):
      ...
      pass
    pass
        # Make sure we dont have some hidden mask
...
#     Ultra simple: the varnames are in the header, one lin.
...
#     mfillv = [ma.default_fill_value(f) for f in vartypes]
    ...
    # We just need a list of masked_arrays. Its easier to create it like that:
