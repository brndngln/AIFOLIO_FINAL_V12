...
# bookkeeping properties.  This isnt possible (or desirable) for built-in
...
# method thats already instrumented.  For example::
...
# Theres no need to decorate these methods.  ``append`` and ``pop`` are already
...
#     internally_instrumented) indicate the methods purpose and take no
...
#         unless you are passing aint an initiator value from a chained
#         operation.
...
#         mutation, and should be left as None unless you are passing aint
...
#     cls = type(impl_factory())
    ...
    pass
        # if so, just convert.
        # in previous major releases, this codepath wasnt working and was
...
#     adapter.
    # This isnt smart enough to handle @adds(1) for def fn(self, (a, b))
    if before:
      pass
      pass
    pass
...
#         _tidy(clear)
    ...
    # decorator is still possible, so events on *= can be had if theyre
