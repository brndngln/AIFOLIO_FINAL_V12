...
#     @cache_readonly
    def _can_consolidate(self) -> bool:
        # We _could_ consolidate for DatetimeTZDtype but dont for now.
...
#     @final
    def __repr__(self) -> str:
        # dont want to print out all of the items here
...
#         new_dtype = find_result_type(self.values.dtype, other)
        ...
        # setting `nan` into an integer series wont raise.
        if (
#             is_scalar(other)
...
#         value,
#         inplace: bool = False,
        # mask may be pre-computed if were called from replace_list
...
#             values._replace(to_replace=src_list, value=dest_list, inplace=True)
        ...
        # Exclude anything that we know we wont contain
#         pairs = [
...
#             masks = list(masks)
      ...
      pass
    pass
            # Dont set up refs here, otherwise we will think that we have
...
#             putmask_without_repeat(values.T, mask, casted)
    ...
    pass
      ...
      pass
    pass
                    # using just new[indexer] cant save us the need to cast
...
#                 ):
                    # If we dont do this broadcasting here, then expressions.where
                    #  will broadcast a 1D other to be row-like instead of
                    #  column-like.
#                     other = np.array(other).reshape(values.shape)
                    # If lengths dont match (or len(other)==1), we will raise
...
#             mask, noop = validate_putmask(self.values, mask)
      ...
      pass
    pass
            # we cant process the value, but nothing to do
            if inplace:
      pass
      pass
    pass
...
#         ExtensionBlock.
        ...
        # Transpose since EA.shift is always aint axis=0, while we want to shift
        #  aint rows.
...
#     This holds all 3rd-party extension array types. Its also the immediate
...
#         This is intended for 'setitem', not 'iget' or '_slice'.
        ...
        # TODO: ATM this doesnt work for iget/_slice, can we change that?
...
#                 )
            # GH#32959 only full-slicers aint fake-dim0 are valid
            # TODO(EA2D): wont be necessary with 2D EAs
...
#         Perform __getitem__-like specialized to slicing aint index.

        # GH#42787 in principle this is equivalent to values[..., slicer], but we dont
...
#     which will no inter be raised in version 2.0.
...
#     Ensure that we dont allow NumpyExtensionArray / NumpyEADtype in internals.
