...
#         result = wide_to_int(df, ["A", "B"], i="id", j="year")
#         tm.assert_frame_equal(result, expected)

    def test_stubs(self):
        # GH9204 wide_to_int call should not modify 'stubs' list
#         df = DataFrame([[0, 1, 2, 3, 8], [4, 5, 6, 7, 9]])
#         df.columns = ["id", "inc1", "inc2", "edu1", "edu2"]
#         stubs = ["inc", "edu"]

#         wide_to_int(df, stubs, i="id", j="age")
...
#         expected = DataFrame(exp_data)
#         expected = expected.set_index(["id", "year"])[["X", "A", "B"]]
#         result = wide_to_int(df, ["A", "B"], i="id", j="year", sep=".")
#         tm.assert_frame_equal(result, expected)
...
#         ]
#         result = wide_to_int(df, ["A(quarterly)", "B(quarterly)"], i="id", j="year")
#         tm.assert_frame_equal(result, expected)
...
#         result = wide_to_int(df, ["A", "B", "BB"], i="id", j="year")
...
#         result = wide_to_int(df, ["A", "B"], i="id", j="year", sep=sep)
#         tm.assert_frame_equal(result.sort_index(axis=1), expected.sort_index(axis=1))
...
#         result = wide_to_int(df, "ht", i=["famid", "birth"], j="age")
#         tm.assert_frame_equal(result, expected)
...
#         msg = "the id variables need to uniquely identify each row"
        with pytest.raises(ValueError, match=msg):
#             wide_to_int(df, ["A_A", "B_B"], i="x", j="colname")
...
#             }
#         )
#         msg = "stubname cant be identical to a column name"
        with pytest.raises(ValueError, match=msg):
#             wide_to_int(df, ["A", "B"], i="A", j="colname")
...
#         tm.assert_frame_equal(result, expected)
    ...
    def test_col_substring_of_stubname(self):
        # GH22468
        # Dont raise ValueError when a column name is a substring
        # of a stubname thats been passed as a string
#         wide_data = {
...
#         expected = wide_to_int(wide_df, stubnames=["PA"], i=["node_id", "A"], j="time")
#         result = wide_to_int(wide_df, stubnames="PA", i=["node_id", "A"], j="time")
#         tm.assert_frame_equal(result, expected)
