...
#         result._cache = {}
        ...
        # weve already validated levels and codes, so shortcut here
...
#         ValueError
#             If length of levels and codes dont match, if the codes for any
...
#             Each array-like gives one levels value for each data point.
...
#         Raises a ValueError for `MultiIndex` because theres no single
...
#     @cache_readonly
    def levels(self) -> FrozenList:
        # Use cache_readonly to ensure that self.get_locs doesnt repeatedly
...
#         Formats each item in tup according to its levels formatter function.
...
#             sentinel: Literal["] | bool | lib.NoDefault = "
            # GH3547 use value of sparsify as sentinel if its "Falsey"
...
#         that it only acts on copies
        ...
        # GH 15110
        # Dont allow a single string for names in a MultiIndex
...
#                 lvl_indexer = np.asarray(k)
    ...
    pass
                # a collection of labels to include from this level (these are ord)
...
#                             raise err
                        ...
                        # GH 42351: No inter ignore not founds & enforced in 2.0
