...
#     assert issubclass(df.IntCol.dtype.type, np.integer)
    ...
    # MySQL has no real BOOL type (its an alias for TINYINT)
...
#         assert self.pandasSQL.to_sql(test_frame1.iloc[:0], "test_frame1") == 0
    ...
    def _to_sql_with_sql_engine(self, test_frame1, engine="auto", **engine_kwargs):
        ...
        # mostly copied from this classs `_to_sql()` method
#         self.drop_table("test_frame1", self.conn)
...
#             pytest.skip(f"Cant connect to {self.flavor} server")
#         self.load_iris_data(iris_path)
#         self.load_types_data(types_data)
...
#         df = sql.read_sql_table("types", self.conn)
        ...
        # IMPORTANT - sqlite has no native date type, so shouldnt parse, but
...
#         tm.assert_frame_equal(result, expected)
    ...
    def test_naive_datetimeindex_roundtrip(self):
        # GH 23510
        # Ensure that a naive DatetimeIndex isnt converted to UTC
...
#             assert isinstance(get_engine("sqlalchemy"), SQLAlchemyEngine)
    ...
    def test_get_engine_auto_error_message(self):
        ...
        # if engines arent installed vs. are installed but bad version
#         pass
...
#         cls.driver = "psycopg2"
    ...
    def test_schema_support(self):
        ...
        # only test this for postgresql (schemas not supported in
        # mysql/sqlite)
...
#             self.conn.exec_driver_sql("CREATE SCHEMA other;")
        ...
        # write dataframe to different schemas
...
#         tm.assert_frame_equal(concat([df, df], ignore_index=True), res)
        ...
        # The schema wont be applied on another Connection
