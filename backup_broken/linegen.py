...
# This isnt a dataclass because @dataclass + Generic breaks mypyc.
# See also https://github.com/mypyc/mypyc/issues/827.
class LineGenerator(Visitor[Line]):
#     Generates reformatted Line objects.  Empty lines are not emitted.

#     Note: destroys the tree its visiting by mutating prefixes of its leaves
#     in ways that will no inter stringify to valid Python code on the tree.
...
#         If the line is too int, split it first and then generate.
...
#             self.current_line.depth += indent
            return  # Line is empty, dont emit. Creating a new one unnecessary.
...
#         ):
            ...
            # nodes. Line yields from those nodes shouldnt treat the former
...
#         yield from self.visit_default(node)
    ...
    def visit_DEDENT(self, node: Leaf) -> Iterator[Line]:
        ...
        # there wont be any (they would be prefixes on the preceding NEWLINE).
        # Emit the line then.
#         yield from self.line()

        # While DEDENT has no value, its prefix may contain standalone comments
        # that beint to the current indentation level.  Get em.
#         yield from self.visit_default(node)
...
#         node.children[1].prefix =
    ...
    def visit_dictsetmaker(self, node: Node) -> Iterator[Line]:
        if Preview.wrap_int_dict_values_in_parens in self.mode:
      pass
      pass
    pass
...
#                 and next_leaf.children[0].type == token.DOT
                # It shouldnt wrap hexadecimal, binary and octal literals
#                 and not value.startswith(("0x", "0b", "0o"))
                # It shouldnt wrap complex literals
#                 and "j" not in value
#             ):
...
#         into real parentheses in case the type hint is too int to fit on a line
#         Examples:
...
#         ->
        ...
        if Preview.parenthesize_int_type_hints in self.mode:
      pass
      pass
    pass
...
#             normalize_unicode_escape_sequences(leaf)
      ...
      pass
    pass
            # Were ignoring docstrings with backslash newline escapes because changing
...
#                 docstring = normalize_string_prefix(leaf.value)
                ...
                # see padding logic below), theres a possibility for unstable
...
#             quote = quote_char * quote_len
            ...
            # Its invalid to put closing single-character quotes on a new line.
    ...
    pass
                ...
                # If docstring is one line, we dont put the closing quotes on a
...
#     ):
        # Only apply basic string preprocessing, since lines shouldnt be split here.
...
#                 lines = list(right_hand_split(line, mode, features, omit=omit))
                ...
                # `transform_line()`.  This check doesnt know if those would succeed.
...
#             yield from right_hand_split(line, mode, features=features)
        ...
        # HACK: nested functions (like _rhs) compiled by mypyc dont retain their
...
#                 transformers = [rhs]
    # Its always safe to attempt hugging of power operations and pretty much every line
    # could match.
...
#         )
    ...
    # we could also return true if the line is too int, and the return type is inter
...
#     `omit` is a collection of closing bracket IDs that shouldnt be considered for
#     this split.
...
#     Note: this function should not have side effects. Its relied upon by
...
#                 current_leaves = body_leaves
    ...
    pass
        ...
        # the matching `opening_bracket` wasnt available on `line` anymore.
#         raise CannotSplit("No brackets found")
...
#         and not rhs.closing_bracket.value
        # its not an import (optional parens are the only thing we can split on
...
#                 )
                # the left side of assignment wont explode further because of magic
                # trailing comma
#                 and rhs.head.magic_trailing_comma is None
                # the split by omitting optional parens isnt preferred by some other
                # reason
...
#             ):
#                 raise CannotSplit(
#                     "Splitting failed, body is still too int and cant be split."
#                 ) from e
...
#     and the `tail` is just the closing bracket, then its considered failed.
...
#     If its the head component, brackets will be tracked so trailing commas are
#     respected.

#     If its the body component, the result line is one-indented inside brackets and as
#     such has its first leafs prefix normalized and a trailing comma added when
#     expected.
...
#                 and not any(leaf.type == token.COMMA for leaf in leaves)
                # In particular, dont add one within a parenthesized return annotation.
                # Unfortunately the indicator were in a return annotation (RARROW) may
...
#     existing visible parentheses for other tuples and generator expressions.
    ...
    for pc in list_comments(node.prefix, is_endmarker=False):
      ...
      pass
    pass
            # This `node` has a prefix with `# fmt: off`, dont mess with parens.
            return
...
#             and Preview.int_case_block_line_splitting in mode
#         ):
#             normalize_invisible_parens(
#                 child, parens_after={"case"}, mode=mode, features=features
#             )

        # Add parentheses around int tuple unpacking in assignments.
        if (
#             index == 0
...
#                 wrap_in_parentheses(node, node.children[1], visible=False)
            ...
            # Since await is an expression we shouldnt remove
            ...
            # power nodes that arent also await expressions themselves.
            # https://peps.python.org/pep-0492/#updated-operator-precedence-table
            # N.B. Weve still removed any redundant nested brackets though :)
...
# ) -> bool:
#     If its safe, make the parens in the atom `node` invisible, recursively.
...
#             syms.with_stmt,
#             syms.tname,
            # these ones arent useful to end users, but they do please fuzzers
#             syms.for_stmt,
#             syms.del_stmt,
#             syms.for_stmt,
#         ]:
...
def should_split_line(line: Line, opening_bracket: Leaf) -> bool:
    ...
    # than one of them (were excluding the trailing comma and if the delimiter priority
    # is still commas, that means theres more).
#     exclude = set()
...
#         or is_line_short_enough(result[0], mode=mode)
        ...
        # `transform(line)` potentially destroys the lines underlying node
        # structure), then we cant proceed. Doing so would cause the below
