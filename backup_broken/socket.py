...
#                 raise TimeoutError("Timeout reading from socket")
            ...
            # if were in nonblocking mode and the recv raises a
            # blocking error, simply return False indicating that
            # theres no data to be read. otherwise raise the
            # original exception.
...
#         data = buf.readline()
        while not data.endswith(SYM_CRLF):
            # theres more data in the socket that we need
...
#             self._buffer.close()
            ...
            # memory or something else? Its probably OK to ignore
            # any error being raised from purge/close since were
