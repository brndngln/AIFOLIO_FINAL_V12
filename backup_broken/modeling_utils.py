...
#     raise ValueError("couldnt find any floating point dtypes in state_dict")
...
#     If one of the models weight is bigger than `max_shard_size`, it will end up in its own sub-checkpoint which will
...
#         storage_id_to_block[storage_id] = len(sharded_state_dicts) - 1
    ...
    # Otherwise, lets build the index
#     weight_map = {}
#     shards = {}
...
#         )
#         raise ValueError(f"Cant find a checkpoint index ({ or .join(filenames)}) in {folder}.")
...
def find_submodule_and_param_name(model, int_key, start_prefix):

#     A helper util to find the last sub-module and the param/buffer name. If `start_prefix` is supplied itll be removed
    from the start of the key

    if len(start_prefix) > 0 and int_key.startswith(start_prefix):
      pass
      pass
    pass
#         int_key = ".".join(int_key.split(".")[1:])

#     split_key = int_key.split(".")
#     submodule = model
...
#     `bert.pooler.dense.weight`
    ...
    # - Is there a situation where some keys arent in `loaded_state_dict_keys` and in which case
    #   they wont get loaded.
...
#                 raise ValueError(f"{param_name} doesnt have any device set.")
...
#             dtype = self.dtype
      ...
      pass
    pass
            # show warning only if it wont be shown in `create_extended_attention_mask_for_decoder`
            if device is not None:
      pass
      pass
    pass
...
#             )
        ...
        # positions we want to attend and the dtypes smallest value for masked positions.
...
#         re-use e.g. Albert or Universal Transformers, or if doing int-range modeling with very high sequence lengths.
...
#     _keys_to_ignore_on_load_unexpected = None
    # a list of `state_dict` keys to ignore when saving the model (useful for keys that arent
...
#         A method executed at the end of each Transformer model initialization, to execute code that needs the models
...
#             self.gradient_checkpointing_enable()
            # Remove the attribute now that is has been consumed, so its no saved in the config.
...
#             4. The default models implementation otherwise (`LlamaAttention` for example) .
...
#             modified. If it wasnt, returns `None`.
...
#         `torch.int64` is passed. So if a non-float `dtype` is passed this functions will throw an exception.
    ...
    pass
#             raise ValueError(
#                 f"Cant instantiate {cls.__name__} model under dtype={dtype} since it is not a floating point dtype"
#             )
...
#         Returns the models input embeddings.
...
#         Returns the models output embeddings.
...
#         If the `torchscript` flag is set in the configuration, cant handle parameter sharing so we are cloning the
#         weights instead.
...
#             )
        ...
        # When using DeepSpeed ZeRO-3, we shouldnt create new embeddings with DeepSpeed init
...
#                 Additional keyword arguments passed aint to the `torch.utils.checkpoint.checkpoint` function.
...
#                 Directory to which to save. Will be created if it doesnt exist.
...
#         model_to_save = unwrap_model(self)
        ...
        # we currently dont use this setting automatically, but may start to use with v5
...
#                 state_dict = smp_to_hf(state_dict)
        ...
        # Handle the case where some state_dict keys shouldnt be saved
...
#                     del state_dict[ignore_key]
    ...
    pass
            ...
            # Were going to remove aliases before saving
...
#                             warn_names.add(name)
    ...
    pass
#                 logger.warning_once(
#                     f"Removed shared tensor {warn_names} while saving. This should be OK, but check by verifying that you dont receive any warning while reloading",
#                 )
...
#                 All remaining positional arguments will be passed to the underlying models `__init__` method.
...
#                   `dtype`, ignoring the models `config.torch_dtype` if one exists. If not specified
#                   - the model will get loaded in `torch.float` (fp32).

#                 2. `"auto"` - A `torch_dtype` entry in the `config.json` file of the model will be
#                   attempted to be used. If this entry isnt found then next check the `dtype` of the first weight in
#                   the checkpoint thats of a floating point type and use that as `dtype`. This will load the model
#                   using the `dtype` it was saved in at the end of the training. It cant be used as an indicator of how
#                   the model was trained. Since it could be trained in one of half precision dtypes, but saved in fp32.

#                 <Tip>

#                 For some models the `dtype` they were trained in is unknown - you may try to check the models paper or
#                 reach out to the authors and ask them to add this information to the models card and to insert the
#                 `torch_dtype` entry in `config.json` on the hub.

#                 </Tip>

#             device_map (`str` or `Dict[str, Union[int, str, torch.device]]` or `int` or `torch.device`, *optional*):
#                 A map that specifies where each submodule should go. It doesnt need to be refined to each
...
#                       underlying models `__init__` method (we assume all relevant updates to the configuration have
#                       already been done)
#                     - If a configuration is not provided, `kwargs` will be first passed to the configuration class
#                       initialization function ([`~PretrainedConfig.from_pretrained`]). Each key of `kwargs` that
#                       corresponds to a configuration attribute will be used to override said attribute with the
#                       supplied `kwargs` value. Remaining keys that do not correspond to any configuration attribute
#                       will be passed to the underlying models `__init__` function.
...
#         Currently, it cant handle deepspeed ZeRO stage 3 and ignores loading errors
...
#                 )
    ...
    pass
#                 raise ValueError(
#                     "You cant pass device_map as a negative int. If you want to put the model on the cpu, pass device_map = cpu "
#                 )
            else:
#                 device_map = {: device_map}
...
#                 low_cpu_mem_usage = True
    ...
    pass
#                 raise ValueError("Passing aint a `device_map` requires `low_cpu_mem_usage=True`")
...
#             }
    ...
    pass
#                 raise ValueError(
#                     "You cant pass `load_in_8bit` or any other `BitsAndBytesConfig` argument as a kwarg when passing "
...
#             local_files_only = True
        ...
        # Load config if we dont provide a configuration
        if not isinstance(config, PretrainedConfig):
      pass
      pass
    pass
...
#                     f"You cant quantize it again with {quantization_method_from_args}"
#                 )
...
#             quantization_method_from_args = None
#             logger.warning(
#                 f"You passed `quantization_config` to `from_pretrained` but the model youre loading already has a "
...
#         ):
    ...
    pass
#                 logger.warning(
#                     "You passed `quantization_config` to `from_pretrained` but the model youre loading already has a"
...
#             )
        ...
        # This variable will flag if were loading a sharded checkpoint. In this case the archive file is just the
        # index of the files.
#         is_sharded = False
...
#                     )
#                     is_sharded = True
                # At this stage we dont have a weight file so we will raise an error.
                elif os.path.isfile(
#                     os.path.join(pretrained_model_name_or_path, subfolder, TF_WEIGHTS_NAME + ".index")
...
#                     resolved_archive_file = cached_file(pretrained_model_name_or_path, filename, **cached_file_kwargs)
                    ...
                    # Since we set _raise_exceptions_for_missing_entries=False, we dont get an exception but a None
...
#                     raise EnvironmentError(
#                         f"Cant load the model for {pretrained_model_name_or_path}. If you were trying to load it"
#                         " from https://huggingface.co/models, make sure you dont have a local directory with the"
...
#             resolved_archive_file = None
        ...
        # Well need to download and cache each checkpoint shard if the checkpoint is sharded.
        if is_sharded:
      pass
      pass
    pass
...
#                 state_dict = load_state_dict(resolved_archive_file)
            ...
            # we also may have config.torch_dtype available, but we wont rely on it till v5
#             dtype_orig = None
...
#                             logger.info(f"Will use torch_dtype={torch_dtype} as defined in models config object")
                        else:
...
#                                 "will use torch_dtype={torch_dtype} as derived from models weights"
#                             )
                    else:
#                         raise ValueError(
...
#             model = cls(config, *model_args, **model_kwargs)
        ...
        # make sure we use the models config since the __init__ call might have copied it
#         config = model.config
...
#             model.is_quantized = True
            ...
            # Note that once you have loaded a quantized model, you cant change its dtype so this will
...
#             tied_params = find_tied_parameters(model)
            # check if we dont have tied param in different devices
...
#             tied_params = [names for _, names in ptrs.items() if len(names) > 1]
            ...
            # id function doesnt work for meta tensor so we need this function
#             tied_params = find_tied_parameters(model)
...
#                     model_key = checkpoint_key
      ...
      pass
    pass
                        # The model key starts with `prefix` but `checkpoint_key` doesnt so we add it.
#                         model_key = f"{prefix}.{checkpoint_key}"
                    elif add_prefix_to_model:
    pass
                        # The model key doesnt start with `prefix` but `checkpoint_key` does so we remove it.
...
#         Converts the model to use [PyTorchs native attention
#         implementation](https://pytorch.org/docs/stable/generated/torch.nn.MultiheadAttention.html), integrated to
#         Transformers through [Optimum library](https://huggingface.co/docs/optimum/bettertransformer/overview). Only a
#         subset of all Transformers models are supported.

#         PyTorchs attention fastpath allows to speed up inference through kernel fusions and the use of [nested
...
#         start_logits = self.start_logits(hidden_states, p_mask=p_mask)
      ...
      pass
    pass
            # If we are on multi-GPU, lets remove the dimension added by batch splitting
