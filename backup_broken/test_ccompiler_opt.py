...
#             assert(len(opt.cpu_dispatch_names()) == 0)
    ...
    pass
            return
        # check sanity of arguments validation
        for baseline, dispatch in (
#             ("unkown_feature - max +min", "unknown max min"), # unknowing features
#             ("#avx2", "$vsx") # groups and polices arent acceptable
#         ) :
            try:
...
#             unknown =
#         )
        # any features in cpu_dispatch must be ignored if its part of baseline
#         self.expect(
...
#         special cases
        ...
        # in icc and msvc, FMA3 and AVX2 cant be separated
...
#                 x86_icc=diff, x86_iccw=diff
#             )
        # in msvc, following features cant be separated too
...
#                 )
        ...
        # in ppc64le VSX and VSX2 cant be separated
#         self.expect_baseline("vsx", ppc64le="vsx vsx2")
        # in aarch64 following features cant be separated
...
#             x86=", ppc64le="
#         )
        # an exception must triggered if native flag isnt supported
...
#             s390x="vxe vxe2"
#         )
        # autovec, skipping features that cant be
