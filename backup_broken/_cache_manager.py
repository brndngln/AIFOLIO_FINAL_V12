...
#         </Tip>
        ...
        # Deletion order matters. Blobs are deleted in last so that the user cant end
...
#         }),
#         warnings=[
#             CorruptedCacheException("Snapshots dir doesnt exist in cached repo: ..."),
...
#     refs_path = repo_path / "refs"
    ...
    if not snapshots_path.exists() or not snapshots_path.is_dir():
      ...
      pass
    pass
#         raise CorruptedCacheException(f"Snapshots dir doesnt exist in cached repo: {snapshots_path}")
...
#     Vaguely inspired by Djangos `timesince` formatter.
...
#         logger.warning(f"Couldnt delete {path_type}: file not found ({path})", exc_info=True)
    except PermissionError:
#         logger.warning(f"Couldnt delete {path_type}: permission denied ({path})", exc_info=True)
