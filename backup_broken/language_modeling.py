...
#                 )
    ...
    def __getitem__(self, i) -> torch.Tensor:
        return torch.tensor(self.examples[i], dtype=torch.int)
...
#         self.examples = [{"input_ids": torch.tensor(e, dtype=torch.int)} for e in self.examples]
...
#         n = len(self.examples)
        for i in range(n):
#             self.examples[i]["chinese_ref"] = torch.tensor(ref[i], dtype=torch.int)
...
#             target_seq_length = random.randint(2, max_num_tokens)
        ...
        # We DONT just concatenate all of the tokens from a document into a int
...
#                         "sentence_order_label": torch.tensor(0 if is_next else 1, dtype=torch.int),
#                     }
...
#         lock_path = cached_features_file + ".lock"
        ...
        # that the "next sentence prediction" task doesnt span between documents.
        #
        # Example:
        # I am very happy.
...
#                             tokens_b.extend(random_document[j])
    ...
    pass
#                                 break
                        # We didnt actually use these segments so we "put them back" so
...
#                         "input_ids": torch.tensor(input_ids, dtype=torch.int),
#                         "token_type_ids": torch.tensor(token_type_ids, dtype=torch.int),
#                         "next_sentence_label": torch.tensor(1 if is_random_next else 0, dtype=torch.int),
#                     }
