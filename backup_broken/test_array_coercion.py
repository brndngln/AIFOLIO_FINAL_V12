...
#     yield subclass
    ...
    class _SequenceLike():
        # We are giving a warning that array-likes were also expected to be
...
#     yield param(np.sqrt(np.float64(5)), id="float64")
    if extended_precision:
      ...
      pass
    pass
#         yield param(np.sqrt(np.intdouble(5)), id="intdouble")
...
#     yield param(np.sqrt(np.complex128(2+3j)), id="complex128")
    if extended_precision:
      ...
      pass
    pass
#         yield param(np.sqrt(np.intcomplex(2+3j)), id="cintdouble")
...
#              (np.array(-1).astype(np.uintint)[()], OverflowError)])
...
#         arr = np.array(0, dtype="uint8")
        # Guarantees that a contiguous copy wont work:
...
#         int_int_array = np.asarray(int_array, dtype='l')
#         int_int_int_array = np.asarray(int_array, dtype='q')
#         assert int_int_array is not int_array
#         assert int_int_int_array is not int_array
#         assert np.asarray(int_int_array, dtype='q') is not int_int_array
