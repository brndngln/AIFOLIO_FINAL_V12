...
#         layer. For better performance and reliability, its recommended to use `HfApi` methods when possible.
...
#         self._api = HfApi(endpoint=endpoint, token=token)
        ...
        #  * the 2nd element being the exception raised if the repository or revision doesnt exist
...
#         path = self._strip_protocol(path)
      ...
      pass
    pass
            # cant list repositories at root
#             raise NotImplementedError("Access to repositories lists is not implemented.")
    ...
    pass
      ...
      pass
    pass
                # cant list repositories at the repository type level
#                 raise NotImplementedError("Access to repositories lists is not implemented.")
...
#         commit_message += fup to depth {maxdepth}  if maxdepth is not None else
...
#             "expand_info", True
#         )  # dont expose it as a parameter in the public API to follow the spec
        if not resolved_path.path_in_repo:
      pass
      pass
    pass
...
#         unhandled_kwargs = set(kwargs.keys()) - {"revision"}
      ...
      pass
    pass
            # for now, lets not handle custom callbacks
            # and lets not handle custom kwargs
