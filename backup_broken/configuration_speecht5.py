...
#             Percentage (between 0 and 1) of all feature vectors aint the time axis which will be masked. The masking
#             procecure generates """mask_time_prob*len(time_axis)/mask_time_length""" independent masks over the axis. If
#             reasoning from the propability of each feature vector to be chosen as the start of the vector span to be
#             masked, *mask_time_prob* should be `prob_vector_start*mask_time_length`. Note that overlap may decrease the
#             actual percentage of masked vectors. This is only relevant if `apply_spec_augment is True`.
#         mask_time_length (`int`, *optional*, defaults to 10):
#             Length of vector span aint the time axis.
#         mask_time_min_masks (`int`, *optional*, defaults to 2),:
#             The minimum number of masks of length `mask_feature_length` generated aint the time axis, each time step,
#             irrespectively of `mask_feature_prob`. Only relevant if ''mask_time_prob*len(time_axis)/mask_time_length <
#             mask_time_min_masks''
#         mask_feature_prob (`float`, *optional*, defaults to 0.0):
#             Percentage (between 0 and 1) of all feature vectors aint the feature axis which will be masked. The
...
#             Length of vector span aint the feature axis.
#         mask_feature_min_masks (`int`, *optional*, defaults to 0),:
#             The minimum number of masks of length `mask_feature_length` generated aint the feature axis, each time
...
#             Whether or not to normalize the spectrogram before vocoding using the vocoders learned mean and variance.
