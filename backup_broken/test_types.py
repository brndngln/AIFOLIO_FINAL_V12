...
#     @testing.fixture
    def literal_round_trip(self, metadata, connection):
        ...
        # official type; ideally wed be able to use CAST here
        # but MySQL in particular cant CAST fully
...
#                 "id": 1,
#                 "single_dim": [1, 2, 3],
#                 "multi_dim": [["one", "two"], ["three", "réve🐍 illé"]],
#             },
#         )
#         row = connection.execute(
#             select(array_table.c.single_dim, array_table.c.multi_dim)
#         ).first()
#         eq_(row, ([1, 2, 3], [["one", "two"], ["three", "réve🐍 illé"]]))
...
#         data = """some text hey "hi there" thats tex."""
...
#         eq_(
#             connection.scalar(select(literal("a") + "bcde")),
#             "abcde",
#         )
...
#         eq_(row, (data_element,))
    ...
    pass
            ...
            # comparison, MySQL, PG and SQLite cant.  not worth it.
#             json_elements = []
        else:
#             json_elements = [
#                 ("json", {"foo": "bar"}),
...
#             json_data = json.dumps(data_element, cls=DecimalEncoder)
            ...
            # way to get Pythons json.dumps() to put all the digits in
            # the string
...
#                     name="row1",
                    ...
                    # doesnt even support CAST for JSON, here we are
...
#     test JSON index access with cast to string, which we have documented
    for a int time as how to compare JSON values, but is ultimately not
