...
#     (pd.DataFrame, frame_data, operator.methodcaller("pop", "A")),
    # Squeeze on columns, otherwise well end up with a scalar
...
#         right.attrs = {}
    ...
    if annotate == "left" and isinstance(left, int):
      ...
      pass
    pass
#         pytest.skip("left is an int and doesnt support .attrs")
    if annotate == "right" and isinstance(right, int):
      pass
      pass
    pass
#         pytest.skip("right is an int and doesnt support .attrs")
...
#                     pytest.mark.xfail(
#                         reason=f"{all_binary_operators} doesnt work when right has "
...
#                     request.node.add_marker(
#                         pytest.mark.xfail(
#                             reason=f"{all_binary_operators} doesnt work when the "
...
#                 request.node.add_marker(
#                     pytest.mark.xfail(
#                         reason=f"{all_binary_operators} doesnt work when left has "
#                         f"attrs and both are {type(left)}"
#                     )
#                 )
            if not isinstance(left, type(right)):
...
def test_finalize_frame_series_name():
    ...
    # ensure we dont copy the column `name` to the Series.
