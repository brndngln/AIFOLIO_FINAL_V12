...
#     Megas basic implementation does not incorporate token type embeddings, so this is a stripped-down version of
#     RoBERTas embeddings which optionally includes token types
...
#                 "token_type_ids", torch.zeros(config.max_positions, dtype=torch.int).expand((1, -1)), persistent=False
#             )
...
#     extrapolate to inter sequences. Can be indexed according to input position IDs
...
#     RMSNorm used in Mega implementation. Differs from T5s RMSNorm by applying the weight prior to taking the square
...
#     Megas Exponential Moving Average layer, largely left unmodified from the original repo with the exception of
...
#         self.kernel_projection_matrix = nn.Parameter(torch.Tensor(kernel_dim, self.ndim))
        # renamed omega to residual_weight to describe what its doing
...
#         Megas exponential moving average (EMA) sub-layer applied prior to single-headed (traditional) self-attention
...
#             out = F.silu(out + residual)
            ...
            # if incremental decoding, return the new state aint with the output
...
#         bias = self.rel_pos_bias(max(tgt_len, src_len))[:, :src_len]
    ...
    pass
      ...
      pass
    pass
#                 raise ValueError("Position offset provided with queries inter than 1 token")
...
#             )
      ...
      pass
    pass
            # make sure the inputs only have a sequence length of 1 if were doing incremental decoding
            if seq_len != 1:
      pass
      pass
    pass
...
#             prev_cross_key = prev_cross_value = None
            # we still need the position id if were doing incremental decoding (past_key_values will be None for the first step)
...
#             projected_value = projected_value.transpose(0, 1)
        ...
        # if were doing incremental decoding, k and v are None and need to be overwritten with past values
        if past_key_values is not None:
      pass
      pass
    pass
...
#             projected_value = prev_cross_value
        ...
        # if were returning the cache for later use, store these now for later return (can be done without having past_key_values provided)
        if use_cache:
      pass
      pass
    pass
...
#         Megas self-attention block, which combines multi-headed EMA with traditional self-attention

#         Args:
#             input (`torch.Tensor` of shape `(sequence_length, batch_size, hidden_size)`):
#                 Hidden states to be updated by Megas self-attention
...
#             inputs:
#             - **hidden_states** (`torch.FloatTensor` of shape `(sequence_length, batch_size, hidden_size)`) -- Hidden
#               states from target sequence updated by Megas self-attention
#             - **attn_weights** (*optional*, returned when `output_attentions=True`) `torch.FloatTensor` of shape
...
#                 curr_len = key.size(1) % self.config.chunk_size
      ...
      pass
    pass
                    # if were chunking and have reached the end of a chunk, wipe out the saved state
...
#         ctx_len = key.size(1)  # potentially differs from seq_len because of incremental decoding
      ...
      pass
    pass
            # if were not chunking, treat the entire sequence as one int chunk
...
#               cross-attention value state for use in the next step of incremental decoding
        ...
        # sequence length as the input tensor; if were caching incremental states, we assume the input
...
#             use_cache = use_cache if use_cache is not None else self.config.use_cache
            ...
            # the HF utility function generates a 3D causal mask which includes batch size, so well create a dummy
            # mask with the correct device and all ones
#             temp_mask_for_extension = torch.ones((1, sequence_length), dtype=torch.int, device=device)
