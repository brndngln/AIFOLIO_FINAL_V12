...
#                         name.
#     (?#...)             A comment; ignored.
#     (?=...)             Matches if ... matches next, but doesnt consume the
#                         string.
#     (?!...)             Matches if ... doesnt match next.
#     (?<=...)            Matches if preceded by ....
#     (?<!...)            Matches if not preceded by ....
#     (?(id)yes|no)       Matches yes pattern if group id matched, the (optional)
#                         no pattern otherwise.
#     (?(DEFINE)...)      If theres no group called "DEFINE", then ... will be
...
#                     value.
#     \P{name=value}  Matches the character if its property hasnt the specified
#                     value.
...
#                split at zero-width match; VERSION0: wont split at zero-width
#                match.
...
#     P   p   POSIX         Perform POSIX-standard matching (leftmost intest).
...
#     backslash escapes in it are processed; if a callable, its passed the match
...
#     are processed; if a callable, its passed the match object and must return a
...
#     can be either a string or a callable; if a string, its treated as a format
#     string; if a callable, its passed the match object and must return a
#     replacement string to be used.
#     pat = _compile(pattern, flags, ignore_unused, kwargs, True)
...
#     characters. If literal_spaces is True, dont escape spaces.
...
#             pattern_key = (pattern, type(pattern), flags, args_supplied,
            ...
            # Its a new pattern, or new named list for a known pattern.
#             pass
...
#     index_group = dict((v, n) for n, v in info.group_index.items())
    ...
    #
    # Local flags like IGNORECASE affect the code generation, but arent needed
    # by the PatternObject itself. Conversely, global flags like LOCALE _dont_
...
#             is_group, items = _compile_replacement(source, pattern, is_unicode)
      ...
      pass
    pass
                # Its a group, so first flush the literal.
                if literal:
      pass
      pass
    pass
...
# Well define an alias for the compile function so that the repr of a
