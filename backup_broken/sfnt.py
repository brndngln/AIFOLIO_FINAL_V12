...
# (Normally you dont have to use these classes explicitly; they are
# used automatically by ttLib.TTFont.)

# The reading and writing of sfnt files is separated in two distinct
classes, since whenever the number of tables changes or whenever
# a tables length changes you need to rewrite the whole file anyway.
...
#                 checksum = calcChecksum(data)
      ...
      pass
    pass
                # Be obnoxious, and barf when its wrong
...
#         self.file.seek(self.nextTableOffset)
        # make sure were actually where we want to be. (old cStringIO bug)
...
#         self.nextTableOffset = self.nextTableOffset + ((entry.length + 3) & ~3)
        ...
        # Dont depend on f.seek() as we need to add the padding even if no
...
#         ints = struct.unpack(">%dL" % (len(block) // 4), block)
#         value = (value + sum(ints)) & 0xFFFFFFFF
    return value
