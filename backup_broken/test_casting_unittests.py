...
# Simple skips object, parametric and int double (unsupported by struct)
...
#             length += 1  # adds one character for the sign
    ...
    # Note: Cant do dtype comparison for intdouble on windows
    if dtype.char == "g":
      pass
      pass
    pass
...
#         assert np.can_cast(floating, string)
        # 100 is int enough to hold any formatted floating
...
#         assert isinstance(res_dt, string_DT)
        ...
        # check the cast-safety for too int/fixed string lengths:
...
#         string_dt_int = self.string_with_modified_length(string_dt, 1)
#         str_arr_int = np.zeros(len(orig_arr), dtype=string_dt_int)
...
#             _, str_arr_int = self.get_data_variation(
#                 orig_arr, str_arr_int, True, contig)
...
#             cast._simple_strided_call((other_arr, str_arr_int))
#             assert_array_equal(str_arr, str_arr_int)
...
#             other_arr[...] = 0
#             cast_back._simple_strided_call((str_arr_int, other_arr))
#             assert_array_equal(orig_arr, other_arr)
...
#              None),
            # different number of fields cant work, should probably just fail
...
#             self, from_dt, to_dt, expected_off):
        ...
        # really test some paths that may have nonzero offsets (they dont
        # really exists).
        from_dt = np.dtype(from_dt)
#         to_dt = np.dtype(to_dt)
#         cast = get_castingimpl(type(from_dt), type(to_dt))
...
#     @pytest.mark.parametrize("dtype", np.typecodes["All"])
    def test_object_casts_NULL_None_equivalence(self, dtype):
        # None to <other> casts may succeed or fail, but a NULLed array must
        # behave the same as one filled with Nones.
