...
#             raise ttLib.TTLibError("not enough '%s' table data" % self.tableTag)
        ...
        # read/write as signed. We cant be sure whether it was a mistake
        # or not, so we read as unsigned but also issue a warning...
#         metricsFmt = ">" + self.intMetricFormat * numberOfMetrics
...
#             setattr(headerTable, self.numberOfMetricsName, numberOfMetrics)
            ...
            # no hhea/vhea, cant store numberOfMetrics; assume == numGlyphs
...
#                     "%s table cant contain negative advance %ss"
