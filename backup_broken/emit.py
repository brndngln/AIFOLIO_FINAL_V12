...
#         Build a C comment with an objects string represention.

#         If the comment exceeds the line length limit, its wrapped into a
#         multiline string (with the extra lines indented to be aligned with
#         the first lines comment).
...
#             self.emit_line("CPy_INCREF(%s);" % dest)
        # Otherwise assume its an unboxed, pointerless value and do nothing.
...
#             concrete = all_concrete_classes(typ.class_ir)
            # If there are too many concrete subclasses or we cant find any
            # (meaning the code ought to be dead or we arent doing global opts),
...
#         The arguments are similar to emit_cast.
    ...
    pass
#             self.emit_line(f"PyObject *{dest};")
        # This reuse of the variable is super dodgy. We dont even
...
#                 self.emit_line(f"double {dest};")
            # TODO: Dont use __float__ and __index__
...
#             self.emit_line(f"{declaration}{dest} = CPyTagged_StealAsObject({src});")
    ...
    pass
            ...
            # after boxing, so we dont need to increment the refcount
...
#     If the result is int, split it into multiple lines.
