...
def test_apply_np_transformer(float_frame, op, how):
    ...
    #  trigger the warning here, but lets put one in just to be sure
...
#         warn = RuntimeWarning
    ...
    with tm.assert_produces_warning(warn, check_stacklevel=False):
        # float_frame fixture is defined in conftest.py, so we dont check the
...
def test_agg_cython_table_transform_frame(df, func, expected, axis):
    ...
    if axis in ("columns", 1):
      ...
      pass
    pass
        # operating blockwise doesnt let us preserve dtypes
