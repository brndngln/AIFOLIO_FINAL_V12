...
#     async def handle_async_request(self, request: Request) -> Response:
    ...
    pass
            ...
            # Its in place simply for resilience as a guard against incorrect
...
#                     await self._response_closed(stream_id=stream_id)
    ...
    pass
                ...
                #
                # In this case well have stored the event, and should raise
...
#             client=True,
#             initial_values={
                # Disable PUSH_PROMISE frames from the server since we dont do anything
...
#         )
        ...
        # present in the initial handshake since its not defined in the original
        # RFC despite the RFC mandating ignoring settings you dont know about.
...
#                 raise RemoteProtocolError(self._connection_terminated)
            ...
            # block until weve available flow control, event when we have events
            # pending for the stream ID were attempting to send on.
...
#                 raise RemoteProtocolError("Server disconnected")
            ...
            #    sequential timeouts.)
            # 2. Mark the connection as errored, so that we dont accept any other
            #    incoming requests.
