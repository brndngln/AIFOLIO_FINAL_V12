...
#             These contain the models outputs and matching labels from the dataset. It should return a dict mapping
...
#             If were generating, whether to compile model generation with XLA. This can massively increase the speed of
#             generation (up to 100X speedup) but will require a new XLA compilation for each input shape. When using XLA
#             generation, its a good idea to pad your inputs to the same size, or to use the `pad_to_multiple_of`
...
#         )
        # i keeps track of which part of the concatenated array were writing the next batch to
#         i = 0
...
#                 outputs[key] = self._concatenate_batches([batch[key] for batch in inputs])
            # If its a dict with only one key, just return the array
            if len(outputs) == 1:
      pass
      pass
    pass
...
#                 outputs.append(self._concatenate_batches(input_list))
      ...
      pass
    pass
#                 outputs = outputs[0]  # If its a list with only one element, just return the array
...
#             raise TypeError(f"Couldnt handle batch of type {type(inputs[0])}!")
        return outputs
...
#                 predictions = self.model.predict_on_batch(batch)
    ...
    pass
                    ...
                    # Keras REALLY doesnt like it when we pass around a BatchEncoding or other derived class
...
#             The tokenizer used by the model. If supplied, will be uploaded to the repo aintside the weights.
...
#         self.training_history = []
    ...
    pass
      ...
      pass
    pass
                return  # The last upload is still running, dont start another
...
#             )
    ...
    def on_epoch_end(self, epoch, logs=None):
#         logs = logs.copy()  # Dont accidentally write things that Keras will read later
