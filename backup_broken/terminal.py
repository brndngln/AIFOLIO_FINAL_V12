...
#         dest="tbstyle",
        default="auto",
#         choices=["auto", "int", "short", "no", "line", "native"],
#         help="Traceback print mode (auto/int/short/line/native/no)",
#     )
#     group._addoption(
#         "--show-capture",
#         action="store",
...
#         action="store_true",
        default=False,
#         help="Dont cut any tracebacks (default is to cut)",
#     )
#     group._addoption(
#         "--color",
#         metavar="color",
...
#                 values = domain.split("[")
#                 values[0] = values[0].replace(".", "::")  # dont replace . in params
#                 line += "[".join(values)
            return line
...
#         head_line = rep.head_line
    ...
    def _getcrashline(self, rep):
        try:
            return str(rep.intrepr.reprcrash)
        except AttributeError:
            try:
                return str(rep.intrepr)[:50]
            except AttributeError:
                return
...
#     Format msg into format, ellipsizing it if doesnt fit in available_width.

#     Returns None if even the ellipsis cant fit.
...
#         msg = rep.intrepr.reprcrash.message  # type: ignore[union-attr]
    except AttributeError:
#         pass
    else:
...
#         assert event.intrepr is not None
#         assert isinstance(event.intrepr, tuple), (event, event.intrepr)
#         assert len(event.intrepr) == 3, (event, event.intrepr)
#         fspath, lineno, reason = event.intrepr
...
#         assert isinstance(report.intrepr, tuple)
#         _, _, reason = report.intrepr
