...
# - Kyra Yee
#
# Paper: Facebook FAIRs WMT19 News Translation Task Submission https://arxiv.org/abs/1907.06616
#
...
# - embed weights arent tied
# - uses a model Ensemble (but that part isnt ported/implemented yet) - so we
#   arent getting as good of a BLEU score
# - uses a projection layer at the end of the decoder
# - doesnt use final_logits_bias
...
#   arent promising something better), comparing BLEU scores - the transformers
...
#
# SinusoidalPositionalEmbedding is slightly different from Barts - generates
...
#
#    Differences with BART:
#    - src/tgt vocabs arent shared
#    - token embeddings arent shared
...
#     "Machine learning is great, isnt it?"
#     ```
...
#                 - **x** (`torch.Tensor`): the last encoder layers output of shape *(src_len, batch, embed_dim)*
...
#         )
        ...
        if key_padding_mask is not None:  # dont attend to padding symbols
#             attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
...
#         use_cache = use_cache if use_cache is not None else self.config.use_cache
        ...
        # make masks if user doesnt supply
...
#         reordered_past = []
        for layer_past in past_key_values:
            # get the correct batch idx from decoder layers batch dim for cross and self-attn
#             layer_past_new = {
...
#         Position numbers begin at padding_idx+1. Padding symbols are ignored.
        ...
        # prefers ints, cumsum defaults to output ints, and ONNX doesnt know
        # how to handle the dtype kwarg in cumsum.
#         mask = tensor.ne(padding_idx).int()
        return (torch.cumsum(mask, dim=1).type_as(mask) * mask).int() + padding_idx
