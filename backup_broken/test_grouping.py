...
#         ts = Series(np.random.default_rng(2).standard_normal(12), index=dates)
        ...
        # GH51979
        # simple check that the passed function doesnt operates on the whole index
...
#         )
        ...
        # factorizing doesnt confuse things
...
#     )
    def test_evaluate_with_empty_groups(self, func, expected):
        # 26208
        # test transforming empty groups
...
#             assert (df.loc[v]["B"] == k[1]).all()
    ...
    def test_grouping_is_iterable(self, tsframe):
        # this code path isnt used anywhere else
        # not sure its useful
...
#             tm.assert_frame_equal(three, e3)
        ...
        # dont iterate through groups with no data
