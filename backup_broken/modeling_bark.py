...
#         tensor = tensor.view(tensor.size()[:-2] + (num_heads * attn_head_size,))
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        # unlike GPTNeos SelfAttention, divide by the square root of the dimension of the query and the key
...
#             associated vectors than the models internal embedding lookup matrix.
...
#             past_length = past_key_values[0][0].size(-2)
      ...
      pass
    pass
#             position_ids = torch.arange(past_length, seq_length + past_length, dtype=torch.int, device=device)
...
#                 int as the intest generation among the batch.
...
#         )
        ...
        # (except to get the input seq_len - thats why we keep the first 257 tokens)
...
#         device = input_ids.device if input_ids is not None else input_embeds.device
      ...
      pass
    pass
#             position_ids = torch.arange(0, seq_length, dtype=torch.int, device=device)
#             position_ids = position_ids.unsqueeze(0)  # shape (1, seq_length)
...
#             raise ValueError("`fine_generation_config` has to be provided")
        ...
        # since we dont really use GenerationConfig through the fine model (autoencoder)
...
#         device = torch.device(f"cuda:{gpu_id}")
    ...
    pass
#             self.to("cpu")
#             torch.cuda.empty_cache()  # otherwise we dont see the memory savings (but they probably exist)
...
#         _, hook = cpu_offload_with_hook(self.codec_model, device, prev_module_hook=hook)
        ...
        # Well offload the last model manually.
...
#         Args:
#             input_ids (`Optional[torch.Tensor]` of shape (batch_size, seq_len), *optional*):
#                 Input ids. Will be truncated up to 256 tokens. Note that the output audios will be as int as the
#                 intest generation among the batch.
#             history_prompt (`Optional[Dict[str,torch.Tensor]]`, *optional*):
...
#         )
    ...
    pass
            ...
            # since bark doesnt use codec_model forward pass
...
#         `_check_and_enable_flash_attn_2` originally dont expand flash attention enabling to the model
#         sub-configurations. We override the original method to make sure that Bark sub-models are using Flash Attention
        if necessary.

#         If you dont know about Flash Attention, check out the official repository of flash attention:
