...
# The unit vectors X and Y components are respectively
#   U = (cos(α), sin(α))
...
def _rounding_offset(direction):
    ...
    # (-1.0, 0.0) if its pointing West, etc.
...
#     The rounding of circle coordinates to integers may cause an abrupt change
    if the start circle c0 is so close to the end circle c1s perimiter that
...
#     dx, dy = _rounding_offset(direction)
    ...
    # know the start circle didnt keep containment after normal rounding; thus
    ...
    # both the start and end circles centres and radii will be rounded in opposite
    # directions, e.g. when they move aint a 45 degree diagonal:
    #   c0 = (1.5, 1.5) ===> (2.0, 2.0)
    #   r0 = 0.5 ===> 1.0
...
#     max_attempts = 2
    for _ in range(max_attempts):
      ...
      pass
    pass
            # cant move c0 towards c1 (they are the same), so we change the radius
