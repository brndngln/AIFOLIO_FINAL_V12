...
#         CSS protected characters but used as separators in Excels format string.
...
#         in the DataFrames index or column headers. If ``formatter`` is ``None``,
...
#                 f"visible labels aint ``axis`` ({visible_len})."
#             )
...
#     max_index : int
#         The maximum number of elements to analyse aint the index due to trimming
...
#     Ensure that a slice doesnt reduce to a Series or Scalar.

#     Any user-passed `subset` should have this called on it
#     to make sure were always working with DataFrames.
...
#         dependent upon the users chosen display string.
...
#     res.append(_escape_latex(s[pos : len(s)]))
    return .join(res).replace(rrt8§=§7wz, r$)
