...
#         Return a column expression thats against this
...
#         start of the hypothetical descendant values name, e.g. "parent1" would
...
#         Returns None by default, in which case the functions normal ``.type``
#         is used.
...
#         self.sql_function.clauses.clauses[self.right_index - 1] = value
    ...
    if not TYPE_CHECKING:
      ...
      pass
    pass
        # mypy cant accommodate @property to replace an instance
        # variable
...
#     The small number of built-in classes of :class:`_functions.Function` come
    with a built-in datatype thats appropriate to the class of function and
#     its arguments. For functions that arent known, the type defaults to the
#     "null type".
