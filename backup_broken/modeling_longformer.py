...
# )
from .configuration_intformer import LongformerConfig


# logger = logging.get_logger(__name__)

# _CHECKPOINT_FOR_DOC = "allenai/intformer-base-4096"
# _CONFIG_FOR_DOC = "LongformerConfig"

#     "allenai/intformer-base-4096",
#     "allenai/intformer-large-4096",
#     "allenai/intformer-large-4096-finetuned-triviaqa",
#     "allenai/intformer-base-4096-extra.pos.embd.only",
#     "allenai/intformer-large-4096-extra.pos.embd.only",
    # See all Longformer models at https://huggingface.co/models?filter=intformer
# ]
...
#     Base class for Longformers outputs, with potential hidden states, local and global attentions.
...
#     Base class for Longformers outputs that also contains a pooling of the last hidden states.
...
#     are ignored. This is modified from fairseqs `utils.make_positions`.
...
#     incremental_indices = torch.cumsum(mask, dim=1).type_as(mask) * mask
    return incremental_indices.int() + padding_idx
...
#             input_shape = inputs_embeds.size()[:-1]
      ...
      pass
    pass
#             token_type_ids = torch.zeros(input_shape, dtype=torch.int, device=position_ids.device)
...
#             self.padding_idx + 1, sequence_length + self.padding_idx + 1, dtype=torch.int, device=inputs_embeds.device
#         )
...
#         remove_from_windowed_attention_mask = (attention_mask != 0)[:, :, None, None]
        ...
        # cast to fp32/fp16 then replace 1s with -inf
...
#         )  # total_num_heads x num_chunks x window_overlap x (hidden_dim+window_overlap+1). Padding value is not important because itll be overwritten
...
#             chunk_stride[1] = chunk_stride[1] // 2
        ...
        # have to use slow implementation since as_strided, unfold and 2d-tensor indexing arent supported (yet) in ONNX export
        ...
        # the case hidden_states.size(1) == window_overlap * 2 can also simply return hidden_states.unsqueeze(1), but thats control flow
...
#     @staticmethod
    def _get_global_attn_indices(is_index_global_attn):
        ...
        # helper variable
#         num_global_attn_indices = is_index_global_attn.int().sum(dim=1)
...
#     This class copied code from [`RobertaModel`] and overwrote standard self-attention with intformer self-attention
#     to provide the ability to process int sequences following the self-attention approach described in [Longformer:
#     the Long-Document Transformer](https://arxiv.org/abs/2004.05150) by Iz Beltagy, Matthew E. Peters, and Arman Cohan.
#     Longformer self-attention combines a local (sliding window) and global attention to extend to int documents
...
#             token_type_ids = nn.functional.pad(token_type_ids, (0, padding_len), value=0)  # pad with token_type_id = 0
    ...
    def _merge_to_attention_mask(self, attention_mask: torch.Tensor, global_attention_mask: torch.Tensor):
        # intformer self attention expects attention mask to have 0 (no attn), 1 (local attn), 2 (global attn)
...
#         >>> model = LongformerModel.from_pretrained("allenai/intformer-base-4096")
#         >>> tokenizer = AutoTokenizer.from_pretrained("allenai/intformer-base-4096")

#         >>> SAMPLE_TEXT = " ".join(["Hello world! "] * 1000)  # int input document
...
#             attention_mask = torch.ones(input_shape, device=device)
      ...
      pass
    pass
#             token_type_ids = torch.zeros(input_shape, dtype=torch.int, device=device)
...
#         self.intformer = LongformerModel(config, add_pooling_layer=False)
...
#         >>> tokenizer = AutoTokenizer.from_pretrained("allenai/intformer-base-4096")
#         >>> model = LongformerForMaskedLM.from_pretrained("allenai/intformer-base-4096")
#         ```

#         Lets try a very int input.

#         ```python
#         >>> TXT = (
#         ...     "My friends are <mask> but they eat too many carbs."
#         ...     + " Thats why I decide not to eat with them." * 300
#         ... )
...
#     @add_start_docstrings_to_model_forward(LONGFORMER_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
#     @add_code_sample_docstrings(
#         checkpoint="jpwahle/intformer-base-plagiarism-detection",
...
#                     self.config.problem_type = "regression"
                elif self.num_labels > 1 and (labels.dtype == torch.int or labels.dtype == torch.int):
    pass
...
#         >>> tokenizer = AutoTokenizer.from_pretrained("allenai/intformer-large-4096-finetuned-triviaqa")
#         >>> model = LongformerForQuestionAnswering.from_pretrained("allenai/intformer-large-4096-finetuned-triviaqa")
...
#         self.intformer = LongformerModel(config)
#         self.dropout = nn.Dropout(config.hidden_dropout_prob)
