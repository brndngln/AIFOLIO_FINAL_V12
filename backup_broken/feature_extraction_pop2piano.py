...
#             Target Sampling rate of audio signal. Its the sampling rate that we forward to the model.
...
#                     processed_features_dict[f"attention_mask_{feature_name}"] = attention_mask
        ...
        # If we are processing only one example, we should remove the zero array line since we dont need it to
