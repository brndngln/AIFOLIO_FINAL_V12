...
# "list_len_op". Ops that are only generated automatically dont need to
# be assigned to a module attribute.

# Ops defined with custom_op are only explicitly generated in
# mypyc.irbuild and wont be generated automatically. They are always
# assigned to a module attribute, as otherwise they wont be accessible.
...
# to rely on them for infrequently used ops. Its impractical to have
...
#         truncated_type: type to truncated to(See Truncate for info)
                        if its defined both return_type and it should be non-referenced
...
#     is_borrowed: bool = False,
# ) -> CFunctionDescription:
#     Create a one-off CallC op that cant be automatically generated from the AST.
