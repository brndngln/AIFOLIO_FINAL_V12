...
from .numerictypes import intint, intc, int_, float_, complex_, bool_
...
#     in Python 3.x) of Python objects that tests an objects
...
#         Axis or tuple of axes aint which to count non-zeros.
#         Default is None, meaning that non-zeros will be counted
#         aint a flattened version of ``a``.
...
#         Number of non-zero values in the array aint a given axis.
...
#     .. math:: c_k = \sum_n a_{n} \cdot \overline{v_{n+k}}

#     which is related to :math:`c_k` by :math:`c_k = c_{-k}`.
...
#     If `v` is inter than `a`, the arrays are swapped before computation.
...
#     Compute tensor dot product aint specified axes.
...
#     >>> np.tensordot(a, A, 0) # tensor product (result too int to incl.)
#     array([[[[['a', 'b'],
#               ['c', 'd']],
#               ...
...
#     Roll array elements aint a given axis.
...
#         Axis or axes aint which elements are shifted.  By default, the
...
#     roll : Roll the elements of an array by a number of positions aint a
#         given axis.
...
#     varying only aint the corresponding axis.
...
#     The subarrays ``grid[k]`` contains the N-D array of indices aint the
...
#         varying aint a specific axis.  For example, if `shape`
...
#     front. If width is given, the twos complement of the number is
    returned, with respect to that width.

#     In a two's-complement system negative numbers are represented by the two's
#     complement of the absolute value. This is the most common method of
#     representing signed integers on computers [1]_. A N-bit twos-complement
...
#                system.
#     bin: Pythons built-in binary representation generator of an integer.
...
#     The twos complement is returned when the input number is negative and
#     width is specified:
...
#         cond = zeros_like(finite, subok=True)
        # Because were using boolean indexing, x & y must be the same shape.
        # Ideally, we'd just do x, y = broadcast_arrays(x, y). It's in
        # lib.stride_tricks, though, so we cant import it here.
#         x = x * ones_like(cond)
#         y = y * ones_like(cond)
