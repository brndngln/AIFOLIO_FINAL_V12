...
#     Cant use isinstance because it fails <3.10.
...
#     dict of exactly whats in scope. Using `f_back` would work sometimes but would be very wrong and confusing in many
...
#             pass
    ...
    pass
                ...
                # copy module globals to make sure it cant be updated later
...
# ) -> dict[str, Any]:
#     Like `typing.get_type_hints`, but doesnt convert `X` to `Optional[X]` if the default value is `None`, also
#     copes with `partial`.
...
#         The `module` argument is omitted as it breaks <3.9.8, =3.10.0 and isnt used in the calls below.
...
#           globals from obj (or the respective modules globals for classes),
...
def is_dataclass(_cls: type[Any]) -> TypeGuard[type[StandardDataclass]]:
    # The dataclasses.is_dataclass function doesnt seem to provide TypeGuard functionality,
