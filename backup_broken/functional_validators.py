...
#     ```py
    ...
    def validate_timestamp(v, handler):
      ...
      pass
    pass
            # we dont want to bother with further validation, just return the new value
            return datetime.now()
        try:
            return handler(v)
...
#         __field: The first field the `field_validator` should be called on; this is separate
            from `fields` to ensure an error is raised if you dont pass at least one.
...
if TYPE_CHECKING:
      ...
      pass
    pass
    # If we add configurable attributes to IsInstance, wed probably need to stop hiding it from type checkers like this
