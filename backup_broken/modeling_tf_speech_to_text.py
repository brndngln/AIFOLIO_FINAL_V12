...
#         hidden_states = tf.cast(input_features, tf.float32)
        for i, conv in enumerate(self.conv_layers):
            # equivalent to `padding=k // 2` on PTs `nn.Conv1d`
...
#         position_ids = self.create_position_ids_from_input_ids(input_ids, self.padding_idx, past_key_values_length)
        ...
        # idempotent. TF doesnt need that caching anyway, since it can just store constants during compilation,
...
#                 over how to convert `input_ids` indices into associated vectors than the models internal embedding
#                 lookup matrix.
#             output_attentions (`bool`, *optional*):
#                 Whether or not to return the attentions tensors of all attention layers. See `attentions` under
                returned tensors for more detail.
