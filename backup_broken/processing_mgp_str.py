...
#         When used in normal mode, this method forwards all its arguments to ViTImageProcessors
#         [`~ViTImageProcessor.__call__`] and returns its output. This method also forwards the `text` and `kwargs`
#         arguments to MgpstrTokenizers [`~MgpstrTokenizer.__call__`] if `text` is not `None` to encode the text. Please
