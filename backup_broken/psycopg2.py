...
#   using psycopg2s ``set_client_encoding()`` method.
...
#    This is to maintain some degree of compatibility with PostgreSQLs own URL
...
# have been shown in benchmarking to improve psycopg2s executemany()
...
# equivalent to psycopg2s ``execute_values()`` handler; an overview of this
# feature and its configuration are at :ref:`engine_insertmanyvalues`.

# .. versionadded:: 2.0 Replaced psycopg2s ``execute_values()`` fast execution
...
# * ``values_only`` - this is the default value.  SQLAlchemys native
#   its default value of ``True``.  This handler rewrites simple
#   parameter sets can be inserted with one statement.

# * ``values_plus_batch``- SQLAlchemys native
#   value of ``True``. Then, psycopg2s ``execute_batch()`` handler is used for
...
# , these
# options make use of psycopg2s ``set_isolation_level()`` connection method,
# rather than emitting a PostgreSQL directive; this is because psycopg2s
...
#    weve detected that ``HSTORE`` oids are available, the
...
# place within SQLAlchemys own marshalling logic, and not that of ``psycopg2``
...
#         self._log_notices(self.cursor)
    ...
    def _log_notices(self, cursor):
        # check also that notices is an iterable, after its already
        # established that we will be iterating through it.  This is to get
        # around test suites such as SQLAlchemys using a Mock object for
        # cursor
...
#         oids = extras.HstoreAdapter.get_oids(dbapi_connection)
    ...
    pass
            ...
            # this flag doesnt actually help in a lot of disconnect
            # situations, so dont rely on it.
    ...
    pass
            ...
            # didnt cut it, fall back onto these.
