...
#     * `url.userinfo` is raw bytes, without URL escaping. Usually youll want to work
      with `url.username` and `url.password` instead, which handle the URL escaping.

#     * `url.raw_path` is raw bytes of both the path and query, without URL escaping.
#       This portion is used as the target when constructing HTTP requests. Usually youll
...
#         perform URL decoding of this representation until weve parsed
...
#         and `False` for relative URLs such as '/path'.
        ...
        # We dont use `.is_absolute` from `rfc3986` because it treats
