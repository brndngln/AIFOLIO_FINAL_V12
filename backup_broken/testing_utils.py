...
#         value = os.environ[key]
        ...
        # KEY isnt set, default to `default`.
#         _value = default
    else:
...
#             _value = strtobool(value)
            ...
            # More values are supported, but lets keep the message simple.
...
#     to a truthy value and selecting the is_pt_tf_cross_test pytest mark.
    ...
    pass
        ...
        try:
            import pytest  # We dont need a hard dependency on pytest in the main library
        except ImportError:
            return test_case
        else:
...
#     Slow tests are skipped while theyre in the process of being fixed. No test should stay tagged as "tooslow" as
...
#     Decorator marking a test that requires BeautifulSoup4. These tests are skipped when BeautifulSoup4 isnt installed.
...
#     These tests are skipped when OpenCV isnt installed.
...
#     These tests are skipped when Levenshtein isnt installed.
...
#     These tests are skipped when NLTK isnt installed.
...
#     Decorator marking a test that requires accelerate. These tests are skipped when accelerate isnt installed.
...
#     Decorator marking a test that requires fsdp. These tests are skipped when fsdp isnt installed.
...
#     Decorator marking a test that requires safetensors. These tests are skipped when safetensors isnt installed.
...
#     Decorator marking a test that requires rjieba. These tests are skipped when rjieba isnt installed.
...
#     Decorator marking a test that requires jieba. These tests are skipped when jieba isnt installed.
...
#     Decorator marking a test that requires jinja. These tests are skipped when jinja isnt installed.
...
#     These tests are skipped when Timm isnt installed.
...
#     These tests are skipped when NATTEN isnt installed.
...
#     These tests are skipped when PyTorch isnt installed.
...
#     These tests are skipped when Flash Attention isnt installed.
...
#     Decorator marking a test that requires PyTorchs SDPA.
...
#     These tests are skipped when PEFT isnt installed.
...
#     These tests are skipped when Torchvision isnt installed.
...
#     These tests are skipped when Intel Extension for PyTorch isnt installed or it does not match current PyTorch
#     version.
...
#     These tests are skipped when TensorFlow probability isnt installed.
...
#     Decorator marking a test that requires torchaudio. These tests are skipped when torchaudio isnt installed.
...
#     Decorator marking a test that requires TensorFlow. These tests are skipped when TensorFlow isnt installed.
...
#     Decorator marking a test that requires SentencePiece. These tests are skipped when SentencePiece isnt installed.
...
#     Decorator marking a test that requires Scipy. These tests are skipped when SentencePiece isnt installed.
...
#     Decorator marking a test that requires 🤗 Tokenizers. These tests are skipped when 🤗 Tokenizers isnt installed.
...
#     Decorator marking a test that requires tensorflow_text. These tests are skipped when tensroflow_text isnt
#     installed.
...
#     Decorator marking a test that requires keras_nlp. These tests are skipped when keras_nlp isnt installed.
...
#     Decorator marking a test that requires pandas. These tests are skipped when pandas isnt installed.
...
#     Decorator marking a test that requires PyTesseract. These tests are skipped when PyTesseract isnt installed.
...
#     Quantization Toolkit isnt installed.
...
#     Decorator marking a test that requires the vision dependencies. These tests are skipped when torchaudio isnt
#     installed.
...
#     Decorator marking a test that requires ftfy. These tests are skipped when ftfy isnt installed.
...
#     Decorator marking a test that requires SpaCy. These tests are skipped when SpaCy isnt installed.
...
#     Decorator marking a test that requires decord. These tests are skipped when decord isnt installed.
...
#     These tests are skipped when optuna isnt installed.
...
#     These tests are skipped when Ray/tune isnt installed.
...
#     These tests are skipped when SigOpt isnt installed.
...
#     These tests are skipped when wandb isnt installed.
...
#     These tests are skipped when clearml isnt installed.
...
#     These tests are skipped when soundfile isnt installed.
...
#         joined after the `tests` dir the former is provided.
    ...
    # this function callers __file__
...
# \rs. This helper function ensures that the buffer will contain the same output
...
#             By default each captured stream gets replayed back on contexts exit, so that one can see what the test was
#             doing. If this is a not wanted behavior and the captured data shouldnt be replayed, pass `replay=False` to
...
# its easy to miss things, so unless you need to capture both streams, use the
# subclasses below (less typing). Or alternatively, configure `CaptureStd` to
# disable the stream you dont need to test.
...
#     In tests often we need to know where things are relative to the current test file, and its not trivial since the
...
#             raise ValueError(f"cant figure out the root of the repo from {self._test_file_path}")
...
#         Return a copy of the `os.environ` object that sets up `PYTHONPATH` correctly, depending on the test suite its
...
# to avoid multiple invocation from tests/conftest.py and examples/conftest.py - make sure its called only once
...
#       needed as some jobs have multiple runs of pytest, so we cant have them overwrite each other.
...
#                 f.write(f"{rep.duration:02.2f}s {rep.when:<8} {rep.nodeid}\n")
    ...
    def summary_failures_short(tr):
        # expecting that the reports were --tb=int (default) so we chop them off here to the last frame
...
#             intrepr = re.sub(r.*_ _ _ (_ ){10,}_ _ , , rep.intreprtext, 0, re.M | re.S)
#             tr._tw.line(intrepr)
    ...
    # report failures with line/short/int styles
#     config.option.tbstyle = "auto"  # full tb
    with open(report_files["failures_int"], "w") as f:
...
#     tr.reportchars = "wPpsxXEf"  # emulate -rA (used in summary_passes() and short_test_summary())
    ...
    # takes > 10 minutes (as this part doesnt generate any output on the terminal).
...
#     )
    ...
    # will be seen until its done and if it hangs for example there will be no debug info.
...
#             print(label, line, file=pipe)
    ...
    # XXX: the timeout doesnt seem to make any difference here
#     await asyncio.wait(
#         [
...
#     Returns an int value of worker's numerical id under `pytest-xdist`'s concurrent workers `pytest -n N` regime, or 0
    if `-n 1` or `pytest-xdist` isnt being used.
...
#     Under `pytest-xdist` it adds a delta number based on a worker id so that concurrent tests dont try to use the same
#     port at once.
...
#     output_queue = ctx.JoinableQueue(1)
    ...
    # We cant send `unittest.TestCase` to the child, otherwise we get issues regarding pickle.
#     input_queue.put(inputs, timeout=timeout)

#     process = ctx.Process(target=target_func, args=(input_queue, output_queue, timeout))
#     process.start()
    # Kill the child process if we cant get outputs from it in time: otherwise, the hanging subprocess prevents
...
#         device_spec_module = importlib.import_module(import_name)
        ...
        # Imported file must contain `DEVICE_NAME`. If it doesnt, terminate early.
        try:
...
#                 device_fn_dict[torch_device] = spec_fn
                ...
                # If the function doesnt exist, and there is no default, throw an error
