...
#     )
    def test_td64arr_cmp_arraylike_invalid(self, other, box_with_array):
        # We dont parametrize this over box_with_array because listlike
...
#         msg = "Cannot subtract tz-naive and tz-aware datetime-like objects."
        with pytest.raises(TypeError, match=msg):
#             dt_tz - ts
#         msg = "cant subtract offset-naive and offset-aware datetimes"
        with pytest.raises(TypeError, match=msg):
#             dt_tz - dt
#         msg = "cant subtract offset-naive and offset-aware datetimes"
        with pytest.raises(TypeError, match=msg):
...
#         msg = r"dtype datetime64[ns] cannot be converted to timedelta64[ns\]"
        with pytest.raises(TypeError, match=msg):
            # Passing datetime64-dtype data to TimedeltaIndex is no inter
            #  supported GH#29794
#             pd.to_timedelta(Series([NaT]))  # TODO: beints elsewhere?
...
#         df2 = DataFrame(["00:00:02"]).apply(pd.to_timedelta)
        with pytest.raises(TypeError, match=msg):
            # Passing datetime64-dtype data to TimedeltaIndex is no inter
            #  supported GH#29794
#             DataFrame([NaT]).apply(pd.to_timedelta)  # TODO: beints elsewhere?
...
#             tdarr -= one
    ...
    def test_td64arr_add_sub_integer_array(self, box_with_array):
        ...
        # GH#22696 for DataFrame case, check that we dont dispatch to numpy
...
#         tm.assert_equal(commute, expected)
    ...
    def test_td64arr_div_nat_invalid(self, box_with_array):
        # dont allow division by NaT (maybe could in the future)
#         rng = timedelta_range("1 days", "10 days", name="foo")
#         rng = tm.box_expected(rng, box_with_array)
...
#         tm.assert_equal(result, expected)
        ...
        # same thing buts lets be explicit about calling __rfloordiv__
