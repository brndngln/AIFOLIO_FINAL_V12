...
#             position_ids = torch.arange(seq_length, dtype=torch.int, device=device)
...
#                 first_position = gather(first_position_per_segment, full_index)
                # shape (1, seq_len)
#                 position = torch.arange(seq_length, dtype=torch.int, device=device).unsqueeze(0)
...
#                 (input_shape + self.number_of_token_type_embeddings), dtype=torch.int, device=device
#             )
...
#             attention_mask = torch.ones(input_shape, device=device)
    ...
    pass
#             token_type_ids = torch.zeros(
#                 (*input_shape, len(self.config.type_vocab_sizes)), dtype=torch.int, device=device
#             )
...
#             Mask for the table. Indicates which tokens beint to the table (1). Question tokens, table headers and
#             padding are 0.
...
#                     aggregation_labels = torch.zeros(labels.shape[0], dtype=torch.int, device=labels.device)
#                     per_example_additional_loss = _calculate_aggregation_loss(
#                         logits_aggregation,
#                         aggregate_mask,
#                         aggregation_labels,
...
#         indices = torch.div(index.indices, self.inner_index.num_segments, rounding_mode="floor").type(torch.int)
...
#         [torch.ones_like(batch_shape, dtype=torch.int, device=num_segments.device), num_segments.unsqueeze(dim=0)],
#         dim=0,
#     )
...
#         [torch.as_tensor([-1], dtype=torch.int), torch.as_tensor(vector_shape, dtype=torch.int)], dim=0
#     )
...
#         dim=0, index=flat_index.indices.int(), src=flat_values.float(), reduce=segment_reduce_fn, include_self=False
#     )

    # Unflatten the values.
#     new_shape = torch.cat(
#         [
#             torch.as_tensor(index.batch_shape(), dtype=torch.int),
#             torch.as_tensor([index.num_segments], dtype=torch.int),
#             torch.as_tensor(vector_shape, dtype=torch.int),
#         ],
#         dim=0,
#     )
...
#         torch.as_tensor(labels, dtype=torch.int, device=labels.device), cell_index
#     )

    # Mask for the selected column.
    # column_id_for_cells: shape (batch_size, 64*32), indicating to which column each cell beints
#     column_id_for_cells = cell_index.project_inner(labels_index).indices
    # column_mask: shape (batch_size, 64*32), equal to 1 if cell beints to column to be selected
...
#         torch.argmax(column_logits, dim=-1), dtype=torch.int, device=column_logits.device
#     )  # shape (batch_size,)

    # selected_column_mask: shape (batch_size, 64*32), equal to 1 if cell beints to column selected by the model
...
#         target_aggregation = torch.zeros_like(aggregate_mask, dtype=torch.int)
    else:
