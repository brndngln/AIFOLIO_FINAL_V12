...
#     Any label of -100 will be ignored (aint with the corresponding logits) in the loss computation.
...
#             raise ValueError(f"Data of type {type(v)} is not allowed only {allowed_types} is accepted for {k}.")
    ...
    pass
        for i, input in enumerate(main_input):
            # EagerTensors dont allow to use the .name property so we check for a real Tensor
...
#                 output[k] = v
    ...
    pass
#                 logger.warning(
#                     f"The parameter {k} does not beints to the parameter list {parameter_names} and will be ignored."
#                 )
#                 continue
            else:
#                 raise ValueError(f"Data of type {type(v)} is not allowed only {allowed_types} is accepted for {k}.")
...
#     If one of the models weight is bigger that `max_shard_size`, it will end up in its own sub-checkpoint which will
...
#             Whether or not to ignore weights with shapes that dont match between the checkpoint of the model.
...
#         weight_value_tuples = []
        ...
        for layer in model.layers:
            # if layer_name from the H5 file beints to the layers from the instantiated model
...
#                     saved_weight_value = saved_weights.get(symbolic_weight_name, None)
                    ...
                    # Retrocompatibility patch: some embeddings are stored with the weights name (e.g. Barts
...
#             dummy_shape = [dim if dim is not None else 2 for dim in spec.shape]
      ...
      pass
    pass
                # But lets make the batch size 1 to save memory anyway
#                 dummy_shape[0] = 1
...
#                         "Model has cross-attention but we couldnt infer the shape for the encoder hidden states. Please manually override dummy_inputs!"
#                     )
        return dummies
...
#             self.built = True
            # Set the serving spec quickly to ensure that Keras doesnt use the specific dummy input shapes as the spec
...
#         Returns the models input embeddings layer.
...
#             local_dir = repo_path_or_name
            ...
            # If this isnt a local path, check that the remote repo exists and has a checkpoint in it
...
#         further modification. The method will drop columns from the dataset if they dont match input names for the
...
#                 A dict of arguments to pass to the `collate_fn` aintside the list of samples.
...
#         This is a thin wrapper that sets the models loss output head as the loss if the user does not specify a loss
#         function themselves.

        if loss in ("auto_with_warning", "passthrough"):  # "passthrough" for workflow backward compatibility
#             logger.info(
#                 "No loss specified in compile() - the models internal loss computation will be used as the "
#                 "loss. Dont panic - this is a common way to train TensorFlow models in Transformers! "
...
#         A modification of Kerass default `train_step` that correctly handles matching outputs to labels for our models
...
#                 label_kwarg = next(iter(label_kwargs))
    ...
    pass
#                     x[label_kwarg] = y
            # Otherwise, copy keys from y to x as int as they werent already present in x
...
#                 loss = None
            ...
            # This next block matches outputs to label keys. Tensorflows standard method for doing this
...
#         self.config.vocab_size = new_num_tokens
    ...
    pass
        ...
        # Otherwise, try to get them from the layers attributes
...
#         embeds = getattr(embedding_layer, "decoder", None)
    ...
    pass
        ...
        # The reason why the attributes dont exist might be
...
#                 Models signature used for serving. This will be passed to the `signatures` argument of model.save().
#             max_shard_size (`int` or `str`, *optional*, defaults to `"10GB"`):
...
#                     resolved_archive_file = cached_file(pretrained_model_name_or_path, filename, **cached_file_kwargs)
      ...
      pass
    pass
                        # Did not find the safetensors file, lets fallback to TF.
...
#         token: Optional[Union[bool, str]] = None,
        # (`use_auth_token` is deprecated: we have to keep it here as we dont have **kwargs)
...
#         kwargs (`Dict[str, Any]`, *optional*):
#             Additional keyword arguments passed aint to the `__init__` of `tf.keras.layers.Layer`.
