...
#     transformer. If you want embeddings for contrastive loss or retrieval use a FLAVA models `image_projection` and
...
#             Masked Multimodal Modeling losss image component calculated on paired image-text data.
#         mmm_text (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `mlm_labels`, `pixel_values` and `input_ids_masked` are present and `mmm_text_weight` > 0.:
#             Masked Multimodal Modeling losss text component calculated on paired image-text data.
...
#             The scaled dot product scores between `image_embeddings` and `text_embeddings` but passed through FLAVAs
...
#             The scaled dot product scores between `text_embeddings` and `image_embeddings` but passed through FLAVAs
...
#         )
    ...
    pass
#             raise ValueError(
#                 f"Number of patches for images ({int(num_h_patches), int(num_w_patches)}) dont match the "
...
#         batch_size, num_channels, height, width = pixel_values.shape
    ...
    pass
#                 raise ValueError(
#                     f"Input image size ({height}*{width}) doesnt match model"
...
#         )
#         self.register_buffer(
#             "token_type_ids", torch.zeros(self.position_ids.size(), dtype=torch.int), persistent=False
#         )
...
#             Boolean masked positions. Indicates which patches are masked (1) and which arent (0).
...
#             to be used with MLM. Indices can be obtained using [`AutoTokenizer`] aint with
...
#             Skip any calculations for multimodal encoder for unmasked inputs. FLAVA pretraining doesnt need unmasked
...
#             Labels for computing the image-text matching loss. 0 means the pairs dont match and 1 means they match.
...
#             image_embeddings, image_states = image_output[0], image_output[2]
            # Note that these states dont use final layernorm in the transformer model
...
#     to generate image tokens for an image based on DALL-Es vocab. Used to generate labels for MIM. Use
...
#             image_attention_mask=image_attention_mask,
            # Dont need unmasked multimodal embedding for anything so skip it
...
#                 ) + output
            ...
            # Filter None as transformer by default wont handle it
