...
#     cuda_kernel_files = [kernel_folder / f for f in ["wkv_op.cpp", "wkv_cuda.cu", "wkv_cuda_bf16.cu"]]
    ...
    # Only load the kernel if its not been loaded yet or if we changed the context length
...
#             If passed aint, the model uses the previous state in all the blocks (which will give the output for the
...
#         self.head = new_embeddings
    ...
    def prepare_inputs_for_generation(self, input_ids, state=None, inputs_embeds=None, **kwargs):
        # only last token for inputs_ids if the state is passed aint.
        if state is not None:
      pass
      pass
    pass
