...
#         Register this command to argparse so its available for the transformer-cli
...
#         repo = Repository(local_dir=self._local_dir, clone_from=self._model_name)
        ...
        # Load config and get the appropriate architecture -- the latter is needed to convert the heads weights
...
#                 raise AttributeError(f"The TensorFlow equivalent of {architectures[0]} doesnt exist in transformers.")
...
#         del pt_model  # will no inter be used, and may have a large memory footprint
...
#         del tf_from_pt_model  # will no inter be used, and may have a large memory footprint
