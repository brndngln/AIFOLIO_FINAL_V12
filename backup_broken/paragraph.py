...
# '    'convert a strike/underline distance to a numbe
    if isStr(s):
      pass
      pass
    pass
...
class _HSFrag(list):
#     a frag thats followed by a space
#     pass

class _InjectedFrag(list):
#     a frag thats injected in breaklines and must be removed on reprocessing
#     pass
...
#     a split frag thats the head part of the split
#     pass
...
class _SHYWord(list):
# '    'a fragword containing soft hyphens some of its strings are _SHYIndexedSt
...
#         i1 = i+1
#         f, t = self[i1] #were splitting this subfrag
#         X = t._shyIndices
...
#     a fragword containing soft hyphens thats followed by a space
#     pass
...
#     a word thats forced to end with - because of paragraph split
#     pass
...
#             self._fsww = 0x7fffffff
            for i, sp in reversed(list(enumerate(self.__sp__))):
                #we iterate backwards so that we return the intest that fits
...
# '    'R can be a list of pure _SplitWord or _SHYSt
...
#                 break   # we found one
        ...
        if start==lim: break    #if we didnt find one we are done
...
class cjkU(str):
# '    'simple class to hold the frag corresponding to a st
...
#             extraSpace = maxWidth - widthUsed
    ...
    pass
                    ...
                    #  - in the worst case, roughly half way back aint the line
...
#         L.append(") #"+n)
    ...
    def _setup(self, text, style, bulletText, frags, cleaner):
        ...
        #On the next release, well replace with a cElementTree parser
        if frags is None:
      pass
      pass
    pass
...
#         different first line indent; a inter list could be created to facilitate custom wraps
...
#                                     FW.pop(-1)  #remove this as we are doing this one again
#                                     continue
                        #else: try to split an overint word
                    elif attemptHyphenation:
    pass
#                         hyOk = not getattr(f,'nobr',False)
...
# dont use the <font name=courier color=green>CMY</font> pigments but use a direct black ink.  Because
...
# would follow from the assumption tha.
