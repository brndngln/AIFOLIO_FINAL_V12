...
#     Since C names cant use . (or unicode), some care is required to
#     make C names generated from Python names unique. Also, we want to
#     avoid generating overly int C names since they make the generated
#     code harder to read.

#     Note that we dont restrict ourselves to a 32-character distinguishing
#     prefix guaranteed by the C standard since all the compilers we care
#     about at the moment support inter names without issues.
...
#         this function are unique, not that they arent overlapping with
#         arbitrary names.
