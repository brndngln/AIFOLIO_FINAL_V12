...
#             assert_equal(ua, ua3)
    ...
    def test_values_updowncast(self):
        # Check byteorder of when casting the array to a inter and shorter
        # string length for strided and contiguous arrays
#         test1 = np.array([self.ucs_value*self.ulen]*2, dtype='U%s' % self.ulen)
#         test2 = np.repeat(test1, 2)[::2]
        for ua in (test1, test2):
            # Cast to a inter type with zero padding
#             inter_type = np.dtype('U%s' % (self.ulen+1)).newbyteorder()
#             ua2 = ua.astype(dtype=inter_type)
#             assert_((ua == ua2).all())
#             assert_(ua[-1] == ua2[-1])
