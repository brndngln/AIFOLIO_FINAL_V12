...
#     expected = DataFrame(columns=df.columns[:2], index=range(0))
    # GH#50744 - Columns after selection shouldnt retain names
...
def test_groupby_multi_corner(df):
    # test that having an all-NA column doesnt mess you up
#     df = df.copy()
#     df["bad"] = np.nan
...
def test_groupby_two_group_keys_all_nan():
    # GH #36842: Grouping over two group keys shouldnt raise an error
...
def test_index_label_overlaps_location():
    # checking we dont have any label/location confusion in the
    # wake of GH5375
...
#             gb2 = df.loc[[]].groupby(axis=axis_number, level=0)
        # default for empty when we cant infer a dtype is float64
...
def test_subsetting_columns_keeps_attrs(klass, attr, value):
    # GH 9959 - When subsetting columns, dont drop attributes
...
def test_groupby_cumsum_skipna_false():
    # GH#46216 dont propagate np.nan above the diagonal
...
def test_groupby_cumsum_timedelta64():
    # GH#46216 dont ignore is_datetimelike in libgroupby.group_cumsum
