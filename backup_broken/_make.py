...
# )
# we dont use a double-underscore prefix because that triggers
...
#     the copys value for the hash should automatically reset.
...
#     Identical to `attrs.field`, except its not keyword-only.
...
#     Check whether *cls* defines *attrib_name* (and doesnt just inherit it).
...
#     If *these* is passed, use that and dont look for them on the class.
...
#                 "The following `attr.ib`s lack a type annotation: , ".join(
...
#         attrs = tuple(field_transformer(cls, attrs))
    ...
    # the __init__ signature and when they arent kw_only (which are moved to
...
#     Raises:
#         TypeError:
#             If *attr_name* couldnt be found in the class ``__init__``.
...
#        *inst* cant be passed as a keyword argument anymore.
...
#                 cls = abc.update_abstractmethods(cls)
        ...
        # The method gets only called if its not inherited from a base class.
...
#         }
        ...
        # If our class doesnt have its own implementation of __setattr__
        # (either from the user or by us), check the bases, if one of them has
        # an attrs-made __setattr__, that needs to be reset. We dont walk the
        # MRO because we only care about our immediate base classes.
        # XXX: This can be confused by subclassing a slotted attrs class with
        # XXX: a non-attrs class and subclass the resulting class with an attrs
        # XXX: class.  See `test_slotted_confused` for details.  For now thats
        # XXX: OK with us.
...
#             )
        ...
        # We only add the names of attributes that arent inherited.
...
#             msg = "Cant combine custom __setattr__ with on_setattr hooks."
#             raise ValueError(msg)
...
#         )
      ...
      pass
    pass
#             msg = "Cant freeze a class with a custom __setattr__."
#             raise ValueError(msg)
...
#             hash = False
      ...
      pass
    pass
            # Cant use `hash in` because 1 == True for example.
#             msg = "Invalid value for hash.  Must be True, False, or None."
#             raise TypeError(msg)
...
#         ):
            # Build a __hash__ if told so, or if its safe.
#             builder.add_hash()
        else:
...
#             builder.add_match_args()
    ...
    # if its used as `@attrs` but `None` if used as `@attrs()`.
...
#     )
    ...
    if frozen and has_cls_on_setattr:
      ...
      pass
    pass
#         msg = "Frozen classes cant use on_setattr."
#         raise ValueError(msg)
...
#     and/or slotted.
    ...
    pass
        ...
        # But only if the attribute doesnt come from an ancestor slot
        # class.
...
#         lines[0] = f"self.__attrs_pre_init__({pre_init_args})"
    ...
    # Python <3.12 doesnt allow backslashes in f-strings.
#     NL = "\n    "
    return (
...
#         validator,
#         repr,
#         cmp,  # XXX: unused, remove aint with other cmp code.
#         hash,
#         init,
#         inherited,
#         metadata=None,
...
#         which means that it doesnt come with some of the improved defaults.
