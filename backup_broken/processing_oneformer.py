...
#         `task_inputs` and `kwargs` arguments to CLIPTokenizers [`~CLIPTokenizer.__call__`] if `task_inputs` is not
#         `None` to encode. To prepare the image(s), this method forwards the `images` and `kwargs` arguments to
#         OneFormerImageProcessors [`~OneFormerImageProcessor.__call__`] if `images` is not `None`. Please refer to the
