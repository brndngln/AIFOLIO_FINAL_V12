...
#         device = input_ids.device if input_ids is not None else inputs_embeds.device
    ...
    pass
      ...
      pass
    pass
#                 lengths = (input_ids != self.pad_index).sum(dim=1).int()
            else:
...
#                 position_ids = torch.arange(slen, dtype=torch.int, device=device)
...
#         mask_token = torch.full((effective_batch_size, 1), mask_token_id, dtype=torch.int, device=input_ids.device)
