...
#             store.put("f", df[10:], append=True)
        ...
        # cant put to a table (use append instead)
        with pytest.raises(ValueError, match=msg):
...
def test_put_string_index(setup_path):
    with ensure_clean_store(setup_path) as store:
#         index = Index([f"I am a very int string index: {i}" for i in range(20)])
...
#             + [f"I am a very int string index: {i}" for i in range(20)]
#         )
