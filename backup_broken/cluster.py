...
#         A random cluster node will be chosen if target_node isnt passed, and primaries
...
#              Set the RedisClusters startup nodes to all of the discovered nodes.
#              If true (default value), the clusters discovered nodes will be used to
#              determine the cluster nodes-slots mapping in the next topology refresh.
#              It will remove the initial passed startup nodes if their endpoints arent
...
#             )
        ...
        # Whenever a new connection is established, RedisClusters on_connect
        # method should be run
        # If the user passed on_connect function well save it and run it
...
#         Get the node that holds the keys slot.
#         If replica set to True but the slot doesnt have any replicas, None is
        returned.
...
#                      key.
#             time: 5, thread-2 acquired `my-lock` now that its available.
#                      thread-2 sets the token to "xyz"
#             time: 6, thread-1 finishes its work and calls release(). if the
#                      token is *not* stored in thread local storage, then
#                      thread-1 would see the token value as "xyz" and would be
#                      able to successfully release the thread-2s lock.

#         In some use cases its necessary to disable thread local storage. For
#         example, if you have code where one thread acquires a lock and passes
#         that lock instance to a worker thread to release later. If thread
#         local storage isn't disabled in this case, the worker thread won't see
#         the token set by the thread that acquired the lock. Our assumption
#         is that these cases arent common and as such default to using
...
#             command_flag = self.command_flags.get(command)
    ...
    pass
            # return the clusters default node
            return [self.nodes_manager.default_node]
...
#         So, dont use this function with EVAL or EVALSHA.
...
#             determine what slots to map the command to; or, if the keys dont
...
#         command = args[0]
    ...
    pass
        ...
        # EVAL and EVALSHA are common enough that its wasteful to go to the
...
#                 raise
                ...
                # Connection retries are being handled in the nodes
                # Retry object.
                # ConnectionError can also be raised if we couldnt get a
...
#                 self.nodes_manager.startup_nodes.pop(target_node.name, None)
                # Reset the cluster nodes connection
...
#                     self.nodes_manager.close()
            ...
            # RedisClusters __init__ can fail before nodes_manager is set
#             pass
...
#         Get the requested node from the clusters nodes.
#         nodes.
...
#         Update the slots node with the redirected one
...
#             self.slots_cache[e.slot_id].append(old_primary)
            # Remove the old replica, which is now a primary, from the slots
            # node list
...
#                 self.default_node = redirected_node
            ...
            # shard. We need to remove all current nodes from the slots list
...
#                 continue
            ...
            # If theres only one server in the cluster, its ``host`` is
...
#             fully_covered = self.check_slots_coverage(tmp_slots)
      ...
      pass
    pass
                # Dont need to continue to the next startup node if all
                # slots are covered
#                 break
...
#         Raise a RedisClusterException if the node is None or doesnt exist in
#         the cluster.
    ...
    pass
#             raise RedisClusterException(
#                 f"Node {host}:{port} doesnt exist in the cluster"
#             )
...
#         Taken code from redis-py and tweak to make it work within a cluster.
        ...
        # NOTE: dont parse the response in this function -- it could pull a
...
#         Appends the executed command to the pipelines command stack
...
#         self.explicit_transaction = False
        ...
        # TODO: Implement
        # we can safely return the connection to the pool here since were
        # sure were no inter WATCHing anything
        # if self.connection:
    pass
...
#         nodes = {}
        ...
        for c in attempt:
            while True:
                ...
                # (it might be possible we have a cached node that no inter
...
#                     is_default_node = True
                ...
                # ( its just a string in the form of host:port )
...
#             n.connection_pool.release(n.connection)
        ...
        # if the response isnt an exception it is a
        # valid response from the node
        # were all done with that command, YAY!
        # if we have more commands to attempt, weve run into problems.
...
#         )
    ...
    pass
            ...
            #
            # If a lot of commands have failed, well be setting the
...
#         connection = self.connection
        for c in self.commands:
            ...
            # is no inter open will result in a
            # connection error raised by redis-py.
            # but redis-py doesnt check in parse_response
