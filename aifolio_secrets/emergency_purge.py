...
PIPELINE_LOCK_PATH = os.path.join(os.path.dirname(__file__), '../logs/pipeline_lock.json')
...
def reroute_dashboard_to_lockdown():
    print('[PURGE] Dashboard rerouted to Lockdown Mode (frontend must check LOCKDOWN_MODE).')
...
def run_emergency_purge(trigger_source, keyword, ai_confirmed, human_confirmed, countdown=0, override_key=None):
...
if __name__ == '__main__':
    ...
    run_emergency_purge(trigger_source, keyword, ai_confirmed, human_confirmed, countdown, override_key)
