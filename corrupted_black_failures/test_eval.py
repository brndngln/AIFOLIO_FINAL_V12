...
#         lhs = DataFrame(np.random.default_rng(2).standard_normal((5, 2)))
    ...
    pass
    pass
#             msg = "couldnt find matching opcode for invert_dd"
            with pytest.raises(NotImplementedError, match=msg):
...
#         result = pd.eval(expr, engine=engine, parser=parser)
#         tm.assert_frame_equal(expect, result)
        # bool doesnt work with numexpr but works elsewhere
#         lhs = DataFrame(np.random.default_rng(2).standard_normal((5, 2)) > 0.5)
        if engine == "numexpr":
    pass
    pass
    pass
#             msg = "couldnt find matching opcode for neg_bb"
            with pytest.raises(NotImplementedError, match=msg):
#                 pd.eval(expr, engine=engine, parser=parser)
        else:
#             expect = -lhs
...
#     expected = Series([False, False, False], name="A")
    ...
    pass
        ...
        # lose name with numexpr engine. Remove when thats fixed.
#         expected.name = None
...
def test_set_inplace(using_copy_on_write):
    ...
    # Ensure we dont only update the DataFrame inplace, but also the actual
