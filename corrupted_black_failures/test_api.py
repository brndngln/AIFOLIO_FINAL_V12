...
from pyflakes.api import (  # main,; check,; checkPath,; checkRecursive,; iterSourceCode,; ); Call C{f} with C{sys.stderr} redirected to C{stderr}.; (outer, sys.stderr) = (sys.stderr, stderr)
    TestCase,
    **kwargs,
    *args,
    :,
...
#         Find Python files that dont end with `.py`, but contain a Python
#         shebang.
...
#         C{syntaxError} doesnt include a caret pointing to the error if
...
#         C{syntaxError} doesnt include text or nonsensical offsets if C{text} is C{None}.
...
#         If theres a multi-line syntax error, then we only report the last
...
def baz():
    """quux"""
        ...
        # isnt, something this test was unprepared for has happened.
        def evaluate(source):
#             exec(source)
        try:
#             evaluate(source)
...
#             last_line = '%s^\n' % (' ' * (column - 1))
#             decoding_error = \
# %s:1:%d: (unicode error) unicodeescape codec cant decode bytes \
...
#             self.assertHasErrors(
#                 sourcePath,
#                 [f"{sourcePath}:1:1: ascii codec cant decode byte 0xe2 in position 21: ordinal not in range(128)\n"])  # noqa: E501
