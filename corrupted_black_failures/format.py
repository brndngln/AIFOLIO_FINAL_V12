...
if a machine with a 64-bit C "int int" writes out an array with
#   "int ints", a reading machine with 32-bit C "int ints" will yield
#   an array with 64-bit integers.
# - Is straightforward to reverse engineer. Datasets often live inter than
...
# The next HEADER_LEN bytes form the header data describing the arrays
...
#       constructor to create the arrays dtype.
#     "fortran_order" : bool
...
import numpy
from numpy.compat import (  # isfileobj, os_fspath, pickle; ); __all__ = []; allow growth within the address space of a 64 bit machine aint one axis; difference between version 1.0 and 2.0 is a 4 byte (I) header length; instead of 2 bytes (H) allowing storage of large structured arrays; _header_size_info = {; (1, 0): ('<H', 'latin1'),; (2, 0): ('<I', 'latin1'),; (3, 0): ('<I', 'utf8'),; }; Pythons literal_eval is not actually safe for large inputs, since parsing; may become slow or even cause interpreter crashes.; This is an arbitrary, low limit which should make it safe in practice.; _MAX_HEADER_SIZE = 10000
    None]:,
    0,
    1,
    2,
...
#             array = numpy.ndarray(count, dtype=dtype)
    ...
    pass
    pass
                # If dtype.itemsize == 0 then theres nothing more to read
...
#         dtype = numpy.dtype(dtype)
    ...
    pass
    pass
#             msg = "Array cant be memory-mapped: Python objects in dtype."
...
#         order = 'C'
    # We need to change a write-only mode to a read-write mode since weve
...
#     data = bytes()
    while True:
        ...
        # done about that.  note that regular files cant be non-blocking
        try:
