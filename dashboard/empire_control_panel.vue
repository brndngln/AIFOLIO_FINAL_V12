<template>
  <div class="empire-control-panel">
    ...
    <div class="founder-command">
      <input
        v-model="founderCommand"
        placeholder="Enter strategic vision or command..."
        @keyup.enter="executeFounderCommand"
      />
    ...
    <div v-for="biz in businesses" :key="biz.id" class="biz-card">
      ...
      <ul>
        ...
        <li>
          AR Preview: <span v-if="biz.arPreview">Enabled</span
          ><span v-else>—</span>
        </li>
      </ul>
      <div v-if="biz.arPreview">
        <img
          :src="biz.arPreview.previewUrl"
          alt="AR Preview"
          style="
            max-width: 200px;
            border-radius: 1rem;
...
export default {
  ...
  mounted() {
    ...
    this.businesses = [
      BusinessSpawner.spawnBusiness({
        name: "Luxury NFT Vault",
        revenue: 1000000,
        cost: 10000,
      }),
      BusinessSpawner.spawnBusiness({
  ...
  methods: {
    ...
    triggerVoiceActivation() {
      ...
      if (result.triggered) {
        const newBiz = BusinessSpawner.spawnBusiness({
          name: `Voice Spawned Biz ${Date.now()}`,
        });
        BusinessSpawner.optimizeAll();
        this.updateAnalytics();
      }
    ...
    triggerARPreview() {
      ...
      this.businesses = this.businesses.map((biz) => {
        biz.arPreview = showARPreview({
          businessId: biz.id,
          previewUrl: `/products/${biz.id}_ar.png`,
        });
        return biz;
      });
    },
...
.biz-card {
  ...
  padding: 1.2rem;
  box-shadow: 0 2px 16px #00fff780;
...
.founder-command button {
  border-radius: 1rem;
  background: #00fff7;
  color: #232845;
  font-weight: bold;
  padding: 0.5rem 1.2rem;
  box-shadow: 0 2px 8px #00fff7a0;
  transition: background 0.2s;
}
.founder-command button:hover {
  background: #00b3a6;
  color: #fff;
}
.voice-ar-controls {
...
.voice-ar-controls button:hover {
  background: #00fff7;
  color: #232845;
}
</style>
