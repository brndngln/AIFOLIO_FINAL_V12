...
def reviewer_training():
    ...
    if request.method == 'POST':
        ...
        for logf in [AUDIT_LOG, AUDIT_BACKUP]:
            with open(logf, 'a') as logfobj:
                logfobj.write(f"REVIEWER_TRAINING: {reviewer} {action} at {datetime.now()}\n")
...
@reviewer_bp.route('/api/analytics/reviewer_escalations', methods=['GET', 'POST', 'PUT'])
def reviewer_escalations():
    ...
    if request.method == 'POST':
        ...
        for logf in [AUDIT_LOG, AUDIT_BACKUP]:
            with open(logf, 'a') as logfobj:
                logfobj.write(f"ESCALATION: {reviewer} escalated at {timestamp} from {ip} - Reason: {reason}\n")
        ...
        with ESCALATION_LOCK:
            ...
            with open(ESCALATION_FILE) as f:
                for l in f:
                    parts = l.strip().split('|')
                    if len(parts) >= 5 and parts[0]==timestamp and parts[1]==reviewer:
                        parts[4] = new_state
                        if len(parts)<6:
        ...
        for logf in [AUDIT_LOG, AUDIT_BACKUP]:
            with open(logf, 'a') as logfobj:
                logfobj.write(f"ESCALATION: {reviewer} {new_state} at {datetime.now()} - Reason: {resolution_reason}\n")
...
def reviewer_stats():
        ...
        with open(REVIEWER_TRAINING_FILE) as f:
            ...
            for l in lines:
                ...
                if len(parts) > 3:
                    ...
                    event = parts[2]
                    timestamp = parts[0]
                    response_time = float(parts[3]) if parts[3].replace('.','',1).isdigit() else 0
                    correct = (parts[4].lower() == 'true') if len(parts)>4 else None
                    s = stats.setdefault(reviewer, {'reviewer': reviewer, 'events': 0, 'total_reviews': 0, 'total_response_time': 0, 'correct': 0, 'last_activity': timestamp, 'escalations': 0})
                    s['events'] += 1
                    s['total_reviews'] += 1
    ...
    try:
        with open(ESCALATION_FILE) as f:
            for l in f.readlines():
                ...
                if len(parts) > 2:
                    reviewer = parts[1]
                    reviewer_escalations[reviewer] = reviewer_escalations.get(reviewer, 0) + 1
    except Exception:
        pass
    metrics = []
    for r, s in stats.items():
        ...
        metrics.append({
            ...
            'avg_response_time': round(s['total_response_time']/total,2) if total else 0,
            'accuracy_rate': round(100*s['correct']/total,1) if total else 0,
            'total_reviews': total,
            'escalation_rate': round(100*reviewer_escalations.get(r,0)/total,1) if total else 0,
