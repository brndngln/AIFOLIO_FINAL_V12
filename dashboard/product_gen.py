...
def generate():
    ...
    if request.method == 'POST':
        ...
        optimized_prompt, prompt_flag = ai_action_with_oversight((prompt, title), lambda x: optimize_prompt(*x), context='optimize_prompt')
        visuals, visuals_flag = ai_action_with_oversight((niche, brand), lambda x: select_visuals(*x), context='select_visuals')
        sales_suggestions = ai_action_with_oversight(optimized_prompt, suggest_sales_improvements, context='sales_optimizer')
        fixed_content, ethics_report = ai_action_with_oversight(optimized_prompt, scan_and_fix, context='ethics_qualityguard')
        final_content = ai_action_with_oversight((fixed_content, category, user_consent), lambda x: legal_scan(*x), context='legal_scan')
        ...
        if prompt_flag: preview += f"<br><b>Prompt Flag:</b> {prompt_flag}"
        if visuals_flag: preview += f"<br><b>Visuals Flag:</b> {visuals_flag}"
    ...
    </form>'''.format(csrf_token)
    return render_template('dashboard.html', content="<h3>Product Generation</h3>" + form + "<a href='/dashboard'>Back</a>")
