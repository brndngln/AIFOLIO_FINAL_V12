...
function renderReviewerStats(stats, escalations) {
    ...
    if (escDiv) {
        escDiv.innerHTML = '<h4>Escalation Workflow</h4>' + (escalations.length ? '<table style="width:100%;border-collapse:collapse;"><thead><tr><th>Reviewer</th><th>Reason</th><th>Timestamp</th><th>State</th><th>Resolution</th><th>Action</th></tr></thead><tbody>' +
        escalations.map(e =>
            `<tr>
                <td>${e.reviewer}</td>
                <td>${e.reason}</td>
        ...
        ).join('')+'</tbody></table>' : '<div style="color:#e74c3c;">No escalations.</div>');
        escDiv.querySelectorAll('.resolve-escalation,.reject-escalation').forEach(btn => {
            btn.onclick = function() {
                ...
                fetch('/api/analytics/reviewer_escalations', {
                    ...
                    body:JSON.stringify({reviewer, timestamp, state, resolution_reason: reason})
                }).then(r=>r.json()).then(()=>fetchReviewerAnalytics());
            }
        });
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load from backend
    fetch('/api/notification_settings').then(r=>r.json()).then(settings => {
    ...
    notifForm.onsubmit = function(e) {
        ...
        fetch('/api/notification_settings', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({in_app: notifInApp.checked, email: notifEmail.checked})
        }).then(r=>r.ok ? notifSaved.style.display='inline' : null);
        setTimeout(()=>{notifSaved.style.display='none';}, 1500);
    };
    ...
    auditForm.onsubmit = function(e) {
        ...
        fetch(`/api/audit/search?q=${encodeURIComponent(q)}&date_from=${encodeURIComponent(dateFrom)}&date_to=${encodeURIComponent(dateTo)}&action_type=${encodeURIComponent(actionType)}`)
        .then(r=>r.json()).then(res => {
            const table = document.getElementById('auditResultsTable');
            const tbody = table.querySelector('tbody');
            auditLoading.style.display = 'none';
            if (res.results && res.results.length) {
                tbody.innerHTML = res.results.map(row => `<tr><td>${row.timestamp}</td><td>${row.user}</td><td>${row.action}</td><td>${row.ip}</td></tr>`).join('');
                table.style.display = '';
                auditEmpty.style.display = 'none';
            } else {
                tbody.innerHTML = '';
    ...
    function pollNotifications() {
        fetch('/api/notifications').then(r=>r.json()).then(data => {
            ...
            if (notifDiv && data.alerts && data.alerts.length) {
                notifDiv.innerHTML = data.alerts.map(a => `<div class='alert'>${a}</div>`).join('');
            }
        });
    ...
    function loadCustomTemplates() {
        fetch('/legal_export/custom/list').then(r=>r.json()).then(list => {
            ...
            listDiv.querySelectorAll('.delete-template').forEach(btn => {
                btn.onclick = function() {
                    fetch('/legal_export/custom/delete/' + btn.dataset.fn, {method:'POST'})
                    .then(r => { if(r.ok) loadCustomTemplates(); });
                };
            });
        });
    }
