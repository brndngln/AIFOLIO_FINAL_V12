...
from ai_engines.prompt_optimizer import optimize_prompt
from ai_engines.visual_context import select_visuals
...
def ai_action_with_oversight(input_data, engine_fn, context=""):
    ...
    with open('../analytics/ai_audit.log', 'a') as logf:
        logf.write(f"AI_ACTION|{datetime.now()}|{engine_fn.__name__}|Context:{context}|Input:{input_data}|Output:{result}\n")
    # Double-backup
...
def login():
    ...
    if request.method == 'POST':
        ...
        if LOGIN_ATTEMPTS[ip] >= LOCKOUT_THRESHOLD:
            ...
            return LOGIN_PAGE + '<p>Too many failed attempts. Locked out for 15 minutes.</p>'
        return LOGIN_PAGE + '<p>Access Denied</p>'
    # GET: render login with CSRF
    csrf_token = generate_csrf_token()
    return LOGIN_PAGE.replace('</form>', f'<input type="hidden" name="csrf_token" value="{csrf_token}"/></form>')
...
def audit_trail():
    ...
    try:
        ...
        pass
    return render_template('dashboard.html', content='<h3>Audit Trail</h3><pre>' + ''.join(logs) + '</pre><a href="/dashboard">Back</a>')
