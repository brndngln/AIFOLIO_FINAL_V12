...
def accessibility_audit():
    """Accessibility audit with CSRF and double audit log."""
    if request.method == 'POST':
        ...
        return render_template('accessibility_report.html', summary=summary, report=report)
...
def accessibility_audit_route():
    ...
    return render_template('dashboard.html', content=f'<h3>Accessibility Audit Results for {route_url}</h3><pre>{summary}</pre><a href="/dashboard">Back</a>')
