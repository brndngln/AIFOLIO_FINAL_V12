...
def test_manual_override():
    ...
    r = requests.post(f'{API_BASE}/rotation/manual_override', json={
        'adminId': ADMIN_ID,
    ...
    assert r.status_code == 200 and r.json().get('success'), 'Manual override should succeed with valid MFA'
    log('Manual override succeeded with valid MFA.')

def test_freeze_toggle():
    log('Testing freeze toggle (pause)...')
    r = requests.post(f'{API_BASE}/rotation/toggle', json={
        'adminId': ADMIN_ID,
        'code': MFA_CODE,
        'enabled': False
    })
    assert r.status_code == 200 and not r.json().get('enabled'), 'Freeze should set enabled to False'
    ...
    assert r.status_code == 200 and r.json().get('enabled'), 'Unfreeze should set enabled to True'
