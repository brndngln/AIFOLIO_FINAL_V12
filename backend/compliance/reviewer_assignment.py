...
def assign_reviewer(violation, exclude=None):
    ...
    candidates = [r for r in reviewers if r['type']=='human' and r['id'] not in exclude]
    if not candidates:
    ...
    for i, v in enumerate(violations):
        if v.get('status') == 'open' and not v.get('assigned_reviewer'):
            ...
            if reviewer_id:
                ...
                v.setdefault('assignment_history', []).append({'reviewer': reviewer_id, 'timestamp': datetime.utcnow().isoformat()})
...
def reviewer_leaderboard():
    ...
    stats = defaultdict(lambda: {'assigned': 0, 'resolved': 0, 'sla_met': 0, 'badges': []})
    ...
    leaderboard = []
    for r in reviewers:
        entry = {'id': r['id'], 'name': r.get('name'), 'type': r['type'], 'accuracy': r.get('accuracy',1.0)}
