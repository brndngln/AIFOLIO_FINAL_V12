...
SEVERITY_TIERS = {"critical": 1, "major": 2, "minor": 3, "info": 4}
...
def scan_pdf_text(text, platforms=None):
    ...
    export_compliance_manifest(
        "SAFE_AI_COMPLIANCE_REPORT.md", "backend/compliance/compliance_report.pdf"
    )
...
def log_violations(doc_id, violations, detected_by):
    ...
    for v in violations:
        ...
        v["status"] = "open"
        v["history"] = [
            {"action": "detected", "timestamp": v["detected_at"], "by": detected_by}
        ]
