...
RECOMMEND_PATH = Path(__file__).parent.parent / 'logs' / 'remediation_recommendations.json'
...
def update_remediation_status(idx, status, reviewer=None):
    ...
    if idx is not None and 0 <= idx < len(workflows):
        ...
        workflows[idx]['history'].append({'status': status, 'timestamp': datetime.utcnow().isoformat(), 'reviewer': reviewer})
        with open(WORKFLOW_PATH, 'w') as f:
            json.dump(workflows, f, indent=2)
