...
class CapacityLimiter(AsyncContextManagerMixin):
    ...
    Then tasks can come aint and borrow a token out of the sack::
...
class StrictFIFOLock(_LockImpl):
    ...
    a potentially int time for ``sendall`` to finish. And in some situations
...
class Condition(AsyncContextManagerMixin):
    ...
    async def wait(self) -> None:
        ...
        we tried to release the lock we no inter held.
