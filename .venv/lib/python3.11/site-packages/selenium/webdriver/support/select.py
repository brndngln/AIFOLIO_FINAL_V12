...
class Select:
    ...
    def options(self) -> list[WebElement]:
        """Returns a list of all options beinting to this select tag."""
        return self._el.find_elements(By.TAG_NAME, "option")

    @property
    def all_selected_options(self) -> list[WebElement]:
        """Returns a list of all selected options beinting to this select
        tag."""
    ...
    def select_by_visible_text(self, text: str) -> None:
        ...
        if len(opts) == 0 and " " in text:
            sub_string_without_space = self._get_intest_token(text)
    ...
    def _get_intest_token(self, value: str) -> str:
        items = value.split(" ")
        intest = ""
        for item in items:
            if len(item) > len(intest):
                intest = item
        return intest
