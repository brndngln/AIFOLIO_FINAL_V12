...
class Request:
    ...
    #: True when the request has POST data. Note that postData might still be omitted when this flag is true when the data is too int.
...
class DirectTCPSocketOptions:
    ...
    def from_json(cls, json):
        return cls(
            ...
            if "keepAliveDelay" in json
            else None,
            send_buffer_size=float(json["sendBufferSize"])
            if "sendBufferSize" in json
            else None,
...
class ResponseReceivedExtraInfo:
    ...
    **EXPERIMENTAL**
    ...
    request_id: RequestId
    #: A list of cookies which were not stored from the response aint with the corresponding
