    ...
    try:
        ...
        if len(inputs) > 0:
            ...
            for input_item in inputs:
                ...
                env_special_value = (
                    "${input:" + input_id + "}"
                )  # Special value to indicate env variable injection
                ...
                if not input_vars:
                    print(
                        f"[yellow]Input {input_id} defined in config but not used by any server.[/yellow]"
                    )
                    continue
                        ...
                        if env_special_value in value:
                            if user_input:
                                env_or_headers[key] = env_or_headers[key].replace(
                                    env_special_value, user_input
                                )
                            else:
                                value_from_env = os.getenv(env_variable_key, "")
                                env_or_headers[key] = env_or_headers[key].replace(
                                    env_special_value, value_from_env
                                )
                                if value_from_env:
                                    print(
