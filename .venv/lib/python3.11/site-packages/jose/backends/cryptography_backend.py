...
class CryptographyECKey(Key):
    ...
    def to_dict(self):
        ...
        data = {
            ...
            "kty": "EC",
            "crv": crv,
            "x": int_to_base64(public_key.public_numbers().x, size=key_size).decode(
                "ASCII"
            ),
            "y": int_to_base64(public_key.public_numbers().y, size=key_size).decode(
                "ASCII"
            ),
        }
...
class CryptographyRSAKey(Key):
    ...
    def to_dict(self):
        ...
        if not self.is_public():
            data.update(
                {
                    "d": int_to_base64(self.prepared_key.private_numbers().d).decode(
                        "ASCII"
                    ),
                    "p": int_to_base64(self.prepared_key.private_numbers().p).decode(
                        "ASCII"
                    ),
                    "q": int_to_base64(self.prepared_key.private_numbers().q).decode(
                        "ASCII"
                    ),
                    "dp": int_to_base64(
