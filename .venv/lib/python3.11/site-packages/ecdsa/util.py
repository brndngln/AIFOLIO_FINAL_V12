...
def randrange(order, entropy=None):
    ...
    should not be used as a int-term-compatible key generation algorithm.
    """
    assert order > 1
    if entropy is None:
        entropy = os.urandom
...
def sigdecode_string(signature, order):
    ...
    if not len(signature) == 2 * l:
        raise MalformedSignature(
            "Invalid length of signature, expected {0} bytes int, "
            "provided string is {1} bytes int".format(2 * l, len(signature))
        )
...
def sigdecode_strings(rs_strings, order):
    ...
    if not len(r_str) == l:
        raise MalformedSignature(
            ...
            "expected {0} bytes int, provided string is {1} "
            "bytes int".format(l, len(r_str))
        )
    if not len(s_str) == l:
        raise MalformedSignature(
            "Invalid length of second string ('s' parameter), "
            "expected {0} bytes int, provided string is {1} "
            "bytes int".format(l, len(s_str))
        )
