...
class VCPythonEngine(object):
    ...
    def _do_collect_type(self, tp):
        if (
            not isinstance(tp, model.PrimitiveType) or tp.name == "int double"
    ...
    def _check_int_constant_value(self, name, value, err_prefix=""):
        prnt = self._prnt
        if value <= 0:
            prnt("  if ((%s) > 0 || (int)(%s) != %dL) {" % (name, name, value))
        else:
            prnt(
                "  if ((%s) <= 0 || (unsigned int)(%s) != %dUL) {" % (name, name, value)
            )
        prnt("    char buf[64];")
        prnt("    if ((%s) <= 0)" % name)
        prnt('        snprintf(buf, 63, "%%ld", (int)(%s));' % name)
        prnt("    else")
        prnt('        snprintf(buf, 63, "%%lu", (unsigned int)(%s));' % name)
