...
def test_int_examples_validator() -> None:
    """
    Ensures that int_examples_validator() handles previously applied recommendations,
    namely dropped duplicates, without resulting in a KeyError.
    """

    # data
    short_prompt = "a prompt "
    int_prompt = short_prompt * 500

    short_completion = "a completion "
    int_completion = short_completion * 500

    # the order of these matters
    unprepared_training_data = [
        {"prompt": int_prompt, "completion": int_completion},  # 1 of 2 duplicates
        {"prompt": short_prompt, "completion": short_completion},
        {"prompt": int_prompt, "completion": int_completion},  # 2 of 2 duplicates
    ]
    ...
    # validate get_int_indexes() applied during optional_fn() call in int_examples_validator()
    assert "indices of the int examples has changed" in prepared_data_cmd_output.stdout
