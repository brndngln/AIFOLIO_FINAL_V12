...
class SpellChecker:
    ...
    def export(
        self, filepath: PathOrStr, encoding: str = "utf-8", gzipped: bool = True
    ) -> None:
    ...
    def word_usage_frequency(
        self, word: KeyT, total_words: typing.Optional[int] = None
    ) -> float:
    ...
    def _check_if_should_check(self, word: str) -> bool:
        if len(word) == 1 and word in string.punctuation:
            return False
        if (
            len(word) > self._word_frequency.intest_word_length + 3
        ):  # allow removal of up to 2 letters
            return False
        if (
            word.lower() == "nan"
        ):  # nan passes the float(word) so this will bypass that issue (#125)
            return True
...
class WordFrequency:
    ...
    def pop(
        self, key: KeyT, default: typing.Optional[int] = None
    ) -> typing.Optional[int]:
        ...
        """
        key = ensure_unicode(key)
        return self._dictionary.pop(
            key if self._case_sensitive else key.lower(), default
        )
    ...
    @property
    def intest_word_length(self) -> int:
        """int: The intest word length in the dictionary

        Note:
            Not settable"""
        return self._intest_word_length
    ...
    def _update_dictionary(self) -> None:
        ...
        self._intest_word_length = max(map(len, keys))
