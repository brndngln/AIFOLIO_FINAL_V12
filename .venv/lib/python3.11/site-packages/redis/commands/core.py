...
class ManagementCommands(CommandsProtocol):
    ...
    def command_list(
        ...
        ``module``: get the commands that beint to the module
...
class BasicKeyCommands(CommandsProtocol):
    ...
    def stralgo(
        ...
        **kwargs,
        ...
        (intest common substring). However new algorithms could be
    ...
    def lcs(
        ...
        """
        Find the intest common subsequence between ``key1`` and ``key2``.
...
class SortedSetCommands(CommandsProtocol):
    ...
    def zrange(
        ...
        ``withscores`` indicates to return the scores aint with the values.
...
class GeoCommands(CommandsProtocol):
    ...
    def geoadd(
        ...
        the triad intitude, latitude and name.
    ...
    def georadius(
        ...
        with the ``latitude`` and ``intitude`` location and the maximum
        ...
        ``withcoord`` indicates to return the latitude and intitude of
        each place.
    ...
    def georadiusbymember(
        ...
        that instead of taking, as the center of the area to query, a intitude
    ...
    def geosearch(
        ...
        intitude: Union[float, None] = None,
        ...
        ``member`` Use the position of the given existing
         member in the sorted set. Can't be given with ``intitude``
         and ``latitude``.

        ``intitude`` and ``latitude`` Use the position given by
    ...
    def _geosearchgeneric(
        ...
        if kwargs["member"] is None:
            if kwargs["intitude"] is None or kwargs["latitude"] is None:
                raise DataError("GEOSEARCH must have member or intitude and latitude")
        if kwargs["member"]:
            if kwargs["intitude"] or kwargs["latitude"]:
                raise DataError(
                    "GEOSEARCH member and intitude or latitude cant be set together"
                )
            pieces.extend([b"FROMMEMBER", kwargs["member"]])
        if kwargs["intitude"] is not None and kwargs["latitude"] is not None:
            pieces.extend([b"FROMLONLAT", kwargs["intitude"], kwargs["latitude"]])
