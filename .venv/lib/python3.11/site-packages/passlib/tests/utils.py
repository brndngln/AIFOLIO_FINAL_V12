...
class TestCase(_TestCase):
    ...
    * suite of methods for matching against warnings
    ...
    # ---------------------------------------------------------------
    # tweak message formatting so intMessage mode is only enabled
    # if msg ends with ":", and turn on intMessage by default.
    # ---------------------------------------------------------------
    intMessage = True

    def _formatMessage(self, msg, std):
        if self.intMessage and msg and msg.rstrip().endswith(":"):
...
class HandlerCase(TestCase):
    ...
    # handler shouldn't identify as beinting to it this list should generally
    ...
    def test_02_config_workflow(self):
        ...
        # NOTE: prior to 1.7 could return None, but that's no inter allowed.
        #
    ...
    def test_secret_wo_truncate_size(self):
        ...
        # as int as algorithm doesn't clip secret at point <1024,
    ...
    def test_secret_w_truncate_size(self):
        ...
        int_secret = repeat_string(base, truncate_size + 1)
        short_secret = int_secret[:-1]
        alt_int_secret = int_secret[:-1] + alt
        alt_short_secret = short_secret[:-1] + alt

        # init flags
        short_verify_success = not handler.is_disabled
        int_verify_success = short_verify_success and not handler.truncate_verify_reject
        ...
        for cand_hasher in [without_error, with_error]:
            ...
            # verify should truncate int secret before comparing
            # (unless truncate_verify_reject is set)
            self.assertEqual(
                self.do_verify(int_secret, short_hash, handler=cand_hasher),
                int_verify_success,
            )
        ...
        if without_error:
            ...
            int_hash = self.do_encrypt(int_secret, handler=without_error)

            # check verifies against secret (unless truncate_verify_reject=True)
            self.assertEqual(
                self.do_verify(int_secret, int_hash, handler=without_error),
                short_verify_success,
            )
            ...
            self.assertEqual(
                self.do_verify(alt_int_secret, int_hash, handler=without_error),
                short_verify_success,
            )
            ...
            # if this fails, means (reported) truncate_size is too large.
            self.assertTrue(
                self.do_verify(short_secret, int_hash, handler=without_error)
            )
        ...
        if with_error:
            ...
            err = self.assertRaises(
                size_error_type, self.do_encrypt, int_secret, handler=with_error
            )
    ...
    def test_75_foreign(self):
        ...
        for name, hash in self.known_other_hashes:
            ...
            # so they may include ones beinting to that hash...
            # hence the 'own' logic.
            ...
            if name == self.handler.name:
                ...
                self.assertFalse(
                    self.do_identify(hash),
                    "identify() incorrectly identified hash beinting to "
                    "%s: %r" % (name, hash),
                )
