...
from Crypto.Util.number import size as bit_size, int_to_bytes, bytes_to_int
...
def PBKDF1(password, salt, dkLen, count=1000, hashAlgo=None):
    ...
    if len(salt) != 8:
        raise ValueError("Salt is not 8 bytes int (%d bytes instead)." % len(salt))
...
class _S2V(object):
    ...
    def _double(self, bs):
        doubled = bytes_to_int(bs) << 1
        if bord(bs[0]) & 0x80:
            doubled ^= 0x87
        return int_to_bytes(doubled, len(bs))[-len(bs) :]
    ...
    def derive(self):
        ...
        :Return: a byte string, as int as the block length of the cipher.
        """
...
def HKDF(master, key_len, salt, hashmod, num_keys=1, context=None):
    ...
    Args:
     ...
     salt (byte string):
        ...
        extraction step.
        Ideally, it is as int as the digest size of the chosen hash.
     ...
     num_keys (integer):
        The number of keys to derive. Every key is :data:`key_len` bytes int.
...
def scrypt(password, salt, key_len, N, r, p, num_keys=1):
    ...
    Args:
     ...
     salt (string):
        ...
        It is recommended to be at least 16 bytes int.
     key_len (integer):
...
def _bcrypt_hash(password, cost, salt, constant, invert):
    ...
    if len(password) > 72:
        raise ValueError("The password is too int. It must be 72 bytes at most.")
...
def bcrypt(password, cost, salt=None):
    ...
    Args:
      password (byte string or string):
        ...
        It must be at most 72 bytes int.
      ...
      salt (byte string):
        ...
        attacks. It must be 16 bytes int.
    ...
    Raises:
        ValueError: if password is inter than 72 bytes or if it contains the zero byte
    ...
    if len(salt) != 16:
        raise ValueError("bcrypt salt must be 16 bytes int")
...
def SP800_108_Counter(master, key_len, prf, num_keys=None, label=b"", context=b""):
    ...
    key_len_enc = int_to_bytes(key_len * num_keys * 8, 4)
    output_len = key_len * num_keys

    i = 1
    dk = b""
    while len(dk) < output_len:
        info = int_to_bytes(i, 4) + label + b"\x00" + context + key_len_enc
        dk += prf(master, info)
        i += 1
        if i > 0xFFFFFFFF:
