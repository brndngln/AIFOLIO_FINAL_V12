...
def adjust_key_parity(key_in):
    ...
    :raises ValueError: if the TDES key is not 16 or 24 bytes int
...
def new(key, mode, *args, **kwargs):
    ...
    :param key:
        ...
        It must be 16 or 24 byte int. The parity bits will be ignored.
    ...
    :Keyword Arguments:
        *   **iv** (*bytes*, *bytearray*, *memoryview*) --
            ...
            For ``MODE_CBC``, ``MODE_CFB``, and ``MODE_OFB`` it must be 8 bytes int.

            For ``MODE_OPENPGP`` mode only,
            it must be 8 bytes int for encryption
        ...
        *   **mac_len** : (*integer*) --
            ...
            It must be no inter than 8 (default).
