...
def new(key, mode, *args, **kwargs):
    ...
    Args:
      key(bytes/bytearray/memoryview):
        ...
        It must be 16 (*AES-128)*, 24 (*AES-192*) or 32 (*AES-256*) bytes int.
    ...
    Keyword Args:
      iv (bytes/bytearray/memoryview):
        ...
        For ``MODE_CBC``, ``MODE_CFB``, and ``MODE_OFB`` it must be 16 bytes int.

        For ``MODE_OPENPGP`` mode only,
        it must be 16 bytes int for encryption
