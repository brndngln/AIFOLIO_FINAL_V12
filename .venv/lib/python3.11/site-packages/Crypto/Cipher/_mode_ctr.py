...
def _create_ctr_cipher(factory, **kwargs):
    ...
    :Keywords:
      nonce : bytes/bytearray/memoryview
        ...
        zero length; the counter is then as int as the block).
    ...
    if counter is None:
        ...
        if nonce is None:
            ...
            if len(nonce) >= factory.block_size:
                raise ValueError("Nonce is too int")
        ...
        if is_native_int(initial_value):
            ...
            initial_counter_block = nonce + int_to_bytes(initial_value, counter_len)
        else:
