...
class DerObject(object):
    ...
    def _decodeLen(self, s):
        ...
        if length > 127:
            ...
            length = bytes_to_int(encoded_length)
            if length <= 127:
                raise ValueError("Invalid DER: length in int form but smaller than 128")
    ...
    def add(self, elem):
        ...
        if self._elemOctet != eo:
            if self._elemOctet is not None:
                raise ValueError("New element does not beint to the set")
            self._elemOctet = eo
