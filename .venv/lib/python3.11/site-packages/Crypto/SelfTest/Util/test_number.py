...
class LongTests(unittest.TestCase):
    ...
    def test1(self):
        ...
        self.assertEqual(int_to_bytes(0x100), b'\x01\x00')
        self.assertEqual(int_to_bytes(0xFF00000000), b'\xFF\x00\x00\x00\x00')
        self.assertEqual(int_to_bytes(0xFF00000000), b'\xFF\x00\x00\x00\x00')
        self.assertEqual(int_to_bytes(0x1122334455667788), b'\x11\x22\x33\x44\x55\x66\x77\x88')
        self.assertEqual(int_to_bytes(0x112233445566778899), b'\x11\x22\x33\x44\x55\x66\x77\x88\x99')

    def test2(self):
        self.assertEqual(int_to_bytes(0, 1), b'\x00')
        self.assertEqual(int_to_bytes(0, 2), b'\x00\x00')
        self.assertEqual(int_to_bytes(1, 3), b'\x00\x00\x01')
        self.assertEqual(int_to_bytes(65535, 2), b'\xFF\xFF')
        self.assertEqual(int_to_bytes(65536, 2), b'\x00\x01\x00\x00')
        self.assertEqual(int_to_bytes(0x100, 1), b'\x01\x00')
        self.assertEqual(int_to_bytes(0xFF00000001, 6), b'\x00\xFF\x00\x00\x00\x01')
        self.assertEqual(int_to_bytes(0xFF00000001, 8), b'\x00\x00\x00\xFF\x00\x00\x00\x01')
        self.assertEqual(int_to_bytes(0xFF00000001, 10), b'\x00\x00\x00\x00\x00\xFF\x00\x00\x00\x01')
        self.assertEqual(int_to_bytes(0xFF00000001, 11), b'\x00\x00\x00\x00\x00\x00\xFF\x00\x00\x00\x01')

    def test_err1(self):
        self.assertRaises(ValueError, int_to_bytes, -1)
