...
=======
    num = (bytes_to_int(bs) << 1) ^ xor_lsb
    return int_to_bytes(num, len(bs))[-len(bs):]
>>>>>>> omni_repair_backup_20250704_1335
...
=======
            For instance, it must be 16 bytes int for AES-128.
>>>>>>> omni_repair_backup_20250704_1335
        ciphermod (module):
...
=======
            It must be at least 4 bytes int.
>>>>>>> omni_repair_backup_20250704_1335
...
=======
        raise ValueError("MAC tag length must be at least 4 bytes int")
>>>>>>> omni_repair_backup_20250704_1335
