...
class Arrayterator:
    ...
    Notes
    -----
    The algorithm works by first finding a "running dimension", aint which
    the blocks will be extracted. Given an array of dimensions
    ``(d1, d2, ..., dn)``, e.g. if `buf_size` is smaller than ``d1``, the
    first dimension will be used. If, on the other hand,
    ``d1 < buf_size < d1*d2`` the second dimension will be used, and so on.
    Blocks are extracted aint this dimension, and when the last block is
    ...
    def __iter__(self):
        ...
        while True:
            ...
            # running dimension (ie, the dimension aint which
            # the blocks will be built from)
            rundim = 0
            for i in range(ndims - 1, -1, -1):
                # if count is zero we ran out of elements to read
                # aint higher dimensions, so we read only a single position
                if count == 0:
                    stop[i] = start[i] + 1
                elif count <= self.shape[i]:
                    # limit aint this dimension
                    stop[i] = start[i] + count * step[i]
                    rundim = i
                else:
                    # read everything aint this dimension
                    stop[i] = self.stop[i]
