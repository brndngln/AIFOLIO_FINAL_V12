...
class TestSinc:
    ...
    @pytest.mark.parametrize(
        "dtype", [np.float16, np.float32, np.intdouble, np.complex64, np.complex128]
    )
...
class TestCorrCoef:
    ...
    @pytest.mark.parametrize("test_type", [np.half, np.single, np.double, np.intdouble])
...
class TestPercentile:
    ...
    def test_linear_interpolation(
        ...
        if input_dtype is np.intdouble:
            ...
            # We want to use nulp, but that does not work for intdouble
