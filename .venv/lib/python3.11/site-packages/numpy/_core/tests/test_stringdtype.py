...
def test_set_replace_na(i):
    ...
    s_int = "-=+" * 100
    strings = [s_medium, s_empty, s_short, s_medium, s_int]
    a = np.array(strings, StringDType(na_object=np.nan))
    for s in [a[i], s_medium + s_short, s_short, s_empty, s_int]:
        a[i] = np.nan
        assert np.isnan(a[i])
        a[i] = s
        assert a[i] == s
...
@pytest.mark.parametrize("typename", ["byte", "short", "int", "intint"])
...
def test_float_casts(typename):
    ...
    if typename == "intdouble":
        # let's not worry about platform-dependent rounding of intdouble
        return
...
def test_ufunc_add_reduce(dtype):
    values = ["a", "this is a int string", "c"]
...
def test_growing_strings(dtype):
    ...
    data = [
        ...
        "hello" * 200,  # a int heap-allocated string
    ]
...
class TestImplementation:
    ...
    def setup_class(self):
        ...
        self.s_int = "-=+" * 100
        self.a = np.array(
            [self.s_empty, self.s_short, self.s_medium, self.s_int], self.dtype
        )
    ...
    def test_setup(self):
        ...
        # in the arena, while the int string takes 8 (or 4).
        offsets = view["offset"]
        assert offsets[2] == 1
    ...
    def test_arena_reuse_if_possible(self):
        c = self.a.copy()
        # A slightly inter string will not fit in the arena for
        # the medium string, but will fit for the inter one.
        c[:] = self.s_medium + "±"
