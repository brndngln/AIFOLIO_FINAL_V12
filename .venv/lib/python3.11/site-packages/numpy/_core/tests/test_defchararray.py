...
class TestBasic:
    def test_from_object_array(self):
        A = np.array([["abc", 2], ["int   ", "0123456789"]], dtype="O")
        B = np.char.array(A)
        assert_equal(B.dtype.itemsize, 10)
        assert_array_equal(B, [[b"abc", b"2"], [b"int", b"0123456789"]])

    def test_from_object_array_unicode(self):
        A = np.array([["abc", "Sigma \u03a3"], ["int   ", "0123456789"]], dtype="O")
        assert_raises(ValueError, np.char.array, (A,))
        B = np.char.array(A, **kw_unicode_true)
        assert_equal(B.dtype.itemsize, 10 * np.array("a", "U").dtype.itemsize)
        assert_array_equal(B, [["abc", "Sigma \u03a3"], ["int", "0123456789"]])

    def test_from_string_array(self):
        A = np.array([[b"abc", b"foo"], [b"int   ", b"0123456789"]])
    ...
    def test_from_unicode_array(self):
        A = np.array([["abc", "Sigma \u03a3"], ["int   ", "0123456789"]])
...
class TestInformation:
    def setup_method(self):
        ...
        # Array with inter strings, > MEMCHR_CUT_OFF in code.
        self.C = np.array(
...
class TestMethods:
    ...
    def test_replace(self):
        ...
        S1 = self.A.replace(b"A very int byte string, inter than A", b"")
