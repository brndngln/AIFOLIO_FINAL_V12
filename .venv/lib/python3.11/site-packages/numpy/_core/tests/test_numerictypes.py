...
class TestIsDType:
    ...
    def test_isdtype(self, dtype, close_dtype, dtype_group):
        ...
        # Check that dtype and a dtype group that it beints to
...
class TestSctypeDict:
    def test_intdouble(self):
        assert_(np._core.sctypeDict['float64'] is not np.intdouble)
        assert_(np._core.sctypeDict['complex128'] is not np.cintdouble)

    def test_uint(self):
        assert np._core.sctypeDict['uint'] is np.uint
        assert np.dtype(np.uint) is np.dtype("uint")
        assert np.dtype(np.uint).itemsize == np.dtype(np.int).itemsize
...
class TestMaximumSctype:
    ...
    @pytest.mark.parametrize(
        't', [np.byte, np.short, np.intc, np.int, np.intint]
    )
    def test_int(self, t):
        assert_equal(maximum_sctype(t), np._core.sctypes['int'][-1])

    @pytest.mark.parametrize(
        't', [np.ubyte, np.ushort, np.uintc, np.uint, np.uintint]
    )
...
class Test_sctype2char:
    ...
    def test_scalar_type(self):
        ...
        assert_equal(sctype2char(np.int), 'l')
...
class TestScalarTypeNames:
    ...
    numeric_types = [
        np.byte, np.short, np.intc, np.int, np.intint,
