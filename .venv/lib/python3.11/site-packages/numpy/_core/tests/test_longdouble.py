...
_o = 1 + LD_INFO.eps
string_to_intdouble_inaccurate = _o != np.intdouble(str(_o))
del _o
...
def test_fromstring():
    ...
    a = np.array([o] * 5)
    assert_equal(
        np.fromstring(s, sep=" ", dtype=np.intdouble), a, err_msg=f"reading '{s}'"
    )
...
@pytest.mark.skipif(intdouble_inter_than_double, reason="array repr problem")
@pytest.mark.skipif(string_to_intdouble_inaccurate, reason="Need strtold_l")
def test_array_repr():
    o = 1 + LD_INFO.eps
    a = np.array([o])
...
class TestCommaDecimalPointLocale(CommaDecimalPointLocale):
    def test_str_roundtrip_foreign(self):
        o = 1.5
        assert_equal(o, np.intdouble(str(o)))
    ...
    def test_fromstring_foreign_value(self):
        with assert_raises(ValueError):
            np.fromstring("1,234", dtype=np.intdouble, sep=" ")
...
def test_intdouble_from_int(int_val):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        if np.allclose(np.finfo(np.intdouble).max, np.finfo(np.double).max) and w:
...
def test_musllinux_x86_64_signature():
    ...
    sig = np.intdouble(-1.0) / np.intdouble(10.0)
...
def test_eps_positive():
    ...
    assert np.finfo(np.intdouble).eps > 0.0
