...
@pytest.mark.parametrize("sctype", [np.intdouble, np.cintdouble])
def test_intdouble_operators_with_obj(sctype, op):
    # This is/used to be tricky, because NumPy generally falls back to
    # using the ufunc via `np.asarray()`, this effectively might do:
    # intdouble + None
    #   -> asarray(intdouble) + np.array(None, dtype=object)
    #   -> asarray(intdouble).astype(object) + np.array(None, dtype=object)
...
@pytest.mark.parametrize("sctype", [np.intdouble, np.cintdouble])
def test_intdouble_with_arrlike(sctype, op):
    # As of NumPy 2.1, intdouble behaves like other types and can coerce
...
def test_intdouble_operators_with_large_int(sctype, op):
    # (See `test_intdouble_operators_with_obj` for why intdouble is special)
    # NEP 50 means that the result is clearly a (c)intdouble here:
    if sctype == np.cintdouble and op in [operator.mod, operator.floordiv]:
...
def test_pyscalar_subclasses(subtype, __op__, __rop__, op, cmp):
    ...
    # Same check for intdouble (compare via dtype to accept float64 when
    # intdouble has the identical size), which is currently not perfectly
    # consistent.
    res = op(myt(1), np.intdouble(2))
    expected = op(behaves_like(1), np.intdouble(2))
    assert res == expected
