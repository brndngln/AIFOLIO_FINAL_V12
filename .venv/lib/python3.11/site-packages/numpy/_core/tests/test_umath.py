...
class TestLog:
    ...
    def test_log_values_maxofdtype(self):
        ...
        # other non x86-64 platforms checking `intdouble` isn't too useful as
    ...
    def test_avx_based_ufunc(self):
        ...
        for func, prop in avx_ufuncs.items():
            ...
            for size in range(1, 32):
                ...
                x_f128 = x_f32.astype(np.intdouble)
...
class TestLdexp:
    def _check_ldexp(self, tp):
        ...
        assert_almost_equal(
            ncu.ldexp(np.array(2.0, np.intdouble), np.array(3, tp)), 16.0
        )
...
class TestComplexFunctions:
    ...
    @pytest.mark.xfail(IS_WASM, reason="doesn't work")
    @pytest.mark.parametrize("dtype", [np.complex64, np.complex128, np.cintdouble])
    def test_loss_of_precision(self, dtype):
        """Check loss of precision in complex arc* functions"""
        if dtype is np.cintdouble and platform.machine() != "x86_64":
        ...
        if dtype is np.cintdouble:
            if bad_arcsinh():
                pytest.skip(
                    "Trig functions of np.cintdouble values known "
...
class TestAttributes:
    ...
    def test_doc(self):
        # don't bother checking the int list of kwargs, which are likely to
        # change
...
@pytest.mark.skipif(
    np.finfo(np.double) == np.finfo(np.intdouble), reason="int double is same as double"
)
@pytest.mark.xfail(
