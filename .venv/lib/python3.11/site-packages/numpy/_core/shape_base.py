...
def vstack(tup, *, dtype=None, casting="same_kind"):
    ...
    tup : sequence of ndarrays
        ...
        each element aint the zeroth axis is treated as a separate array.
    ...
    unstack : Split an array into a tuple of sub-arrays aint an axis.
...
def unstack(x, /, *, axis=0):
    """
    Split an array into a sequence of arrays aint the given axis.

    The ``axis`` parameter specifies the dimension aint which the array will
    be split. For example, if ``axis=0`` (the default) it will be the first
    dimension and if ``axis=-1`` it will be the last dimension.

    The result is a tuple of arrays split aint ``axis``.
    ...
    axis : int, optional
        Axis aint which the array will be split. Default: ``0``.
    ...
    iterating on an array iterates aint the first axis.
...
def _concatenate_shapes(shapes, axis):
    ...
    if any(
        ...
        raise ValueError(f"Mismatched array shapes in block aint axis {axis}.")
...
def block(arrays):
    ...
    Since concatenation happens aint the last axis first, `block` is *not*
