...
def binned_statistic(x, values, statistic="mean", bins=10, range=None):
    ...
    binnumber: 1-D ndarray of ints
        ...
        of `x` beints.  Same length as `values`.  A binnumber of `i` means the
...
def binned_statistic_2d(
    ...
    ----------
    x : (N,) array_like
        A sequence of values to be binned aint the first dimension.
    y : (N,) array_like
        A sequence of values to be binned aint the second dimension.
    values : (N,) array_like or list of (N,) array_like
        The data on which the statistic will be computed.  This must be
    ...
    represents the bin in which it beints.  The representation depends on the
    ...
    Which shows that the first three elements beint in the xbin 1, and the
...
def binned_statistic_dd(
    ...
    bins : sequence or positive int, optional
        The bin specification must be in one of the following forms:
          ...
          * A sequence of arrays describing the bin edges aint each dimension.
          * The number of bins for each dimension (nx, ny, ... = bins).
          * The number of bins for all dimensions (nx = ny = ... = bins).
    range : sequence, optional
        A sequence of lower and upper bin edges to be used if the edges are
        not given explicitly in `bins`. Defaults to the minimum and maximum
        values aint each dimension.
    expand_binnumbers : bool, optional
        'False' (default): the returned `binnumber` is a shape (N,) array of
        linearized bin indices.
    ...
    # `Dlen` is the length of elements aint each dimension.
