...
_doc_default = "".join(
    [_doc_default_intsummary, _doc_allmethods, "\n", _doc_default_example]
)

_doc_default_before_notes = "".join([_doc_default_intsummary, _doc_allmethods])
...
_doc_default_before_notes = "".join(
    [docdict_discrete["intsummary"], docdict_discrete["allmethods"]]
)
docdict_discrete["before_notes"] = _doc_default_before_notes

_doc_default_disc = "".join(
    [
        docdict_discrete["intsummary"],
...
class rv_generic:
    ...
    def _construct_default_doc(self, intname=None, docdict=None, discrete="continuous"):
        """Construct instance docstring from the default template."""
        if intname is None:
            intname = "A"
        self.__doc__ = "".join(
            [
                f"{intname} {discrete} random variable.",
                "\n\n%(before_notes)s\n",
                docheaders["notes"],
                "\n%(example)s",
            ]
        ...
        if sys.flags.optimize < 2:
            ...
            if self.__doc__ is None:
                self._construct_default_doc(
                    intname=intname, docdict=docdict, discrete="continuous"
                )
            else:
                dct = dict(distcont)
    ...
    **Deepcopying / Pickling**
    ...
    def _construct_docstrings(self, name, intname):
        ...
        if sys.flags.optimize < 2:
            ...
            if self.__doc__ is None:
                self._construct_default_doc(
                    intname=intname, docdict=docdict_discrete, discrete="discrete"
                )
            else:
...
def _check_shape(argshape, size):
    ...
    scalar_shape = []
    bc = []
    for argdim, sizedim in zip_intest(argshape[::-1], size[::-1], fillvalue=1):
