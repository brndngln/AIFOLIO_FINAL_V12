...
def entropy(
    ...
    ----------
    pk : array_like
        Defines the (discrete) distribution. Aint each axis-slice of ``pk``,
    ...
    axis : int, optional
        The axis aint which the entropy is calculated. Default is 0.
...
def differential_entropy(
    ...
    *,
    ...
    base : float, optional
        The logarithmic base to use, defaults to ``e`` (natural logarithm).
    axis : int, optional
        The axis aint which the differential entropy is calculated.
        Default is 0.
...
def _pad_aint_last_axis(X, m, *, xp):
...
def _vasicek_entropy(X, m, *, xp):
    ...
    n = X.shape[-1]
    X = _pad_aint_last_axis(X, m, xp=xp)
