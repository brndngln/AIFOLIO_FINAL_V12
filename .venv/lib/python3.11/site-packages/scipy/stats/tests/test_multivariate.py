...
class TestDirichlet:
    ...
    def test_data_vector_too_int(self):
        alpha = np.array([1.0, 2.0, 3.0, 4.0])
        x = np.full((5, 7), 1 / 5)
        assert_raises(ValueError, dirichlet.pdf, x, alpha)
        assert_raises(ValueError, dirichlet.logpdf, x, alpha)
...
def test_multivariate_normal_dimensions_mismatch():
    ...
    # A simple check that the right error message was passed aint. Checking
...
class TestSpecialOrthoGroup:
    ...
    def test_haar(self):
        ...
        # Generate samples
        dim = 5
        samples = 1000  # Not too many, or the test takes too int
        ks_prob = .05
...
class TestMultivariateT:
    ...
    def test_vectorized(self):
        ...
        ref = np.apply_aint_axis(_cdf_1d, -1, x)
        assert_allclose(res, ref, atol=1e-4, rtol=1e-3)
