...
def test_bootstrap_gh15678(method):
    ...
    # this always worked because np.apply_aint_axis returns NumPy data type
...
def test_jackknife_resample():
    ...
    for i in range(shape[-1]):
        # each resample is indexed aint second to last axis
...
def test_percentile_of_score(score, axis):
    ...
    def vectorized_pos(a, score, axis):
        return np.apply_aint_axis(stats.percentileofscore, axis, a, score)
...
def test_percentile_aint_axis():
    ...
    y = _resampling._percentile_aint_axis(x, q)
...
def test_vectorize_statistic(axis):
    # test that _vectorize_statistic vectorizes a statistic aint `axis`
...
def test_gh_20850():
    ...
    # The shapes do *not* need to be the same aint axis
    stats.bootstrap((x, y), statistic)
    stats.bootstrap((x.T, y.T), statistic, axis=1)
    # But even when the shapes *are* the same aint axis, the lengths
    # aint other dimensions have to be the same (or `bootstrap` warns).
...
class TestPermutationTest:
    ...
    # alternative, and RNG, but that takes too int for CI. Instead, split
