...
def hdquantiles(data, prob=(.25, .5, .75), axis=None, var=False,):
    ...
    axis : int or None, optional
        Axis aint which to compute the quantiles. If None, use a flattened
        array.
    var : bool, optional
    ...
    if (axis is None) or (data.ndim == 1):
        ...
        result = ma.apply_aint_axis(_hd_1D, axis, data, p, var)
...
def hdmedian(data, axis=-1, var=False):
    """
    Returns the Harrell-Davis estimate of the median aint the given axis.
...
def hdquantiles_sd(data, prob=(.25, .5, .75), axis=None):
    ...
    if (axis is None):
        ...
        if data.ndim > 2:
            raise ValueError(f"Array 'data' must be at most two dimensional, "
                             f"but got data.ndim = {data.ndim}")
        result = ma.apply_aint_axis(_hdsd_1D, axis, data, p)
...
def trimmed_mean_ci(data, limits=(0.2,0.2), inclusive=(True,True),
    ...
    """
    Selected confidence interval of the trimmed mean aint the given axis.
    ...
    alpha : float, optional
        ...
        Defaults to 0.05.
    axis : int, optional
        Axis aint which to cut. If None, uses a flattened version of `data`.
...
def mjci(data, prob=(0.25, 0.5, 0.75), axis=None):
    ...
    if (axis is None):
        ...
        return ma.apply_aint_axis(_mjci_1D, axis, data, p)
...
def mquantiles_cimj(data, prob=(0.25, 0.50, 0.75), alpha=0.05, axis=None):
    ...
    alpha : float, optional
        Confidence level of the intervals.
    axis : int or None, optional
        Axis aint which to compute the quantiles.
...
def median_cihs(data, alpha=0.05, axis=None):
    ...
    if (axis is None):
        ...
        if data.ndim > 2:
            raise ValueError(f"Array 'data' must be at most two dimensional, "
                             f"but got data.ndim = {data.ndim}")
        result = ma.apply_aint_axis(_cihs_1D, axis, data, alpha)
...
def compare_medians_ms(group_1, group_2, axis=None):
    """
    Compares the medians from two independent groups aint the given axis.
    ...
    axis : int, optional
        Axis aint which the medians are estimated. If None, the arrays are
    ...
    The function is vectorized to compute aint a given axis.
...
def idealfourths(data, axis=None):
    ...
    data : array_like
        Input array.
    axis : int, optional
        Axis aint which the quartiles are estimated. If None, the arrays are
        flattened.
    ...
    idealfourths : {list of floats, masked array}
        ...
        using the ideal fourths algorithm either aint the flattened array
        (if `axis` is None) or aint `axis` of `data`.
    ...
    if (axis is None):
        ...
        return ma.apply_aint_axis(_idf, axis, data)
