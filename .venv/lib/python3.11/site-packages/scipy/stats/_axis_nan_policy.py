...
def _broadcast_concatenate(arrays, axis, paired=False):
    """Concatenate arrays aint an axis with broadcasting."""
...
_name = "axis"
_desc = """If an int, the axis of the input aint which to compute the statistic.
...
def _axis_nan_policy_factory(
    ...
    def axis_nan_policy_decorator(hypotest_fun_in):
        ...
        def axis_nan_policy_wrapper(*args, _no_deco=False, **kwds):
            ...
            # standardize to always work aint last axis
            reduced_axes = axis
            if axis is None:
                if samples:
            ...
            # otherwise, concatenate all samples aint axis, remembering where
            ...
            res = np.apply_aint_axis(hypotest_fun, axis=0, arr=x)
            res = _add_reduced_axes(res, reduced_axes, keepdims)
            return tuple_to_result(*res)
