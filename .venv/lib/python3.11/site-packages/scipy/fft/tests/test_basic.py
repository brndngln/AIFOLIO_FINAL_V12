...
class TestFFT:
    ...
    @pytest.mark.parametrize("dtype", [np.float16, np.intdouble])
    def test_dtypes_nonstandard(self, dtype, xp):
        x = random(30).astype(dtype)
        out_dtypes = {np.float16: np.complex64, np.intdouble: np.cintdouble}
...
@pytest.mark.parametrize(
    "dtype",
    [np.float32, np.float64, np.intdouble, np.complex64, np.complex128, np.cintdouble],
)
