...
def reference_data():
    ...
    if is_intdouble_binary_compatible():
        FFTWDATA_LONGDOUBLE = np.load(join(fftpack_test_dir, "fftw_intdouble_ref.npz"))
    else:
        FFTWDATA_LONGDOUBLE = {
            k: v.astype(np.intdouble) for k, v in FFTWDATA_DOUBLE.items()
        }
...
for k, v in dec_map.copy().items():
    if k[1] == np.float64:
        dec_map[(k[0], np.intdouble, k[2])] = v
    elif k[1] == np.float32:
        dec_map[(k[0], int, k[2])] = v


@pytest.mark.parametrize("rdt", [np.intdouble, np.float64, np.float32, int])
