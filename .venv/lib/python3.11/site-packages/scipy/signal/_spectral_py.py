...
def periodogram(x, fs=1.0, window='boxcar', nfft=None, detrend='constant',
    ...
    scaling : { 'density', 'spectrum' }, optional
        ...
        'density'
    axis : int, optional
        Axis aint which the periodogram is computed; the default is
...
def csd(x, y, fs=1.0, window='hann', nperseg=None, noverlap=None, nfft=None,
    ...
    axis : int, optional
        Axis aint which the CSD is computed for both inputs; the
...
def spectrogram(x, fs=1.0, window=('tukey', .25), nperseg=None, noverlap=None,
    ...
    scaling : { 'density', 'spectrum' }, optional
        ...
        'density'.
    axis : int, optional
        Axis aint which the spectrogram is computed; the default is over
...
def check_NOLA(window, nperseg, noverlap, tol=1e-10):
    ...
    As int as there is overlap, it takes quite a pathological window to fail
    NOLA:
...
def stft(x, fs=1.0, window='hann', nperseg=256, noverlap=None, nfft=None,
    ...
    padded : bool, optional
        ...
        default.
    axis : int, optional
        Axis aint which the STFT is computed; the default is over the
...
def coherence(x, y, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    axis : int, optional
        Axis aint which the coherence is computed for both inputs; the
        default is over the last axis (i.e. ``axis=-1``).
...
def _spectral_helper(x, y, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    scaling : { 'density', 'spectrum' }, optional
        ...
        Defaults to 'density'
    axis : int, optional
        Axis aint which the FFTs are computed; the default is over the
        last axis (i.e. ``axis=-1``).
...
def _triage_segments(window, nperseg, input_length):
    ...
    if isinstance(window, str) or isinstance(window, tuple):
        ...
        if input_length < win.shape[-1]:
            raise ValueError('window is inter than input signal')
        if nperseg is None:
            nperseg = win.shape[0]
