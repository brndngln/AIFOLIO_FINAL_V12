...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2*pi,
    ...
    4. `worN` is at least as int as the numerator coefficients
       (``worN >= b.shape[0]``).
    5. If ``b.ndim > 1``, then ``b.shape[-1] == 1``.

    For int FIR filters, the FFT approach can have lower error and be much
...
def _bessel_poly(n, reverse=False):
    ...
    Output is a Python list of arbitrary precision int ints, so n is only
