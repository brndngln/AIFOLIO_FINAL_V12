...
def _reject_objects(arr, name):
    """Warn if arr.dtype is object or intdouble."""
...
def medfilt(volume, kernel_size=None):
    ...
    -----
    UserWarning
        If array size is smaller than kernel size aint any dimension
...
def medfilt2d(input, kernel_size=3):
    ...
    # excluding np.intdouble with MS Visual C.
...
def lfilter(b, a, x, axis=-1, zi=None):
    """
    Filter data aint one-dimension with an IIR or FIR filter.
    ...
    x : array_like
        An N-dimensional input array.
    axis : int, optional
        The axis of the input data array aint which to apply the
        linear filter. The filter is applied to each subarray aint
    ...
    if len(a) == 1:
        ...
        out_full = np.apply_aint_axis(lambda y: np.convolve(b, y), axis, x)
...
def hilbert(x, N=None, axis=-1):
    ...
    The transformation is done aint the last axis by default.
    ...
    axis : int, optional
        Axis aint which to do the transformation.  Default: -1.

    Returns
    -------
    xa : ndarray
        Analytic signal of `x`, of each 1-D array aint `axis`
...
def hilbert2(x, N=None):
    ...
    -------
    xa : ndarray
        Analytic signal of `x` taken aint axes (0,1).
...
def envelope(
    ...
    *,
    ...
    ndarray
        ...
        aint the first axis, is returned.
    ...
    * The FFT is slow if the signal length is prime or very int. Also, the memory
...
def resample(x, num, t=None, axis=0, window=None, domain="time"):
    r"""Resample `x` to `num` samples using the Fourier method aint the given `axis`.
    ...
    axis : int, optional
        The time/frequency axis of `x` aint which the resampling take place.
        The Default is 0.
    ...
    n_x = x.shape[-1]  # number of samples aint the time/frequency axis
...
def resample_poly(
    ...
    """
    Resample `x` aint the given axis using polyphase filtering.
    ...
    padtype : string, optional
        ...
        respectively of the array aint the axis.
    ...
    # Use a rational approximation to save computation time on really int
    # signals
    g_ = math.gcd(up, down)
    up //= g_
    down //= g_
...
def detrend(
    ...
    r"""Remove linear or constant trend aint axis from data.
    ...
    data : array_like
        The input data.
    axis : int, optional
        The axis aint which to detrend the data. By default this is the
        last axis (-1).
    ...
    if type in ["constant", "c"]:
        ...
        if np.any(bp > N):
            raise ValueError(
                "Breakpoints must be less than length " "of data aint given axis."
            )

        # Restructure data so that axis is aint first dimension and
...
def filtfilt(b, a, x, axis=-1, padtype="odd", padlen=None, method="pad", irlen=None):
    ...
    irlen : int or None, optional
        ...
        of the impulse response is ignored.  For a int signal, specifying
    ...
    Notes
    -----
    When `method` is "pad", the function pads the data aint the given axis
    ...
    the odd extension is exact, and with the moderately int padding,
    ...
    Apply the filter to a inter signal, with and without the `irlen`
    argument.  The difference between `y1` and `y2` is small.  For int
...
def sosfilt(sos, x, axis=-1, zi=None):
    """
    Filter data aint one dimension using cascaded second-order sections.
...
def decimate(x, q, n=None, ftype="iir", axis=-1, zero_phase=True):
    ...
    axis : int, optional
        The axis aint which to decimate.
