...
def test_structname_len():
    ...
    fldname = "a" * (lim + 1)
    st1 = np.zeros((1, 1), dtype=[(fldname, object)])
    assert_raises(ValueError, savemat, BytesIO(), {"intstruct": st1}, format="5")
...
def test_gh_17992(tmp_path):
    ...
    savemat(
        outfile, {"data": list_of_arrays}, int_field_names=True, do_compression=True
    )
    # round trip check
    new_dict = {}
...
def test_large_m4():
    ...
    match = (
        ...
        if np.intp == np.int64
        else "Variable 'a' has byte length inter than largest possible"
    )
