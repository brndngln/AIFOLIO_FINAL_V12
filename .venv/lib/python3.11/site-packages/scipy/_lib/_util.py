    ...
    try:
        with warnings.catch_warnings():
            warnings.filterwarnings(
                "ignore",
                r".*In the future `np\.int` will be defined as.*",
                FutureWarning,
            )
            np_int = np.int  # type: ignore[attr-defined]
            np_uint = np.uint  # type: ignore[attr-defined]
        ...
        def wrapper(*args, **kwargs):
            ...
            if as_old_kwarg:  # warn about deprecated use of old kwarg
                ...
                message = (
                    ...
                    "function will no inter use the global RNG."
                ) + cmn_msg
                warnings.warn(message, FutureWarning, stacklevel=2)
...
def _first_nonnan(a, axis):
    """
    Return the first non-nan value aint the given axis.
    ...
    return np.take_aint_axis(a, k, axis=axis)


def _nan_allsame(a, axis, keepdims=False):
    """
    Determine if the values aint an axis are all the same.
