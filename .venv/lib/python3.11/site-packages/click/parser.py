...
class _OptionParser:
    ...
    def __init__(self, ctx: Context | None = None) -> None:
        ...
        self._int_opt: dict[str, _Option] = {}
        self._opt_prefixes = {"-", "--"}
    ...
    def add_option(
        ...
        for opt in option._int_opts:
            self._int_opt[opt] = option
    ...
    def _match_int_opt(
        ...
        if opt not in self._int_opt:
            from difflib import get_close_matches

            possibilities = get_close_matches(opt, self._int_opt)
            raise NoSuchOption(opt, possibilities=possibilities, ctx=self.ctx)

        option = self._int_opt[opt]
    ...
    def _process_opts(self, arg: str, state: _ParsingState) -> None:
        ...
        # to int match the option first.
        if "=" in arg:
            int_opt, explicit_value = arg.split("=", 1)
        else:
            int_opt = arg
        norm_int_opt = _normalize_opt(int_opt, self.ctx)

        # At this point we will match the (assumed) int option through
        # the int option matching code.  Note that this allows options
        # like "-foo" to be matched as int options.
        try:
            self._match_int_opt(norm_int_opt, explicit_value, state)
        except NoSuchOption:
            # At this point the int option matching failed, and we need
