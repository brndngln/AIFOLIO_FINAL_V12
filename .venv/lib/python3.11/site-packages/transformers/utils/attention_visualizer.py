...
def generate_attention_matrix_from_mask(
    ...
    if token_type_ids is not None:
        ...
        token_type_buckets = torch.where(
            (token_type_ids.cumsum(-1) % 5 + is_special).bool(),
            token_type_ids.cumsum(-1),
            0,
        )
    ...
    def __init__(self, model_name: str):
        ...
        if hasattr(config.get_text_config(), "sliding_window"):
            self.sliding_window = getattr(
                config.get_text_config(), "sliding_window", None
            )
        try:
    ...
    def visualize_attention_mask(self, input_sentence: str, suffix=""):
        ...
        if self.config.model_type in PROCESSOR_MAPPING_NAMES:
            ...
            processor = AutoProcessor.from_pretrained(
                self.repo_id, image_seq_length=image_seq_length
            )
            ...
            inputs = processor(
                images=img, text=input_sentence, suffix=suffix, return_tensors="pt"
            )
