...
def _process_regular_parameters(
    ...
    docstring = ""
    source_args_dict = (
        get_args_doc_from_source([ModelArgs, ImageProcessorArgs])
        if source_args_dict is None
        else source_args_dict
    )
    missing_args = {}
        ...
        if docstring_init:
            ...
            for param_name, param_type_annotation in cls.__annotations__.items():
                ...
                if is_documented:
                    ...
                    if param_type == "":
                        print(
                            f"🚨 {param_name} for {cls.__qualname__} in file {cls.__code__.co_filename} has no type"
                        )
                    param_type = param_type if "`" in param_type else f"`{param_type}`"
                    # Format the parameter docstring
                    if additional_info:
