...
class CompressedTensorsConfig(QuantizationConfigMixin):
    ...
    def __init__(
        self,
        config_groups: Optional[
            dict[str, Union["QuantizationScheme", list[str]]]
        ] = None,  # noqa: F821
        format: str = "dense",
        ...
        if isinstance(self.quant_type, str):
            ...
            if (
                ...
                if torch.xpu.is_available():
                    ...
                    ) >= version.parse("0.11.0") and version.parse(
        ...
        if isinstance(self.quant_type, str):
            ...
            if "quant_type_kwargs" in d and "layout" in d["quant_type_kwargs"]:
                ...
                if isinstance(d["quant_type_kwargs"]["layout"], list):
                    assert (
                        len(d["quant_type_kwargs"]["layout"]) == 2
                    ), "layout saves layout name and layout kwargs"
                    assert isinstance(
                        d["quant_type_kwargs"]["layout"][0], str
                    ), "layout name must be a string"
                    assert isinstance(
                        d["quant_type_kwargs"]["layout"][1], dict
