...
def require_huggingface_hub_greater_or_equal(version: str):
    ...
    def decorator(test_case):
        return unittest.skipUnless(
            is_huggingface_hub_greater_or_equal(version),
            f"test requires huggingface_hub version >= {version}",
        )(test_case)
...
def require_flash_attn_3(test_case):
    ...
    """
    return unittest.skipUnless(
        is_flash_attn_3_available(), "test requires Flash Attention 3"
    )(test_case)
    ...
    if isinstance(test_case, type):
        ...
        def wrapper(*args, **kwargs):
            if token is not None:
                with patch(
                    "huggingface_hub.utils._headers.get_token", return_value=token
                ):
...
def require_torchcodec(test_case):
    ...
    """
    return unittest.skipUnless(is_torchcodec_available(), "test requires Torchvision")(
        test_case
    )
...
def require_torch_large_accelerator(test_case, memory: float = 20):
    ...
    if torch_device != "cuda" and torch_device != "xpu":
        return unittest.skip(
            reason=f"test requires a GPU or XPU with more than {memory} GiB of memory"
        )(test_case)

    torch_accelerator_module = getattr(torch, torch_device)

    return unittest.skipUnless(
        torch_accelerator_module.get_device_properties(0).total_memory / 1024**3
        > memory,
...
def require_auto_round(test_case):
    ...
    """
    return unittest.skipUnless(is_auto_round_available(), "test requires autoround")(
        test_case
    )
...
def require_speech(test_case):
    ...
    """
    return unittest.skipUnless(is_speech_available(), "test requires torchaudio")(
        test_case
    )
...
class Expectations(UserDict[PackedDeviceProperties, Any]):
    ...
    def find_expectation(
        self, properties: DeviceProperties = (None, None, None)
    ) -> Any:
        """
        ...
        (result_key, result) = max(
            ...
            key=lambda x: (
                Expectations.score(
                    properties, x[0]
                ),  # x[0] is a device properties tuple (device_type, major, minor)
                x[0][1]
                if x[0][1] is not None
                else -1,  # This key is the major version, -1 if major is None
                x[0][2]
                if x[0][2] is not None
                else -1,  # This key is the minor version, -1 if minor is None
            ),
        )
