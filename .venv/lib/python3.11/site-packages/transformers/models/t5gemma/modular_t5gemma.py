    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(encoder, dict):
            ...
            assert isinstance(
                encoder, T5GemmaModuleConfig
            ), f"{type(encoder)} is not supported."
        ...
        if isinstance(decoder, dict):
            ...
            ), f"{type(decoder)} is not supported."
        ...
        self.initializer_range = kwargs.get(
            "initializer_range", decoder.initializer_range
        )
...
class T5GemmaEncoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.pre_self_attn_layernorm = T5GemmaRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_self_attn_layernorm = T5GemmaRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )

        # mlp
        self.mlp = T5GemmaMLP(config)
        self.pre_feedforward_layernorm = T5GemmaRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_feedforward_layernorm = T5GemmaRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class T5GemmaDecoderLayer(T5GemmaEncoderLayer):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.pre_cross_attn_layernorm = T5GemmaRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_cross_attn_layernorm = T5GemmaRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class T5GemmaClassificationHead(nn.Module):
    ...
    def __init__(
        self, hidden_size: int, num_labels: int, classifier_dropout_rate: float = 0.0
    ):
        super().__init__()
...
class T5GemmaPreTrainedModel(Gemma2PreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        if pad_token_id is None:
            raise ValueError(
                "self.model.config.decoder.pad_token_id has to be defined."
            )
...
def make_default_2d_attention_mask(
    ...
    if token_ids is not None:
        ...
        attention_mask = (token_ids != pad_token_id).to(
            hidden_states.device, torch.long
        )
    else:
...
class T5GemmaEncoder(T5GemmaPreTrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [
                T5GemmaEncoderLayer(config, layer_idx)
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if attention_mask is None:
            attention_mask = make_default_2d_attention_mask(
                input_ids, inputs_embeds, self.config.pad_token_id
            )
        ...
        if not isinstance(self_attn_mask_mapping := attention_mask, dict):
            ...
            self_attn_mask_mapping = {
                ...
                "sliding_attention": create_sliding_window_causal_mask(
                    **mask_kwargs,
                    or_mask_function=sliding_window_bidirectional_mask_function(
...
class T5GemmaDecoder(T5GemmaEncoder):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [
                T5GemmaDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
...
class T5GemmaModel(T5GemmaPreTrainedModel):
    def __init__(self, config: T5GemmaConfig):
        ...
        if not config.is_encoder_decoder:
            raise ValueError(
                "T5GemmaModel only support encoder-decoder modeling. Use `T5GemmaEncoderModel` instead."
            )
...
class T5GemmaEncoderModel(T5GemmaPreTrainedModel):
    def __init__(self, config: T5GemmaConfig):
        ...
        if config.is_encoder_decoder:
            raise ValueError(
                "T5GemmaEncoderModel only supports encoder-only model. Use `T5GemmaModel` instead."
            )
    ...
    def _tie_weights(self):
        ...
        if self.config.tie_word_embeddings:
            self._tie_or_clone_weights(
                self.lm_head.out_proj, self.get_decoder().get_input_embeddings()
            )
...
class T5GemmaForSequenceClassification(T5GemmaPreTrainedModel):
    def __init__(
        self, config: T5GemmaConfig, is_encoder_decoder: Optional[bool] = None
    ):
        """
    ...
    def forward(
        ...
        if self.config.is_encoder_decoder and (
            input_ids is None and inputs_embeds is not None
        ):
        ...
        if self.config.pad_token_id is None:
            ...
            if self.config.is_encoder_decoder:
                ...
                last_non_pad_token = torch.clamp(
                    last_non_pad_token, max=decoder_input_ids.shape[-1] - 1
                )
        else:
            last_non_pad_token = -1
            logger.warning_once(
