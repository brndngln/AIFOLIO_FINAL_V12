    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if (
            ...
            # cross_attentions
            key_states = (
                self.k_proj(key_value_states).view(*kv_input_shape).transpose(1, 2)
            )
            value_states = (
                self.v_proj(key_value_states).view(*kv_input_shape).transpose(1, 2)
            )
        elif past_key_value is not None:
            # reuse k, v, self_attention
            key_states = (
                self.k_proj(hidden_states).view(*kv_input_shape).transpose(1, 2)
            )
            value_states = (
                self.v_proj(hidden_states).view(*kv_input_shape).transpose(1, 2)
            )
...
class PatchTSMixerBlock(nn.Module):
    ...
    def __init__(self, config: PatchTSMixerConfig):
        ...
        self.mixers = nn.ModuleList(
            [PatchTSMixerLayer(config=config) for _ in range(num_layers)]
        )
...
class PatchTSMixerPretrainHead(nn.Module):
    ...
    def forward(self, hidden_features):
        ...
        forecast = self.base_pt_block(
            hidden_features
        )  # [batch_size x n_vars x num_patch x patch_length]
        return forecast
...
def forecast_masking(
    ...
    mask = mask.unsqueeze(-1).repeat(
        1, 1, 1, num_features
    )  # mask: [bs x num_channels x num_patch x patch_len]
    if unmasked_channel_indices is not None:
        mask[:, unmasked_channel_indices, :, :] = 0
...
class PatchTSMixerForPretraining(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        )  # tensor [batch_size x nvars x num_patch x patch_length]
    ...
    def forward(
        ...
        if self.prediction_channel_indices is not None:
            if self.distribution_output:
                ...
                if future_values is not None and return_loss is True:
                    loss_val = loss(
                        y_hat, future_values[..., self.prediction_channel_indices]
                    )
        else:
            if self.distribution_output:
                distribution = self.distribution_output.distribution(
...
class InjectScalerStatistics4D(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, loc: torch.Tensor, scale: torch.Tensor):
        ...
        concat_stats = self.map_scale_expansion(
            concat_stats
        )  # [batch_size x n_channels x num_patch x (2*expansion)]
        concat_stats = self.map_scale_compression(
            concat_stats
        )  # [batch_size x n_channels x num_patch x 2]

        inputs = torch.cat(
            [inputs, concat_stats], dim=-1
        )  # [batch_size x channels x num_patch x d_model+2]
        inputs = self.inverse_trans_expansion(
            inputs
        )  # [batch_size x channels x num_patch x (expansion*d_model)]
        inputs = self.inverse_trans_compression(
            inputs
        )  # [batch_size x channels x num_patch x d_model]
...
class PatchTSMixerForRegression(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        if target_values is not None and return_loss is True:
            if self.distribution_output:
                if self.distribution_output == "negative_binomial" and torch.any(
                    target_values < 0
                ):
                    raise Exception(
                ...
                y_hat = tuple(
                    [item.view(-1, self.config.num_targets) for item in y_hat]
                )
