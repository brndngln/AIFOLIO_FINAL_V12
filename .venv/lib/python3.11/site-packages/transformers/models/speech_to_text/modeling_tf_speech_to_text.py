        ...
        if getattr(self, "conv_layers", None) is not None:
            for i, layer in enumerate(self.conv_layers):
                with tf.name_scope(layer.name):
                    layer.build(
                        [None, None, self.in_channels]
                        if i == 0
                        else [None, None, self.mid_channels // 2]
                    )
...
class TFSpeech2TextSinusoidalPositionalEmbedding(keras.layers.Layer):
    ...
    def call(self, input_ids: tf.Tensor, past_key_values_length: int = 0) -> tf.Tensor:
        ...
        embeddings = self._get_embedding(
            self.padding_idx + 1 + seq_len + self.offset + past_key_values_length,
            self.embedding_dim,
            self.padding_idx,
        )
        return tf.reshape(
            tf.gather(embeddings, tf.reshape(position_ids, (-1,)), axis=0),
            (bsz, seq_len, -1),
        )
        ...
        return {
            "input_features": tf.TensorSpec(
                (
                    None,
                    None,
                    self.config.input_feat_per_channel * self.config.input_channels,
                ),
                tf.float32,
                name="input_features",
            ),
