...
class MLCDRotaryEmbedding(nn.Module):
    ...
    def forward(self, num_patches_height: int, num_patches_width: int) -> torch.Tensor:
        ...
        hpos_ids = (
            torch.arange(num_patches_height, device=self.inv_freq.device)
            .unsqueeze(1)
            .expand(-1, num_patches_width)
        )
        wpos_ids = (
            torch.arange(num_patches_width, device=self.inv_freq.device)
            .unsqueeze(0)
...
class MLCDVisionTransformer(nn.Module):
    def __init__(self, config: MLCDVisionConfig):
        ...
        self.vision_rotary_embedding = MLCDRotaryEmbedding(
    ...
    def forward(
        ...
        rotary_pos_emb = self.vision_rotary_embedding(
...
class MLCDPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, MLCDVisionEmbeddings):
            ...
            factor = self.config.initializer_factor
            pos_emb_std = (
                module.config.hidden_size // module.config.num_attention_heads // 2
            ) ** -0.5 * factor
