...
class DFineMultiscaleDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        value = encoder_hidden_states.reshape(
        ...
        if reference_points.shape[-1] == 2:
            ...
            sampling_locations = (
                reference_points.reshape(
                    batch_size, sequence_length, 1, self.n_levels, 1, 2
                )
...
class DFineDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states_2 = nn.functional.dropout(
            hidden_states_2, p=self.dropout, training=self.training
        )
        ...
        hidden_states_2 = nn.functional.dropout(
            hidden_states_2, p=self.activation_dropout, training=self.training
        )
        ...
        hidden_states = self.final_layer_norm(
            hidden_states.clamp(min=-65504, max=65504)
        )
...
class DFinePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, DFineMultiscaleDeformableAttention):
            ...
            grid_init = grid_init.reshape(module.n_heads, 1, 2).tile(
                [1, sum(module.num_points_list), 1]
            )
            scaling = torch.concat(
                [torch.arange(1, n + 1) for n in module.num_points_list]
            ).reshape(1, -1, 1)
            grid_init *= scaling
            with torch.no_grad():
...
class DFineIntegral(nn.Module):
    ...
    def forward(
        ...
        pred_corners = F.linear(pred_corners, project.to(pred_corners.device)).reshape(
            -1, 4
        )
...
class DFineDecoder(DFinePreTrainedModel):
    ...
    def __init__(self, config: DFineConfig):
        ...
        self.eval_idx = (
            config.eval_idx
            if config.eval_idx >= 0
            else config.decoder_layers + config.eval_idx
        )
        ...
        self.reg_scale = nn.Parameter(
            torch.tensor([config.reg_scale]), requires_grad=False
        )
        self.max_num_bins = config.max_num_bins
        self.d_model = config.d_model
        ...
        self.lqe_layers = nn.ModuleList(
            [DFineLQE(config) for _ in range(config.decoder_layers)]
        )
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            query_pos_embed = self.query_pos_head(ref_points_detach).clamp(
                min=-10, max=10
            )
            ...
            if i == 0:
                ...
                new_reference_points = F.sigmoid(
                    self.pre_bbox_head(output[0]) + inverse_sigmoid(ref_points_detach)
                )
            ...
            if self.bbox_embed is not None:
                pred_corners = (
                    self.bbox_embed[i](hidden_states + output_detach)
        ...
        if self.class_embed is not None and self.bbox_embed is not None:
            ...
            intermediate_predicted_corners = torch.stack(
                intermediate_predicted_corners, dim=1
            )
...
class DFineForObjectDetection(DFinePreTrainedModel):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        predicted_corners = (
            outputs.intermediate_predicted_corners if return_dict else outputs[4]
        )
        initial_reference_points = (
            outputs.initial_reference_points if return_dict else outputs[5]
        )
...
class DFineMLP(nn.Module):
    ...
    def forward(self, stat_features: torch.Tensor) -> torch.Tensor:
        for i, layer in enumerate(self.layers):
            stat_features = (
                self.act(layer(stat_features))
                if i < self.num_layers - 1
                else layer(stat_features)
            )
...
class DFineLQE(nn.Module):
    def __init__(self, config: DFineConfig):
        ...
        self.reg_conf = DFineMLP(
            4 * (self.top_prob_values + 1), config.lqe_hidden_dim, 1, config.lqe_layers
        )

    def forward(self, scores: torch.Tensor, pred_corners: torch.Tensor) -> torch.Tensor:
        batch_size, length, _ = pred_corners.size()
        prob = F.softmax(
            pred_corners.reshape(batch_size, length, 4, self.max_num_bins + 1), dim=-1
        )
...
class DFineRepVggBlock(nn.Module):
    ...
    def __init__(self, config: DFineConfig, in_channels: int, out_channels: int):
        ...
        self.conv1 = DFineConvNormLayer(
            config, hidden_channels, out_channels, 3, 1, padding=1
        )
        self.conv2 = DFineConvNormLayer(
            config, hidden_channels, out_channels, 1, 1, padding=0
        )
        self.activation = nn.Identity() if activation is None else ACT2CLS[activation]()
...
class DFineCSPRepLayer(nn.Module):
    ...
    def __init__(
        ...
        self.bottlenecks = nn.ModuleList(
            [
                DFineRepVggBlock(config, hidden_channels, hidden_channels)
...
class DFineRepNCSPELAN4(nn.Module):
    def __init__(self, config: DFineConfig, act: str = "silu", numb_blocks: int = 3):
        ...
        self.conv1 = DFineConvNormLayer(
            config, conv1_dim, conv3_dim, 1, 1, activation=act
        )
        self.csp_rep1 = DFineCSPRepLayer(
            config, conv3_dim // 2, conv4_dim, num_blocks=numb_blocks
        )
        self.conv2 = DFineConvNormLayer(
            config, conv4_dim, conv4_dim, 3, 1, activation=act
        )
        self.csp_rep2 = DFineCSPRepLayer(
            config, conv4_dim, conv4_dim, num_blocks=numb_blocks
        )
        self.conv3 = DFineConvNormLayer(
            config, conv4_dim, conv4_dim, 3, 1, activation=act
        )
        self.conv4 = DFineConvNormLayer(
            config, conv3_dim + (2 * conv4_dim), conv2_dim, 1, 1, activation=act
        )

    def forward(self, input_features: torch.Tensor) -> torch.Tensor:
        # Split initial features into two branches after first convolution
        split_features = list(
            self.conv1(input_features).split((self.conv_dim, self.conv_dim), 1)
        )
...
class DFineSCDown(nn.Module):
    def __init__(self, config: DFineConfig, kernel_size: int, stride: int):
        super().__init__()
        self.conv1 = DFineConvNormLayer(
            config, config.encoder_hidden_dim, config.encoder_hidden_dim, 1, 1
        )
        self.conv2 = DFineConvNormLayer(
            config,
            config.encoder_hidden_dim,
...
class DFineEncoder(nn.Module):
    def __init__(self, config: DFineConfig):
        ...
        self.layers = nn.ModuleList(
            [DFineEncoderLayer(config) for _ in range(config.encoder_layers)]
        )
...
class DFineHybridEncoder(nn.Module):
    ...
    def __init__(self, config: DFineConfig):
        ...
        self.encoder = nn.ModuleList(
            [DFineEncoder(config) for _ in range(len(self.encode_proj_layers))]
        )
        # top-down fpn
        ...
        for _ in range(len(self.in_channels) - 1, 0, -1):
            lateral_layer = DFineConvNormLayer(
                config, self.encoder_hidden_dim, self.encoder_hidden_dim, 1, 1
            )
