...
class SmolVLMImageProcessor(BaseImageProcessor):
    ...
    def pad(
        ...
        padded_masks = [
            [np.zeros(pad_size, dtype=np.int64) for _ in range(max_num_images)]
            for _ in range(batch_size)
        ]
    ...
    def preprocess(
        ...
        data = {
            "pixel_values": np.array(images_list)
    ...
    def get_number_of_image_patches(self, height: int, width: int, images_kwargs=None):
        ...
        """
        do_image_splitting = (
            images_kwargs.get("do_image_splitting", None) or self.do_image_splitting
        )
        max_image_size = (
            images_kwargs.get("max_image_size", None) or self.max_image_size
        )
        size = images_kwargs.get("size", None) or self.size

        if do_image_splitting:
            height, width = _resize_output_size_rescale_to_max_len(
                height, width, max_len=size["longest_edge"]
            )
            height, width = _resize_output_size_scale_below_upper_bound(
                height, width, max_len=4096
            )
            aspect_ratio = width / height

            if width >= height:
                resized_width = (
                    math.ceil(width / max_image_size["longest_edge"])
                    * max_image_size["longest_edge"]
                )
                resized_height = int(width / aspect_ratio)
                resized_height = (
                    math.ceil(height / max_image_size["longest_edge"])
                    * max_image_size["longest_edge"]
                )
            elif height > width:
                resized_height = (
