...
def get_resize_output_image_size(
    ...
    resolution_max_side = (
        max(height, width) if resolution_max_side is None else resolution_max_side
    )
...
class SmolVLMVideoProcessor(BaseVideoProcessor):
    ...
    def __init__(self, **kwargs: Unpack[SmolVLMVideoProcessorInitKwargs]):
        ...
        if "video_sampling" in kwargs:
            ...
            self.size = get_size_dict(
                kwargs["video_sampling"]["video_size"],
    ...
    def resize(
        ...
        **kwargs,
        ...
        if size.longest_edge:
            ...
            raise ValueError(
                f"Size must contain 'height' and 'width' keys, or 'longest_edge' key. Got {size}."
            )

        video = F.resize(
            video, new_size, interpolation=interpolation, antialias=antialias
        )
        ...
        video = F.resize(
            video, max_size, interpolation=interpolation, antialias=antialias
        )
        return video
    ...
    def sample_frames(
        ...
        if skip_secs > 0 and (metadata["duration"] - 2 * skip_secs) > (
            num_frames * fps
        ):
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        if do_sample_frames:
            ...
            for video, metadata in zip(videos, video_metadata):
                video, timestamps, duration = self.sample_frames(
                    video, metadata, num_frames, fps, skip_secs
                )
        ...
        if do_pad:
            ...
            pixel_attention_mask = reorder_videos(
                processed_padded_mask_grouped, grouped_videos_index
            )
