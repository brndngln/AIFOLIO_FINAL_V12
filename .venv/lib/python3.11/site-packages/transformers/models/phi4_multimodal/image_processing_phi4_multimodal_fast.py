...
class Phi4MultimodalImageProcessorFast(BaseImageProcessorFast):
    ...
    def dynamic_preprocess(
        self, image, image_size, patch_size, mask_size, max_num=36, min_num=1
    ):
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        if size.height != size.width:
            raise ValueError(
                "Phi4MultimodalFastImageProcessor only supports square sizes."
            )
        ...
        for image in images:
            ...
            global_image = self.resize(
                processed_image, size, interpolation=interpolation, antialias=False
            )
            ...
            hd_image_reshape = hd_image_reshape.reshape(
            ...
            attention_mask_reshape = attention_mask_reshape.reshape(
            ...
            hd_image_reshape = torch.cat(
                [global_image.unsqueeze(0), hd_image_reshape], dim=0
            )
            hd_attention_mask_reshape = torch.cat(
        ...
        images_transformed = [
            self.pad_to_max_num_crops(im, max_crops) for im in images_transformed
        ]
