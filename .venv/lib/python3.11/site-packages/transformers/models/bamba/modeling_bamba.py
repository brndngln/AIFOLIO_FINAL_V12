...
class HybridMambaAttentionDynamicCache(modeling_jamba.HybridMambaAttentionDynamicCache):
    ...
    def __init__(
        self, config: BambaConfig, batch_size, dtype=torch.float16, device=None
    ):
                ...
                self.conv_states += [
                    torch.zeros(
                        ...
                        (
                            ...
                            + 2 * config.mamba_n_groups * ssm_state_size
                        ),
                        conv_kernel_size,
                        device=device,
                        dtype=dtype,
        ...
        **kwargs,
        ...
        if is_fast_path_available and "cuda" in self.in_proj.weight.device.type:
            return self.cuda_kernels_forward(
                hidden_states, cache_params, cache_position, attention_mask, seq_idx
            )
...
class BambaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: BambaConfig, layer_idx: int, layer_type: str = "mamba"):
        ...
        self.pre_ff_layernorm = BambaRMSNorm(
    ...
    def __init__(self, config: BambaConfig):
        ...
        for i in range(config.num_hidden_layers):
            decoder_layers.append(
                BambaDecoderLayer(
                    config, layer_idx=i, layer_type=config.layers_block_type[i]
                )
            )
    ...
    def forward(
        ...
        **kwargs: Unpack[BambaFlashAttentionKwargs],
        ...
        for decoder_layer in self.layers:
            ...
            layer_mask = (
                mamba_mask if decoder_layer.layer_type == "mamba" else causal_mask
            )
