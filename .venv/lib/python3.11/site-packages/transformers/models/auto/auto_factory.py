...
def auto_class_update(
    cls, checkpoint_for_example: str = "google-bert/bert-base-cased", head_doc: str = ""
):
...
def add_generation_mixin_to_remote_model(model_class):
    ...
    has_custom_generate_in_class = hasattr(
        model_class, "generate"
    ) and "GenerationMixin" not in str(getattr(model_class, "generate"))
    has_custom_prepare_inputs = hasattr(
        model_class, "prepare_inputs_for_generation"
    ) and "GenerationMixin" not in str(
...
class _LazyAutoMapping(OrderedDict[type[PretrainedConfig], _LazyAutoMappingValue]):
    ...
    ) -> Union[_LazyAutoMappingValue, _T]:
        try:
    ...
    def register(
        self, key: type[PretrainedConfig], value: _LazyAutoMappingValue, exist_ok=False
    ) -> None:
        """
