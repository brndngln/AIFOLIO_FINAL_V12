...
class Idefics2ImageProcessorFast(BaseImageProcessorFast):
    ...
    def split_images(
        ...
        batch_split_images = [
            [image[i] for image in batch_split_images]
            for i in range(len(batch_split_images[0]))
        ]
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        split_images = reorder_images(
            split_images_grouped, grouped_images_index, is_nested=True
        )
        if do_image_splitting:
            # flattenened the doubly nested list to a nested list
            for i, group_images in enumerate(split_images):
                split_images[i] = [
                    image for sublist in group_images for image in sublist
                ]
        ...
        for shape, stacked_images in grouped_images.items():
            if do_resize:
                stacked_images = self.resize(
                    stacked_images, size, interpolation=interpolation
                )
            resized_images_grouped[shape] = stacked_images
        resized_images = reorder_images(
            resized_images_grouped, grouped_images_index, is_nested=True
        )
        ...
        processed_images = reorder_images(
            processed_images_grouped, grouped_images_index, is_nested=True
        )
        ...
        if do_pad:
            ...
            for i, images in enumerate(processed_images):
                for j, image in enumerate(images):
                    (
                        processed_images_padded[i, j],
                        pixel_attention_masks[i, j],
                    ) = self.pad(image, (max_height, max_width))
        ...
        if do_pad:
            ...
            data = {
                "pixel_values": torch.stack(
                    [torch.stack(images) for images in processed_images]
                )
            }
        else:
