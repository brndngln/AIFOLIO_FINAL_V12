    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        attention_mask = torch.zeros(
            [1, 1, seq_length, seq_length], device=query_states.device, dtype=torch.bool
        )
...
class Glm4vVisionModel(Glm4vPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.blocks = nn.ModuleList(
            [Glm4vVisionBlock(config) for _ in range(config.depth)]
        )
...
class Glm4vTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Glm4vTextConfig, layer_idx: int):
        ...
        self.post_attention_layernorm = Glm4vRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_self_attn_layernorm = Glm4vRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_mlp_layernorm = Glm4vRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class Glm4vTextModel(Qwen2_5_VLTextModel):
    def __init__(self, config: Glm4vTextConfig):
        ...
        self.layers = nn.ModuleList(
            [
                Glm4vTextDecoderLayer(config, layer_idx)
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if pixel_values_videos is not None:
            ...
            )  # GLM-4.1V use image_token_id for video
            mask_unsqueezed = mask.unsqueeze(-1)
            mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)
            ...
            for i in range(len(text)):
                ...
                for frame_idx in range(len(video_grid_thw)):
                    if self.image_token in text[i]:
                        num_image_tokens = (
                            video_grid_thw[frame_idx].prod() // merge_length
                        )
                        text[i] = text[i].replace(
                            self.image_token, "<|placeholder|>" * num_image_tokens, 1
                        )
