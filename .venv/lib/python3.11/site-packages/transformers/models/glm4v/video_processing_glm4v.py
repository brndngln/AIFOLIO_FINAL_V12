...
class Glm4vVideoProcessor(BaseVideoProcessor):
    ...
    def sample_frames(
        ...
        if duration <= self.max_duration:
            ...
            frame_indices = [
                min(max_frame_idx, int(math.ceil(i * video_fps / self.fps)))
                for i in range(n)
            ]
        else:
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        timestamps_list = []
        if do_sample_frames:
            if video_metadata is None or (
                isinstance(video_metadata, list) and video_metadata[0] is None
            ):
                raise ValueError(
            ...
            raise AssertionError(
                "Must set `do_sample_frames=True` to sample frames from GLM-4.1V Model."
            )
