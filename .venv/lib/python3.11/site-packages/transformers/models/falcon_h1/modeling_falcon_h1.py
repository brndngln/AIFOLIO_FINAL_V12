...
class FalconHybridMambaAttentionDynamicCache(DynamicCache):
    ...
    def to_legacy_cache(self) -> tuple[tuple[torch.Tensor], tuple[torch.Tensor]]:
        raise NotImplementedError(
            "FalconHybridMambaAttentionDynamicCache does not have a legacy cache equivalent."
        )
...
class FalconH1RMSNormGated(torch.nn.Module):
    ...
    def forward(self, hidden_states, gate=None):
        ...
        hidden_states = hidden_states.view(
            batch_size, seq_len, self.n_groups, int(dim // self.n_groups)
        )
        ...
        hidden_states = (
            self.weight.view(self.n_groups, int(dim // self.n_groups)) * hidden_states
        )
...
class FalconH1Mixer(nn.Module):
    ...
    def __init__(self, config: FalconH1Config, layer_idx: int):
        ...
        self.conv_kernel_size = config.mamba_d_conv
        self.intermediate_size = (
            int(config.mamba_expand * self.hidden_size)
...
class FalconH1DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: FalconH1Config, layer_idx: int):
        ...
        self.input_layernorm = FalconH1RMSNorm(
...
class FalconH1PreTrainedModel(PreTrainedModel):
    ...
    _supports_cache_class = (
        True  # Note: only supports FalconHybridMambaAttentionDynamicCache
    )
    _is_stateful = True
...
class FalconH1Model(FalconH1PreTrainedModel):
    def __init__(self, config: FalconH1Config):
        ...
        self.final_layernorm = FalconH1RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        ...
        **kwargs,
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if attention_mask is not None:
                ...
                )[:, :, -sequence_length:, :].to(dtype)
                padding_mask = (
                    causal_mask[:, :, :, :mask_length] + padding_attention_mask
                )
        ...
        **kwargs,
        ...
        if not empty_past_kv:
            ...
            past_key_values = FalconHybridMambaAttentionDynamicCache(
                ...
                self.dtype,
                devices=[
                    self.model.layers[i].mamba.conv1d.weight.device
