...
class Gemma3nTextConfig(Gemma2Config, PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            isinstance(intermediate_size, Sequence)
            and (intsize_len := len(intermediate_size)) != num_hidden_layers
        ):
            raise ValueError(
        ...
        if layer_types is None:
            self.layer_types = [
                "full_attention" if i % 5 == 0 else "sliding_attention"
...
class Gemma3nAudioAttention(nn.Module):
    ...
    def _extract_block_context(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if (
            hidden_states.ndim > 2 and x_unfolded.ndim > 3
        ):  # Check if inner dimensions (like N, H) exist
    ...
    def forward(
        ...
        final_condition_for_where = torch.logical_and(
            ...
            condition_from_causality.to(
                condition_from_input_validity.device
            ),  # Ensure same device
        )
...
class Gemma3nAudioEncoder(PreTrainedModel):
    ...
    def forward(
        ...
        indices = torch.clamp(
            indices, max=audio_mel_mask.shape[1] - 1
...
class Gemma3nTextAltUp(nn.Module):
    ...
    def __init__(self, config: Gemma3nTextConfig):
        ...
        self.register_buffer(
            "router_input_scale",
            torch.tensor(self.config.hidden_size**-1.0),
            persistent=False,
        )
...
class Gemma3nTextAttention(Gemma3Attention):
    def __init__(self, config: Gemma3nTextConfig, layer_idx: int):
        ...
        first_kv_shared_layer_idx = (
            self.config.num_hidden_layers - self.config.num_kv_shared_layers
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if (
            ...
            key_states = past_key_value.key_cache[self.kv_shared_layer_index][
                :, :, indices
            ]
            value_states = past_key_value.value_cache[self.kv_shared_layer_index][
                :, :, indices
            ]
        else:
...
@auto_docstring(
    custom_intro="The base Gemma 3n language model without a language modeling head."
)
class Gemma3nTextModel(Gemma3TextModel):
    ...
    def project_per_layer_inputs(
        ...
        per_layer_projection: torch.Tensor = self.per_layer_model_projection(
            inputs_embeds
        )
...
@auto_docstring(
    custom_intro="The base Gemma 3n language model with a language modeling head."
)
        ...
        **lm_kwargs,
        ...
        if input_ids is not None:
            ...
            inputs_embeds = torch.where(
                expanded_vision_mask, vision_embeds, inputs_embeds
            )
            ...
            inputs_embeds = torch.where(
                expanded_audio_mask, audio_embeds, inputs_embeds
            )
        else:
...
class Gemma3nForConditionalGeneration(PaliGemmaForConditionalGeneration):
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(self, **super_kwargs):
        raise AttributeError(
            "Do not inherit _prepare_4d_causal_attention_mask_with_cache_position from PaliGemma"
        )
