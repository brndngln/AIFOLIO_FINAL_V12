...
class DiaMultiChannelEmbedding(nn.Module):
    ...
    def __init__(self, config: DiaDecoderConfig):
        ...
        self.embed = nn.Embedding(
            config.vocab_size * config.num_channels, config.hidden_size
        )
        self.hidden_size = config.hidden_size
        self.num_channels = config.num_channels
        offsets = (
            torch.arange(config.num_channels, dtype=torch.long) * config.vocab_size
        )  # (C,)
        self.register_buffer("offsets", offsets, persistent=False)

    def forward(self, audio_codes: torch.Tensor) -> torch.Tensor:
        tokens = (audio_codes + self.offsets.to(audio_codes.device)).squeeze(1)
        embeds = self.embed(tokens).view(
            tokens.shape[0], audio_codes.shape[1], -1, self.hidden_size
        )
...
class DiaSelfAttention(nn.Module):
    ...
    def __init__(
        self,
        config: Union[DiaEncoderConfig, DiaDecoderConfig],
        layer_idx: int,
...
class DiaCrossAttention(nn.Module):
    ...
    def __init__(self, config: DiaDecoderConfig, layer_idx: int):
        ...
        self.k_proj = nn.Linear(
            self.cross_hidden_size, self.num_key_value_heads * self.head_dim, bias=False
        )
        self.v_proj = nn.Linear(
            self.cross_hidden_size, self.num_key_value_heads * self.head_dim, bias=False
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_values is not None and is_updated:
            ...
            value_states = past_key_values.cross_attention_cache.value_cache[
                self.layer_idx
            ]
        else:
            key_states = (
                self.k_proj(cross_attention_states).view(cross_shape).transpose(1, 2)
            )
            value_states = (
                self.v_proj(cross_attention_states).view(cross_shape).transpose(1, 2)
            )
...
class DiaEncoder(DiaPreTrainedModel):
    def __init__(self, config: DiaEncoderConfig):
        ...
        self.layers = nn.ModuleList(
            [
                DiaEncoderLayer(config, layer_idx)
...
class DiaDecoder(DiaPreTrainedModel):
    ...
    def __init__(self, config: DiaDecoderConfig):
        ...
        self.layers = nn.ModuleList(
            [
                DiaDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = DiaRMSNorm(config.hidden_size, eps=config.norm_eps)
    ...
    def forward(
        ...
        **kwargs,
        ...
        bsz, seq_len, channels = (
            ...
            -1,
            self.config.decoder_config.num_channels,
        )
        ...
        **kwargs,
        ...
        if labels is not None:
            loss = self.loss_function(
                logits=audio_logits, labels=labels, vocab_size=self.vocab_size, **kwargs
            )
