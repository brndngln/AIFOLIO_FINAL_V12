...
class DiaProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        if audio is not None:
            ...
            compression_rate = math.prod(
                self.audio_tokenizer.config.downsampling_ratios
            )
            max_encoded_sequence_len = (
                input_audios["padding_mask"][0].shape[-1] // compression_rate
            )
            ...
            for padding_mask, audio in zip(
                input_audios["padding_mask"], input_audios["input_values"]
            ):
                ...
                current_audio_len = (
                    math.ceil(padding_mask.sum(dim=-1) / base_pad_len) * base_pad_len
                )
                ...
                with torch.no_grad():
                    audio = audio[None, ..., :current_audio_len].to(
                ...
                input_ids = torch.nn.functional.pad(
                    input_ids,
                    pad=(0, 0, padding_len + 1, 0, 0, 0),
                    mode="constant",
                    value=audio_bos_token_id,
                )
                num_valid_inputs = (
                    encoded_sequence_len + 1 + max_delay
                )  # sequence + bos + delay
                num_valid_inputs += 0 if generation else 1  # eos if training
                attention_mask = torch.tensor(
                    [0] * padding_len + [1] * num_valid_inputs, dtype=torch.long
                )[None, :]
        ...
        # TTS generation
            ...
            decoder_attention_mask = torch.ones(
                size=(batch_size, 1 + max_delay), dtype=torch.long
            )
        else:
            raise ValueError(
                "If you try to train, you should provide audio data as well."
            )
        ...
        data.update(
            {
                "decoder_input_ids": delayed_decoder_input_ids,
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        if audio_prompt_len is not None:
            audio_prompt_len = torch.tensor(
                audio_prompt_len, device=decoder_input_ids.device, dtype=torch.long
            )
            start_of_generation_idx = audio_prompt_len[None].expand(
                decoder_input_ids.shape[0]
            )
        else:
            start_of_generation_idx = (
                decoder_input_ids[:, :, 0] == audio_bos_token_id
            ).sum(dim=-1)
        # -1 for the eos token
        end_of_generation_idx = (
            decoder_input_ids.shape[1]
            - (decoder_input_ids[:, :, 0] == audio_pad_token_id).sum(dim=-1)
            - 1
        )
        ...
        with torch.no_grad():
            for i in range(start_of_generation_idx.shape[0]):
                output_i = output_sequences[
                    i, :, start_of_generation_idx[i] : end_of_generation_idx[i]
                ][None, ...]
                output_i = output_i.to(self.audio_tokenizer.device)
                audio_i = (
                    self.audio_tokenizer.decode(audio_codes=output_i)
                    .audio_values.cpu()
                    .squeeze()
                )
    ...
    def save_audio(
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        if isinstance(saving_path, (str, Path)):
            ...
            isinstance(saving_path, (list, tuple))
            and all(isinstance(p, (str, Path)) for p in saving_path)
        ):
            raise ValueError(
                "Invalid input path. Please provide a string, or a list of strings"
            )
    ...
    def build_indices(
        ...
        # (0..seq_len-1)
        sequence_idx = torch.arange(seq_len, dtype=torch.int32)[None, :].expand(
            bsz, seq_len
        )[..., None]
        ...
        batch_idx = torch.arange(bsz, dtype=torch.int32)[:, None, None].expand(
            bsz, seq_len, num_channels
        )
        channel_idx = torch.arange(num_channels, dtype=torch.int32)[
            None, None, :
        ].expand(bsz, seq_len, num_channels)

        all_idx = torch.stack(
            [
                batch_idx.reshape(-1),
                valid_sequence_idx.reshape(-1),
    ...
    def apply_audio_delay(
        ...
        gathered_audio = audio[batch_idx, valid_sequence_idx, channel_idx].view(
            audio.size()
        )
        ...
        final_audio = torch.where(
            mask_bos, bos_token_id, torch.where(mask_pad, pad_token_id, gathered_audio)
        )
