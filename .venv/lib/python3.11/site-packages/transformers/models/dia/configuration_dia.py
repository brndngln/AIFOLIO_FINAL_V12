...
class DiaConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder_config = (
            encoder_config if encoder_config is not None else DiaEncoderConfig()
        )
        self.decoder_config = (
            decoder_config if decoder_config is not None else DiaDecoderConfig()
        )
        self.norm_eps = norm_eps
        self.delay_pattern = (
            delay_pattern
            if delay_pattern is not None
            else [0, 8, 9, 10, 11, 12, 13, 14, 15]
        )
        self.initializer_range = initializer_range
        self.use_cache = use_cache
