...
class DiaGenerationMixin(GenerationMixin):
    ...
    def _prepare_model_inputs(
        ...
        if self._uses_cfg:
            ...
            if model_kwargs.get("attention_mask", None) is not None:
                model_kwargs["attention_mask"] = model_kwargs["attention_mask"].repeat(
                    2, 1
                )
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        if decoder_input_ids is None or decoder_attention_mask is None:
            ...
            if decoder_input_ids is None:
                decoder_input_ids = torch.full(
                    (real_batch_size, 1, num_channels),
            ...
            decoder_attention_mask = torch.ones(
                size=(real_batch_size, decoder_input_ids.shape[1]),
                dtype=torch.long,
                device=device,
            )
        ...
        valid_input_size = (
            ...
            - (decoder_input_ids[:, :, 0] == self.config.pad_token_id).sum(dim=-1).max()
        )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        batch_size = (
            encoder_outputs[0].shape[0] // 2
            if self._uses_cfg
            else encoder_outputs[0].shape[0]
        )
        input_ids = input_ids.reshape(
            batch_size, self.config.decoder_config.num_channels, -1
        ).transpose(1, 2)

        # Base method handles most things except CFG and the delay pattern mask
        model_inputs = super().prepare_inputs_for_generation(
            input_ids, encoder_outputs=encoder_outputs, **kwargs
        )
        ...
        if (
            model_inputs.get("use_cache", False)
            and model_inputs["cache_position"][0] > 0
        ):
            model_inputs["decoder_input_ids"] = model_inputs["decoder_input_ids"][
                :, -1, :
            ][:, None, :]

        # Be compile friendly
        model_inputs["decoder_input_ids"] = model_inputs[
            "decoder_input_ids"
        ].contiguous()
    ...
    def apply_delay_mask(
        input_ids: torch.Tensor, pad_id: int, delay_mask: Optional[torch.Tensor]
    ) -> torch.Tensor:
        ...
        input_ids[:, :mask_len, :] = torch.where(
            valid_mask == pad_id, valid_input, valid_mask
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        output_sequences = output_sequences.reshape(bsz, num_channels, -1).transpose(
            1, 2
        )

        # Apply delay mask
        output_sequences = self.apply_delay_mask(
            output_sequences, self.config.pad_token_id, delay_mask
        )
