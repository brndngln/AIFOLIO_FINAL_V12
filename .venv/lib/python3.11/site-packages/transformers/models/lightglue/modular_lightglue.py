...
class LightGlueImageProcessor(SuperGlueImageProcessor):
    ...
    def plot_keypoint_matching(
        ...
        if is_matplotlib_available():
            ...
            raise ImportError(
                "Please install matplotlib to use `plot_keypoint_matching` method"
            )
...
class LightGluePositionalEncoder(nn.Module):
    def __init__(self, config: LightGlueConfig):
        ...
        self.projector = nn.Linear(
            2, config.descriptor_dim // config.num_attention_heads // 2, bias=False
        )
    ...
    def forward(
        ...
        output = (
            (embeddings, projected_keypoints) if output_hidden_states else (embeddings,)
        )
        return output
...
class LightGlueMLP(CLIPMLP):
    def __init__(self, config: LightGlueConfig):
        ...
        self.layer_norm = nn.LayerNorm(
            config.intermediate_size, elementwise_affine=True
        )
...
class LightGlueTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        encoder_attention_mask = (
            attention_mask.reshape(-1, 2, 1, 1, num_keypoints)
            .flip(1)
        ...
        cross_intermediate_states = torch.cat(
            [self_attention_descriptors, cross_attention_output], dim=-1
        )
        ...
        if output_hidden_states:
            cross_attention_hidden_states = (
                cross_intermediate_states,
                cross_output_states,
            )
            all_hidden_states = (
                all_hidden_states
                + (
                    self_attention_descriptors.reshape(
                        batch_size, num_keypoints, descriptor_dim
                    ),
                )
...
def sigmoid_log_double_softmax(
    ...
    certainties = nn.functional.logsigmoid(matchability0) + nn.functional.logsigmoid(
        matchability1
    ).transpose(1, 2)
    scores0 = nn.functional.log_softmax(similarity, 2)
    scores1 = nn.functional.log_softmax(
        similarity.transpose(-1, -2).contiguous(), 2
    ).transpose(-1, -2)
    scores = similarity.new_full(
        (batch_size, num_keypoints_0 + 1, num_keypoints_1 + 1), 0
    )
...
class LightGlueMatchAssignmentLayer(nn.Module):
    def __init__(self, config: LightGlueConfig):
        ...
        self.final_projection = nn.Linear(
            self.descriptor_dim, self.descriptor_dim, bias=True
        )
    ...
    def forward(self, descriptors: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        m_descriptors = (
            m_descriptors
            / torch.tensor(self.descriptor_dim, device=m_descriptors.device) ** 0.25
        )
        m_descriptors = m_descriptors.reshape(
            batch_size // 2, 2, num_keypoints, descriptor_dim
        )
...
def get_matches_from_scores(
    ...
    matches = (
        torch.stack([matches0, matches1]).transpose(0, 1).reshape(batch_size * 2, -1)
    )
    matching_scores = (
        torch.stack([matching_scores0, matching_scores1])
        .transpose(0, 1)
...
class LightGlueForKeypointMatching(LightGluePreTrainedModel):
    ...
    def __init__(self, config: LightGlueConfig):
        ...
        self.keypoint_detector_descriptor_dim = (
            config.keypoint_detector_config.descriptor_decoder_dim
        )
        self.descriptor_dim = config.descriptor_dim
        ...
        self.transformer_layers = nn.ModuleList(
            [
                LightGlueTransformerLayer(config, layer_idx=i)
                for i in range(config.num_hidden_layers)
            ]
        )
        self.match_assignment_layers = nn.ModuleList(
            [
                LightGlueMatchAssignmentLayer(config)
                for _ in range(config.num_hidden_layers)
            ]
        )
        self.token_confidence = nn.ModuleList(
            [
                LightGlueTokenConfidenceLayer(config)
    ...
    def _keypoint_processing(
        ...
        keypoint_encoding_output = self.positional_encoder(
            keypoints, output_hidden_states=output_hidden_states
        )
    ...
    def _get_early_stopped_image_pairs(
        ...
        if layer_index < self.num_layers - 1:
            ...
            ratio_confident = (
                1.0 - (keypoint_confidences < threshold).float().sum(dim=1) / num_points
            )
    ...
    def _get_keypoint_matching(self, descriptors, mask, layer_index, early_stops=None):
        ...
        matches, matching_scores = get_matches_from_scores(
    ...
    def _do_layer_keypoint_pruning(
        ...
        descriptors_matchability = self.match_assignment_layers[
            layer_index
        ].get_matchability(descriptors)
        pruned_keypoints_mask = self._get_pruning_mask(
            keypoint_confidences, descriptors_matchability, layer_index
        )
        pruned_keypoints_mask = pruned_keypoints_mask.masked_fill(
            mask == 0, torch.tensor(False)
        )
        ...
        pruned_indices = pad_sequence(
            pruned_indices, batch_first=True, padding_value=-1
        )
    ...
    def _do_final_keypoint_pruning(
        ...
        indices, matches, matching_scores = (
            tensor.reshape(batch_size // 2, 2, -1)
            for tensor in [indices, matches, matching_scores]
        )
        indices0 = indices[:, 0]
        indices1 = indices[:, 1]
        matches0 = matches[:, 0]
    ...
    def _match_image_pair(
        ...
        mask = (
            mask.reshape(batch_size * 2, initial_num_keypoints)
            if mask is not None
            else None
        )
        descriptors = descriptors.reshape(
            batch_size * 2, initial_num_keypoints, self.keypoint_detector_descriptor_dim
        )
        ...
        pruned_keypoints_indices = torch.arange(
            0, initial_num_keypoints, device=device
        ...
        for layer_index in range(self.num_layers):
            ...
            if mask is not None:
                ...
                extended_attention_mask = torch.ones(
                    (batch_size, input_shape[-2]), device=keypoints.device
                )
            ...
            if do_early_stop:
                if layer_index < self.num_layers - 1:
                    ...
                    keypoint_confidences = self.token_confidence[layer_index](
                        descriptors
                    )
                ...
                if torch.any(early_stopped_pairs):
                    ...
                    (
                        early_stopped_matches,
                        early_stopped_matching_scores,
                    ) = self._get_keypoint_matching(
                    ...
                    if do_keypoint_pruning:
                        final_pruned_keypoints_indices.extend(
                            list(pruned_keypoints_indices[early_stops])
                        )
                        final_pruned_keypoints_iterations.extend(
                            list(pruned_keypoints_iterations[early_stops])
                        )
        ...
        if do_early_stop and do_keypoint_pruning:
            ...
            matches, matching_scores = self._get_keypoint_matching(
                descriptors, mask, self.num_layers - 1
            )
            final_pruned_keypoints_iterations = (
                torch.ones_like(matching_scores) * self.num_layers
            )
    ...
    def forward(
        ...
        if labels is not None:
            raise ValueError(
                "LightGlue is not trainable, no labels should be provided."
            )
        ...
        descriptors = descriptors.reshape(
            batch_size, 2, -1, self.keypoint_detector_descriptor_dim
        ).to(pixel_values)
