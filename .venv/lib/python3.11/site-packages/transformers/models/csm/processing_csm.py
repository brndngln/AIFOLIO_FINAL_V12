...
class CsmProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        if not hasattr(tokenizer, "audio_eos_token"):
            ...
            self.audio_eos_token_id = tokenizer.convert_tokens_to_ids(
    ...
    def _get_encoded_length(
        ...
        if (
            ...
            or strides is None
            or dilations is None
            or use_causal_conv is None
        ):
            return cur_length
    ...
    def __call__(
        ...
        **kwargs: Unpack[CsmProcessorKwargs],
        ...
        if sum(n_audio_in_text) > 0 and n_audio != sum(n_audio_in_text):
            if audio is None:
                raise ValueError(
                    "No audio were provided, but there are audio tokens in the prompt"
                )
            else:
                raise ValueError(
        ...
        if audio is not None:
            ...
            )  # not supported by the feature extractor
            ...
            for n_audio in n_audio_in_text:
                if n_audio == 0:
                    ...
                    input_values_cutoffs.append(
                        torch.tensor(
                            [el.shape[-1] for el in audio[offset : offset + n_audio]]
                        ).cumsum(dim=-1)
                    )
                    offset += n_audio
            ...
            input_values_cutoffs = [
                torch.nn.functional.pad(
                    cut_idxs, (0, max_len - cut_idxs.shape[-1]), value=-1
                )
        ...
        if output_labels:
            ...
            if depth_decoder_labels_ratio <= 1.0:
                rand_idxs = torch.randperm(n_audio_frames)[
                    : int(n_audio_frames * (1 - depth_decoder_labels_ratio))
                ]
            ...
            labels = torch.where(
                (data["input_ids"] == self.audio_token_id)
                | (data["input_ids"] == self.audio_eos_token_id),
                data["input_ids"],
                -100,
            )
