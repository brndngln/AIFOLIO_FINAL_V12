...
class GraniteMoeHybridConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if layer_types is not None and any(
            layer_type not in ["mamba", "attention"] for layer_type in layer_types
        ):
            raise ValueError(
                "layer_types must be a list strings in  [`mamba` `attention`]"
            )
    ...
    def layers_block_type(self):
        return (
            self.layer_types if self.layer_types else ["mamba"] * self.num_hidden_layers
        )
