...
class MiniMaxCache(DynamicCache):
    ...
    def batch_repeat_interleave(self, repeats: int):
        for layer_idx in range(len(self)):
            if self.linear_cache[layer_idx] != []:
                self.linear_cache[layer_idx] = self.linear_cache[
                    layer_idx
    ...
    def batch_select_indices(self, indices: torch.Tensor):
        for layer_idx in range(len(self)):
            if self.linear_cache[layer_idx] != []:
                self.linear_cache[layer_idx] = self.linear_cache[layer_idx][
                    indices, ...
                ]
            else:
...
class MiniMaxLightningAttention(nn.Module):
    def __init__(self, config: MiniMaxConfig, layer_idx: int):
        ...
        self.qkv_proj = nn.Linear(
            config.hidden_size, self.num_attention_heads * self.head_dim * 3, bias=False
        )
    ...
    def decay_factors(self, slope_rate):
        ...
        key_decay = torch.exp(
            -slope_rate * (self.block_size - block_size_range[:, None])
        )
        ...
        diagonal_decay = torch.where(
            diagonal_decay >= 0, -diagonal_decay, float("-inf")
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        qkv_states = qkv_states.reshape(
            batch_size, seq_len, self.num_attention_heads, 3 * self.head_dim
        )

        query_states, key_states, value_states = torch.split(
            qkv_states, self.head_dim, dim=3
        )
        ...
        if attn_weights_inter is None:
            ...
            for i in range(num_blocks):
                ...
                # intra: ( Q @ K.T ) @ V -> QK * V
                attn_weights_intra = torch.matmul(
                    current_query_states, current_key_states.transpose(-1, -2)
                )
                attn_output_intra = torch.matmul(
                    attn_weights_intra * current_diagonal_decay, current_value_states
                )

                # inter: Q @ ( K.T @ V ) -> Q * KV
                attn_output_inter = torch.matmul(
                    current_query_states * current_query_decay, attn_weights_inter
                )
                ...
                next_attn_weights_inter = torch.matmul(
                    (current_key_states * current_key_decay).transpose(-1, -2),
            ...
            for i in range(seq_len):
                ...
                current_attn_weights_inter = torch.matmul(
                    current_key_states.transpose(-1, -2), current_value_states
                )
                attn_weights_inter = (
                    ratio * attn_weights_inter + current_attn_weights_inter
                )
                current_attn_output = torch.matmul(
                    current_query_states, attn_weights_inter
                )
...
class MiniMaxSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList(
            [MiniMaxBlockSparseTop2MLP(config) for _ in range(self.num_experts)]
        )
...
class MiniMaxDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_states = (
            residual * self.attn_alpha_factor + hidden_states * self.attn_beta_factor
        )
        ...
        hidden_states = (
            residual * self.mlp_alpha_factor + hidden_states * self.mlp_beta_factor
        )
