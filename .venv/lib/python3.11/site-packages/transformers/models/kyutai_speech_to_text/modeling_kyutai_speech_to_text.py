...
class KyutaiSpeechToTextConv1dPaddingCache:
    ...
    def __init__(
        ...
        if len(from_args_num_layers) != 1 or from_args_num_layers.pop() != num_layers:
            ...
            mode in ["constant", "replicate"] for mode in per_layer_padding_mode
        ):
    ...
    def update(self, hidden_states: torch.Tensor, layer_idx: int):
        ...
        if padding > 0:
            ...
            padding_states = torch.empty(
                batch_size, in_channels, padding, dtype=dtype, device=device
            )
...
class KyutaiSpeechToTextEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer(
            "audio_tokens_offsets", audio_tokens_offsets, persistent=False
        )

    def forward(self, input_ids):
        input_ids = torch.where(
            input_ids == self.embed_tokens.padding_idx,
            input_ids,
            input_ids + self.audio_tokens_offsets,
        )
...
class KyutaiSpeechToTextLinear(nn.Module):
    def __init__(self, input_dim, output_dim, num_codebooks, use_flexible_linear=False):
        ...
        if not use_flexible_linear:
            ...
            self.linear = KyutaiSpeechToTextFlexibleLinear(
...
class KyutaiSpeechToTextGatingMLP(nn.Module):
    def __init__(self, config, use_flexible_linear=False):
        ...
        if num_layers == 1:
            ...
            self.fc1 = KyutaiSpeechToTextFlexibleLinear(
                hidden_size, ffn_dim, num_layers
            )
            self.fc2 = KyutaiSpeechToTextFlexibleLinear(
                ffn_dim // 2, hidden_size, num_layers
            )
...
class KyutaiSpeechToTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(
        ...
        self.self_attn = KYUTAI_SPEECH_TO_TEXT_ATTENTION_CLASSES[
        ...
        self.input_layernorm = KyutaiSpeechToTextRMSNorm(
            self.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = KyutaiSpeechToTextRMSNorm(
            self.hidden_size, eps=config.rms_norm_eps
        )
        ...
        self.layers = nn.ModuleList(
            [
                KyutaiSpeechToTextDecoderLayer(
                    config, layer_idx, use_flexible_linear=False
                )
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = KyutaiSpeechToTextRMSNorm(
...
class KyutaiSpeechToTextForConditionalGeneration(
    ...
    def __init__(self, config):
        ...
        self.codec_model.generation_config = GenerationConfig.from_model_config(
            config.codec_config
        )
    ...
    def _prepare_generation_config(self, *args, **kwargs):
        ...
        model_kwargs["audio_window_size"] = (
            generation_config.audio_window_size
    ...
    def _prepare_model_inputs(
        ...
        if audio_window_size is None:
            audio_window_size = self.codec_model.get_encoded_length(
                model_kwargs["input_values"].shape[-1]
            ).item()
        ...
        for method in cache_methods:
            setattr(
                self.codec_model,
                method,
                types.MethodType(getattr(self, method).__func__, self.codec_model),
            )
        ...
        if "past_key_values" in temporary_model_kwargs:
            model_kwargs["encoder_past_key_values"] = temporary_model_kwargs[
                "past_key_values"
            ]
        ...
        for layer_name in self.codec_model.encoder._mimiconv1d_layer_names:
            per_layer_padding.append(
                self.codec_model.encoder.get_submodule(layer_name).padding_total
            )
            per_layer_padding_mode.append(
                self.codec_model.encoder.get_submodule(layer_name).pad_mode
            )
            per_layer_in_channels.append(
                self.codec_model.encoder.get_submodule(layer_name).in_channels
            )
            ...
            if cache_position[-1] - 1 >= end:
                ...
                with torch.no_grad():
                    ...
                    input_values_end_idx = (
                        start + audio_window_size
                    ) * self.config.frame_size
                    current_input_values = input_values[
                        ..., input_values_start_idx:input_values_end_idx
                    ]
                ...
                current_window.copy_(
                    torch.tensor([start, end], device=current_window.device).expand(
                        current_window.shape[0], -1
                    )
                )
    ...
    def from_pretrained(cls, *args, **kwargs):
        ...
        vars(model.codec_model.generation_config).update(
            {"_from_model_config": False, **codec_model_attrs}
        )
