...
class MimiEuclideanCodebook(nn.Module):
    ...
    def quantize(self, hidden_states):
        ...
        dists = torch.cdist(hidden_states[None].float(), self.embed[None].float(), p=2)[
            0
        ]
...
class MimiModel(MimiPreTrainedModel):
    ...
    def get_encoded_length(self, input_length: torch.LongTensor) -> torch.LongTensor:
        ...
        for layer_name in self.encoder._mimiconv1d_layer_names:
            output_length = self.encoder.get_submodule(layer_name)._get_output_length(
                output_length
            )
    ...
    def get_audio_codes_mask(
        self, padding_mask: torch.Tensor, padding_side: str = "right"
    ):
        """
        ...
        audio_codes_mask = torch.arange(
            encoded_lengths.max(), device=encoded_lengths.device
        ).expand(len(encoded_lengths), -1)
    ...
    def encode(
        ...
        use_streaming = (
            use_streaming if use_streaming is not None else self.config.use_streaming
        )
        ...
        if use_streaming and padding_cache is None:
            ...
            for layer_name in self.encoder._mimiconv1d_layer_names:
                per_layer_padding.append(
                    self.encoder.get_submodule(layer_name).padding_total
                )
                per_layer_padding_mode.append(
                    self.encoder.get_submodule(layer_name).pad_mode
                )
                per_layer_in_channels.append(
                    self.encoder.get_submodule(layer_name).in_channels
                )
