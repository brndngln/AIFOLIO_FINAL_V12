        ...
        if (
            ...
            for d in current_segment_list:
                if (
                    ...
                    and len(d["tokens"]) > 2
                    and d["tokens"][-2] >= timestamp_begin
                ):
    ...
    for i in range(len(current_segments)):
        ...
        if return_token_timestamps:
            token_timestamps_list[i] = F.pad(
                ...
                pad=pad,
                value=token_timestamps_list[i][-1]
    ...
    def _extract_token_timestamps(
        ...
        if "beam_indices" in generate_outputs:
            ...
            if num_input_ids is not None and num_input_ids > 1:
                ...
                beam_indices_first_step_unrolled = torch.ones(
                ...
                unrolled_beam_indices = torch.cat(
                    [beam_indices_first_step_unrolled, beam_indices], dim=-1
                )
            else:
            ...
            unrolled_beam_indices = unrolled_beam_indices.masked_fill(
                unrolled_beam_indices == -1, 0
            )
            ...
            weights = torch.stack(
                [
                    torch.index_select(
                        weights[:, :, i, :], dim=0, index=unrolled_beam_indices[:, i]
                    )
