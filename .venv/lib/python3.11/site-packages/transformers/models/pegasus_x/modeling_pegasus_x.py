...
class PegasusXSinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(
        ...
        div_term = torch.exp(
            torch.arange(
                half_d_feature, device=input_embeds.device, dtype=torch.int64
            ).type_as(input_embeds)
...
class PegasusXDecoder(PegasusXPreTrainedModel):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                PegasusXDecoderLayer(config, layer_idx=i)
    ...
    def forward(
        ...
        position_ids = self.embed_positions(
            inputs_embeds, past_key_values_length, position_ids
        )
