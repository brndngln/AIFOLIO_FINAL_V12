...
class Qwen2VLVideoProcessor(BaseVideoProcessor):
    ...
    def sample_frames(
        ...
        if num_frames is not None:
            ...
            max_frames = (
                math.floor(min(max_frames, total_num_frames) / frame_factor)
                * frame_factor
            )
            num_frames = total_num_frames / metadata["fps"] * fps
            num_frames = min(
                min(max(num_frames, min_frames), max_frames), total_num_frames
            )
    ...
    def get_num_of_video_patches(
        ...
        temporal_patch_size = (
            videos_kwargs.get("temporal_patch_size", None) or self.temporal_patch_size
        )
