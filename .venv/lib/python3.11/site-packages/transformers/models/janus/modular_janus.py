    ...
    def __init__(
        ...
        **kwargs,
        ...
        if vision_config is None:
            logger.info(
                "`vision_config` is None. Initializing with default JanusVisionConfig values"
            )
        ...
        if vq_config is None:
            logger.info(
                "`vq_config` is None. Initializing with default JanusVQVAEConfig values"
            )
...
class JanusVisionEncoder(SiglipEncoder):
    def __init__(self, config: JanusVisionConfig):
        ...
        self.layers = nn.ModuleList(
            [JanusVisionEncoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class JanusVQVAE(ChameleonVQVAE):
    ...
    def decode(self, image_tokens: torch.LongTensor) -> torch.FloatTensor:
        ...
        if (
            ...
            != self.quantize.quant_state_dims[0] * self.quantize.quant_state_dims[1]
        ):
            raise ValueError(
...
class JanusModel(JanusPreTrainedModel):
    def __init__(self, config: JanusConfig):
        ...
        self.generation_embeddings = nn.Embedding(
    ...
    def forward(
        ...
        **kwargs,
        ...
        if pixel_values is not None:
            ...
            image_attention_mask = image_attention_mask.unsqueeze(-1).expand(
                -1, -1, embed_dim
            )
...
class JanusForConditionalGeneration(JanusPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_config.get_generation_mode() not in (
        ...
        if generation_config.guidance_scale is None:
            logger.warning(
                "`guidance_scale` is required for CFG but not provided. Setting to default value of 5."
            )
        ...
        if generation_config.guidance_scale and generation_config.guidance_scale > 1:
            ...
            generation_config.guidance_scale = (
                None  # Reset to prevent processor duplication.
            )
        ...
        mask = (input_tokens[batch_size:, :] != generation_config.bos_token_id) & (
            ...
            != generation_config.generation_kwargs["boi_token_id"]
        )
        ...
        if model_kwargs.get("past_key_values", None) is None:
            ...
            model_kwargs["past_key_values"] = self._get_cache(
                ...
                max_cache_len=max(
                    generation_config.max_length, num_image_tokens + seq_len
                ),
                device=device,
        ...
        generated_tokens = torch.zeros(
            (batch_size, num_image_tokens), dtype=dtype, device=device
        )
        ...
        for i in range(num_image_tokens):
            ...
            model_inputs["cache_position"] = model_inputs["cache_position"].to(
                inputs_embeds.device
            )
