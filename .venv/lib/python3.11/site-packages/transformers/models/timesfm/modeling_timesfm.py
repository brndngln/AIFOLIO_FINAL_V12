    ...
    def forward(self, seq_length=None, position=None):
        ...
        if position is None:
            ...
            position = torch.arange(
                seq_length, dtype=torch.float32, device=self.inv_timescales.device
            ).unsqueeze(0)
        elif position.ndim != 2:
            raise ValueError(
                f"position must be 2-dimensional, got shape {position.shape}"
            )

        scaled_time = position.view(*position.shape, 1) * self.inv_timescales.view(
            1, 1, -1
        )
...
class TimesFmDecoderLayer(nn.Module):
    ...
    def __init__(self, config: TimesFmConfig, layer_idx: int):
        ...
        self.input_layernorm = TimesFmRMSNorm(
...
class TimesFmModel(TimesFmPreTrainedModel):
    def __init__(self, config: TimesFmConfig):
        ...
        self.freq_emb = nn.Embedding(
            num_embeddings=config.freq_size, embedding_dim=config.hidden_size
        )
        self.layers = nn.ModuleList(
            [
                TimesFmDecoderLayer(config, layer_idx)
    ...
    @staticmethod
    def _timesfm_shift_padded_seq(
        mask: torch.Tensor, seq: torch.Tensor
    ) -> torch.Tensor:
...
class TimesFmModelForPrediction(TimesFmPreTrainedModel):
    ...
    def _preprocess(
        ...
        for i, ts in enumerate(inputs):
            ...
            padding = torch.zeros(
                input_len + self.horizon_len, dtype=ts.dtype, device=ts.device
            )
            if input_len < self.context_len:
                num_front_pad = self.context_len - input_len
                ts = torch.cat(
                    [torch.zeros(num_front_pad, dtype=ts.dtype, device=ts.device), ts],
                    dim=0,
                )
                padding = torch.cat(
                    [
                        torch.ones(
                            num_front_pad, dtype=ts.dtype, device=padding.device
                        ),
                        padding,
                    ],
                    dim=0,
    ...
    def _postprocess_output(
        ...
        output_ts = output_ts.view(
            b, n, self.config.horizon_length, len(self.config.quantiles) + 1
        )
    ...
    def forward(
        ...
        num_decode_patches = (
            self.horizon_len + output_patch_len - 1
        ) // output_patch_len
    ...
    def _timesfm_moving_average(
        ...
        kernel = (
            torch.ones(window_size, dtype=arr.dtype, device=arr.device) / window_size
        )
        # Apply convolution to calculate the moving average
        smoothed_arr = F.conv1d(
            arr_padded.view(1, 1, -1), kernel.view(1, 1, -1)
        ).squeeze()
