...
class InternVLProcessor(ProcessorMixin):
        ...
        for prompt in text:
            ...
            while self.image_token in new_prompt or self.video_token in new_prompt:
                if self.image_token in new_prompt and (
                    ...
                    current_patch_index = (
                        video_patch_indices[video_index - 1] if video_index > 0 else 0
                    )
                    end_patch_index = video_patch_indices[video_index]
                    start_index = (
                        video_num_patches_indices[current_patch_index]
                        if video_index > 0
                        else 0
                    )
    ...
    def __call__(
        ...
        **kwargs: Unpack[InternVLProcessorKwargs],
        ...
        if videos is not None:
            ...
            video_num_patches = [
                1 for frames in num_frames_per_video for _ in range(frames)
            ]
        ...
        if images is not None or videos is not None:
            ...
            if videos is not None and video_index != len(videos):
                raise ValueError(
                    "Number of video placeholders in the prompt does not match the number of videos."
                )

            # Concatenate the interleaved image and video patches (function agnostic to the patches type (list, numpy array, torch tensor))
            image_videos_inputs = {
                "pixel_values": concatenate_list(image_video_patches)
            }
        ...
        return BatchFeature(
            data={**text_inputs, **image_videos_inputs}, tensor_type=return_tensors
        )
    ...
    def _get_num_multimodal_tokens(self, image_sizes=None, **kwargs):
        ...
        if image_sizes is not None:
            ...
            num_image_patches = [
                self.image_processor.get_number_of_image_tokens(
            ...
            num_image_tokens = [
                2 + (self.image_seq_length * num_patches)
