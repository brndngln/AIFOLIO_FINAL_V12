...
class InternVLVisionAttention(JanusVisionAttention):
    def __init__(self, config: InternVLVisionConfig):
        ...
        self.q_norm = (
            InternVLVisionRMSNorm(self.embed_dim) if qk_norm else nn.Identity()
        )
        self.k_norm = (
            InternVLVisionRMSNorm(self.embed_dim) if qk_norm else nn.Identity()
        )
    ...
    def forward(
        ...
        attention_output, attention_weights = self.attention(
            self.layernorm_before(
                hidden_states
            ),  # in InternVLVision, layernorm is applied before self-attention
...
class InternVLVisionEncoder(nn.Module):
    def __init__(self, config: InternVLVisionConfig) -> None:
        ...
        self.layer = nn.ModuleList(
            [InternVLVisionLayer(config) for i in range(config.num_hidden_layers)]
        )
