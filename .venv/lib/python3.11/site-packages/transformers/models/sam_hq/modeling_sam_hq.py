...
class SamHQVisionLayer(GradientCheckpointingLayer):
    def __init__(self, config, window_size):
        ...
        self.attn = SAM_HQ_VISION_ATTENTION_CLASSES[config._attn_implementation](
            config, window_size
        )
...
class SamHQTwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = SAM_HQ_ATTENTION_CLASSES[config._attn_implementation](
            config, downsample_rate=1
        )
        self.layer_norm1 = nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)

        self.cross_attn_token_to_image = SAM_HQ_ATTENTION_CLASSES[
        ...
        self.cross_attn_image_to_token = SAM_HQ_ATTENTION_CLASSES[
            config._attn_implementation
        ](config, downsample_rate=attention_downsample_rate)
...
class SamHQTwoWayTransformer(nn.Module):
    def __init__(self, config: SamHQMaskDecoderConfig):
        ...
        for i in range(self.num_hidden_layers):
            self.layers.append(
                SamHQTwoWayAttentionBlock(config, skip_first_layer_pe=(i == 0))
            )

        self.final_attn_token_to_image = SAM_HQ_ATTENTION_CLASSES[
            config._attn_implementation
        ](config)
...
class SamHQMaskDecoder(nn.Module):
    def __init__(self, config: SamHQMaskDecoderConfig):
        ...
        self.upscale_layer_norm = SamHQLayerNorm(
        ...
        self.hq_mask_mlp = SamHQFeedForward(
            self.hidden_size, self.hidden_size, self.hidden_size // 8, 3
        )
        self.num_mask_tokens = self.num_mask_tokens + 1

        # Compress ViT features
        self.compress_vit_conv1 = nn.ConvTranspose2d(
            config.vit_dim, self.hidden_size, kernel_size=2, stride=2
        )
        self.compress_vit_norm = SamHQLayerNorm(
            self.hidden_size, data_format="channels_first"
        )
        self.compress_vit_conv2 = nn.ConvTranspose2d(
            self.hidden_size, self.hidden_size // 8, kernel_size=2, stride=2
        )

        # Embedding encoder
        self.encoder_conv1 = nn.ConvTranspose2d(
            self.hidden_size, self.hidden_size // 4, kernel_size=2, stride=2
        )
        self.encoder_norm = SamHQLayerNorm(
            self.hidden_size // 4, data_format="channels_first"
        )
        self.encoder_conv2 = nn.ConvTranspose2d(
            self.hidden_size // 4, self.hidden_size // 8, kernel_size=2, stride=2
        )
    ...
    def forward(
        ...
        has_intermediate = (
            intermediate_embeddings is not None and len(intermediate_embeddings) > 0
        )
        ...
        if has_intermediate:
            ...
            compressed_vit_features = self.activation(
                self.compress_vit_norm(compressed_vit_features)
            )
        ...
        output_tokens = torch.cat(
            [self.iou_token.weight, self.mask_tokens.weight, self.hq_token.weight],
            dim=0,
        )
        ...
        if hq_features.shape[0] == 1:
            ...
            hq_features.shape[0] == batch_size
            and batch_size * point_batch_size != batch_size
        ):
        ...
        masks_sam = (
            hyper_in[:, :, : self.num_mask_tokens - 1] @ upscaled_embedding
        ).reshape(batch_size, point_batch_size, -1, height, width)
        masks_hq = (
            hyper_in[:, :, self.num_mask_tokens - 1 :] @ upscaled_embedding_hq
        ).reshape(batch_size, point_batch_size, -1, height, width)
        ...
        if multimask_output:
            ...
            masks_sam = torch.gather(
                masks_sam,
                2,
                sort_indices[..., None, None].expand(
                    -1, -1, -1, masks_sam.shape[3], masks_sam.shape[4]
                ),
            )
        ...
        masks_hq = masks[
            :, :, slice(self.num_mask_tokens - 1, self.num_mask_tokens), :, :
        ]
        if hq_token_only:
            masks = masks_hq
        else:
