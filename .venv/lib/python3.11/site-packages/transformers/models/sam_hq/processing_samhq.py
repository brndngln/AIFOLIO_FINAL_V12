...
class SamHQProcessor(ProcessorMixin):
    ...
    def _normalize_and_convert(
        ...
        if input_points is not None:
            input_points = self._normalize_batch_coordinates(
        ...
        if input_boxes is not None:
            input_boxes = self._normalize_batch_coordinates(
                input_boxes, original_sizes, is_bounding_box=True
            )
            input_boxes = np.array(input_boxes)

        # Update processor with converted inputs
        if input_boxes is not None:
            encoding_image_processor["input_boxes"] = self._to_tensor(
                input_boxes, 3, return_tensors
            )
        if input_points is not None:
            encoding_image_processor["input_points"] = self._to_tensor(
                input_points, 4, return_tensors
            )
        if input_labels is not None:
            encoding_image_processor["input_labels"] = self._to_tensor(
                input_labels, 3, return_tensors
            )
    ...
    def _check_and_preprocess_points(
        ...
        input_points = self._preprocess_input(
            input_points, "Input points must be a list of list of floating points."
        )

        input_labels = self._preprocess_input(
            input_labels, "Input labels must be a list of list integers."
        )
    ...
    def _normalize_batch_coordinates(
        self, inputs, original_sizes, is_bounding_box=False
    ):
        """
        ...
        if len(original_sizes) != len(inputs):
            ...
            return [
                self._normalize_coordinates(
                    self.target_size, item, size, is_bounding_box=is_bounding_box
                )
