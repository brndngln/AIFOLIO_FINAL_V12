...
class RichInterface:
    def __init__(
        self, model_name: Optional[str] = None, user_name: Optional[str] = None
    ):
    ...
    def print_status(
        self, model_name: str, generation_config: GenerationConfig, model_kwargs: dict
    ):
...
class ChatArguments:
    ...
    verbose: bool = field(
        default=False,
        metadata={"help": "Whether to show runtime warnings in the chat interface."},
    )
...
class ChatCommand(BaseTransformersCLICommand):
    ...
    def parse_generate_flags(self, generate_flags: list[str]) -> dict:
        ...
        generate_flags_as_dict = {
            k: v.lower() if v.lower() in ["true", "false"] else v
            for k, v in generate_flags_as_dict.items()
        }
        generate_flags_as_dict = {
            k: "null" if v == "None" else v for k, v in generate_flags_as_dict.items()
        }
        ...
        generate_flags_as_dict = {
            k: f'"{v}"' if not is_number(v) else v
            for k, v in generate_flags_as_dict.items()
        }
        ...
        generate_flags_string = ", ".join(
            [f"{k}: {v}" for k, v in generate_flags_as_dict.items()]
        )
    ...
    def parse_eos_tokens(
        ...
        if eos_tokens is not None:
            all_eos_token_ids.extend(
                tokenizer.convert_tokens_to_ids(eos_tokens.split(","))
            )
    ...
    def handle_non_exit_user_commands(
        ...
        if user_input == "!clear":
            ...
            for flag in new_generate_flags:
                ...
                parsed_new_generate_flags = self.parse_generate_flags(
                    new_generate_flags
                )
            ...
            valid_command = False
            interface.print_color(
                text=f"'{user_input}' is not a valid command. Showing help message.",
                color="red",
            )
            interface.print_help()
    ...
    def run(self):
        ...
        generation_config, model_kwargs = self.get_generation_parameterization(
        ...
        interface = RichInterface(
            model_name=args.model_name_or_path_positional, user_name=user
        )
        interface.clear()
        chat = self.clear_chat_history(args.system_prompt)
                ...
                if user_input.startswith("!"):
                    ...
                    if user_input == "!exit":
                        ...
                        (
                            ...
                            generation_config,
                            model_kwargs,
                        ) = self.handle_non_exit_user_commands(
