...
class HybridCache(Cache):
    ...
    def get_mask_sizes(
        ...
        if self.is_sliding[layer_idx]:
            ...
            local_mask_kv_offset = torch.clamp(
                first_cache_position - self.sliding_window + 1, min=0
            )
            # This is not general (see HybridChunkedCache for the whole general case), but it's what the cache returns
...
class HybridChunkedCache(Cache):
    ...
    def update(
        ...
        update_fn = (
            self._sliding_update if self.is_sliding[layer_idx] else self._static_update
        )
        return update_fn(
            cache_position,
            layer_idx,
