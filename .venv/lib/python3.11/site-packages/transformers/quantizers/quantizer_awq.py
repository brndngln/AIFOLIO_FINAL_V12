...
class AwqQuantizer(HfQuantizer):
        ...
        if self.quantization_config.version == AWQLinearVersion.IPEX:
            ...
            if device_map is None:
                ...
                if isinstance(device_map, dict) and any(
                    forbidden in device_map.values()
                    for forbidden in ("cpu", torch.device("cpu"), "disk")
                ):
                    raise ValueError(
    ...
    def update_torch_dtype(self, torch_dtype):
        if torch_dtype is None:
            ...
            logger.warning(
                "We suggest you to set `torch_dtype=torch.float16` for better efficiency on CUDA with AWQ."
            )
        return torch_dtype
