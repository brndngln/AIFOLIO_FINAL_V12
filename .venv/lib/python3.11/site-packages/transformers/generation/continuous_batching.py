...
class PagedAttentionCache(Cache):
    def __init__(
        ...
        if num_blocks is None or block_size is None:
            ...
            num_blocks, block_size = compute_optimal_blocks(
                ...
                config,
                generation_config,
                initial_prompt_shapes or [],
                dtype,
                median_prefill_length=200,
            )
            logger.info(
                f"Using calculated num_blocks={num_blocks}, block_size={block_size}"
            )
        ...
        for idx in range(config.num_hidden_layers):
            layer_device = (
                layer_device_map[idx] if layer_device_map is not None else device
            )
            new_layer_key_cache = torch.zeros(
                self.cache_shape, dtype=self.dtype, device=layer_device
            )
    ...
    def free_blocks(self, request_id: str) -> None:
        ...
        if request_id in self._block_tables:
            ...
            logger.warning(
                f"Attempted to free blocks for non-existent request_id: {request_id}"
            )
    ...
    def update(
        ...
        **kwargs,
        ...
        k_cache_flat = self.key_cache[layer_idx].view(
            self.num_key_value_heads, total_slots, self.head_dim
        )
        v_cache_flat = self.value_cache[layer_idx].view(
            self.num_key_value_heads, total_slots, self.head_dim
        )
        k_cache_flat[:, write_index, :] = key_states[0]
        v_cache_flat[:, write_index, :] = value_states[0]
        return (
            k_cache_flat[None, :, read_index, :],
            v_cache_flat[None, :, read_index, :],
        )
...
class Scheduler(ABC):
    ...
    def __init__(
        self, cache: PagedAttentionCache, retain_cache_on_finish: bool = False
    ):
...
class FIFOScheduler(Scheduler):
    ...
    def _allocate_blocks_if_needed(self, state: RequestState, len_next_tokens: int):
        ...
        if occupancy < len_next_tokens or (len(state.allocated_blocks) == 0):
            blocks_needed = (
                (len_next_tokens - occupancy + 1) // self.cache.block_size
            ) + 1
    ...
    def schedule_batch(self, token_budget: int) -> list[RequestState]:
        ...
        for state in candidates:
            self._prepare_request_for_processing(
                state, token_budget, request_ids_to_remove_from_waiting
            )
        ...
        self.waiting_requests_order = deque(
            [
                req_id
                for req_id in self.waiting_requests_order
                if req_id not in request_ids_to_remove_from_waiting
            ]
        )
...
def compute_optimal_blocks(
    ...
    if device.type == "cuda":
        ...
        logger.warning(
            "MPS memory estimation is approximate. Using conservative defaults."
        )
        return 2048, 256
    else:
        logger.warning(
            f"Unsupported device type {device.type} for optimal block calculation. Using defaults."
        )
        return 32, 128
    ...
    memory_per_token = (
        2 * num_kv_heads * head_dim * dtype_size * num_hidden_layers
    )  # For K and V caches
    ...
    if median_prefill_length is None and inputs:
        ...
        median_prefill_length = (
            int(statistics.median(non_empty_inputs)) if non_empty_inputs else 64
        )
    ...
    initial_block_size = max(
        MIN_BLOCK_SIZE, total_tokens // (max_concurrent_sequences * 2)
    )
...
def create_document_mask(cumulative_seqlens_q, cumulative_seqlens_k):
    ...
    is_causal = (
        ~(cumulative_seqlens_q[1:] - cumulative_seqlens_q[:-1] == 1)
        * cumulative_seqlens_q[1:]
    )
    apply_causal = (
        torch.bucketize(q_indices, is_causal, right=True)[:, None] == k_doc_ids
    )
    # TODO don't apply on prefill splitting
    causal_mask = torch.triu(
        torch.ones(total_q, total_k, device=q_doc_ids.device), diagonal=1
    ).bool()
...
class ContinuousBatchProcessor:
    ...
    def _configure_batch_parameters(self):
        ...
        if user_batch_tokens is not None:
            ...
            self.max_context_len = getattr(
                self.generation_config, "max_position_embeddings", 2048
            )
    ...
    def _handle_request_error(self, error, state: RequestState):
        ...
        if isinstance(state.request_id, str):
            state.static_outputs = self.scheduler.get_active_request_static_outputs(
                state.request_id
            )
        else:
    ...
    def prepare_next_batch(self):
        ...
        self.metrics.record_queue_metrics(
            len(self.scheduler.active_requests), len(self.scheduler.waiting_requests)
        )
    ...
    def _build_tensors(
        ...
        self.cumulative_seqlens_q[: len(cumulative_seqlens_q)] = to_tensor(
            cumulative_seqlens_q
        )
        self.cumulative_seqlens_k[: len(cumulative_seqlens_k)] = to_tensor(
            cumulative_seqlens_k
        )
            ...
            if len(state.remaining_prompt_ids) == 0:
                ...
                if state.update_with_token(token):
                    self.metrics.record_request_completion(
                        state.created_time, state.request_id
                    )
                    self.scheduler.finish_request(
                        state.request_id, evict_from_cache=(not self.manual_eviction)
                    )
...
class ContinuousBatchingManager:
    ...
    def __init__(
        ...
        self.streaming = streaming
        self.log_prob_generation = getattr(
            generation_config, "log_prob_generation", False
        )
        ...
        self.logit_processor = self.model._get_logits_processor(
    ...
    def add_request(
        ...
        self.input_queue.put(
            state, block=True, timeout=10
        )  # XXX: pass timeout as fn arg?
    ...
    def __iter__(self):
        ...
        while (
            ...
            result = self.get_result(
                timeout=0.1
            )  # allow the model to run for 10 seconds
        ...
        try:
            ...
            if hasattr(self.generation_config, "scheduler"):
                ...
                if scheduler is None:
                    logger.warning(
                        f"Scheduler '{scheduler}' not found. Defaulting to FIFO."
                    )
            ...
            if self.profile:
                tracing_schedule = schedule(
                    skip_first=2, warmup=3, active=200, repeat=100, wait=1
                )
                trace_handler = tensorboard_trace_handler(
                    dir_name="/fsx/arthur/transformers",
                    use_gzip=True,
                    worker_name="paged_compile",
                )
                activities = [
                ...
                with profile(
                    ...
                    while (
                        ...
                        or batch_processor.has_pending_requests()
                    ):
    ...
    def _inner_generation_loop(
        self, batch_processor: ContinuousBatchProcessor, is_first: bool = False
    ):
    ...
    def _handle_critical_error(
        self, error, batch_processor: Optional[ContinuousBatchProcessor]
    ):
