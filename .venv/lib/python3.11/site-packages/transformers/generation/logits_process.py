...
class DiaClassifierFreeGuidanceLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        cond_logits, uncond_logits = scores.split(unguided_bsz, dim=0)
        scores_processed = (
            cond_logits + (cond_logits - uncond_logits) * self.guidance_scale
        )
        ...
        if self.guidance_top_k is not None:
            ...
            _, top_k_indices = torch.topk(
                scores_processed, k=self.guidance_top_k, dim=-1
            )
...
class DiaEOSChannelFilterLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, num_channels: int, eos_token_id: int):
        if num_channels < 1:
            raise ValueError(
                f"Audio codebooks need at least one channel, but found {num_channels} channels."
            )
        if eos_token_id < 1:
            raise ValueError(
                f"Expected `eos_token_id` to be a positive integer, found {eos_token_id} instead."
            )
...
class DiaEOSDelayPatternLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        self.delay_pattern = torch.tensor(
            delay_pattern, device=device, dtype=torch.int
        )[None, :]
        self.eos_token_id = eos_token_id
    ...
    def __call__(
        ...
        if self.active_batches is None:
            ...
            self.active_batches = torch.zeros(
                size=(scores.shape[0],), device=self.device, dtype=torch.bool
            )
