...
class TokenClassificationPipeline(ChunkPipeline):
    ...
    def __call__(
        ...
        _inputs, is_split_into_words, offset_mapping, delimiter = self._args_parser(
            inputs, **kwargs
        )
    ...
    def preprocess(self, sentence, offset_mapping=None, **preprocess_params):
        ...
        if is_split_into_words:
            ...
            if not isinstance(sentence, list):
                raise ValueError(
                    "When `is_split_into_words=True`, `sentence` must be a list of tokens."
                )
            words = sentence
            sentence = delimiter.join(
                words
            )  # Recreate the sentence string for later display and slicing
            ...
            if not isinstance(sentence, str):
                raise ValueError(
                    "When `is_split_into_words=False`, `sentence` must be an untokenized string."
                )
        ...
        if is_split_into_words and not self.tokenizer.is_fast:
            raise ValueError(
                "is_split_into_words=True is only supported with fast tokenizers."
            )
