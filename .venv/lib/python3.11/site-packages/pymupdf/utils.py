...
def replace_image(
    page: pymupdf.Page, xref: int, *, filename=None, pixmap=None, stream=None
):
...
def get_image_info(
    page: pymupdf.Page, hashes: bool = False, xrefs: bool = False
) -> list:
...
def do_widgets(
    ...
    def join_widgets(pdf, acro_fields, xref1, xref2, name):
        ...
        if kids1.pdf_is_array():
            re_target(
                pdf, acro_fields, xref1, kids1, xref2, kids2
            )  # pylint: disable=arguments-out-of-order
        elif kids2.pdf_is_array():
            re_target(
                pdf, acro_fields, xref2, kids2, xref1, kids1
            )  # pylint: disable=arguments-out-of-order
        else:
            new_target(
                pdf, acro_fields, xref1, w1, xref2, w2, name
            )  # pylint: disable=arguments-out-of-order
...
def getLinkText(page: pymupdf.Page, lnk: dict) -> str:
    ...
    link_names = dict(  # existing ids and their xref
        [
            (x[0], x[2]) for x in page.annot_xrefs() if x[1] == pymupdf.PDF_ANNOT_LINK
...
def draw_bezier(
    ...
    img = page.new_shape()
    Q = img.draw_bezier(
        pymupdf.Point(p1), pymupdf.Point(p2), pymupdf.Point(p3), pymupdf.Point(p4)
    )
    img.finish(
        color=color,
        fill=fill,
...
def draw_sector(
    ...
    img = page.new_shape()
    Q = img.draw_sector(
        pymupdf.Point(center), pymupdf.Point(point), beta, fullSector=fullSector
    )
    img.finish(
        color=color,
        fill=fill,
...
class Shape:
        ...
        for i, p in enumerate(points):
            if i == 0:
                if not (self.last_point == pymupdf.Point(p)):
                    self.draw_cont += (
                        _format_g(pymupdf.JM_TUPLE(pymupdf.Point(p) * self.ipctm))
                        + " m\n"
                    )
                    self.last_point = pymupdf.Point(p)
            else:
                self.draw_cont += (
                    _format_g(pymupdf.JM_TUPLE(pymupdf.Point(p) * self.ipctm)) + " l\n"
                )
            self.updateRect(p)
    ...
    def draw_rect(self, rect: rect_like, *, radius=None) -> pymupdf.Point:
        ...
        if radius is None:  # standard rectangle
            self.draw_cont += (
                _format_g(
                    pymupdf.JM_TUPLE(list(r.bl * self.ipctm) + [r.width, r.height])
                )
                + " re\n"
            )
            self.updateRect(r)
    ...
    def draw_zigzag(
        ...
        matrix = pymupdf.Matrix(
            pymupdf.util_hor_matrix(p1, p2)
        )  # normalize line to x-axis
    ...
    def insert_text(
        ...
        *,
        ...
        if morphing:
            m1 = pymupdf.Matrix(
                1, 0, 0, 1, morph[0].x + self.x, height - morph[0].y - self.y
            )
            mat = ~m1 * morph[1] * m1
        ...
        *,
        ...
        for i, line in enumerate(t0):
            ...
            for j in range(num_words):
                ...
                # int word: split across multiple lines - char by char ...
                if len(just_tab) > 0:
                ...
                lbuff += " "  # finish int word
                rest = maxwidth - pixlen(lbuff)  # int word stored
        ...
        templ = (
            lambda a, b, c, d: f"1 0 0 1 {_format_g((a, b))} Tm /{c} {_format_g(d)} Tf "
        )
...
def fill_textbox(
    ...
    def norm_words(width, words):
        """Cut any word in pieces no inter than 'width'."""
        nwords = []
        word_lengths = []
        for w in words:
            ...
            # word inter than rect width - split it in parts
            n = len(wl_lst)
            while n > 0:
                wl = sum(wl_lst[:n])
                if wl <= width:
    ...
    for i, line in enumerate(textlines):
        ...
        # cut in parts any words that are inter than rect width
...
def has_annots(doc: pymupdf.Document) -> bool:
    ...
    for i in range(doc.page_count):
        for item in doc.page_annot_xrefs(i):
            # pylint: disable=no-member
            if not (
                item[1] == pymupdf.PDF_ANNOT_LINK or item[1] == pymupdf.PDF_ANNOT_WIDGET
            ):  # pylint: disable=no-member
                return True
    return False
...
# -------------------------------------------------------------------
def subset_fonts(
    doc: pymupdf.Document, verbose: bool = False, fallback: bool = False
) -> OptInt:
...
# -------------------------------------------------------------------
def xref_copy(
    doc: pymupdf.Document, source: int, target: int, *, keep: list = None
) -> None:
