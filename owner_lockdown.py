    ...
    def decorator(func: Callable):
        def wrapper(*args, **kwargs):
            if not biometric_check():
                raise OwnerApprovalError('OWNER biometric approval required for: ' + action_desc)
...
def never_without_you(owner_signature: str):
    if owner_signature != 'VALID_OWNER_SIGNATURE':
        raise OwnerApprovalError('Action rejected: OWNER digital signature missing or invalid (Never Without You Principle).')
    return True
