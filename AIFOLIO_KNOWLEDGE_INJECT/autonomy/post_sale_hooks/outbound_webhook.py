...
def post_sale_event_to_webhooks(event_name, payload):
    ...
    for url in urls:
        ...
        try:
            resp = requests.post(
                url, json={"event": event_name, "payload": payload}, timeout=10
            )
            if resp.status_code in (200, 201, 202):
                logging.info(
                    f"[AIFOLIO] Outbound webhook success: {url} event={event_name}"
                )
            else:
                logging.error(
                    f"[AIFOLIO] Outbound webhook failed: {url} code={resp.status_code} body={resp.text}"
                )
        ...
        if elapsed > 2.0:
            logging.warning(
                f"[AIFOLIO][PERF] Outbound webhook {url} took {elapsed:.2f}s"
            )
