...
def run_all_hooks(sale_data):
    ...
    start = time.time()
    try:
        results["send_confirmation_email"] = send_confirmation_email(
            sale_data["buyer_email"], sale_data["vault_name"]
        )
        hook_metrics.record("send_confirmation_email", True, time.time() - start)
    except Exception as e:
        hook_metrics.record(
            "send_confirmation_email", False, time.time() - start, error=str(e)
        )
    # Trigger Upsell Suggestion
    start = time.time()
    try:
        results["trigger_upsell_suggestion"] = trigger_upsell_suggestion(
            sale_data["buyer_email"], sale_data["vault_name"]
        )
        hook_metrics.record("trigger_upsell_suggestion", True, time.time() - start)
    except Exception as e:
        hook_metrics.record(
            "trigger_upsell_suggestion", False, time.time() - start, error=str(e)
        )
    # Log Receipt
    start = time.time()
    try:
        ...
        hook_metrics.record(
            "log_receipt_to_db", False, time.time() - start, error=str(e)
        )
    # Update Smart Price
    start = time.time()
    try:
        ...
        hook_metrics.record(
            "update_smart_price", False, time.time() - start, error=str(e)
        )
    # Request Review
    start = time.time()
    try:
        results["request_review"] = request_review.schedule_email(
            sale_data["buyer_email"], delay_hours=24
        )
    ...
    start = time.time()
    try:
        analytics_result = track_analytics.record_sale(
            sale_data["vault_id"], sale_data.get("buyer_metadata", {})
        )
        anomaly = (
            analytics_result.get("anomaly", False)
            if isinstance(analytics_result, dict)
            else False
        )
        results["track_analytics"] = analytics_result
        hook_metrics.record(
            "track_analytics", True, time.time() - start, anomaly=anomaly
        )
    except Exception as e:
    ...
    start = time.time()
    try:
        cross_result = cross_sell_recommender.recommend_next(
            sale_data["buyer_email"], current_vault=sale_data["vault_id"]
        )
        results["cross_sell_recommender"] = cross_result
        hook_metrics.record("cross_sell_recommender", True, time.time() - start)
    except Exception as e:
        hook_metrics.record(
            "cross_sell_recommender", False, time.time() - start, error=str(e)
        )
    # Send Preview Bundle
    start = time.time()
    try:
        results["send_preview_bundle"] = send_preview_bundle(
            sale_data["buyer_email"], sale_data.get("vault_preview_data", {})
        )
        hook_metrics.record("send_preview_bundle", True, time.time() - start)
    except Exception as e:
        hook_metrics.record(
            "send_preview_bundle", False, time.time() - start, error=str(e)
        )
    ...
    try:
        ...
        compliance = (
            compliance_result.get("compliance", False)
            if isinstance(compliance_result, dict)
            else False
        )
        results["file_tax_compliance"] = compliance_result
        hook_metrics.record(
            "file_tax_compliance", True, time.time() - start, compliance=compliance
        )
    except Exception as e:
        hook_metrics.record(
            "file_tax_compliance", False, time.time() - start, error=str(e)
        )
    # Tag Buyer CRM
    start = time.time()
    try:
    ...
    start = time.time()
    try:
        results["affiliate_attribution"] = affiliate_attribution(
            sale_data["buyer_email"], sale_data.get("referral_data", {})
        )
        hook_metrics.record("affiliate_attribution", True, time.time() - start)
    except Exception as e:
        hook_metrics.record(
            "affiliate_attribution", False, time.time() - start, error=str(e)
        )
    # Fraud Check (fraud)
    start = time.time()
    try:
        fraud_result = fraud_check(
            sale_data.get("buyer_data", {}), sale_data.get("sale_metadata", {})
        )
        fraud = (
            fraud_result.get("fraud_flag", False)
    ...
    post_sale_event_to_webhooks(
        "post_sale_hooks_complete", {"sale_data": sale_data, "hook_results": results}
    )
    return results
    try:
        send_preview_bundle(
