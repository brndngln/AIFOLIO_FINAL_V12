...
try:
    ...
    class MockConfig:
        ...
        SIM_LAUNCH_CAL_EVENT_DELAY_CHANCE = (
            0.15  # Chance an event is marked as 'Delayed_sim'
        )
...
SIMULATED_EVENT_TYPES = [
    ...
    "Flash Sale_sim",
    "Webinar_sim",
    "Content Update Release_sim",
]
SIMULATED_EVENT_STATUSES = [
    "1_Planned",
    "2_Preparation",
    "3_Ready",
    "4_In Progress_sim",
...
SIMULATED_VAULT_NAME_PREFIXES_LC = [
    "Ultimate",
    "Complete",
    "Secret",
    "Advanced",
...
SIMULATED_VAULT_NAME_SUFFIXES_LC = ["Guide", "Kit", "Playbook", "System", "Vault Drop"]
SIMULATED_NICHE_CATEGORIES_LC = [
    "AI",
    "Health",
    "Marketing",
    "Productivity",
...
class LaunchCalendarDataSimulator:
    """Simulates data for a Notion Launch Calendar with anti-sentience safeguards."""
    ...
    def get_simulated_launch_calendar_events(
        ...
        if num_events is None:
            num_to_simulate = random.randint(
                config.SIM_LAUNCH_CAL_NUM_EVENTS_MIN,
        ...
        for i in range(num_to_simulate):
            ...
            scheduled_datetime = current_time + timedelta(
            ...
            if status == "6_Delayed_sim" or (
                status != "1_Planned"
                and random.random() < config.SIM_LAUNCH_CAL_EVENT_DELAY_CHANCE
            ):
            ...
            if status in ["4_In Progress_sim", "5_Completed"]:
                actual_start_datetime = scheduled_datetime - timedelta(
                    hours=random.uniform(0, 1)
                )
                if status == "5_Completed":
                    actual_end_datetime = actual_start_datetime + timedelta(
                        days=random.uniform(0.5, 5)
                    )  # Event duration
                    # Ensure end is after start
                    if actual_end_datetime < actual_start_datetime:
                        actual_end_datetime = actual_start_datetime + timedelta(hours=1)
            ...
            entry = {
                ...
                "actual_start_datetime_utc_sim": actual_start_datetime.isoformat()
                if actual_start_datetime
                else None,
                "actual_end_datetime_utc_sim": actual_end_datetime.isoformat()
                if actual_end_datetime
                else None,
                "associated_product_id_sim": f"prod_sim_{uuid.uuid4().hex[:8]}"
                if "Vault" in event_type or "Bundle" in event_type
                else None,
                "responsible_team_sim": random.choice(
                    [
                        "Marketing_sim",
                        "Content_sim",
                        "Sales_sim",
                        "Automated_sim (placeholder)",
                    ]
                ),
                "notes_sim": notes,
                "created_at_sim": (
                    current_time - timedelta(days=random.randint(10, 90))
                ).isoformat(),
            }
...
if __name__ == "__main__":
    ...
    print(
        f"Generated {len(calendar_data['launch_calendar_events_sim'])} event entries."
    )

    print("\n📅 Simulated Launch Calendar Events (Specific Number: 5): 📅")
    calendar_data_specific = simulator.get_simulated_launch_calendar_events(
        num_events=5
    )
    print(json.dumps(calendar_data_specific, indent=2))
    print(
        f"Generated {len(calendar_data_specific['launch_calendar_events_sim'])} event entries."
    )
