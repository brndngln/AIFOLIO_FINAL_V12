...
export default function EliteAuditComplianceDashboard({ module = "core" }) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const data =
        exportFormat === "json" ? await res.json() : await res.text();
      setExported(
  ...
  return (
    <div
      className="elite-audit-dashboard"
      style={{
        background: "#181e2b",
        color: "#b3e9ff",
        borderRadius: 16,
    ...
    >
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <span style={{ marginLeft: 16 }}>
          <strong>Total Events:</strong> {compliance.total}
        </span>
      </div>
      <div style={{ marginBottom: 16 }}>
        <button
      ...
      <div style={{ marginTop: 32, opacity: 0.8 }}>
        <em>
          All audit logs are immutable, append-only, and SAFE AI-compliant.
          Owner control and full transparency guaranteed.
        </em>
      </div>
    </div>
  );
