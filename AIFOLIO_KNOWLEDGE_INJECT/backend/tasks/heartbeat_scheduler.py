...
COMPLIANCE_EXPORT_URL = os.getenv(
    "COMPLIANCE_EXPORT_URL", "http://localhost:8000/api/export/compliance"
)
...
def scheduler_loop():
    while True:
        now = datetime.utcnow()
        # Heartbeat
        if now.hour == HEARTBEAT_HOUR and (
            not os.path.exists(LAST_HEARTBEAT_FILE)
            or (
                datetime.utcnow()
                - datetime.fromisoformat(open(LAST_HEARTBEAT_FILE).read().strip())
            ).days
            >= 1
        ):
            send_heartbeat()
        # Compliance export
        if now.hour == EXPORT_HOUR and (
            not os.path.exists(LAST_EXPORT_FILE)
            or (
                datetime.utcnow()
                - datetime.fromisoformat(open(LAST_EXPORT_FILE).read().strip())
            ).days
            >= 1
        ):
            export_compliance()
