...
class FractalLicensingEngine:
        ...
        FractalLicensingEngine._vaults = [
            {
                **v,
                ...
                if v.get("value", 0) > 10000
                else "Medium",
            }
            for v in vaults
            if v.get("earnings", 0) > 0
        ]
    ...
    def generate_licensing_variant(vault_id, region=None, language=None, audience=None):
        ...
        base = next(
            (v for v in FractalLicensingEngine._vaults if v["id"] == vault_id), None
        )
        if not base:
            return None
        variant = {
        ...
        FractalLicensingEngine._licensing_variants.setdefault(vault_id, []).append(
            variant
        )
        return variant
    ...
    def detect_abuse(vault_id, user, fingerprint):
        ...
        FractalLicensingEngine._abuse_log.append(
            {
                "vault_id": vault_id,
                "user": user,
