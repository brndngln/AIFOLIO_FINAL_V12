...
class EventRouter:
    ...
    def get_recent_events(self, limit=100):
        ...
        try:
            if self.redis:
                ...
                cursor.execute(
                    "SELECT ts, event, payload FROM event_log ORDER BY ts DESC LIMIT ?",
                    (limit,),
                )
    ...
    def log_event(self, entry):
        ...
        try:
            ...
            cursor = self.db.cursor()
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS event_log (ts TEXT, event TEXT, payload TEXT)"
            )
            cursor.execute(
                "INSERT INTO event_log (ts, event, payload) VALUES (?, ?, ?)",
