...
function renderEventBreakdownPieChart(eventBreakdown) {
  ...
  container.innerHTML =
    '<h3>Event Breakdown (Pie)</h3><canvas id="pieChart" width="300" height="200"></canvas>';
...
function renderRiskOverTimeLineChart(riskOverTime) {
  ...
  container.innerHTML =
    '<h3>Risk Over Time (Line)</h3><canvas id="riskLineChart" width="400" height="120"></canvas>';
...
function renderHeatmapWidget() {
  ...
  container.innerHTML =
    '<h3>Audit Event Heatmap (Simulated)</h3><canvas id="heatmapCanvas" width="400" height="120"></canvas>';
...
function showEventDrilldown(eventType) {
  fetch("/api/audit-log")
    ...
    .then((entries) => {
      ...
      modal.innerHTML =
        `<h3>Drilldown: ${eventType}</h3>` +
        details
          .map(
            (d) =>
              `<div style='border-bottom:1px solid #eee;padding:4px 0;'><b>${d.time}</b> [${d.user}]<br>${d.details}</div>`,
          )
          .join("") +
...
function renderComplianceSummary() {
  ...
  if (!container) {
    ...
    container.innerHTML =
      '<h2>Compliance Summary</h2><div id="summaryContent"></div>';
    document
      .querySelector("main")
      .insertBefore(container, document.getElementById("alerts"));
  }
  fetch("/api/compliance-charts")
    .then((res) => res.json())
    .then((data) => {
      const totalEvents = Object.values(data.eventBreakdown).reduce(
        (a, b) => a + b,
        0,
      );
...
function renderComplianceCheckWidget(checks) {
  ...
  if (!checks) return;
  container.innerHTML =
    '<h3>Automated Compliance Checks</h3><canvas id="complianceBar" width="350" height="120"></canvas>';
  ...
  window.complianceBarObj = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Status",
          data: checks.map((c) => (c.status === "PASS" ? 1 : 0)),
          backgroundColor: checks.map((c) =>
            c.status === "PASS" ? "#0c0" : "#f33",
          ),
        },
      ],
    },
...
function renderUserActivityWidget(activity) {
  ...
  container.innerHTML =
    ...
    activity
      .map(
        (u) =>
          `<tr><td>${u.user}</td><td>${u.actions}</td><td>${u.last_active}</td><td>${u.ip || ""}</td><td>${u.device || ""}</td></tr>`,
      )
      .join("") +
    "</table>";
}
...
function renderAnomalyTimelineWidget(timeline) {
  ...
  if (!timeline) return;
  container.innerHTML =
    '<h3>Anomaly Timeline</h3><canvas id="anomalyLine" width="350" height="120"></canvas>';
  ...
  window.anomalyLineObj = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Anomalies",
          data: timeline.map((t) => t.anomalies),
          fill: false,
          borderColor: "#f33",
          tension: 0.2,
        },
...
function renderGeolocationAnalyticsWidget(geo) {
  ...
  if (!geo) return;
  container.innerHTML =
    '<h3>Geolocation Analytics</h3><canvas id="geoBar" width="350" height="120"></canvas>';
...
function renderDeviceRiskWidget(devices) {
  ...
  if (!devices) return;
  container.innerHTML =
    '<h3>Device Risk</h3><canvas id="deviceRiskBar" width="350" height="120"></canvas>';
  ...
  window.deviceRiskBarObj = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Risk",
          data: devices.map((d) => d.risk),
          backgroundColor: devices.map((d) => (d.risk > 0.5 ? "#f33" : "#0c0")),
        },
      ],
    },
    options: {
...
function renderCustomAnomalyDetectionWidget(anomalies) {
  ...
  if (!anomalies) return;
  container.innerHTML =
    "<h3>Custom Anomaly Detection</h3>" +
    anomalies
      .map(
        (a) =>
          `<div style='border-bottom:1px solid #eee;padding:4px 0;'><b>${a.type}</b> [${a.user}] <span style='color:${a.severity === "high" ? "#f33" : a.severity === "medium" ? "#fa0" : "#0c0"}'>${a.severity.toUpperCase()}</span> <span style='float:right'>${a.time}</span></div>`,
      )
      .join("");
}
...
function renderExternalComplianceFeedsWidget(feeds) {
  ...
  if (!feeds) return;
  container.innerHTML =
    "<h3>External Compliance Feeds</h3><ul>" +
    feeds
      .map(
        (f) =>
          `<li><a href="${f.url}" target="_blank" rel="noopener">${f.feed}</a></li>`,
      )
      .join("") +
    "</ul>";
}
...
function renderAdvancedReportingWidget(data) {
  ...
  container.innerHTML =
    ...
    `<b>Trends:</b> ` +
    data.trends.map((t) => `${t.date}: ${t.events} events`).join(" | ") +
    "<br>" +
    `<b>Outliers:</b><ul>` +
    data.outliers
      .map((o) => `<li>${o.type} [${o.user}] at ${o.time}</li>`)
      .join("") +
    "</ul>";
}
...
function renderRegulatoryAPIWidget(data) {
  ...
  if (!data) {
    container.innerHTML =
      "<h3>Regulatory API Status</h3><span>Loading...</span>";
    return;
  }
