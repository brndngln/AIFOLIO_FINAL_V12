...
#             *salt* and *digest* are both raw (unencoded) bytes.
        ...
        # that just picks out relevant digest, and doesnt bother
        # with full structure validation each time its called.
...
#             Returns a list of digest algorithms; e.g. ``["sha-1"]``
        # XXX: this could be sped up by writing custom parsing routine
        # that just picks out relevant names, and doesnt bother
        # with full structure validation each time its called.
...
#             raise ValueError("sha-1 must be in algorithm list of scram hash")
    ...
    def _calc_needs_update(self, **kwds):
        # marks hashes as deprecated if they dont include at least all default_algs.
        # XXX: should we deprecate if they arent exactly the same,
...
#                 other = self._calc_checksum(secret, alg)
                ...
                # but dont need to be that strict, and want to be able
                # to parse hashes containing algs not supported by platform.
                # its fine if we fail here though.
...
##            if digest and data != digest:
    pass
##                raise ValueError("password doesnt match digest")
##            else:
##                digest = data
##        elif not digest:
    pass
