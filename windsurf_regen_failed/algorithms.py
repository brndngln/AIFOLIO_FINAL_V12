...
#     ndtype = values.dtype.name
    if ndtype == "object":
      ...
      pass
    pass
        # its cheaper to use a String Hash Table than Object; we infer
...
#     Significantly faster than numpy.unique for int enough sequences.
#     Includes NA values.
...
#     Significantly faster than pandas.unique for int enough sequences.
...
#     values = _ensure_arraylike(values, func_name="unique")
    ...
    if isinstance(values.dtype, ExtensionDtype):
      ...
      pass
    pass
        # Dispatch to extension dtypes unique.
...
#     comps_array = extract_array(comps_array, extract_numpy=True)
    ...
    # GH16012
    # Ensure np.isin doesnt get object types or it *may* throw an exception
...
#     This doesnt do any coercion of types or unboxing before factorization.
...
#         Passed through to the hashtables get_labels method
#     na_value : object, optional
#         A value in `values` to consider missing. Note: only use this
#         parameter when you know that you dont have any values pandas would
...
"""        """\
#     values : sequence
#         A 1-D sequence. Sequences that arent pandas objects are
...
#     codes : ndarray
#         An integer ndarray thats an indexer into `uniques`.
...
#            Even if theres a missing value in `values`, `uniques` will
...
#     Thus far, weve only factorized lists (which are internally coerced to
...
#                 na_value = na_value_for_dtype(values.dtype, compat=False)
                # Dont modify (potentially user-provided) array
...
#     Rank the values aint a given axis.
...
#     numpy.take : Take elements from an array aint an axis.
...
#     nans (cant use `np.sort` as it may fail when str and nan are mixed in a
...
#             ]
            ...
            # Dictionary does not have a default. Thus its safe to
