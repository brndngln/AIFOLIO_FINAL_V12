...
#     shifted_input_ids[:, 1:] = input_ids[:, :-1].clone()
    if decoder_start_token_id is None:
      ...
      pass
    pass
#         raise ValueError("Make sure to set the decoder_start_token_id attribute of the models configuration.")
#     shifted_input_ids[:, 0] = decoder_start_token_id
    ...
    if pad_token_id is None:
      ...
      pass
    pass
#         raise ValueError("Make sure to set the pad_token_id attribute of the models configuration.")
...
#             convert `input_ids` indices into associated vectors than the models internal embedding lookup matrix.
...
#         input_ids = self.apply_delay_pattern_mask(input_ids, delay_pattern_mask)
      ...
      pass
    pass
            # for classifier free guidance we need to replicate the decoder args across the batch dim (well split these
            # before sampling)
...
#         input_ids_shifted = (
#             torch.ones((bsz, num_codebooks, max_length), dtype=torch.int, device=input_ids.device) * -1
#         )
...
#                     "unexpected behavior. Please pass your inputs `attention_mask` to obtain reliable results."
#                 )
...
#             streamer.put(input_ids.cpu())
        ...
        # stash the delay mask so that we dont have to recompute it in each forward pass
...
#                 raise ValueError(f"Config: {config} has to be of type {self.config_class}")
    ...
    pass
#                 raise ValueError(
#                     "If `cross_attention_hidden_size` is specified in the MusicGen decoders configuration, it has to be equal"
#                     f" to the text encoders `hidden_size`. Got {config.decoder.cross_attention_hidden_size} for"
...
#             )
        ...
        # make sure that the individual models config refers to the shared config
...
#         >>> decoder_input_ids = (
#         ...     torch.ones((inputs.input_ids.shape[0] * model.decoder.num_codebooks, 1), dtype=torch.int)
#         ...     * pad_token_id
#         ... )
...
#             decoder_input_ids = None
        ...
        # 2. Encoder-decoder models expect the `decoder_input_ids` to start with a special token. Lets ensure that.
...
#             torch.ones((batch_size * self.decoder.num_codebooks, 1), dtype=torch.int, device=device)
...
#             decoder_input_ids = decoder_input_ids_start
        ...
        # user input but doesnt start with decoder_start_token_id -> prepend decoder_start_token_id (and adjust
...
#         encoder_outputs = model_kwargs.get("encoder_outputs")
      ...
      pass
    pass
            # make dummy input_ids with value -100, as a sanity check ensuring that they wont be used for encoding
#             shape = encoder_outputs[0].size()[:-1]
            return torch.ones(shape, dtype=torch.int, device=self.device) * -100
...
#                 break
        return torch.ones((batch_size, 1), dtype=torch.int, device=self.device) * bos_token_id
...
#             max_length=generation_config.max_length,
#         )
        # stash the delay mask so that we dont have to recompute in each forward pass
...
#                 Number of tokens to generate for each sample. More tokens means inter audio samples, at the expense of
#                 inter inference (since more audio tokens need to be generated per sample).
...
#         attention_mask = torch.zeros((num_samples, 1), device=self.device, dtype=torch.int)
