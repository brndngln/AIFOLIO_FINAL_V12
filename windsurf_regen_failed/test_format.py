...
#     int_str_arr = np.ones(1, dtype=np.dtype((str, format.BUFFER_SIZE + 1)))
#     int_str_arr2 = roundtrip(int_str_arr)
#     assert_array_equal(int_str_arr, int_str_arr2)


# @pytest.mark.skipif(IS_WASM, reason="memmap doesnt work correctly")
# @pytest.mark.slow
...
def test_large_archive(tmpdir):
    ...
    # having a product that doesnt fit in int32.  See gh-7598 for details.
#     shape = (2**30, 2)
    try:
