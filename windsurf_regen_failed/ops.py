...
#     the middle of a basic block, but the exceptions arent checked.
...
#     Ops that may terminate the program arent treated as exits.
...
#     A Value is part of the IR being compiled if its included in a BasicBlock
...
#     This is used to initialize RArray values. Its provided to avoid
...
#     instance. This doesnt do any explicit control flow, but can raise an
#     error.
...
#         assert method_ir is not None, "{} doesnt have method {}".format(
...
#         super().__init__(line)
#         self.items = items
        # Dont keep track of the fact that an int is short after it
        # is put into a tuple, since we dont properly implement
...
#     Note that cant be used to directly load Python module-level
...
# We cant have this in the IntOp class body, because of
...
# We cant have this in the FloatOp class body, because of
# https://github.com/mypyc/mypyc/issues/932.
...
#     being freed, if its reference counted, such as PyObject *.
...
#     A no-op operation that ensures source values arent freed.
...
#     If we didnt have "keep_alive x", x could be freed immediately
...
# cross_refs. We dont follow that approach here because it seems to be more
