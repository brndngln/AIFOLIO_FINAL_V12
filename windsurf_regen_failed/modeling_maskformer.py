...
#     MaskFormers pixel level module output. It returns both the last and (optionally) the hidden states from the
...
#     MaskFormers pixel decoder module output, practically a Feature Pyramid Network. It returns the last hidden state
...
#         attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
#             Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
#             sequence_length)`. Attentions weights from Detrs decoder after the attention softmax, used to compute the
...
#     For efficiency reasons, the labels dont include the no_object. Because of this, in general, there are more
...
#             pred_probs = pred_probs.softmax(-1)
            # Compute the classification cost. Contrary to the loss, we dont use the NLL,
            # but approximate it in 1 - proba[target class].
            # The 1 is a constant that doesnt change the matching, it can be ommitted.
...
#                 inner layers of the Detrs Decoder.
#         Returns:
#             `Dict[str, Tensor]`: A dict of `torch.Tensor` containing two keys:
#             - **loss_cross_entropy** -- The loss computed using cross entropy on the predicted and ground truth labels.
#             - **loss_mask** -- The loss computed using sigmoid focal loss on the predicted and ground truth masks.
...
#         ]
        for i, layer in enumerate(self.layers):
            ...
            # We cant give instance attributes integer names i.e. self.0 is not permitted and so need to register
            # explicitly
...
#         Segmentation](https://arxiv.org/abs/2107.06278). It first runs the backbones features into a Feature Pyramid
...
#     The MaskFormers transformer module.
...
#             more detail.
#         output_attentions (`bool`, *optional*):
#             Whether or not to return the attentions tensors of Detrs decoder attention layers.
        return_dict (`bool`, *optional*):
...
#         pixel_embeddings = outputs.pixel_decoder_last_hidden_state
        # get the auxiliary predictions (one for each decoders layer)
