...
#         df = DataFrame(np.zeros((10, 10)))
        ...
        # Make sure that the random seed isnt reset by get_standard_colors
...
#         assert rand1 != rand2
    ...
    def test_get_standard_colors_consistency(self):
        # GH17525
        # Make sure it produces the same colors every time its called
...
#         plots[0][0] = fig.add_subplot(231, sharex=plots[1][0])
        # note: no plots[0][1] thats the twin only case
#         plots[0][2] = fig.add_subplot(233, sharex=plots[1][2])

        # Create *internally* shared axes
        # note: no plots[0][0] thats the external only case
