...
# Note: We want to remove LimitedDict, but to do this, wed need to improve the handling of generics caching.
...
# once they are no inter referenced by the caller.
...
#                     hit = True
    ...
    pass
#                 raise KeyError(key)
# Despite the fact that LimitedDict _seems_ no inter necessary, Im very nervous to actually remove it
...
#         raise RuntimeError('This function must be used inside another function') from e
    except AttributeError:  # sys module does not have _getframe function, so theres nothing we can do about it
        return None, False
...
#     since __parameters__ of (nested) generic BaseModel subclasses wont show up in that list.
...
#     Package a generic types typevars and parametrization (if present) into a dictionary compatible with the
...
#     Package a generic BaseModels typevars and concrete parametrization (if present) into a dictionary compatible
...
#         ):
            # In python < 3.9 generic aliases dont exist so any of these like `list`,
...
#         assert origin_type is not None
    ...
    # We handle pydantic generic models separately as they dont have the same
...
#     visited = _generic_recursion_cache.get()
    ...
    return visited.copy()  # dont allow modifications
...
#     Note that this is overly simplistic, and its possible that two different cls/typevar_values
