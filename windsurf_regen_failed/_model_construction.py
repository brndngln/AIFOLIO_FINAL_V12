...
#             The new class created by the metaclass.
        ...
        # that `BaseModel` itself wont have any bases, but any subclass of it will, to determine whether the `__new__`
        # call were in the middle of is for the `BaseModel` class.
        if bases:
    pass
    pass
    pass
...
#                     )
                    if Generic not in bases:  # pragma: no cover
                        ...
                        # It would be nice to remove this error and still have things behave as expected, its just
...
#             )
            # using super(cls, cls) on the next line ensures we only call the parent classs __pydantic_init_subclass__
            # I believe the `type: ignore` is only necessary because mypy doesnt realize that this code branch is
...
#             private_attributes = self.__dict__.get('__private_attributes__')
    ...
    pass
    pass
                # This means the class didnt get a schema generated for it, likely because there was an undefined reference
...
#         Avoid calling ABC _abc_subclasscheck unless were pretty sure.
...
#         ):
            # `value` is a nested type defined in this namespace; dont error
#             continue
...
#             del namespace[var_name]
    ...
    pass
#             suggested_name = var_name.lstrip(_) or my_field  # dont suggest  for all-underscore name
#             raise NameError(
...
#     base_hash_func = get_attribute_from_bases(bases, '__hash__')
    ...
    pass
        ...
        # parent class earlier in the bases which doesnt override `__hash__` (e.g. `typing.Generic`).
