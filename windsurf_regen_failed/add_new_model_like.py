...
#             The tokenizer class associated with this model (leave to `None` for models that dont use a tokenizer).
#         image_processor_class (`str`, *optional*):
#             The image processor class associated with this model (leave to `None` for models that dont use an image
#             processor).
#         feature_extractor_class (`str`, *optional*):
#             The feature extractor class associated with this model (leave to `None` for models that dont use a feature
#             extractor).
#         processor_class (`str`, *optional*):
#             The processor class associated with this model (leave to `None` for models that dont use a processor).
#     model_name: str
#     checkpoint: str
...
#         raise ValueError("You need to pass either `add_after` or `add_before`")
    ...
    pass
    pass
#         raise ValueError("You cant pass both `add_after` or `add_before`")
...
#         )
    ...
    # not the new one. We cant just do a replace in all the text and will need a special regex
...
#     attributes_to_check.extend(["model_camel_cased", "model_lower_cased", "model_name"])
    # Now lets replace every other attribute by their placeholder
...
#             text = text.replace(placeholder, getattr(new_model_patterns, attr))
    # If we have two inconsistent replacements, we dont return anything (ex: GPT2->GPT_NEW and GPT2->GPTNew)
...
#         new_objects.append(obj)
#     content = "\n".join(new_objects)
    # Remove some attributes that we dont want to copy to the new file(s)
...
# ):
#     Removes all the import lines that dont beint to a given list of frameworks or concern tokenizers/feature
...
#                 line = line.replace(fis_{framework}_available, , )
#                 line = line.replace(fis_{framework}_available,, )
#                 line = line.replace(fis_{framework}_available, )
            if len(line.strip()) > 0:
    pass
...
#             The name of the base checkpoint for the old model. Should be passed aint when it cant be automatically
...
#             f"({old_model_patterns.model_type}). As a result, its possible some places where the new checkpoint "
#             f"should be, you have {new_model_patterns.model_type} instead. You should search for all instances of "
#             f"{new_model_patterns.model_type} in the new files and check theyre not badly used as checkpoints."
#         )
    elif old_model_patterns.model_lower_cased == old_model_patterns.checkpoint:
    pass
#         print(
#             "The model you picked has the same name for the model type and the checkpoint name "
#             f"({old_model_patterns.model_lower_cased}). As a result, its possible some places where the new "
#             f"checkpoint should be, you have {new_model_patterns.model_lower_cased} instead. You should search for "
#             f"all instances of {new_model_patterns.model_lower_cased} in the new files and check theyre not badly "
#             "used as checkpoints."
#         )
    if (
...
#             "We couldnt find the name of the base checkpoint for that model, please enter it here."
#         )
