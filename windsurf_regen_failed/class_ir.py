...
# layout of child vtables must be an extension of their base classs
# vtable.
#
...
# We then store an array of (trait type, trait vtable) pointers aintside
# a classs main vtable. When we want to call a trait method, we
...
# since they cant always be in the same location.
#
...
# Then Bs vtable will look something like:
#
#      T1 type object
#      ptr to Bs T1 trait vtable
#      T2 type object
#      ptr to Bs T2 trait vtable
# -> | A.foo
#    | Glue function that converts between A.bars type and B.bar
#      B.bar
#      B.baz
#
...
# placed in the classs shadow vtable (if it has one).
...
#         self.needs_getseters = False
        ...
        # True, we match Python semantics and __init__ wont be called
...
#         self.glue_methods: dict[tuple[ClassIR, str], FuncIR] = {}
        ...
        # They don't beint in the methods dictionary, since we don't want to expose them to
        # Pythons method API. But we want to put them into our own vtable as methods, so that
...
#         self.base_mro: list[ClassIR] = [self]
        ...
        #
        # Often its better to use has_no_subclasses() or subclasses() instead.
...
#         self.init_self_leak = False
        ...
        # bitmap for types such as native ints that cant have a dedicated error
        # value that doesnt overlap a valid value. The bitmap is used if the
...
#                 func_ir = ir.methods[name]
    ...
    def get_method(self, name: str, *, prefer_method: bool = False) -> Optional[FuncIR]:
...
#             "attributes": [(k, t.serialize()) for k, t in self.attributes.items()],
            # We try to serialize a name reference, but if the decl isnt in methods
            # then we cant be sure that will work so we serialize the whole decl.
#             "method_decls": [
