...
# Support code for building Python extensions on Windows.
    ...
    # 2. Force windows to use gcc (were struggling with MSVC and g77 support)
...
#                                                             dry_run, force)
        ...
        # 1. Check for import library on Windows.  Build if it doesnt exist.

#         build_import_library()

        # Check for custom msvc runtime library on Windows. Build if it doesnt exist.
...
#                 linker_so='g++ -shared')
        ...
        # we cant pass it through set_executables because pre 2.2 would fail
#         self.compiler_cxx = ['g++']
...
def find_python_dll():
    ...
    # - in system32,
    # - ortherwise (Sxs), I dont know how to get it.
#     stems = [sys.prefix]
...
#     The .def file will be overwritte.
#     dump = dump_table(dll)
...
def build_msvcr_library(debug=False):
    ...
    # If the version number is None, then we couldnt find the MSVC runtime at
...
#     lib_file = os.path.join(sys.prefix, 'libs', lib_name)
    if not os.path.isfile(lib_file):
      ...
      pass
    pass
        # didnt find library file in virtualenv, try base distribution, too,
...
if sys.platform == 'win32':
    ...
    try:
        ...
        # one, and we cant retrieve it from python
...
#                          (maj, min)) from None
    # Dont be fooled, it looks like an XML, but it is not. In particular, it
