...
#     when makeNames=True, for instances that dont specify an instance name
#     in the designspace. This part of the name generation can be overriden
#     because its not specified by the STAT table.
...
#     interpolableUserRegion: Region = {}
    for axis in doc.axes:
      ...
      pass
    pass
            # Mypy doesnt support narrowing union types via hasattr()
...
#     subDoc = DesignSpaceDocument()
    # Dont include STAT info
    # FIXME: (Jany) lets think about it. Not include = OK because the point of
    # the splitting is to build VFs and well use the STAT data of the full
...
# ) -> List[RuleDescriptor]:
    ...
    #    - axis is point (C-AP),
    #       - and point in conditions range (C-AP-in)
    #            (in this case remove the condition because its always true)
    ...
    #         scrap the condition because its always true
