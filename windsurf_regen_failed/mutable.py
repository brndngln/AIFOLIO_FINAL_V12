...
# ``collections.MutableMapping``; the part thats important to this example is
...
# to define a ``__getstate__`` that doesnt include the ``_parents`` dictionary.
...
# pickling process of the parents object-relational state so that the
...
#         Dictionary of parent objects :class:`.InstanceState`->attribute
#         name on the parent.
...
#             Wrap the target data members value with
#             ``Mutable``.
...
#         sqltype = types.to_instance(sqltype)
        ...
        # and well lose our ability to link that type back to the original.
...
#             _APPLIED_KEY = "_ext_mutable_listener_applied"
            ...
            for prop in mapper.column_attrs:
                if (
                    # all Mutable types refer to a Column thats mapped,
