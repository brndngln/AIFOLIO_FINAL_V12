...
#         any missing values in `data`, then well end up with different
#         rows since we consider `{}` NA, but `.astype(object)` doesnt.
#         super().test_stack()
...
#     @pytest.mark.xfail(reason="Cant compare dicts.")
...
#         well be able to dispatch unique.
...
def custom_assert_series_equal(left, right, *args, **kwargs):
    # NumPy doesnt handle an array of equal-length UserDicts.
