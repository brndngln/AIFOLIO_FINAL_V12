...
#         assert_equal(0, np.add.identity)
    ...
    def test_numpy_float_python_int_addition(self):
        # Check that numpy float and python ints can be added correctly.
...
#             z = np.ones((1, y.shape[0]))
            # This shouldnt cause a segmentation fault:
#             np.dot(z, y)
...
#         assert_(type(dat.nonzero()[1]) is np.ndarray)
    ...
    def test_recarray_tolist(self):
        ...
        # Comparisons fail for NaN, so we cant use random memory
        # for the test.
...
#         x = Subclass(5)
#         x[(0,)] = 2  # shouldnt raise an exception
#         assert_equal(x[0], 2)
...
#         f.close()
    ...
    def test_complex_scalar_warning(self):
        for tp in [np.csingle, np.cdouble, np.cintdouble]:
#             x = tp(1+2j)
...
#         np.array([['abc', 2], ['int   ', '0123456789']], dtype=np.string_)
#         assert_equal(np.dtype('S10').itemsize, 10)
...
#         assert_(b.flags.f_contiguous)
    ...
    def test_squeeze_axis_handling(self):
        ...
        # that dont support axis specification
        # when squeezing
...
#     @pytest.mark.slow_pypy
    def test_memoryleak(self):
        # Ticket #1917 - ensure that array data doesnt leak
        for i in range(1000):
...
#                 assert_equal(tostr(b[1]), tostr(val))
    ...
    def test_string_truncation_ucs2(self):
        ...
        # adjusted for NumPys four byte unicode.
#         a = np.array(['abcd'])
