...
#             all_hidden_states_text += (text_embeds,)
    ...
    pass
    pass
#             attention_mask = torch.ones(input_shape, dtype=torch.int, device=input_ids.device)
...
#         image_embeds_with_ln = self.vision_model.visual.forward_post(image_embeds.type(self.vision_model.dtype))
        # first layer is a special case because we dont have the output from the cross-encoder yet
#         cross_modal_text = self.cross_modal_text_transform(text_embeds)
#         text_token_type_embeddings = self.token_type_embeddings(
#             torch.zeros(1, dtype=torch.int, device=input_ids.device)
...
#             torch.full((1,), image_token_type_idx, dtype=torch.int, device=input_ids.device)
...
#             torch.full((1,), 1, dtype=torch.int, device=self.bridgetower.token_type_embeddings.weight.device)
#         ).expand_as(image_embeds_with_ln)
