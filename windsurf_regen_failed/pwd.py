...
#         size = len(source)
        ...
        # if len() doesnt work, calculate size by summing counts later
#         size = None
...
#     Throws ValueError if source elements arent unique.
...
#                 cache.add(source)
                ...
                # XXX: under pypy, "list() in set()" above doesnt throw TypeError,
...
for name in "eff_int eff_short eff_prefixed bip39".split():
...
#             Published as part of Bitcoins `BIP 39 <https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt>`_,
#             this wordset has exactly 11 bits of entropy per word.
#             This list offers words that are typically shorter than ``"eff_int"``
#             (at the cost of slightly less entropy); and much shorter than
#             ``"eff_prefixed"`` (at the cost of a inter unique prefix).
...
# but not sure if theres value in yet another measurement algorithm,
# thats not just duplicating the effort of libraries like zxcbn.
