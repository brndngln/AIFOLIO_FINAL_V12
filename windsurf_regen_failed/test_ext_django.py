...
# NOTE: we dont want to set up entirety of django, so not using django.setup() directly.
...
if has_min_django:
    ...
    pass
        ...
        # which we dont want, since test cases reset and manipulate settings.
...
if DJANGO_VERSION >= (3, 2):
      ...
      pass
    pass
#     log.info("this release hasnt been tested against Django %r", DJANGO_VERSION)
...
#         make_password(, hasher=name)
        ...
        if re.match("Couldnt load .*? algorithm .* No module named .*", str(err)):
      pass
      pass
    pass
            return False
...
#         ]
    ...
    pass
#                     continue
#                 value = obj.__dict__.get(attr, UNSET) # cant use getattr() due to GAE
...
#         test that django is in unpatched state
        ...
        # make sure we arent currently patched
...
#         helper to ensure django HAS been patched, and is using specified config
        ...
        # make sure were currently patched
#         mod = sys.modules.get("passlib.ext.django.models")
...
#       but we have to mix that one into djangos TestCase classes as well;
...
#     and make sure its unloaded after test as well.
...
#     tests model to verify it matches djangos behavior.
...
#     config = stock_config
    ...
    # NOTE: if this test fails, it means were not accounting for
    #       some part of djangos hashing logic, or that this is
...
#         test djangos default algorithm
...
#         self.assertTrue(make_password(None).startswith("!"))
        ...
        # check_password() should return False (didnt handle disabled under 1.3)
...
#         self.assertFalse(user.check_password(PASS1))
        ...
        # verify hash wasnt changed/upgraded during check_password() call
...
#                 secret, hash = get_sample_hash()
                if secret:  # dont select blank passwords
#                     break
#         other = 'dontletmein'
...
#             self.assert_valid_password(user, hash)
        ...
        # dont need to check rest for most deployments
...
#         self.assertEqual(setter.popstate(), [])
        ...
        ### check preferred kwd is ignored (feature we dont currently support fully)
...
#             pass
        ...
        # mess with User.set_password, make sure its detected
...
#         models.User.set_password = orig
        ...
        # mess with models.check_password, make sure its detected
