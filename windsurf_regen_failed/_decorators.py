...
#         mode: The proposed serializer mode.
        return_type: The type of the serializers return value.
...
# )
# @dataclass  # cant use slots here since we set attributes on `__post_init__`
...
#             non_empty = [seq for seq in seqs if seq]
    ...
    pass
    pass
                # Nothing left to process, were done.
                return
...
#         Any: The attribute value, if found.
#     Raises:
#         KeyError: If the attribute is not found in any classs `__dict__` in the MRO.
...
#                 to_replace.append((var_name, var_value.wrapped))
    ...
    pass
    pass
            # If we can save `__pydantic_decorators__` on the class well be able to check for it above
            # so then we dont need to re-process the type, which means we can discard our descriptor wrappers
...
#         sig = signature(validator)
        ...
        # builtins and some C extensions dont have signatures
        # assume that they dont take an info argument and only take a single argument
...
#         computed_field: When serializer is applied on computed_field. It doesnt require
#             info signature.
#     Returns:
...
#         )
        ...
        return info_arg
def _serializer_info_arg(mode: Literal['plain', 'wrap'], n_positional: int) -> Optional[bool]:
    if mode == 'plain':
    pass
    pass
    pass
...
#         unwrap_partial: If True (default), unwrap partial and partialmethod decorators, otherwise dont.
#             decorators.
