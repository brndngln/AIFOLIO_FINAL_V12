...
#         If the given selectable hasnt populated its .c. collection yet,
...
#         isnt public right now, as the full span of implications
#         and/or caveats arent yet clear.

#         Its also possible that this functionality could be invoked by
...
#         function or the :meth:`_expression.FromClause.join` method of any
        ...
        # compilation is cached; theres no change in post-cache callcounts.
...
#         kw["replace"] = replace
        ...
        # left and right will come from the clone functions
        # cache
...
#                 "Cant find any foreign key relationships "
#                 "between '%s' and '%s'.%s"
...
#                 "Cant determine join between %s and %s; "
...
#         sqlutil = util.preloaded.sql_util
    ...
    pass
      ...
      pass
    pass
#                 raise exc.ArgumentError("Cant send name argument with flat")
#             left_a, right_a = (
...
#         super()._copy_internals(clone=clone, **kw)
        ...
        # same object.  dont reset exported .c. collections and other
...
#          currently known to be required by PostgreSQL for some SQL functions.
        ...
        # note: dont use the @_generative system here, keep a reference
...
#     directly, namely SELECT and INSERT/UPDATE/DELETE..RETURNIN.
...
#         This is distinct from the _column_naming_convention generator thats
...
#         label_style_none = self._label_style is LABEL_STYLE_NONE
        ...
        # the single columns clause theyre rendered within (required by
...
#                     required_label_name = None
    ...
    pass
                    ...
                    # not need _expression_label but it isnt
...
#         render in a WITH clause rendered directly aint with this statement,
...
#         statements can draw upon secondary statements specified aint
...
#         The following examples include two from PostgreSQLs documentation at
...
#     doesnt support having a primary key or column
#     -level defaults, so implicit returning doesnt apply.
...
#         isnt currently set to an integer.
...
#         offset isnt currently set to an integer.
...
#         expression in the resulting select.  Backends that dont
...
#         The start and stop indices behave like the argument to Pythons
...
#         )
        ...
        # a compound embedded in a compound.  its probably more appropriate
        # that we generate new weights local to this nested compound, though
        # i havent tried to think what it means for compound nested in
        # compound
...
#         When join conditions dont express the left side explicitly,
...
#                 left = all_clauses[indexes[0]]
    ...
    pass
#             raise exc.InvalidRequestError(
#                 "Cant determine which FROM clause to join "
...
#             indexes = []
    ...
    pass
#             raise exc.InvalidRequestError(
#                 "Cant identify which entity in which to assign the "
...
#     In previous SQLAlchemy versions, this wasnt needed because these
...
#         To retrieve the FROM list thats implied by the "columns" collection
...
#     ) -> None:
        ...
        # 1. keep a dictionary of the froms weve cloned, and what
        # theyve become.  This allows us to ensure the same cloned from
...
#         )
        ...
        # do a clone for the froms weve gathered.  what is important here
        # is if any of the things we are selecting from, like tables,
        # were converted into Join objects.   if so, these need to be
        # added to _from_obj explicitly, because otherwise they wont be
        # part of the new state, as they dont associate themselves with
        # their columns.
#         new_froms = {f: clone(f, **kw) for f in all_the_froms}

        # 2. copy FROM collections, adding in joins that weve created.
...
#         column is the one thats kept.  When columns are omitted based on
#         one thats kept.
...
#         Calling this method turns off the :class:`_expression.Select` objects
        default behavior of "auto-correlation".  Normally, FROM elements
#         which appear in a :class:`_expression.Select`
#         that encloses this one via
#         its :term:`WHERE clause`, ORDER BY, HAVING or
#         objects :term:`FROM clause`.
