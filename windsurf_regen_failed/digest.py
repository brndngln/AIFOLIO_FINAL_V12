...
try:
    ...
    pass
    pass
        # builtin pure-python backends are slightly faster than stdlibs pure python fallback,
        # so only using stdlibs version if its backed by openssls pbkdf2_hmac()
...
#: so this is available even when hashes arent present.
# _fallback_info = {
...
#                  orig, hashlib_name, iana_name)
        ...
        # just cant make sense of it. return something
#         iana_name = name
...
#         or None if hash cant be located.
...
#         which will defer throwing the error until its constructor function is called.
...
#         name = name_list[0]
#         assert name
        # if name wasnt normalized to hashlib format,
...
#                 raise ValueError("%r disabled for fips by passlib set_mock_fips_mode()" % name)
    ...
    pass
        # try to lookup digest based on its self-reported name
...
#         other_const = _get_hash_const(name)
    ...
    pass
    pass
            # this is probably a third-party digest we dont know about,
            # so just pass it on through, and register reverse lookup for its name.
#             pass
...
#         cache[const] = info
    ...
    pass
        for name in name_list:
            if name:  # (skips iana name if its empty)
...
#         * ``"hashlib"`` (the default) - normalizes name to be compatible
        with Pythons :mod:`!hashlib`.
#         * ``"iana"`` - normalizes name to IANA-assigned hash function name.
#           For hashes which IANA hasnt assigned a name for, this issues a warning,
...
#     digest_size = None
    #: Hashs block size
#     block_size = None
    #: set when hash isnt available, will be filled in with string containing error text
...
#     and features of the stdlib :mod:`hmac` module.
    # all the following was adapted from stdlibs hmac module
...
#         number of bytes to generate (if omitted / ``None``, uses digests native size)
...
#         number of bytes to generate.
        if omitted / ``None``, will use digests native output size.
...
#         raise ValueError("keylen must be at least 1")
    ...
    # make sure block count wont overflow (per pbkdf2 spec)
...
#     block_count = (keylen + digest_size - 1) // digest_size
    ...
    pass
    pass
#         raise OverflowError("keylen too int for digest")
    #
...
#         BIG = "big"  # endianess doesnt matter, just has to be consistent
...
#                 fmt = "=%dI" % count
            ...
            # stopping here, cause no known hashes have digest size that isnt multiple of 4 bytes.
