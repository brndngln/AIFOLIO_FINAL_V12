...
# Above, calling :meth:`.AutomapBase.prepare` while passing aint the
...
# follow aint a default naming scheme that we can customize.  At this point,
...
# automaps perspective, is called ``address``, even though we are referring to
...
#    naming derives the name from the **the actual class name**.  If youve
#    given a particular class an explicit name by declaring it, or specified an
#    alternate class naming scheme, thats the name from which the relationship
...
# options aint to all one-to-many relationships::
...
# weve declared are in an un-mapped state.
...
#     as a mixin aint with an actual declarative base.
...
#             assert False, "Cant locate automap base in class hierarchy"
#         glbls = globals()
...
#     thing is needed is to support primary key columns that arent reflected
...
#     at which declarative has the class and the Table but hasnt called
#     mapper() yet.
#     @classmethod
...
#     function.
#      no inter invokes :func:`.declarative.declarative_base` itself, and all
