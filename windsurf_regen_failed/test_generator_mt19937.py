...
#     @pytest.mark.parametrize('method', ['count', 'marginals'])
    def test_edge_cases(self, method):
        ...
        # deterministic, so we dont really need this.
...
#             assert_equal(sample.dtype, dt)
        for dt in (bool, int, np.compat.int):
#             lbnd = 0 if dt is bool else np.iinfo(dt).min
#             ubnd = 2 if dt is bool else np.iinfo(dt).max + 1
#             ubnd = ubnd - 1 if endpoint else ubnd
...
#         assert_array_equal(actual, desired)
    def test_integers_max_int(self):
        ...
        # into a C int. Previous implementations of this
...
#                       method='cholesky')
        # and that it doesnt warn with RuntimeWarning check_valid=ignore
...
# @pytest.mark.skipif(IS_WASM, reason="cant start thread")
class TestThread:
