...
#             context_position = torch.arange(qlen, dtype=torch.int)[:, None]
#             memory_position = torch.arange(klen, dtype=torch.int)[None, :]
...
#         n = -relative_position
#         num_buckets //= 2
#         ret += (n < 0).to(torch.int) * num_buckets
#         n = torch.abs(n)
...
#         self.out_proj = nn.Linear(config.hidden_size, config.num_labels)
    def forward(self, features, **kwargs):
#         x = features[:, 0, :]  # take <s> token (equiv. to BERTs [CLS] token)
#         x = self.dropout(x)
#         x = self.dense(x)
#         x = torch.tanh(x)
#         x = self.dropout(x)
...
#     are ignored. This is modified from fairseqs `utils.make_positions`. :param torch.Tensor x: :return torch.Tensor:
