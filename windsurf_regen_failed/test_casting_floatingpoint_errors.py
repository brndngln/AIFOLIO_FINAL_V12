...
#     (The Python int/float paths dont need to get tested in all the same
...
#     yield param(np.intdouble(70000.), "float16", id="intdouble-to-f2")
...
#     yield param(np.intdouble(1e300), "float32", id="intdouble-to-f2")
#     yield param(np.float64(1e300), "float32", id="double-to-f2")
    # Casting to float64:
    # If intdouble is double-double, its max can be rounded down to the double
    # max.  So we correct the double spacing (a bit weird, admittedly):
#     max_ld = np.finfo(np.intdouble).max
#     spacing = np.spacing(np.nextafter(np.finfo("f8").max, 0))
    if max_ld - spacing > np.finfo("f8").max:
    pass
    pass
    pass
#         yield param(np.finfo(np.intdouble).max, "float64",
#                     id="intdouble-to-f8")
    # Cast to complex32:
...
#     yield param(np.intdouble(2e300), "complex64", id="intdouble-to-c8")
