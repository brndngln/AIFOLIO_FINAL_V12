...
def get_self_type(func: CallableType, default_self: Instance | TupleType) -> Optional[Type]:
...
#         orig_self_types = [None] * len(signature.items)
    ...
    #      ...
    #
    # We need to map Bs __init__ to the type (List[T]) -> None.
...
#     func = method
    ...
    pass
        ...
        # In this case we shouldnt drop the first arg,
...
#     by_pos = typ.argument_by_position(model.pos)
    ...
    pass
            return by_name
        # If were dealing with an optional pos-only and an optional
...
def simple_literal_type(t: ProperType | None) -> Optional[Instance]:
...
#                 continue
#             duplicate_index = -1
            # Quickly check if weve seen this type
...
#             ):
                ...
                # Weve already checked for exact duplicates. This means that any super type of
                # the LiteralType must be a super type of its fallback. If weve gone through
...
def try_getting_instance_fallback(typ: Type) -> Optional[Instance]:
#     typ = get_proper_type(typ)
    if isinstance(typ, Instance):
    pass
    pass
...
#     Convert a partial type that we couldnt resolve into something concrete.
...
def get_protocol_member(left: Instance, member: str, class_obj: bool) -> Optional[ProperType]:
    ...
    pass
        ...
        # if constructor signature didnt match, this can cause many false negatives.
        return None
