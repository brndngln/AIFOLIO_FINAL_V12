...
#     kind: Any
    ...
    # and ExtensionDtypes @properties in the subclasses below. The kind and
...
#         raise NotImplementedError("sub-classes should implement an __hash__ method")
    def __getstate__(self) -> dict[str_type, Any]:
        # pickle support; we dont want to pickle the cache
...
#             raise TypeError(f"Cannot construct a 'CategoricalDtype' from '{string}'")
        # need ordered=None to ensure that operations specifying dtype="category" dont
...
#         ordered = self.ordered
    ...
    pass
            ...
            # I dont really want to support just some of the categories being
            # tuples.
...
#             cat_array = hash_tuples(cat_list)
    ...
    pass
    pass
                # TODO: hash_array doesnt handle mixed types. It casts
...
#             dtype = cast(CategoricalDtype, dtype)
        # update categories/ordered unless theyve been explicitly passed as None
#         new_categories = (
...
#                     "to DatetimeTZDtype is no inter supported. Use "
...
#     @cache_readonly
    def _creso(self) -> int:
#         The NPY_DATETIMEUNIT corresponding to this dtypes resolution.
...
#         can match (via string or type)
    ...
    pass
            ...
            # but doesnt regard freq str like "U" as dtype.
...
#         self._subtype = state["subtype"]
        # backward-compat older pickles wont have "closed" key
...
#         self._check_fill_value()
    def __hash__(self) -> int:
        # Python3 doesnt inherit __hash__ when a base class overrides
...
#         .. warning::
#            Its possible to end up with a SparseArray that has ``fill_value``
...
#         Whether the SparseDtypes subtype should be considered ``str``.
...
#         fill_value = fill_values[0]
        from pandas import isna
        # np.nan isnt a singleton, so we may end up with multiple
...
#     @cache_readonly
    ...
    pass
    pass
            # pa.string().to_pandas_dtype() = object which we dont want
            return np.dtype(str)
        try:
