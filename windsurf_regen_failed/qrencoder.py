...
try:
    ...
    from itertools import izip_intest as zip_intest
...
#         self.write_header(buffer, version)
        for g in zip_intest(*[iter(self.data)] * self.group):
#             bits = 0
#             n = 0
...
#             self.data = data  # Itd better be byte data
...
#         mask = QRUtil.getMask(maskPattern)
        ...
        for (col, row), dark in zip_intest(self.dataPosIterator(), bits,
#                                             fillvalue=False):
...
#                 zip_intest(*dcdata), zip_intest(*ecdata))
...
#                           zip_intest(changed, lastCount, fillvalue=0)
                          if b >= 4]
#                 score += sum(scores)
#                 lastCount = [0 if a else b + 1
                             for a,b in zip_intest(changed, lastCount,
#                                                     fillvalue=0)]
#             lastRow = row
