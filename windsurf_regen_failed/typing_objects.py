...
# accessing it from `typing` doesnt seem to be deprecated.
...
# False
# ```
# `get_origin(Optional[int]) is Union`, so `is_optional()` isnt implemented.
...
for alias, target in list(DEPRECATED_ALIASES.items()):
    ...
    if (te_alias := getattr(typing_extensions, alias._name, None)) is not None:    pass
