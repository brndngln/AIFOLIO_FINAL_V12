...
#             builder.accept(stmt)
    ...
    # Dont complain about empty unreachable blocks, since mypy inserts
    # those after `if MYPY`.
    elif block.body:
    pass
#         builder.add(
...
def transform_expression_stmt(builder: IRBuilder, stmt: ExpressionStmt) -> None:
    ...
    pass
        # Docstring. Ignore
        return
    # ExpressionStmts do not need to be coerced like other Expressions, so we shouldnt
...
#     first_lvalue = lvalues[0]
    if stmt.type and isinstance(stmt.rvalue, TempNode):
      ...
      pass
    pass
        # This is actually a variable annotation without initializer. Dont generate
...
#     res = builder.binary_op(target_value, rreg, op, stmt.line)
    ...
    # but when target doesnt support that we need to manually assign
...
def transform_import(builder: IRBuilder, node: Import) -> None:
    ...
    # table-driven and compact. Heres how it works:
    #
    ...
    # helper function that will perform all of the groups imports in one go.
...
#         as_names_literal = builder.add(LoadLiteral(tuple(as_names), object_rprimitive))
    ...
    # since that case *shouldnt* load everything into the globals dict.
    # This probably doesnt matter much and the code runs basically right.
        import_from_many_op,
...
# ) -> None:
    ...
    # I dont actually understand why a bunch of it is the way it is.
...
#     builder.add(Branch(_y_init, stop_block, main_block, Branch.IS_ERROR))
    ...
    # If it wasnt, this reraises the exception.
...
#         builder.nonlocal_control[-1].gen_break(builder, line)
    ...
    def else_body() -> None:
        ...
        # but it wont automatically propagate.
...
#         builder.add(Branch(_y, stop, ok, Branch.IS_ERROR))
        ...
        # Everythings fine. Yield it.
#         builder.activate_block(ok)
#         builder.assign(to_yield_reg, _y, line)
#         builder.nonlocal_control[-1].gen_continue(builder, line)

        # Try extracting a return value from a StopIteration and return it.
        # If it wasnt, this rereaises the exception.
#         builder.activate_block(stop)
#         builder.assign(result, builder.call_c(check_stop_op, [], line), line)
#         builder.nonlocal_control[-1].gen_break(builder, line)
