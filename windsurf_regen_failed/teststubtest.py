...
#             output.getvalue()
            # remove cwd as its not available from outside
...
#     If youre reading this and bored, feel free to refactor this and make it more like
#     other mypy tests.
...
#         yield Case(stub="def bar() -> int: ...", runtime="async def bar(): return 5", error="bar")
        pass
        # Dont error for this one -- we get false positives otherwise
...
#         yield Case(stub="A = Tuple[int, str]", runtime="A = (int, str)", error="A")
        # Error if an alias isnt present at runtime...
#         yield Case(stub=B = str, runtime=, error=B)
        # ... but only if the alias isnt private
...
#             error=None,
#         )
        # Stub methods are allowed to have @final even if the runtime doesnt...
#         yield Case(
#             stub.
            class E:
#                 @final
...
#             error="K.foo",
#         )
        # This test wouldnt pass,
        # because the runtime cant set __final__ on instances of builtins.property,
...
"""            """,
            ...
            # but we dont generate special errors for `foo` missing `...` part.
#             error="X2.__new__",
#         )
#     @collect_cases
...
#         assert output == "Success: no issues found in 1 module\n"
    def test_allowlist(self) -> None:
        # Cant use this as a context because Windows
