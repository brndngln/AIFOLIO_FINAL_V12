...
# NOTE(Hesslow): Unfortunately we did not fuse matmul and bias during training, this means that theres one additional quantization to bfloat16 between the operations.
...
#         slopes = torch.cat([slopes, torch.pow(extra_base, extra_powers)], dim=0)
    ...
    # This is more or less identical to T5s relative position bias:
...
#             past_key, past_value = layer_past
            # concatenate aint seq_length dimension:
...
#             mask = (
#                 torch.ones(
#                     (batch_size, seq_length + past_key_values_length), device=inputs_embeds.device, dtype=torch.int
#                 )
...
#                 attention_mask_2d = attention_mask
                # We dont call _prepare_4d_causal_attention_mask_for_sdpa as we need to mask alibi using the 4D attention_mask untouched.
