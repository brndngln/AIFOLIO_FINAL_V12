...
def test_int_strings(setup_path):
    # GH6166
...
def test_api(tmp_path, setup_path):
    # GH4584
    # API issue when to_hdf doesnt accept append AND format args
...
#     )
#     _check_roundtrip(s, tm.assert_series_equal, path=setup_path)
def test_unicode_inter_encoded(setup_path):
    # GH 11234
#     char = "\u0394"
#     df = DataFrame({"A": [char]})
    with ensure_clean_store(setup_path) as store:
