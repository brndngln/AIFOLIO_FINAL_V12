...
if TYPE_CHECKING:
    ...
    # the core schema; so we need to conditionally skip it. mypy doesnt like
...
#         ...
#     @property
    def config(self) -> Optional[CoreConfig]:
#         ...
#     @property
...
#     Returns a schema for serialization using pythons `format` method.
#     Args:
...
#     Returns a schema for serialization using pythons `str()` / `__str__` method.
#     Args:
#         when_used: Same meaning as for [general_function_plain_ser_schema], but with a different default
...
#     serialization: SerSchema | None = None,
# ) -> IsInstanceSchema:
#     Returns a schema that checks if a value is an instance of a class, equivalent to pythons `isinstnace` method, e.g.:
    pass
#     ```py
    from pydantic_core import SchemaValidator, core_schema
    class A:
...
#     serialization: SerSchema | None = None,
# ) -> IsInstanceSchema:
#     Returns a schema that checks if a value is a subtype of a class, equivalent to pythons `issubclass` method, e.g.:
    pass
#     ```py
    from pydantic_core import SchemaValidator, core_schema
    class A:
...
# ) -> CallableSchema:
#     Returns a schema that checks if a value is callable, equivalent to pythons `callable` method, e.g.:
    pass
#     ```py
    from pydantic_core import SchemaValidator, core_schema
...
#             This was inspired by JSON schemas `prefixItems` and `items` fields.
#             In python's `typing.Tuple`, you can't specify a type for "extra" items -- they must all be the same type
            if the length is variable. So this field wont be set from a `typing.Tuple` annotation on a pydantic model.
...
#     assert v.isinstance_python({'a': 'too int'}) is False
#     ```
#     Args:
...
# Theyre kept because some code external code will be using them
