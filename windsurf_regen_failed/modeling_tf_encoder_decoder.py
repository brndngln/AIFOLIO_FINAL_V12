...
#             into associated vectors than the models internal embedding lookup matrix.
...
#                 raise ValueError(f"config: {config} has to be of type {self.config_class}")
    ...
    pass
#                 raise ValueError(
#                     "If `cross_attention_hidden_size` is specified in the decoders configuration, it has to be equal"
#                     f" to the encoders `hidden_size`. Got {config.decoder.cross_attention_hidden_size} for"
...
#             )
    ...
    def tf_to_pt_weight_rename(self, tf_weight):
        # Matt: The TF and PT weights dont align because our TF base classes have an extra layer compared to PT models
        ...
        # not the case, and I wasnt sure how else to go from the config to the correct MainLayer name!

        # This override is only needed in the case where were crossloading weights from PT. However, since weights are
...
#                 All remaning positional arguments will be passed to the underlying models `__init__` method.
...
#             encoder_inputs.update(kwargs_encoder)
            ...
            # The `labels` shouldnt be passed to `self.encoder` below, because it is a based model without this
