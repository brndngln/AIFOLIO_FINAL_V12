...
#                 "it doesnt have a language model head."
#             )
...
#         model_args = set(inspect.signature(self.prepare_inputs_for_generation).parameters)
        ...
        # `prepare_inputs_for_generation` doesnt accept them, then a stricter check can be made ;)
...
#             raise ValueError("`decoder_start_token_id` has to be defined for encoder-decoder generation.")
        ...
        # decoder-only models should use left-padding for generation (cant be checked with `trace=True`)
...
#                         " created by passing the corresponding arguments to generate or by the models config default"
...
#         This beam search function is heavily inspired by Flaxs official example:
...
#             not_max_length_yet = state.cur_len < max_length
            ...
            #   length_penalty. Positive length_penalty favors inter sequences, thus we use max_length there.
...
#             still_open_beam = ~(jnp.all(state.is_sent_finished) & (early_stopping is True))
        ...
        def beam_search_body_fn(state, input_ids_length=1):
            ...
            # Collect the current position slice aint length to feed the fast
...
#             topk_log_probs += add_penalty * np.array(-1.0e7)
            ...
            # Combine sequences, scores, and flags aint the beam dimension and compare
