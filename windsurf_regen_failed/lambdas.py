...
#      information about the lambdas Python code as well as the tracked closure
...
#     This lambda doesnt normally produce its real SQL expression outside of the
...
#         expr = self._setup_binds_for_tracked_expr(expr)
        ...
        # that is exactly what people will be doing, it doesnt work, so
        # Im not really sure how to handle this right now.
        # expected_binds = [
        #    b._orig_key
...
#             opts=kw,
#         )
        # TODO: A LOT A LOT of tests.   for _resolve_with_args, we dont know
...
#     @classmethod
    def get(cls, fn, lambda_element, lambda_kw, **kw):
        try:
            # TODO: validate kw havent changed?
...
#         self.bindparam_trackers = []
        ...
        # based on whats inside its closure variables.
...
#             "lambdas state.  Evaluate this variable outside of the "
...
#             )
            ...
            # create a parameter, otherwise, its some kind of Python
            # object in the closure and we want to track that, to make
            # sure it doesnt change to something else, or if it does,
...
#         After the user-defined lambda has been invoked to produce a statement
        for re-use, run it through coercion rules to both check that its the
...
#             raise exc.InvalidRequestError(
#                 "Cant invoke Python callable %s() inside of lambda "
