...
#             getattr(doc,arn)(*args)
    ...
    pass
    pass
#                 raise NotImplementedError("Cant handle ActionFlowable(%s)" % action)
            else:
#                 raise
        except:
...
#         the template size or if thats not available to the
#         doc size.
...
def _ktAllow(f):
# '    'return true if allowed in containers like KeepTogethe
...
#         this data should derive from flowables. Well have
...
#         shouldnt normally be called directl.
#         self.page += 1
...
#                     self._nextPageTemplateIndex = self.pageTemplates.index(t)
                    return
#             raise ValueError("cant find template(%s)"%pt)
...
#                     self._nextFrameIndex = self.pageTemplate.frames.index(f)
                    return
#             raise ValueError("cant find frame(%s) in %r(%s) which has frames %r"%(fx,self.pageTemplate,self.pageTemplate.id,[(f,f.id) for f in self.pageTemplate.frames]))
...
#         first = flowables[0]
        # if we insert a page break before, well process that, see it again,
...
#         del self._multiBuildEdits
    ...
    #its up to you to ensure that they maintain the needed consistency
    def afterInit(self):
#         pass
...
#     really accurate would be to do two passes, and I dont
