...
#       SQLAlchemys unit of work performs all INSERTs before DELETEs within a
...
# objects position in the :func:`.ordering_list` is synchronized with the
...
#     relationships ``collection_class`` option.  e.g.::
        from sqlalchemy.ext.orderinglist import ordering_list
        class Slide(Base):
#             __tablename__ = 'slide'
#             id = Column(Integer, primary_key=True)
...
#           Name of the attribute that stores the objects order in the
#           relationship.
...
#           ``reorder()`` if youre doing ``append()`` operations with
...
#             self._order_entity(index, entity, True)
    # As of 0.5, _reorder is no inter semi-private
#     _reorder = reorder
    def _order_entity(self, index, entity, reorder=True):
#         have = self._get_order_value(entity)
        # Dont disturb existing ordering if reorder is False
