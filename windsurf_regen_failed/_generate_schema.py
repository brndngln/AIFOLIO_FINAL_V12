...
#                     " (use check_fields=False if youre inheriting from the model and intended this)",
...
# ) -> core_schema.CoreSchema:
    ...
    # note that this wont work for any Annotated types that get wrapped by a function validator
...
#         schema['items_schema'] = apply_validators(inner_schema, each_item_validators, field_name)
    ...
    pass
        ...
        # this is super arbitrary but its the V1 behavior
...
#         schema: The schema to add the encoder to.
    ...
    # Decimal.__class__.__mro__ (and probably other cases) doesnt include Decimal itself
...
#         pydantic_js_functions = metadata.setdefault('pydantic_js_functions', [])
        ...
        # but if it does itll fail by inserting the duplicate
...
#         decide whether to use a `__pydantic_core_schema__` attribute, or generate a fresh schema.
        ...
        # avoid calling `__get_pydantic_core_schema__` if weve already visited this object
...
#             raise PydanticUndefinedAnnotation.from_name_error(e) from e
        ...
        # if obj is still a ForwardRef, it means we cant evaluate it, raise PydanticUndefinedAnnotation
...
#         The idea is that well evolve this into adding more and more user facing methods over time
#         as they get requested and we figure out what the right API for them is.
    ...
    pass
            # NewType, cant use isinstance because it fails <3.7
...
#     collection of (v1-style) ValidatorDecoratorInfos and checking if any of them have `always=True`.
...
def _extract_get_pydantic_json_schema(tp: Any, schema: CoreSchema) -> Optional[GetJsonSchemaFunction]:
...
#         ref = get_type_ref(tp)
        # return the reference if were either (1) in a cycle or (2) it was already defined
...
def resolve_original_schema(schema: CoreSchema, definitions: dict[str, CoreSchema]) -> Optional[CoreSchema]:
