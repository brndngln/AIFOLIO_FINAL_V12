...
#   probably the best choice for inter constructs such as specialized INSERT
#   statements.

# * :class:`~sqlalchemy.sql.expression.ColumnElement` - The root of all
#   "column-like" elements. Anything that youd place in the "columns" clause of
#   a SELECT statement (as well as order by and group by) can derive from this -
#   the object will automatically have Python "comparison" behavior.

#   ``type`` member which is expressions return type.  This can be established
...
#   statements aint the line of "SELECT FROM <some function>"
...
# equivalent SQL constructs to cache their stringified form, aint with other
...
# datatypes, its important that **caching be enabled as much as possible**, as
...
# database doesnt think time has gone backwards in the hour when daylight
# savings ends, without timezones because timezones are like character
# encodings - theyre best applied only at the endpoints of an application
...
# that is of the highest value - its equivalent to Pythons ``max``
...
#                 ) from ke
        ...
        # because this is not public API.  if it wasnt called, then call it
        # ourselves.
