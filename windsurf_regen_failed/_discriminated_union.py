...
#         PydanticUserError:
#             - If a model in union doesnt have a discriminator field.
...
#     to make it easier to maintain state while recursively walking the provided CoreSchema.
    ...
    def __init__(self, discriminator: str, definitions: dict[str, core_schema.CoreSchema]):
        ...
        # It must be the python name of the field, and *not* the fields alias. Note that as of now,
        # all members of a discriminated union _must_ use a field with the same name as the discriminator.
        # This may change if/when we expose a way to manually specify the TaggedUnionSchemas choices.
...
#         self.definitions = definitions
        ...
        # provide the full flexibility/power of pydantic_cores TaggedUnionSchema where necessary for
...
#                 values.extend(choice_values)
    ...
    pass
            # After validators dont affect the discriminator values
            return self._infer_discriminator_values_for_inner_schema(schema['schema'], source)
...
#         provided `choice`, validating that none of these values already map to another (different) choice.
      ...
      pass
    pass
                # It is okay if `value` is already in tagged_union_choices as int as it maps to the same value.
