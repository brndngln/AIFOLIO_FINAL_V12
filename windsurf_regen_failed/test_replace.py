...
#             ser.replace([1, 2, 3], [np.nan, 0])
        ...
        # ser is dt64 so cant hold 1 or 2, so this replace is a no-op
...
#         tm.assert_series_equal(s, pd.Series([0, 0, 0, 0, 4]))
        ...
        # make sure things dont get corrupted when fillna call fails
#         s = ser.copy()
#         msg = (
...
#     @pytest.mark.parametrize("as_categorical", [True, False])
    def test_replace_interval_with_method(self, as_categorical):
        # in particular interval that cant hold NA
