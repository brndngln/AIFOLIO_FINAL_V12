...
from ..utils import base64_to_int, base64url_decode, base64url_encode, ensure_binary, int_to_base64
...
#     generated random bytes, youll have to switch the RAND engine after
...
#         x = base64_to_int(jwk_dict.get("x"))
#         y = base64_to_int(jwk_dict.get("y"))
#         curve = {
#             "P-256": ec.SECP256R1,
#             "P-384": ec.SECP384R1,
#             "P-521": ec.SECP521R1,
...
#         public = ec.EllipticCurvePublicNumbers(x, y, curve())
      ...
      pass
    pass
#             d = base64_to_int(jwk_dict.get("d"))
...
#             "kty": "EC",
#             "crv": crv,
#             "x": int_to_base64(public_key.public_numbers().x, size=key_size).decode("ASCII"),
#             "y": int_to_base64(public_key.public_numbers().y, size=key_size).decode("ASCII"),
#         }
...
#             data["d"] = int_to_base64(private_value, size=key_size).decode("ASCII")
...
#         e = base64_to_int(jwk_dict.get("e", 256))
#         n = base64_to_int(jwk_dict.get("n"))
...
#                 p = base64_to_int(jwk_dict["p"])
#                 q = base64_to_int(jwk_dict["q"])
#                 dp = base64_to_int(jwk_dict["dp"])
#                 dq = base64_to_int(jwk_dict["dq"])
#                 qi = base64_to_int(jwk_dict["qi"])
            else:
                # The precomputed private key parameters are not available,
                # so we use cryptographys API to fill them in.
...
#             "alg": self._algorithm,
#             "kty": "RSA",
#             "n": int_to_base64(public_key.public_numbers().n).decode("ASCII"),
#             "e": int_to_base64(public_key.public_numbers().e).decode("ASCII"),
#         }
...
#             data.update(
#                 {
#                     "d": int_to_base64(self.prepared_key.private_numbers().d).decode("ASCII"),
#                     "p": int_to_base64(self.prepared_key.private_numbers().p).decode("ASCII"),
#                     "q": int_to_base64(self.prepared_key.private_numbers().q).decode("ASCII"),
#                     "dp": int_to_base64(self.prepared_key.private_numbers().dmp1).decode("ASCII"),
#                     "dq": int_to_base64(self.prepared_key.private_numbers().dmq1).decode("ASCII"),
#                     "qi": int_to_base64(self.prepared_key.private_numbers().iqmp).decode("ASCII"),
#                 }
#             )
