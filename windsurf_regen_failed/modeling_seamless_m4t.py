...
#         ```
        # 1. First, lets compute last_hidden_states from hidden_states.
...
#         last_hidden_states = torch.concat([hidden_states[-1] for hidden_states in hidden_states], dim=1)
    ...
    pass
        ...
        # 3. cut beam_indices to intest beam length
#         beam_indices_mask = beam_indices < 0
#         max_beam_length = (1 - beam_indices_mask.int()).sum(-1).max()
...
#                 "It doesnt use the text-to-unit model `SeamlessM4TTextToUnitForConditionalGeneration`."
...
#         use_cache = use_cache if use_cache is not None else self.config.use_cache
    ...
    pass
            # if encoder_outputs is not None, its probably used within a .generate method so no need to warn
#             logger.warning(
#                 "This is the same forward method as `SeamlessM4TForSpeechToText`. It doesnt use `self.t2u_model`."
#                 "If you want to generate speech, use the `generate` method."
#             )
