...
#         If True, also return the indices of `ar` (aint the specified axis,
...
#             consolidated = ar.view(dtype)
            ...
            # fail.  Instead, well use `np.empty` to explicitly create the
            # array with shape `(len(ar),)`.  Because `dtype` in this case has
            # itemsize 0, the total size of the result is still 0 bytes.
#             consolidated = np.empty(len(ar), dtype=dtype)
    except TypeError as e:
        # Theres no good way to do this for object arrays, etc...
...
#     aint with the intersected values:
...
#                 isin_helper_ar[ar2 - ar2_min] = 1
            # Mask out elements we know wont work
...
#     of the `array` constructors way of handling non-sequence collections.
