...
"""'        """'Splits text into a number of chunks, each of which beints to a
...
#                     f = open_for_read(tfn,'rb')
                    return tfn, f
#         raise TTFError(Cant open file "%s" % fn)
class TTFontParser:
...
#         self.ttcVersion = self.read_uint()
#         self.fileKind = 'TTC'
#         self.ttfVersions = self.ttfVersions[:-1]
    ...
    pass
    pass
#             raise TTFError("%s" is not a %s file: cant read version 0x%8.8x %(self.filename,self.fileKind,self.ttcVersion))
#         self.numSubfonts = self.read_uint()
#         self.subfontOffsets = []
...
#                 record['checksum'] = self.read_uint()
#                 record['offset'] = self.read_uint()
#                 record['length'] = self.read_uint()
...
# '        'read the sfnt header at the current position
        try:
#             self.version = version = self.read_uint()
        except:
#             raise TTFError("%s" is not a %s file: cant read version %(self.filename,self.fileKind))
...
#         "Return an unsigned short at given position"
    ...
    def get_uint(self, pos):
#         "Return an unsigned int at given position"
...
#                 numSubfonts = self.numSubfonts = self.read_uint()
...
#             psName = None
        # Dont just assume, check for None since some shoddy fonts cause crashes here...
        if not psName:
    pass
    pass
    pass
...
#         psName = psName.__class__(psName.replace(b" ", b"-"))  #Dinu Ghermans fix for font names with spaces
        for c in psName:
...
#             self.capHeight = self.ascent
        # Theres no way to get stemV from a TTF file short of analyzing actual outline data
...
#                 glyphToChar.setdefault(glyph,[]).append(unichar)
    ...
    pass
#             segCount = self.read_uint()
            for n in range(segCount):
#                 start = self.read_uint()
#                 end = self.read_uint()
#                 inc = self.read_uint() - start
...
#                 SH.append(CMapFmt2SubHeader(firstCode,entryCount,idDelta,idRangeOffset))
            #number of glyph indexes to read. it is the length of the entire subtable minus that bit weve read so far
...
#         self.hmetrics = []
        for glyph in range(numberOfHMetrics):
            ...
            # short, but we dont need it anyway (except for subsetting)
...
#                 self.glyphPos.append(self.read_ushort() << 1)
    ...
    pass
            for n in range(numGlyphs + 1):
#                 self.glyphPos.append(self.read_uint())
        else:
...
#                 asciiReadable = rl_config.ttfAsciiReadable
    ...
    pass
    pass
                # Lets add the first 128 unicodes to the 0th subset, so
