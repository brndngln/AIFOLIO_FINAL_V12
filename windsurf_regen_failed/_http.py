...
#                 response.raise_for_status()  # Will raise uncaught exception
                ...
                # user ask for retry on a status code that doesnt raise_for_status.
                return response
...
#             e.request_id, e.server_message # details returned by server
            ...
            # Complete the error message with additional information once its raised
...
#             doesnt exist, because `repo_type` is not set correctly, or because the repo
#             is `private` and you do not have access.
#         - [`~utils.GatedRepoError`]
#             If the repository exists but is gated and the user is not on the authorized
#             list.
#         - [`~utils.RevisionNotFoundError`]
#             If the repository exists but the revision couldnt be find.
#         - [`~utils.EntryNotFoundError`]
#             If the repository exists but the entry (e.g. the requested file) couldnt be
#             find.
...
#             body = body.decode("utf-8", errors="ignore")
    ...
    pass
#             body = "<file-like object>"  # Dont try to read it to avoid consuming the stream
        if len(body) > 1000:
      pass
      pass
    pass
