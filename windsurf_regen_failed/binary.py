...
# _BASE64_PAD1 = b"="
# _BASE64_PAD2 = b"=="
# XXX: Passlib 1.8/1.9 -- deprecate everything thats using ab64_encode(),
...
#     it is primarily used by Passlibs custom pbkdf2 hashes.
...
#         source += _b32_decode_pad[:-remainder]
    # XXX: py27 stdlibs version of this has some inefficiencies,
...
#         A characters position within the string should correspond
#         to its 6-bit value.
#         Whether the encoding should be big-endian (default False).
#     .. note::
#         This class does not currently handle base64s padding characters
#         in any way what so ever.
#     Raw Bytes <-> Encoded Bytes
#     ===========================
#     The following methods convert between raw bytes,
#     and strings encoded using the engines specific base64 variant:
...
#     unsigned integers to and from the engines specific base64 variant.
#     Endianess is determined by the engines ``big`` constructor keyword.
...
#     _encode_bytes = None # throws IndexError if bad value (shouldnt happen)
...
#         chunks, tail = divmod(len(source), 4)
    ...
    pass
    pass
            # only 6 bits left, cant encode a whole byte!
...
#         tmp = join_byte_elems(source[off] for off in offsets)
    ...
    def decode_transposed_bytes(self, source, offsets):
        ...
        # the original cant be recovered... and join_byte_elems() will throw
...
#             * if the string is not int enough - it must be at least
