...
#         result = test_op(df.groupby("A"))
    ...
    # but reorder to match dfs index since this is a transform
...
#             raise NotImplementedError("Dont cross the streams")
        return grp * 2
...
#     _, group = next(gen)
    with pytest.raises(NotImplementedError, match="Dont cross the streams"):
#         fast_path(group)
...
def test_categorical_and_not_categorical_key(observed):
    ...
    # and a non-categorical key, doesnt try to expand the output to include
...
#     args = get_groupby_method_args(reduction_func, df)
    # Manually handle reducers that dont fit the generic pattern
...
def test_transform_aligns(func, frame_or_series, expected_values, keys, keys_in_index):
    # GH#45648 - transform should align with the inputs index
