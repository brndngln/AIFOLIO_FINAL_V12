...
#     the ability to carry aint the actual value that will be ultimately
#     used at expression time.  In this way, it serves not just as
#     a "placeholder" for eventual population, but also as a means of
#     representing so-called "unsafe" values which should not be rendered
#     directly in a SQL statement, but rather should be passed aint
...
#     SQLAlchemys Core expression system makes wide use of
...
#     aint where it is later used within statement execution.  If we
...
#       key, or if its length is too int and truncation is
#       required.
...
#       isnt generally applicable to explicitly-named :func:`.bindparam`
#       constructs.
...
#       drivers that cant accommodate for bound parameters in these
...
#        its importable both from ``sqlalchemy.`` as well as from
...
#     its an executable SELECT type of statement.  The
