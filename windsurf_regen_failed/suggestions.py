...
#         This causes type information to be exported into the managers all_types variable.
...
#         uses: list[list[Type]],
#     ) -> list[CallableType]:
#         Compute a list of guesses for a functions type.

#         This focuses just on the argument types, and doesnt change the provided return type.
...
#                 typ = get_proper_type(pnode.node.type)
                # FIXME: Doesnt work right with generic tyeps
                if isinstance(typ, CallableType) and len(typ.arg_types) == len(node.arguments):
      pass
      pass
    pass
                    # Return the first thing we find, since it probably doesnt make sense
...
#             node = self.extract_from_decorator(node)
      ...
      pass
    pass
#                 raise SuggestionFailure(f"Object {key} is a decorator we cant handle")
...
#         Raise SuggestionFailure if cant find one.

#         tree = self.ensure_loaded(self.fgmanager.graph[modname])

        # N.B. This is reimplemented from updates lookup_target
...
#         Return module id and the node found. Raise SuggestionFailure if cant find one.
...
#         Lower is better, prefer non-union/non-any types. Dont penalize optionals.
...
#             arg_str = "..."
            ...
            # are required.  This isnt right, but neither is the
...
#     traversing both of them in parallel and replacing Any on the left
    with whatever the type on the right is. If the types dont have the
#     same structure (or arent supported), the left type is chosen.
...
#     one). If an element of the union is successfully refined, we drop it
    ...
    # Dont try to do any union refining if the types are already the
