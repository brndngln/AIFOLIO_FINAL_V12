...
#                 name = arg.variable.name
                ...
                # attributes. cls cant be used to define attributes so we can ignore it.
...
#                 self.analyze_lvalue(target)
        ...
        # raised by the last statement in the body, which usually isnt a
        # problem.
#         stmt.body.accept(self)
...
#                 guard = s.guards[i]
    ...
    pass
#                     guard.accept(self)
                # We already entered a block, so visit this blocks statements directly
...
#                 continue
      ...
      pass
    pass
                # In a function, dont rename the first definition, as it
                # may be an argument that must preserve the name.
#                 to_rename = refs[1:]
            else:
                # At module top level, dont rename the final definition,
...
#         This method ensures that the second assignment to x doesnt introduce a new
#         variable.
...
#                 this name (if this is a new definition)
    ...
    pass
      ...
      pass
    pass
            # Cant redefine within try/with a block.
...
#                 var_blocks[name] = block
                ...
                # This doesnt support arbitrary redefinition.
#                 var_blocks[name] = -1
            return True
...
#     If theres a reference to a variable defined in with outside the
#     statement, or if theres any trickiness around variable visibility
#     (e.g. function definitions), we give up and wont perform renaming.
...
#         self.bound_vars: list[str] = []
        # Stack of names that cant be safely renamed, per scope (* means that
...
#             self.analyze_lvalue(lvalue.expr)
    ...
    def visit_import(self, imp: Import) -> None:
        # We dont support renaming imports
        for id, as_id in imp.ids:
...
#             self.record_skipped(as_id or id)
    ...
    def visit_import_all(self, imp: ImportAll) -> None:
        # Give up, since we dont know all imported names yet
#         self.reject_redefinition_of_vars_in_scope()
