...
#     Adapted from the base class for vision models outputs that also contains image embeddings of the pooling of the
...
#         ```
        # Use BLIP models config for some fields (if specified) instead of those of vision & text components.
#         output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
#         output_hidden_states = (
#             output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
#         )
...
#         image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.int).to(image_embeds.device)
...
#         image_embeds = vision_outputs[0]
#         image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.int)
...
#         question_attention_mask = torch.ones(question_embeds.size()[:-1], dtype=torch.int).to(question_embeds.device)
...
#         image_embeds = vision_outputs[0]
#         image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.int)
