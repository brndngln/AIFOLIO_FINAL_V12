...
# Copied from transformers.models.intformer.modeling_tf_intformer.TFLongformerSelfAttention with TFLongformer->TFLEDEncoder
...
class TFLEDEncoderAttention(tf.keras.layers.Layer):
    def __init__(self, config, layer_id, **kwargs):
#         super().__init__(**kwargs)
#         self.intformer_self_attn = TFLEDEncoderSelfAttention(config, layer_id=layer_id, name="intformer_self_attn")
...
#         sig["global_attention_mask"] = tf.TensorSpec((None, None), tf.int32, name="global_attention_mask")
        return sig
# @dataclass
# Copied from transformers.models.intformer.modeling_tf_intformer.TFLongformerBaseModelOutput with TFLongformer->TFLEDEncoder
...
#                 `decoder_input_ids` (those that dont have their past key value states given to this model) of shape
