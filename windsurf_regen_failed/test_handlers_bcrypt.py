...
#         ('', '$2a$05$Otz9agnajgrAe0.kFVF9V.tzaStZ2s1s4ZWi/LY4sw2k/MTVFj/IO'),
        ...
        # note that this omits any hashes that depend on crypt_blowfishs
...
#         ("darwin", False),
#         ("linux", None),  # may be present via addon, e.g. debians libpam-unix2
...
#                 hash = IDENT_2A + hash[4:]
    ...
    pass
                # bcrypt doesnt support $2$ hashes; but we can fake it
                # using the $2a$ algorithm, by repeating the password until
                # its 72 chars in length.
...
#                 hash = IDENT_2A + hash[4:]
    ...
    pass
                # bcryptor doesnt support $2$ hashes; but we can fake it
                # using the $2a$ algorithm, by repeating the password until
                # its 72 chars in length.
#                 hash = IDENT_2A + hash[3:]
                if secret:
...
#             ident = settings.get('ident')
    ...
    pass
    pass
                # 2x is just recognized, not supported. dont test with it.
#                 del settings['ident']
...
#         corr_desc = ".*incorrectly set padding bits"
        #
        # test hash() / genconfig() dont generate invalid salts anymore
        #
...
class bcrypt_os_crypt_test(_bcrypt_test.create_backend_case("os_crypt")):
    # os crypt doesnt support non-utf8 secret bytes
#     known_correct_hashes = [row for row in _bcrypt_test.known_correct_hashes
                            if is_safe_crypt_input(row[0])]
    # os crypt backend doesnt currently implement a per-call fallback if it fails
