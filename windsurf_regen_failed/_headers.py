...
# occurs repeatedly. But, of course, they cant necessarily be split by
# .split(b","), because quoting.
#
# Given all this mess (case insensitive, duplicates allowed, order is
# important, ...), there doesnt appear to be any standard way to handle
# headers in Python -- theyre almost like dicts, but... actually just
# arent. For now we punt and just use a super simple representation: headers
# are a list of pairs
#
...
def get_comma_header(headers: Headers, name: bytes) -> List[bytes]:
    ...
    # any Transfer-Encoding is reject ones that arent just "chunked", so
...
def set_comma_header(headers: Headers, name: bytes, new_values: List[bytes]) -> Headers:
    ...
    #
    # Simply calling `.title()` is a blunt approach, but its correct
    # here given the cases where were using `set_comma_header`...
    #
