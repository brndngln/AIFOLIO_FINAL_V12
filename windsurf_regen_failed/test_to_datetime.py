...
#             [199934, datetime(1999, 3, 4)],
            # int integer date partially parsed to datetime(2012,1,1), length > 8
...
#         msg = "|".join(
#             [
#                 r^time data ".*" doesnt match format ".*", at position 0.
#                 f"{PARSING_ERR_MSG}$",
...
#         assert result is arr
    ...
    # Doesnt work on Windows since tzpath not set correctly
#     @td.skip_if_windows
...
#         )
      ...
      pass
    pass
            # Doesnt hold for utc=True, since that will astype
...
#         result = to_datetime(dts_with_oob, errors="coerce", cache=cache)
      ...
      pass
    pass
            # FIXME: shouldnt depend on cache!
#             expected = DatetimeIndex(
...
#             ValueError,
#             match=(
#                 r^time data "True" doesnt match format "%Y%m%d",
...
#         msg = "|".join(
#             [
#                 r^time data "a" doesnt match format "%H:%M:%S", at position 0.
#                 f"{PARSING_ERR_MSG}$",
...
#         assert res is NaT
      ...
      pass
    pass
#             msg = r^time data ".*" doesnt match format ".*", at position 0.
            with pytest.raises(ValueError, match=msg):
#                 to_datetime(value, errors="raise", format=format)
        else:
...
#     @pytest.mark.parametrize("format", [(None), ("%H:%M:%S")])
    def test_datetime_invalid_index(self, values, format):
        # GH24763
        # Not great to have logic in tests, but this ones hard to
        # parametrise over
...
#         tm.assert_series_equal(mixed[:-2], expected)
        # well check mixed[-1] and mixed[-2] match now and today to within
...
#         msg = (
#             r^cannot assemble the datetimes: time data ".+" doesnt
...
#     @pytest.mark.parametrize("exact", [True, False])
    def test_to_datetime_iso8601_fails(self, input, format, exact):
        ...
        # `format` is inter than the string, so this fails regardless of `exact`
        with pytest.raises(
#             ValueError,
#             match=(
#                 rf"time data "{input}" doesnt match format "
...
#                 f", at position 0. {PARSING_ERR_MSG}$",
#                 f^time data ".*" doesnt match format ".*", at position 0.
#                 f"{PARSING_ERR_MSG}$",
#             ]
#         )
        with pytest.raises(
...
#     @pytest.mark.xfail(reason="fails to enforce dayfirst=True, which would raise")
    def test_to_datetime_respects_dayfirst(self, cache):
        ...
        # The msg here is not important since it isnt actually raised yet.
...
#         msg = (
#             r^time data " " doesnt match format "%m/%d/%Y",
...
#             ValueError,
#             match=(
#                 r^time data "03/30/2011" doesnt match format
...
#         msg = (
#             r^time data "01-02-2011 00:00:00" doesnt match format
...
#                 "2015-32-02",
#                 "%Y-%d-%m",
#                 ^time data "2015-32-02" doesnt match format "%Y-%d-%m",
#                 f"at position 0. {PARSING_ERR_MSG}$",
#             ),
#             (
#                 "2015-04-31",
...
#             tm.assert_index_equal(res, exp)
        ...
        # these really need to have yearfirst, but we dont support
        if not yearfirst:
      pass
      pass
    pass
...
#         )
        ...
        # we dont support dayfirst/yearfirst here:
...
#         result5 = DatetimeIndex([date_str])[0]
        ...
        # others are not, and cant be changed because it is used in
        # time series plot
