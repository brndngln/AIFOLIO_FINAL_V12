...
#         ...
    ...
    def accept(self, node: Statement | Expression, *, can_borrow: bool = False) -> Optional[Value]:
#         Transform an expression or a statement.

#         If can_borrow is true, prefer to generate a borrowed reference.
#         Borrowed references are faster since they dont require reference count
...
#             out: the BasicBlock that is run if the module has already been loade.
...
#         self.add_bool_branch(comparison, out, needs_import)
    ...
    def get_module(self, module: str, line: int) -> Value:
        # Python 3.7 has a nice PyImport_GetModule function that we cant use :(
...
#             self.add(Unreachable())
    ...
    def disallow_class_assignments(self, lvalues: list[Lvalue], line: int) -> None:
        ...
        # variables that arent marked ClassVar, since we blatantly
...
#                 symbol = symbol.func
      ...
      pass
    pass
                # New semantic analyzer doesnt create ad-hoc Vars for special forms.
...
#                         reg_type = self.node_type(lvalue)
                    ...
                    # in the current functions environment class. Next, define a target that
...
#         target = self.make_spill_target(value.type)
        # Shouldnt be able to fail, so -1 for line
...
#         value_type = self.get_dict_value_type(expr)
    ...
    def _analyze_iterable_item_type(self, expr: Expression) -> Type:
        ...
        # This logic is copied from mypys TypeChecker.analyze_iterable_item_type.
...
#             callee_node = callee_node.impl
        ...
        # removed, not just singledispatch functions (which we dont do now just in case those
...
#         self.symtables[-1][symbol] = target
    ...
    def node_type(self, node: Expression) -> RType:
      ...
      pass
    pass
            # TODO: Dont special case IntExpr
...
#         )
    ...
    # Lacks a good type because there wasnt a reasonable type in 3.5 :(
...
#                 assert arg.initializer is not None
                ...
                # If it is constant, dont bother storing it
