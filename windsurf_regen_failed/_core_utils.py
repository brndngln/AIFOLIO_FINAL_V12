...
# schema building because one of its members refers to a definition that was not yet defined when the union
...
#     Produces the ref to be used for this type by pydantic_cores core schemas.
...
# ) -> core_schema.CoreSchema | None:
    ...
    pass
    pass
        # in this case, there are no missing refs to potentially substitute, so theres no need to walk the schema
        # this is a common case (will be hit for all non-generic models), so its worth optimizing for
        return None
...
#         new_inner_schema = self.walk(schema['schema'], f)
    ...
    pass
    pass
            # This means wed be returning a "trivial" definitions schema that just wrapped the inner schema
