...
#             youre using with your RAG instance.
...
#         n_docs = n_docs if n_docs is not None else self.config.n_docs
        # shift tokens left (from original Pytorchs version)
#         target = tf.concat(
...
#         loss = self.hf_compute_loss(target, rag_logprobs, from_logits=True, reduce_loss=reduce_loss)
    ...
    def hf_compute_loss(self, labels, y_pred, smooth_epsilon=0.0, from_logits=True, reduce_loss=False):
        # Matt: As written, this loss is not XLA-compatible, but its doing some very weird things
        #       and I dont feel comfortable converting it.
...
#         assert len(target.shape) == len(rag_logprobs.shape)
        # last-axis gathering only - use 2D-reshape-trick for Torchs style nD gathering
...
#         output = tf.fill(new_shape, pad_token_id)
        # Normal tensor doesnt support slice assignment, so we need tf.Variable
