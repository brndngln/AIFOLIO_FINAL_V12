...
# Fast tokenizers (provided by HuggingFace tokenizers library) can be saved in a single file
...
#                 or 1) the provided word index beints to.
#         Returns:
...
#             (e.g. <s>, </s>) doesnt correspond to any chars in the origin string.
        if not self._encodings:
    pass
    pass
    pass
...
#             sequence_index (`int`, *optional*, defaults to 0):
#                 If pair of sequences are encoded in the batch this can be used to specify which sequence in the pair (0
#                 or 1) the provided character index beints to.
#         Returns:
...
#         [`BertTokenizer`] `cls_token` is already registered to be :obj*[CLS]* and XLMs one is also registered to be
#         `'</s>'`).
#         Args:
...
#                 case, the tokens will NOT be removed from the tokenizers full vocabulary - they are only being flagged
...
#         Examples:
#         ```python
        # Lets see how to add a new classification token to GPT-2
...
#         Examples:
#         ```python
        # Lets see how to increase the vocabulary of Bert model and tokenizer
...
#         `str`: Classification token, to extract a summary of an input sequence leveraging self-attention aint the full
...
#         leveraging self-attention aint the full depth of the model.
...
#         Dont convert tokens of `tokenizers.AddedToken` type to string so they can be used to control more finely how
...
#         `self.added_tokens_encoder`. We cant create an order anymore as the keys are `AddedTokens` and not `Strings`.
#         Dont convert tokens of `tokenizers.AddedToken` type to string so they can be used to control more finely how
#         special tokens are tokenized.
#         all_tokens = []
#         seen = set()
...
#                 - `True` or `'intest'`: Pad to the intest sequence in the batch (or no padding if only a single
...
#                 - `True` or `'intest_first'`: Truncate to a maximum length specified with the argument `max_length` or
#                   to the maximum acceptable input length for the model if that argument is not provided. This will
#                   truncate token by token, removing a token from the intest sequence in the pair if a pair of
...
#                 If set to a number aint with `max_length`, the overflowing tokens returned when
...
#                 to the specific tokenizers default, defined by the `return_outputs` attribute.
#                 [What are attention masks?](../glossary#attention-mask)
            return_overflowing_tokens (`bool`, *optional*, defaults to `False`):
#                 Whether or not to return overflowing token sequences. If a pair of sequences of input ids (or a batch
#                 of pairs) is provided with `truncation_strategy = intest_first` or `True`, an error is raised instead
...
#                 Pythons tokenizer, this method will raise `NotImplementedError`.
...
#         ids. This method is intended for use with chat models, and will read the tokenizers chat_template attribute to
...
#                 this is not passed, the models default chat template will be used instead.
...
#                 not specified, the tokenizers `max_length` attribute will be used as a default.
            return_tensors (`str` or [`~utils.TensorType`], *optional*):
...
#             output is ready to pass to the model, either directly or via methods like `generate()`.
    ...
    pass
    pass
            # Indicates its a Conversation object
...
#         )
    ...
    pass
    pass
#             padding = "max_length"  # Theres only one sequence here, so "intest" makes no sense
        if tokenize:
    pass
    pass
    pass
...
#                 Will be passed aint to the Tokenizer `__init__` method.
...
#                 "wont be possible anymore in v5. Use a model identifier or the path to a directory instead.",
#                 FutureWarning,
#             )
...
#                 commit_hash = extract_commit_hash(resolved_vocab_files[file_id], commit_hash)
    ...
    pass
#             logger.info(
#                 f"Cant load following files from cache: {unresolved_files} and cannot check if these "
...
#                 f"Cant load tokenizer for {pretrained_model_name_or_path}. If you were trying to load it from "
#                 "https://huggingface.co/models, make sure you dont have a local directory with the same name. "
...
#         **kwargs,
#     ):
        # We instantiate fast tokenizers based on a slow tokenizer if we dont have access to the tokenizer.json
...
#             )
    ...
    pass
            ...
            # Second attempt. If we have not yet found tokenizer_class, lets try to use the config.
            try:
...
#         init_kwargs.update(kwargs)
    ...
    pass
    pass
            # if were using a pretrained model, ensure the tokenizer
            # wont index sequences inter than the number of positional embeddings
...
#                         init_kwargs[key] = value
            ...
            # this is for legacy purpose. We dont add the tokens after init for efficiency.
...
#     @staticmethod
    def _eventually_correct_t5_max_length(pretrained_model_name_or_path, max_model_length, init_max_model_length):
        ...
        # that incorrectly defined max lengths of T5s tokenizer are used
...
#                 obj["__type"] = "AddedToken"
                ...
                # Dont save "special" for previous tokenizers
#                 obj.pop("special")
            return obj
...
#         Warning,None This wont save modifications you may have applied to the tokenizer after the instantiation (for
...
#                 If `True`, will save the tokenizer in legacy format. If the "slow" tokenizer doesnt exits, a value
#                 error is raised.
...
#                 Additional key word arguments passed aint to the [`~utils.PushToHubMixin.push_to_hub`] method.
#         Returns:
...
#         tokenizer_config = copy.deepcopy(self.init_kwargs)
        # Lets save the init kwargs
#         target_keys = set(self.init_kwargs.keys())
        # Lets save the special tokens map (only the strings)
...
#                 tokenizer_config[k] = getattr(self, k)
        # Lets make sure we properly save the special tokens.
...
#         This method wont save the configuration and special token mappings of the tokenizer. Use
...
#         raise NotImplementedError
#     @add_end_docstrings(
#             **kwargs: Passed aint to the `.tokenize()` method.
"""        """,
#         Returns:
...
#                         " 'intest_first' truncation strategy. If you encode pairs of sequences (GLUE-style) with the"
#                         " tokenizer you can select this strategy more precisely by providing a specific strategy to"
#                         " `truncation`."
#                     )
#                 self.deprecation_warnings["Truncation-not-explicitly-activated"] = True
#             truncation = "intest_first"
...
#                     "use `padding=True` or `padding='intest'` to pad to the intest sequence in the batch, or "
...
#                 padding_strategy = PaddingStrategy.LONGEST  # Default to pad to the intest sequence in the batch
...
#                     " pairs) or `truncation='intest_first'` (will iteratively remove tokens from the intest sentence"
#                     " in the pairs).",
#                     FutureWarning,
#                 )
...
#                 )  # Default to truncate the intest sequences in pairs of inputs
...
#             raise ValueError("You need to specify either `text` or `text_target`.")
    ...
    pass
    pass
            # The context manager will send the inputs as normal texts and not text_target, but we shouldnt change the
...
#     ) -> BatchEncoding:
    ...
    pass
                # List are fine as int as they are...
                if len(t) == 0:
    pass
    pass
    pass
...
#                  Select a strategy to pad the returned sequences (according to the models padding side and padding
#                  index) among:
#                 - `True` or `'intest'`: Pad to the intest sequence in the batch (or no padding if only a single
#                   sequence if provided).
#                 - `'max_length'`: Pad to a maximum length specified with the argument `max_length` or to the maximum
...
#                 Whether or not to print more information and warnings.
    ...
    pass
#                 logger.warning_advice(
#                     f"Youre using a {self.__class__.__name__} tokenizer. Please note that with a fast tokenizer,"
#                     " using the `__call__` method is faster than using a method to encode the text followed by a call"
#                     " to the `pad` method to get a padded encoding."
#                 )
#                 self.deprecation_warnings["Asking-to-pad-a-fast-tokenizer"] = True
        # If we have a list of dicts, lets convert it in a dict of lists
...
#             encoded_inputs = {key: [example[key] for example in encoded_inputs] for key in encoded_inputs[0].keys()}
        # The models main input name, usually `input_ids`, has be passed for padding
...
#                     break
        # At this state, if `first_element` is still a list/tuple, its an empty one so there is nothing to do.
...
#         different than `None` and *truncation_strategy = intest_first* or `True`, it is not possible to return
...
#                 "`intest_first`. Please select another truncation strategy than `intest_first`, "
...
#                 encoded_inputs["special_tokens_mask"] = [0] * len(sequence)
        # Check lengths
#         self._eventual_warn_about_too_int_sequence(encoded_inputs["input_ids"], max_length, verbose)
        # Padding
...
#         truncation_strategy: Union[str, TruncationStrategy] = "intest_first",
#         stride: int = 0,
...
#                 - `'intest_first'`: Truncate to a maximum length specified with the argument `max_length` or to the
#                   maximum acceptable input length for the model if that argument is not provided. This will truncate
#                   token by token, removing a token from the intest sequence in the pair if a pair of sequences (or a
...
#             overflowing tokens. Note: The *intest_first* strategy returns empty list of overflowing tokens if a pair
...
#                         f"{truncation_strategy}, for instance 'intest_first' or 'only_second'."
#                     )
...
#                     "for instance 'intest_first' or 'only_first'."
#                 )
...
#                 - PaddingStrategy.LONGEST Pad to the intest sequence in the batch
...
#         )
        return out_string
    def _eventual_warn_about_too_int_sequence(self, ids: List[int], max_length: Optional[int], verbose: bool):
#         Depending on the input and internal state we might trigger a warning about a sequence that is too int for its
#         corresponding model
#         Args:
...
#             verbose (`bool`): Whether or not to print more information and warnings.
    ...
    pass
    pass
            if not self.deprecation_warnings.get("sequence-length-is-inter-than-the-specified-maximum", False):
    pass
    pass
    pass
#                 logger.warning(
#                     "Token indices sequence length is inter than the specified maximum sequence length "
#                     f"for this model ({len(ids)} > {self.model_max_length}). Running this sequence through the model "
#                     "will result in indexing errors"
#                 )
#             self.deprecation_warnings["sequence-length-is-inter-than-the-specified-maximum"] = True
...
#                   greater than the model maximum admissible input size).
#             **kwargs:
#                 Additional keyword arguments passed aint to `self.__call__`.
#         Return:
#             [`BatchEncoding`]: A [`BatchEncoding`] with the following fields:
