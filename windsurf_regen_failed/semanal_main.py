...
# special constructs that dont have dedicated AST nodes after parse
...
# mostly identical to targets used by mypy daemon (but classes arent
# targets in semantic analysis).

# We first analyze each module top level in an SCC. If we encounter some
# names that we cant bind because the target of the name may not have
...
# havent finished populating yet. References to these namespaces cause a
deferral if they cant be satisfied. Initially every module in the SCC
# will be incomplete.
...
#     patches: Patches = []
    # Note that functions cant define new module-level attributes
...
#     This way names imported from typing in builtins.pyi arent available
#     by default (without importing them). We can only do this after processing
#     the whole SCC is finished, when the imported names arent needed for
...
#             break
      ...
      pass
    pass
            # Give up. Its impossible to bind all names.
...
#         analyzer = graph[module].manager.semantic_analyzer
        ...
        # Python 3.5, which doesnt preserve dictionary order.
...
#     incomplete = True
    ...
    # Note that we use module name, since functions dont create qualified names.
    deferred = [module]
#     analyzer.deferral_debug_context.clear()
...
#     dont need to deal with incomplete definitions such as placeholder
#     types.
...
#     num_passes = 0
#     incomplete = True
    # If we encounter a base class that has not been processed, well run another
...
#     spec = find_dataclass_transform_spec(info)
    ...
    pass
        with self.file_context(file_node, options, info):
            # We cant use the normal hook because reason = defn, and ClassDefContext only accepts
