...
# https://pairlist2.pair.net/mailman/listinfo/reportlab-user.)
...
def _getImage():
    ...
    return Image
def Color2Hex(c):
    #assert isinstance(colorobj, colors.Color) #these checks dont work well RGB
...
#         self.__dict__['_dpi'] = dpi
        #the _rl_renderPM.gstate object doesnt support hasattr so we use this as a proxy test for isbuiltin
...
#         and 3 control points of a cubic bezier curve for the curvto opertato.
...
#         However, it must be true that theta1-theta0 > 0.
        ...
        # pretty much just like Robert Kerns pdfgeom.BezierArc
