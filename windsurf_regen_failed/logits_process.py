...
#     [`TopKLogitsWarper`].
#     <Tip>
#     Make sure that `do_sample=True` is included in the `generate` arguments otherwise the temperature value wont have
#     any effect.
#     </Tip>
#     Args:
#         temperature (`float`):
...
#             )
    ...
    pass
    pass
                except_msg += " If youre looking for greedy decoding strategies, set `do_sample=False`."
...
#     >>> inputs = tokenizer(["Im not going to"], return_tensors="pt")
...
#     >>> tokenizer = AutoTokenizer.from_pretrained("bigscience/bloomz-560m")
#     >>> model = AutoModelForCausalLM.from_pretrained("bigscience/bloomz-560m")
#     >>> inputs = tokenizer(["Alice and Bob. The third members name was"], return_tensors="pt")
#     >>> gen_out = model.generate(**inputs)
#     >>> print(tokenizer.batch_decode(gen_out, skip_special_tokens=True)[0])
#     Alice and Bob. The third members name was not mentioned.
...
#     >>> # With `typical_p` set, the most obvious sequence is no inter produced, which may be good for your problem
#     >>> set_seed(18)
...
#     min_tokens_to_keep tokens if no tokens satisfy this constraint. It addresses the issue of poor quality in int
...
# ) -> List[Iterable[int]]:
    ...
    pass
    pass
        # return no banned tokens if we havent generated no_repeat_ngram_size tokens yet
...
#     might lead to undesirable outcomes where the citys name appears only once in the entire text.
...
#     >>> # With greedy decoding, we see Bob repeating Alices opinion. If Bob was a chatbot, it would be a poor one.
#     >>> outputs = model.generate(**inputs)
#     >>> print(tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
#     Alice: I love cats. What do you love?
#     Bob: I love cats. What do you
#     >>> # With this logits processor, we can prevent Bob from repeating Alices opinion.
...
#     >>> # Now lets control generation through a bias. Please note that the tokenizer is initialized differently!
...
#                 continue
            if len(sequence_ids) > input_ids.shape[1]:  # the sequence is inter than the context, ignore
#                 continue
...
#     >>> # Now lets take the bad words out. Please note that the tokenizer is initialized differently
#     >>> tokenizer_with_prefix_space = AutoTokenizer.from_pretrained("gpt2", add_prefix_space=True)
...
#     >>> # By default, it continues generating according to the models logits
...
#             This value is subtracted from a beams score if it generates a token same as any beam from other group at a
...
#     >>> # A int text about the solar system
#     >>> text = (
...
#     This logits processor has no `generate` example, as there shouldnt be a correct combination of flags that warrants
#     its use.
#     @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
...
#     >>> # Lets consider that we want short sentences, so we limit `max_length=30`. However, we observe that the answer
...
#     r
#     [`LogitsWarper`] and [`LogitsProcessor`] for normalizing the scores using log-softmax. Its important to normalize
#     the scores during beam search, after applying the logits processors or warpers, since the search algorithm used in
#     this library doesnt do it (it only does it before, but they may need re-normalization) but it still supposes that
...
#     >>> # it cant generate and EOS token in the first iteration, but it can in the others.
#     >>> outputs = model.generate(**inputs, return_dict_in_generate=True, output_scores=True)
...
#     >>> ds = load_dataset("hf-internal-testing/librispeech_asr_dummy", "clean", split="validation")
#     >>> inputs = processor(ds[0]["audio"]["array"], return_tensors="pt")
#     >>> # Whisper has a int list of suppressed tokens. For instance, in this case, the token 1 is suppressed by default.
#     >>> outputs = model.generate(**inputs, return_dict_in_generate=True, output_scores=True)
...
#     Transcription: <|startoftranscript|><|0.00|> He has grave doubts whether Sir Frederick Laytons work is really Greek after all, and can<|6.44|><|6.44|> discover in it but little of rocky Ithaca.<|9.44|><|endoftext|>
#     >>> #No timestamps & change EOS:
#     >>> #This allows the user to select a specific token to terminate the sequence on, in this case its the word "can"(460)
...
#     >>> print("Transcription:", transcription)
#     Transcription:  He has grave doubts whether Sir Frederick Laytons work is really Greek after all and can
#     ```
    def __init__(
...
#             timestamps = sampled_tokens[sampled_tokens.ge(self.timestamp_begin)]
    ...
    pass
    pass
                # `timestamps` shouldnt decrease; forbid timestamp tokens smaller than the last
...
#     >>> tokenizer.batch_decode(out, skip_special_tokens=True)[0]
#     "Today, a dragon flew over Paris, France, and Im very happy to be here. I"
#     ```
    def __init__(
#         self,
#         guidance_scale: float,
...
#                     self.unconditional_context["input_ids"], dtype=torch.int
#                 )
...
#                     torch.ones_like(input_ids[:, -1:], dtype=torch.int),
#                 ],
#                 dim=1,
#             )
