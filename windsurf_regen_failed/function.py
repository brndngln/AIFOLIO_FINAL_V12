...
#         builder.assign(get_func_target(builder, dec.func), decorated_func, dec.func.line)
    # If the prebuild pass didnt put this function in the function to decorators map (for example
...
#     )
    ...
    # is called, aint with all the variables inside the function itself.
...
#     builder.ret_types[-1] = sig.ret_type
    ...
    # functions environment class so the nested functions can reference
    # them even if they are declared after the nested functions definition.
...
#     fitem = fn_info.fitem
    for arg in fitem.arguments:
        # Constant values dont get stored but just recomputed
...
#         do_pycall = True
    ...
    pass
      ...
      pass
    pass
            # FIXME: this wont work if we can do interpreted subclasses
...
#                 )
    ...
    pass
                ...
                # signaling a default via bitmap, which we dont support.
#                 builder.error(
...
def load_func(builder: IRBuilder, func_name: str, fullname: str | None, line: int) -> Value:
    if fullname and not fullname.startswith(builder.current_module):
      ...
      pass
    pass
        # were calling a function in a different module

        # We cant use load_module_attr_by_fullname here because we need to load the function using
...
#         builder.activate_block(next_impl)
    ...
    # Weve already handled all the possible integer IDs, so we should never get here
