...
#         dim_per_head = embed_dim // num_heads
    ...
    pass
#             warnings.warn(
#                 "Youd better set embed_dim (d_model) in DetaMultiscaleDeformableAttention to make the"
...
#             _cur += height * width
#             level_ids.append(grid.new_ones(height * width, dtype=torch.int) * level)
...
#         device = pixel_values.device
      ...
      pass
    pass
#             pixel_mask = torch.ones(((batch_size, height, width)), dtype=torch.int, device=device)
...
#         spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.int, device=source_flatten.device)
...
#     _tied_weights_keys = [r"bbox_embed.\d+"]
    # We cant initialize the model on meta device as some weights are modified during the initialization
...
#     @torch.jit.unused
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord):
        ...
        # doesnt support dictionary with non-homogeneous values, such
...
#         This is not really a loss, it is intended for logging purposes only. It doesnt propagate gradients.
...
#     For efficiency reasons, the targets dont include the no_object. Because of this, in general, there are more
...
#         self.giou_cost = giou_cost
      ...
      pass
    pass
#             raise ValueError("All costs of the Matcher cant be 0")
...
#             labels (`list[int`):
#                 A list of values to label predictions beinting at each level. A label can be one of {-1, 0, 1}
...
#         _, pred_inds_with_highest_quality = nonzero_tuple(match_quality_matrix == highest_quality_foreach_gt[:, None])
        ...
        # with gt_A, but it has larger overlap with gt_B, its matched index will still be gt_B.
...
#             matched_labels (Tensor): a vector of length N, the matchers label
...
#                 indices.append(
#                     (
#                         torch.tensor([], dtype=torch.int, device=anchors.device),
#                         torch.tensor([], dtype=torch.int, device=anchors.device),
#                     )
#                 )
#                 continue
