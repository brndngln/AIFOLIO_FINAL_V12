...
#         pos_seq = torch.arange(0, seq_len).int().to(hidden_states.device)
...
#         hidden_states = hidden_states + positional_conv_embedding
    ...
    pass
    pass
#             padding_mask = attention_mask.ne(1).int()
        else:
#             padding_mask = torch.zeros(hidden_states.shape[:2], dtype=torch.int, device=hidden_states.device)
...
#         output_lengths = self._get_feat_extract_output_lengths(non_padded_lengths).to(torch.int)
...
#         batch_size, sequence_length, hidden_size = hidden_states.size()
    ...
    pass
    pass
            # apply SpecAugment aint time axis with given mask_time_indices
...
#             that dont have their past key value states given to this model) of shape `(batch_size, 1)` instead of all
...
#                 Whether or not to return the attentions tensors of the decoders cross-attention layers.
...
#                 output_sequence_length, input_sequence_length)` -- The outputs of the decoders cross-attention layers.
