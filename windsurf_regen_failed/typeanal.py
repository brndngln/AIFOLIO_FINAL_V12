...
#         typ = self.visit_unbound_type_nonoptional(t, defining_literal)
    ...
    pass
    pass
            # We dont need to worry about double-wrapping Optionals or
...
#             ):
#                 self.fail(
#                     fCant use bound type variable "{t.name}" to define generic alias,
#                     t,
...
#         count = len(an_args)
    ...
    pass
            ...
            # I didnt find this in the PEP, but it sounds reasonable.
            return list(an_args)
...
#         ]
    def cannot_resolve_type(self, t: UnboundType) -> None:
        # TODO: Move error message generation to messages.py. Wed first
...
#             item = self.anal_type(t.args[0])
    ...
    pass
    pass
#                 self.fail("Type[...] cant contain another Type[...]", t, code=codes.VALID_TYPE)
...
#     ) -> Type:
#         Figure out what an unbound type that doesnt refer to a TypeInfo node means.
...
#         )
    ...
    pass
        ...
        # were using this enum value specifically within context of
...
#         raise NotImplementedError("ParamSpec literals cannot have unbound TypeVars")
    def visit_callable_type(self, t: CallableType, nested: bool = True) -> Type:
        # Every Callable can bind its own type variables, if theyre not in the outer scope
...
#                 ret_type=self.anal_type(t.ret_type, nested=nested),
                # If the fallback isnt filled in yet,
...
#             sym = self.lookup_qualified(t.name, t)
    ...
    pass
        ...
        # TODO: What if its an Instance? Then use t.type.fullname?
        return None
...
#                     )
    ...
    def visit_tuple_type(self, t: TupleType) -> Type:
        # Types such as (t1, t2, ...) only allowed in assignment statements. Theyll
...
#         any_type = AnyType(TypeOfAny.special_form)
        # If the fallback isnt filled in yet, its type will be the falsey FakeInfo
#         fallback = (
#             t.partial_fallback
...
#                 msg = f"Invalid type: {t.simple_name()} literals cannot be used as a type"
                ...
                # string, its unclear if the user meant to construct a literal type
...
#         prefix = tvar_def.prefix
        # we dont set the prefix here as generic arguments will get updated at some point
...
#                 first = unpack_types[0]
    ...
    pass
    pass
                    # UnpackType doesnt have its own line/column numbers,
...
#             ]
        ...
        # a string, try expanding it in case its a type alias or something.
...
#         arg = get_proper_type(arg)
    ...
    pass
            ...
            # 3. If theres some other underlying problem with the parameter.
            #
...
#                 self.fail(msg, ctx, code=codes.VALID_TYPE)
                ...
                # helpful, but it generally wont make sense in the context of a Literal[...].
                return None
...
#                     tvars.append(tvar_expr)
        # When finding type variables in the return type of a function, dont
        # look inside Callable types.  Type variables only appearing in
        # functions in the return type beint to those functions, not the
        # function were currently analyzing.
...
#             )
            # Ideally, wed check whether the type is quoted or `from __future__ annotations`
...
#     Also emit a suitable error if this is not due to implicit Anys.
    if len(t.args) == 0:
    pass
    pass
    pass
...
#             assert isinstance(node.node, TypeVarLikeExpr)
    ...
    pass
            # Dont query the second argument to Annotated for TypeVars
...
#     *new* type aliases: e.g. F[List[int]], F[List[List[int]]], etc. So we cant detect
#     recursion. It is a known problem in the literature, recursive aliases and generic types
#     dont always go well together. It looks like there is no known systematic solution yet.
...
#         super().__init__(any)
    ...
    def visit_typeddict_type(self, t: TypedDictType) -> bool:
        # typeddict is checked during TypedDict declaration, so dont typecheck it here.
        return False
...
#     Note that we cant use normal union simplification, since this function
