...
#             all_vars.update(b.must_be_defined)
        # For the rest of the things, we only care about branches that werent skipped.
...
#             must_be_defined = set()
        # Everything that wasnt defined in all branches but was defined
...
#         self._scope().branch_stmts[-1].record_nested_branch(result)
    def skip_branch(self) -> None:
        # Only skip branch if were outside of "root" branch statement.
...
#         assert len(self.scopes) > 0
    ...
    def is_possibly_undefined(self, name: str) -> bool:
#         assert len(self._scope().branch_stmts) > 0
        # A variable is undefined if its in a set of `may_be_defined` but not in `must_be_defined`.
...
#     - A variable thats defined only part of the time.
...
#             self.msg.variable_may_be_undefined(name, context)
    def process_definition(self, name: str) -> None:
        # Was this name previously used? If yes, its a used-before-definition error.
...
#         self.tracker.end_branch_statement()
    ...
    pass
            ...
            # If the loop doesnt have a `break` either the function will return or
            # execute the `else`.
...
#                 x = 1
                ...
                # not were trying to process `finally` or not.
                return 0
#             finally:
...
#         has_break = loop.has_break
    ...
    pass
            ...
            # a single branch statement (were guaranteed that the body is executed at least once).
...
#             self.variable_may_be_undefined(o.name, o)
            # We dont want to report the error on the same variable multiple times.
...
#                 self.var_used_before_def(o.name, o)
    ...
    pass
            ...
            # we know its a used-before-definition scenario.
...
#                 names = mod.split(".")
    ...
    pass
    pass
                    # `names` should always be nonempty, but we dont want mypy
