...
def test_set_name_methods(idx, index_names):
    # so int as these are synonyms, we dont need to test set_names
...
def test_set_levels(idx):
    # side note - you probably wouldnt want to use levels and codes
...
#         idx.set_names([names[0]])
    ...
    # shouldnt scalar data error, instead should demand list-like
