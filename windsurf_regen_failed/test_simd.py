...
#         Create list of consecutive numbers according to number of vectors lanes.
...
#         vdata_a, vdata_b = self.load(data_a), self.load(data_b)
        ...
        # py level of npyv_set_* dont support ignoring the extra specified lanes or
        # fill non-specified lanes with zero.
#         vset = self.set(*data_a)
#         assert vset == data_a
        # py level of npyv_setf_* dont support ignoring the extra specified lanes or
...
#     skip_sfx = dict()
    if not npyv:
      ...
      pass
    pass
#         skip = f"target {pretty_name} isnt supported by current machine"
    elif not npyv.simd:
    pass
#         skip = f"target {pretty_name} isnt supported by NPYV"
    else:
...
#                                "doesnt support single-precision"
        if not npyv.simd_f64:
      pass
      pass
    pass
#             skip_sfx["f64"] = f"target {pretty_name} doesnt"\
