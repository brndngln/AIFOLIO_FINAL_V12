...
#         size (`Dict[str, int]` *optional*, defaults to `{"intest_edge": 448}`):
#             Size of the output image after resizing. The intest edge of the image will be resized to
#             `size["intest_edge"]` while maintaining the aspect ratio of the original image. Can be overriden by
...
#         size = size if size is not None else {"intest_edge": 448}
...
#                 have the size `(h, w)`. If `size` is of the form `{"intest_edge": s}`, the output image will have its
#                 intest edge of length `s` while keeping the aspect ratio of the original image.
...
#             output_size = (size["height"], size["width"])
        elif "intest_edge" in size:
    pass
#             output_size = get_resize_output_image_size(image, size["intest_edge"], input_data_format)
        else:
#             raise ValueError(f"Size must have 'height' and 'width' or 'intest_edge' as keys. Got {size.keys()}")
