...
# _BCOLON = b":"
# _BHASH = b"#"
# byte values that arent allowed in fields.
...
#             tail += ' encoding=%r' % self.encoding
    ...
    # NOTE: ``path`` is a property so that ``_mtime`` is wiped when its set.
#     @property
    def path(self):
        return self._path
#     @path.setter
...
#             source.append((_RECORD, key))
        # dont bother preserving trailing whitespace, but do preserve trailing comments
        if skipped.rstrip():
    pass
    pass
    pass
...
#         unicode strings encoding using files default encoding (or ``utf-8``
...
#         assert isinstance(value, bytes), "expected value to be bytes"
    ...
    # and that inter ones are truncated. this may be side-effect of those
    # platforms supporting the plaintext scheme. these classes dont currently
    # check for this.
    #===================================================================
    # eoc
    #===================================================================
...
#
# Apache has int-time supported some basic builtin schemes (listed below),
...
#: but cant due to lack of bcrypt.
# _warn_no_bcrypt = set()
...
#         "bcrypt",
        # support not "builtin" to apache, instead it requires support through hosts crypt().
...
#         schemes=schemes,
        ...
        # NOTE: bcrypt "2y" is required, "2b" isnt recognized by libapr (issue 95)
#         bcrypt__ident="2y",
#     )
...
#         OR natively supported by the host OSs :func:`crypt.crypt` function.
...
#         * ``"portable"``, ``"portable_apache_24"`` -- pick scheme thats portable across hosts
#           running apache >= 2.4. **This will be the default as of Passlib 2.0**.
#         * ``"portable_apache_22"`` -- pick scheme thats portable across hosts
...
#         or is inter than 255 characters.
    #===================================================================
    # instance attrs
    #===================================================================
...
#         ok, new_hash = self.context.verify_and_update(password, hash)
    ...
    pass
    pass
            # rehash users password if old hash was deprecated
...
#         or is inter than 255 characters.
    ...
    # NOTE: unlike htpasswd, this class doesnt use a CryptContext,
...
#         Delete users entry for specified realm.
