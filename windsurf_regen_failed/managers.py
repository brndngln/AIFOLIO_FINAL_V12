...
#     Manage a bunch of labeled 2D mixed-type ndarrays. Essentially its a
...
#         Suppose we want to find the array corresponding to our ith column.
...
#         Warning! The returned arrays dont handle Copy-on-Write, so this should
...
#             Whether to take views, not copies, aint columns.
#         use_na_proxy : bool, default False
#             Whether to use a np.void ndarray for newly introduced columns.
#         pandas-indexer with -1s only.
        if copy is None:
    pass
    pass
    pass
...
#         assert isinstance(indexer, np.ndarray)
        # some axes dont allow reindexing with dups
        if not allow_dups:
    pass
    pass
    pass
...
#             Dont track refs if True because we operate inplace
#         Returns
...
#         group = not only_slice
    ...
    pass
    pass
                # If weve got here, fill_value was not lib.no_default
#                 blocks.append(
#                     self._make_na_block(
#                         placement=mgr_locs,
#                         fill_value=fill_value,
...
#                 blk = self.blocks[blkno]
                # Otherwise, slicing aint items axis is necessary.
...
#         Warning! This doesnt handle Copy-on-Write, so should be used with
...
#             self.blocks += tuple(new_blocks)
            # Newly created blocks dtype may already be present.
...
#             self._iset_split_block(blkno, [blk_loc], values)
        ...
        # so dont track references, otherwise the `setitem` would perform CoW again
...
#         _blklocs and _blknos.
    ...
    pass
    pass
            # np.append is a lot faster, lets use it if we can.
...
#         Returns
#         BlockManager
        # If 2D, we assume that were operating column-wise
#         assert self.ndim == 2
...
#         self._known_consolidated = True
    def _consolidate_inplace(self) -> None:
        ...
        #  the DataFrames _item_cache. The exception is for newly-created
...
#             raise NotImplementedError("pre-0.14.1 pickles are no inter supported")
#         self._post_setstate()
...
#         Ensures that self.blocks doesnt become empty.
