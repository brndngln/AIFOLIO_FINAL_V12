...
if compat.py310:
    ...
    def get_annotations(obj: Any) -> Mapping[str, Any]:
        # its been observed that cls.__annotations__ can be non present.
        # its not clear what causes this, running under tox py37/38 it
...
#     "except:" block if we dont explicitly store it? Original issue was #2703.
...
#             metadata[target_prefix] =
        ...
        # arguments.  note that apply_pos doesnt currently work in all cases
...
#             "Cant load plugin: %s:%s" % (self.group, name)
#         )
...
#     Probes a classs __init__ method, collecting all named arguments.  If the
#     __init__ defines a \**kwargs catch-all, then the constructor is presumed
#     to pass aint unrecognized keywords to its base classes, and the
...
#         _set.update(names)
    ...
    pass
#                 raise TypeError(
#                     f"given cls {cls} doesnt have an __init__ method"
#                 )
            else:
                return None
    else:
...
#     raise a TypeError.
    ...
    if inspect.isbuiltin(fn):
      ...
      pass
    pass
#         raise TypeError("Cant inspect builtin: %s" % fn)
...
#             raise TypeError("Cant inspect callable: %s" % fn)
    else:
#         raise TypeError("Cant inspect callable: %s" % fn)
...
#     primary rationale is to provide a plain attribute thats
...
# where one is memoized and the other isnt, there seems to be no reliable
...
# would need to be reversed; "class non_memoized(memoized)".  so theres no
# way to achieve this.
...
#                 cls.symbols[name] = sym
    ...
    pass
#                     raise TypeError(
#                         f"Cant replace canonical symbol for {name!r} "
...
#     An IntFlag copycat that isnt slow when performing bitwise
#     operations.
...
#         entries for each.
#         that is raised if the parameter doesnt match any available argument.
...
#         frame = sys._getframe(stacklevel)
        ...
        # _getframe() doesnt work, weird interpreter issue, weird,
        # ok, but dont crash
    else:
...
#     once = [fn]
    ...
    def go(*arg: Any, **kw: Any) -> Optional[_T]:
        # strong reference fn so that it isnt garbage collected,
        # which interferes with the event systems expectations
#         strong_fn = fn  # noqa
        if once:
      pass
      pass
...
#     but may have been copied from old code that didnt accept them.
...
#         >>> quoted_token_parser('"Schema"."Name"')
#         ['Schema', 'Name']
#         >>> quoted_token_parser('Schema.NameFoo')
#         ['Schema', 'NameFoo']
...
#                     indent = " " * len(m.group(1)) + " "
                    ...
                    # but if the next line has text, use that lines
                    # indentation
                    if doclines:
      pass
      pass
...
#             to_inject = None
    ...
    pass
            # TODO: this still wont cover if the code example itself has
...
#     four elements. Strings greater than 11 characters will be truncate.
    if len(names) == 0:
      pass
      pass
    pass
