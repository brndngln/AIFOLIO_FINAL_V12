...
#                 result = node.type
    ...
    pass
    pass
                # We special-case TypedDict, because they dont define any constructor.
...
#                 self.visit_call_expr_inner(e)
            # Its really a special form that only looks like a call.
...
#         object_type = get_proper_type(object_type)
    ...
    pass
            ...
            # We "unwrap" it to a regular type, as the class/instance method difference doesnt
...
#                 infer_callee = self.typeddict_callable_from_context(callee)
        # We dont show any errors, just infer types in a generic TypedDict type,
...
#         info = self.chk.scope.enclosing_class()
    ...
    pass
    pass
            # Dont mess with partial types in superclasses.
            return None
...
#                 del partial_types[var]
    ...
    pass
            # Call x[y].method(...); may infer type of x if its a partial defaultdict.
...
#             typename = partial_type.type.fullname
        ...
        # TODO: Dont infer argument expression twice.
        if (
...
#             erased_ctx = remove_optional(erased_ctx)
    ...
    pass
    pass
            # Another special case: the return type is a type variable. If its unrestricted,
            ...
            #    the expression `3` normally has an inferred type of `builtins.int`: but if its
...
#                 new_args.append(arg)
        # Dont show errors after we have only used the outer context for inference.
...
#             ):
                ...
                #       cant represent this so we special case it, as this is a pretty common
                #       thing. This doesnt quite work with all possible subclasses of dict
                ...
                #       a little tricky to fix so its left unfixed for now.
...
#                     )
                # If the regular two-phase inference didnt work, try inferring type
...
#                     freeze_all_type_vars(applied)
                    return applied
                # If it didnt work, erase free variables as <nothing>, to avoid confusing errors.
...
#         using messages if its not None. If messages is given, context must also be given.
...
#         ok = True  # False if weve found any error
...
#                 and
                ...
                # dictionaries and we cant tell it from their types
...
#         arg_types = self.infer_arg_types_in_empty_context(args)
        # Step 1: Filter call targets to remove ones where the argument counts dont match
...
#                 union_interrupted = True
    ...
    pass
                    ...
                    # we dont want to introduce internal inconsistencies.
#                     unioned_result = (
...
#         )
    ...
    pass
            ...
            #       In particular, the notes line and column numbers need to be the same
            #       as the errors.
...
#         NamedTuple, which has a definitive "shape". If so, we dont move the corresponding
...
#         item of the desctructured union. Returns None if there is no match.
        ...
        # hang for int time because of a weird overload call. The caller will get
...
#                 res_items.extend(sub_result)
                ...
                # Some item doesnt match, return soon.
                return None
...
#         2. ...Infer a Literal[...] if were in a literal context. For example, if we
#            were analyzing the "3" in "foo(3)" where "foo" has a signature of
#            "def foo(Literal[3]) -> None", wed want to infer that the "3" has a
#            type of Literal[3] instead of Instance.
#         3. ...Infer an Instance that remembers the original Literal if were declaring
...
#            the comments in Instances constructor for more details.
...
#         )
    ...
    pass
    pass
            # Its actually a type expression X | Y.
...
#             left_type = self.accept(left)
    ...
    pass
                ...
                # We allow `in` for a union of containers and iterables as int as at least one of them matches the
                # type of the left operand, as the operation will simply return False if the unions container/iterator
                # type doesnt match the left operand.
...
#                 encountered_partial_type = False
                for item_type in item_types:
                    # Keep track of whether we get type check errors (these wont be reported, they
...
#                         cont_type = self.chk.analyze_container_item_type(item_type)
    ...
    pass
    pass
                        # We dont really know if this is an error or not, so just shut up.
...
#                     ):
                        # its not a container, but it is an iterable
...
#         right_type = get_proper_type(right_type)
        # If either the LHS or the RHS are Any, we cant really concluding anything
...
#         right_op = lookup_operator(rev_op_name, right_type)
        ...
        # turns out, its not as simple as just trying to call __op__ first and
        # __rop__ second.
        #
...
#         ):
            # When we do "A() + B()" where B is a subclass of A, well actually try calling
            # Bs __radd__ method first, but ONLY if B explicitly defines or overrides the
            # __radd__ method.
            #
            # This mechanism lets subclasses "refine" the expected outcome of the operation, even
            # if theyre located on the RHS.
            #
            # As a special case, the alt_promote check makes sure that we dont use the
...
#             any_type = AnyType(TypeOfAny.special_form)
        ...
        # call the __op__ method (even though its missing).
        if not variants:
    pass
    pass
    pass
...
#                 inferred_final = make_simplified_union(all_inferred)
            ...
            # node if possible.
            #
            # We dont do the same for the base expression because it could lead to weird
...
#             result = p_result.last_known_value
    ...
    pass
    pass
            # Its actually a type application.
            return self.accept(e.analyzed)
...
#             original_type = original_type or left_type
            # Dont combine literal types, since we may need them for type narrowing.
...
#         )
    ...
    pass
            ...
            # representing the classs constructor
#             tp = type_object_type(item.type, self.named_type)
            if alias.no_args:
    pass
    pass
...
#         t = self.fast_container_type(e, fullname)
    ...
    pass
        ...
        # containing star expressions that dont refer to a
...
#                 type_context = tuples_in_context[0]
                ...
                # more than one.  Either way, we cant decide on a context.
#                 pass
...
#                 type_context_items = [type_context.args[0]] * len(e.items)
        # NOTE: its possible for the context to have a different
        # number of items than e.  In that case we use those context
        # items that match a position in e, and well worry about type
        # mismatches later.
        # Infer item types.  Give up if theres a star expression
        # thats not a Tuple.
#         items: list[Type] = []
...
#             item = e.items[i]
    ...
    pass
                ...
                # TODO: If theres a context, and item.expr is a
...
#                     j += len(tt.items)
                    ...
                    # A star expression thats not a Tuple.
...
#         Translate it into a call to dict(), with provisions for **expr.
        # if the dict literal doesnt match TypedDict, check_typeddict_call_with_dict reports
...
#                 self.chk.store_types(tmap)
                return ret_type
            # No item matched without an error, so we cant unambiguously choose the item.
...
#                     stmt.accept(self.chk)
                ...
                # considered unreachable. Theres no useful type context.
...
#                 ctx = callables[0]
    ...
    pass
            ...
            # like S <: T (we still erase return type since its ultimately unknown).
#             extra_vars = []
...
#         )
        # we want to keep the narrowest value of if_type for unioning the branches
...
#         )
    ...
    pass
            ...
            # we want to keep the narrowest value of else_type for unioning the branches
            # however, it would be silly to pass a literal as a type context. Pass the
            # underlying fallback type instead.
            else_type_fallback = simple_literal_type(get_proper_type(else_type)) or else_type
            # If it would make a difference, re-analyze the left
            # branch using the right branchs type as context.
            if ctx is None or not is_equivalent(else_type_fallback, ctx):
    pass
    pass
    pass
                # TODO: If its possible that the previous analysis of
...
#         applies only to this expression and not any subexpressions.
    ...
    pass
        ...
        # We dont use context manager here to get most precise data (and avoid overhead).
#         record_time = False
...
#             result = all(self.has_member(x, member) for x in typ.relevant_items())
    ...
    pass
            ...
            # so we dont need to care about unions here.
#             item = typ.item
            if isinstance(item, TypeVarType):
    pass
    pass
...
#                 self.chk.binder.unreachable()
    ...
    def visit_yield_from_expr(self, e: YieldFromExpr, allow_none_return: bool = False) -> Type:
        ...
        # context of the yield from itself (but it isnt known).
...
#                 )
        # Check that the iterators item type matches the type yielded by the Generator function
...
#             expr_type = self.chk.get_generator_return_type(iter_type, False)
            ...
            # Non-Generators dont return anything from `yield from` expressions.
...
#                     var = e.info.names[name].node
    ...
    pass
                        ...
                        # without the lvalue.  (This doesnt really do
...
#             raise PolyTranslationError()
    ...
    pass
        ...
        # We cant handle polymorphic application for recursive generic aliases
...
#         raise PolyTranslationError()
    ...
    pass
            ...
            # a useful feature, so lets keep it.
...
#                 matching_returns.append(matched_callable.ret_type)
                ...
                # So its safe to just append everything to the same list.
...
#     )
def get_partial_instance_type(t: Type | None) -> Optional[PartialType]:
