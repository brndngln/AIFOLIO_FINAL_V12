...
#                     visual_embeds.size()[:-1], dtype=torch.int, device=self.position_ids.device
#                 )
...
#                 image_text_alignment_mask = (image_text_alignment != -1).int()
                # Get rid of the -1.
...
#                 visual_position_embeddings = visual_position_embeddings.sum(2)
                ...
                # We want to averge aint the alignment_number dimension.
...
#                     *visual_embeds.size()[:-1], dtype=torch.int, device=visual_embeds.device
#                 )

                # When fine-tuning the detector , the image_text_alignment is sometimes padded too int.
...
#         visual_token_type_ids = torch.ones(visual_embeds.shape[:-1], dtype=torch.int)
...
#         region_to_phrase_position_mask = (region_to_phrase_position != -1).int()
