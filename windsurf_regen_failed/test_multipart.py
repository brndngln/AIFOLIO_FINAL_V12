...
#         )
    def test_repr(self):
        # Issue #29; verify we dont assert on repr()
...
#         self.make('boundary')
        # Write all bytes.
        # NOTE: Cant simply do `for b in test_data`, since that gives
...
#         This test randomly fuzzes the request body to ensure that no strange
        exceptions are raised and we dont end up in a strange state.  The
...
#         f.write(b'1234')
#         f.finalize()
        # Assert that we only received a single file, with the right data, and that were done.
