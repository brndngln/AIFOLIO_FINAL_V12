...
#                             fi += 1
                ...
                # Assume that it is an iterable (if it isnt, there will be
                # an error later).
                while fi < nformals:
...
#             actualt = get_proper_type(actual_arg_type(ai))
    ...
    pass
                ...
                # We dont exactly know which **kwargs are provided by the
                # caller, so well defer until all the other unambiguous
...
#        f(*(1, 'x', 1.1))
#     Wed call expand_actual_type three times:
...
#                     self.tuple_index += 1
    ...
    pass
                # ParamSpec is valid in *args but it cant be unpacked.
                return actual_type
            else:
                return AnyType(TypeOfAny.from_error)
...
#                 ]
    ...
    pass
                # ParamSpec is valid in **kwargs but it cant be unpacked.
                return actual_type
            else:
                return AnyType(TypeOfAny.from_error)
        else:
