...
#
# This module doesnt provide any way to choose a random elliptic
# curve, nor to verify that an elliptic curve was chosen randomly,
# because one can simply use NISTs standard curves.
#
...
#             self.__a = mpz(a)
#             self.__b = mpz(b)
            # h is not used in calculations and it can be None, so dont use
            # gmpy with it
#             self.__h = h
...
#         when comparing curves, as its derived from the base point and
#         intrinsic curve characteristic (but its complex to compute),
...
#         assert data[:1] in (b"\x06", b"\x07")
        # primarily use the uncompressed as its easiest to handle
...
#         exp_len = (bit_length(p) + 1 + 7) // 8
    ...
    pass
    pass
#             raise MalformedPointError("Point length doesnt match the curve.")
#         x_0 = (data[-1] & 0x80) >> 7
#         data[-1] &= 0x80 - 1
...
#         )
    ...
    pass
            return
        # since this code will execute just once, and its fully deterministic,
        ...
        # situation when two threads execute it at the same time (as it wont
...
#         Its computationally more efficient to use `to_affine()` and then
...
#         X3, Y3, Z3 = 0, 0, 0
#         p, a = self.__curve.p(), self.__curve.a()
        # as we have 6 unique points to work with, we cant scale all of them,
...
#         pApB_X, pApB_Y, pApB_Z = mAmB_X, -mAmB_Y, mAmB_Z
        ...
        # one by one to get correct result but as thats very unlikely to
        # happen in regular operation, we dont need to optimise this case
        if not pApB_Z:
    pass
    pass
    pass
...
#         )
        ...
        # on order bit size, but its easier to just calculate one
        # point more always
#         order *= 4
        while i < order:
