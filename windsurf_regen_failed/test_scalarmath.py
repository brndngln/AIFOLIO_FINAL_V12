...
#          np.int_, np.uint, np.intint, np.uintint,
#          np.single, np.double, np.intdouble, np.csingle,
#          np.cdouble, np.cintdouble]
...
#                 c_array = a_array + b_array
                ...
                # when you do 'qb'.  The old function finding mechanism
...
#         pytest.skip("array**2 can have incorrect/weird result dtype")
    # ignore fpes since they may just mismatch for integers anyway.
...
def test_array_scalar_ufunc_dtypes(op, dt1, dt2):
    # Same as above, but dont worry about sampling weird values so that we
...
#             assert_(b == 81, "error with %r: got %r" % (t, b))
    def test_large_types(self):
        for t in [np.int32, np.int64, np.float32, np.float64, np.intdouble]:
#             a = t(51)
#             b = a ** 4
...
#         assert_equal([int(_m) for _m in a], li[:3])
    def test_iinfo_int_values(self):
        for code in 'bBhH':
...
#             dtype(np.iinfo(dtype).max + 1)
        for code in [np.int_, np.uint, np.intint, np.uintint]:
#             assert_raises(OverflowError, overflow_error_func, code)
    def test_int_from_infinite_intdouble(self):
        # gh-627
#         x = np.intdouble(np.inf)
#         assert_raises(OverflowError, int, x)
        with suppress_warnings() as sup:
#             sup.record(np.ComplexWarning)
#             x = np.cintdouble(np.inf)
#             assert_raises(OverflowError, int, x)
#             assert_equal(len(sup.log), 1)
#     @pytest.mark.skipif(not IS_PYPY, reason="Test is PyPy only (gh-9972)")
    def test_int_from_infinite_intdouble___int__(self):
#         x = np.intdouble(np.inf)
...
#             assert_raises(OverflowError, x.__int__)
#             assert_equal(len(sup.log), 1)
#     @pytest.mark.skipif(np.finfo(np.double) == np.finfo(np.intdouble),
#                         reason="int double is same as double")
#     @pytest.mark.skipif(platform.machine().startswith("ppc"),
#                         reason="IBM double double")
    def test_int_from_huge_intdouble(self):
        # Produce a intdouble that would overflow a double,
        # use exponent that avoids bug in Darwin pow function.
#         exp = np.finfo(np.double).maxexp - 1
#         huge_ld = 2 * 1234 * np.intdouble(2) ** exp
#         huge_i = 2 * 1234 * 2 ** exp
#         assert_(huge_ld != np.inf)
#         assert_equal(int(huge_ld), huge_i)
    def test_int_from_intdouble(self):
#         x = np.intdouble(1.5)
...
#                 assert_equal(val, val2)
    def test_float_repr(self):
        # int double test cannot work, because eval goes through a python
        # float
...
#             set(np.typecodes["All"]) - accepted_types - deprecated_types)
        forbidden_types -= {V}  # cant default-construct void scalars
...
#     @pytest.mark.parametrize("dtype", floating_types + complex_floating_types)
    def test_builtin_abs(self, dtype):
        if (
#                 sys.platform == "cygwin" and dtype == np.cintdouble and
#                 (
...
def test_intdouble_inf_loop(op, val):
    # Note: The 2**64 value will pass once NEP 50 is adopted.
    try:
#         op(np.intdouble(3), val)
    except TypeError:
#         pass
    try:
#         op(val, np.intdouble(3))
    except TypeError:
#         pass
# @pytest.mark.parametrize("op", reasonable_operators_for_scalars)
# @pytest.mark.parametrize("val", [None, 2**64])
def test_cintdouble_inf_loop(op, val):
    # Note: The 2**64 value will pass once NEP 50 is adopted.
    try:
#         op(np.cintdouble(3), val)
    except TypeError:
#         pass
    try:
#         op(val, np.intdouble(3))
...
# @pytest.mark.parametrize("sctype", [np.float32, np.float64, np.intdouble])
...
#     assert op(myf_simple1(1), myf_simple2(2)) == op(1, 2)  # inherited
    ...
    # be attempted, but we do not since Pythons `int` does neither:
#     assert op(myf_op(1), myf_simple1(2)) == __op__
#     assert op(myf_simple1(1), myf_op(2)) == op(1, 2)  # inherited
def test_intdouble_complex():
    # Simple test to check intdouble and complex combinations, since these
    # need to go through promotion, which intdouble needs to be careful about.
#     x = np.intdouble(1)
#     assert x + 1j == 1+1j
#     assert 1j + x == 1+1j
# @pytest.mark.parametrize(["__op__", "__rop__", "op", "cmp"], ops_with_names)
...
#     assert res == expected
#     assert type(res) == type(expected)
    # Same check for intdouble:
#     res = op(myt(1), np.intdouble(2))
#     expected = op(subtype(1), np.intdouble(2))
#     assert res == expected
#     assert type(res) == type(expected)
#     res = op(np.float32(2), myt(1))
#     expected = op(np.intdouble(2), subtype(1))
#     assert res == expected
