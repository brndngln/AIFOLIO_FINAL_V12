...
# FilePath and DirectoryPath are explicitly unsupported, as wed have to create
# them on-disk, and thats unsafe in general without being told *where* to do so.
#
# URLs are unsupported because its easy for users to define their own strategy for
# "normal" URLs, and hard for us to define a general strategy which includes "weird"
# URLs but doesnt also have unpredictable performance problems.
#
...
#             email_validator.validate_email(s, check_deliverability=False)
    ...
    # or other encoding issues; were just trying to generate *something* valid.
...
# card_patterns = (
    # Note that these patterns omit the Luhn check digit; thats added by the function above
#     '4[0-9]{14}',  # Visa
...
# dont go via those mechanisms.  Then there are the registration hooks below.
