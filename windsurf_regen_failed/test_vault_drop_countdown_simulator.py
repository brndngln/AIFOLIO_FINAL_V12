...
# parent_dir = os.path.dirname(os.path.dirname(current_dir)) # This should be aifolio_empire
...
#         self.test_target_datetime_future_obj = datetime.now(timezone.utc) + timedelta(days=5)
#         self.test_target_datetime_past_obj = datetime.now(timezone.utc) - timedelta(days=5)
        # ISO strings formatted with 'Z' for passing to the simulator
#         self.test_target_datetime_future_iso = self.test_target_datetime_future_obj.isoformat().replace('+00:00', 'Z')
#         self.test_target_datetime_past_iso = self.test_target_datetime_past_obj.isoformat().replace('+00:00', 'Z')
    def test_simulator_initialization(self):
#         self.assertIsNotNone(self.simulator)
    def test_get_simulated_countdown_future_basic_structure(self):
#         result = self.simulator.get_simulated_countdown(self.test_target_datetime_future_iso, self.test_vault_id)
#         self.assertIn("action_id_sim", result)
#         self.assertTrue(result["action_id_sim"].startswith(f"countdown_{self.test_vault_id}_"))
...
#         self.assertEqual(result["target_time_utc_sim"], self.test_target_datetime_future_iso)
...
#         result = self.simulator.get_simulated_countdown(self.test_target_datetime_past_iso, self.test_vault_id)
...
#         self.assertTrue(result["display_text_sim"] in ["DROP IS LIVE!", "EVENT ACTIVE!"])
...
#         self.assertIsInstance(countdown_data["total_seconds_remaining_sim"], int) # Should be int after casting in simulator
...
#         result1 = self.simulator.get_simulated_countdown(self.test_target_datetime_future_iso, self.test_vault_id)
#         result2 = self.simulator.get_simulated_countdown(self.test_target_datetime_future_iso, self.test_vault_id)
#         self.assertNotEqual(result1["action_id_sim"], result2["action_id_sim"])
    def test_invalid_datetime_format(self):
#         invalid_dt_string = "not-a-datetime"
#         result = self.simulator.get_simulated_countdown(invalid_dt_string, self.test_vault_id)
#         self.assertIn("error_sim", result)
#         self.assertEqual(result["error_sim"], "Invalid target datetime format provided.")
#         self.assertEqual(result["raw_target_sim"], invalid_dt_string)
#         self.assertFalse(result["countdown_active_sim"])
#         self.assertEqual(result["vault_id_sim"], self.test_vault_id) # Assuming vault_id is still correctly passed through in error cases
...
#             self.assertEqual(simulator_module.config.__class__.__name__, "MockConfig", "Simulator module should be using MockConfig.")
...
#             result = temp_simulator.get_simulated_countdown(self.test_target_datetime_future_iso, self.test_vault_id)
#             self.assertIn("action_id_sim", result) # Basic check that it produced output
            # Verify a value from MockConfig to be sure
#             self.assertEqual(simulator_module.config.SIM_COUNTDOWN_MAX_JITTER_SECONDS, 5)
#         finally:
...
#                 simulator_module.config = original_module_config
            elif hasattr(simulator_module, config): # If it was set by us but wasnt there before
#                 delattr(simulator_module, 'config')
            # Optional: Reload the module to ensure its in a clean state for other tests,
            # though direct restoration above should be sufficient if MockConfig isnt global.
