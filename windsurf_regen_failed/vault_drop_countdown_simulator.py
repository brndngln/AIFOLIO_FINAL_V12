...
#         logger.info("VaultDropCountdownSimulator initialized. Operations stateless.")
    def get_simulated_countdown(self, target_datetime_utc_iso_original: str, vault_id_sim: str) -> Dict[str, Any]:
...
#             target_time_utc = datetime.fromisoformat(target_datetime_utc_iso.replace('Z', '+00:00'))
            if target_time_utc.tzinfo is None:
    pass
    pass
    pass
#                  target_time_utc = target_time_utc.replace(tzinfo=timezone.utc) # Ensure tz aware
        except ValueError as e:
#             logger.error(f"Invalid target_datetime_utc_iso format: {target_datetime_utc_iso_original}. Error: {e}") # Log original
            return {
...
#                 "vault_id_sim": vault_id_sim,
#                 "countdown_active_sim": False,
#                 "display_text_sim": "DROP IS LIVE!" if random.random() > 0.1 else "EVENT ACTIVE!",
#                 "details_sim": {
#                     "days_sim": 0,
#                     "hours_sim": 0,
#                     "minutes_sim": 0,
...
#                 "anti_sentience_notes_sim": "; ".join(anti_sentience_notes) if anti_sentience_notes else "Standard simulation."
#             }
...
#                 "minutes_sim": minutes,
#                 "seconds_sim": seconds,
#                 "total_seconds_remaining_sim": int(time_remaining.total_seconds()) if time_remaining.total_seconds() > 0 else 0
#             },
#             "target_time_utc_sim": target_datetime_utc_iso,
#             "status_message_sim": status_message_sim,
#             "glitch_message_sim": glitch_message_sim,
#             "jitter_applied_seconds_sim": jitter_seconds,
#             "current_datetime_utc_sim": current_time_utc.isoformat().replace('+00:00', 'Z'),
#             "anti_sentience_notes_sim": "; ".join(anti_sentience_notes) if anti_sentience_notes else "Standard simulation."
#         }
# Example Usage:
...
#     future_time = datetime.now(timezone.utc) + timedelta(days=1, hours=2, minutes=30, seconds=15)
...
#     short_target_iso = short_future_time.isoformat().replace('+00:00', 'Z')
    for i in range(5):
#         print(f"  Call {i+1}:")
#         cd_short = simulator.get_simulated_countdown(short_target_iso, f"delta_promo_run_{i}")
#         print(f"    Display: {cd_short.get('display_text_sim')}, Status: {cd_short.get('status_message_sim')}, Jitter: {cd_short.get('jitter_applied_seconds_sim')}s")
        if i < 4 : time.sleep(0.5) # Small delay between simulated calls
