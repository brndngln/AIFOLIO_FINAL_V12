...
#     full_tokens = full_tokens[0]
    ...
    pass
#         tokens = torch.cat(
#             [torch.zeros(max_n_lyric_tokens - len(full_tokens), dtype=torch.int).to(full_tokens.device), full_tokens]
#         )
...
#             music_tokens.append(sampled_tokens)
      ...
      pass
    pass
                # Be extra paranoid and make sure the encoder weights cant
...
#         The commit loss, which ensure that the encoders computed embeddings are close to the codebook vectors, is
#         computed.
...
#         batch_size = tokens.shape[0]
        with torch.no_grad():
#             tokens = tokens.view(batch_size, -1).int()
...
#         batch_size = lyric_and_music_tokens.shape[0]
        with torch.no_grad():
#             lyric_and_music_tokens = lyric_and_music_tokens.view(batch_size, -1).int()
...
#     The `JukeboxMusicTokenConditioner` takes music tokens as an input (coresponding to the codes of the VQVAEs
...
#         bins_ = (self.embed_dim * normalised_position).floor().int().detach()
...
#                 (labels.shape[0], self.nb_relevant_lyric_tokens), dtype=torch.int, device=labels.device
#             )
...
#         Extracts current levels conditioning music tokens.
...
#                 music_tokens_cond = torch.cat((music_tokens_cond, init_cond), dim=-1).int()
...
#         Encodes the hidden states (raw audio) using the VQVAEs encoder. Returns latent_states.
...
#         >>> set_seed(0)
#         >>> zs = [torch.zeros(1, 0, dtype=torch.int) for _ in range(3)]
...
#         music_tokens = [
#             torch.zeros(n_samples, 0, dtype=torch.int, device=labels[0].device) for _ in range(len(self.priors))
#         ]
