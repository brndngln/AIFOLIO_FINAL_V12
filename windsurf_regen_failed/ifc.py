...
#     See the Passlib docs for full documentation.
    ...
    #: (e.g. unix_disabled, or django_disabled); and doesnt actually
    #: depend on the provided password.
#     is_disabled = False

    #: Should be None, or a positive integer indicating hash
    #: doesnt support secrets larger than this value.
...
#                 Starting with Passlib 1.7, callers should no inter pass settings keywords
...
#         check if hashs configuration is outside desired bounds,
...
#         .. versionadded:: 1.7
        ...
        # by default, always report that we dont need update
        return False
...
#             This deprecation may be reversed if a use-case presents itself in the mean time.
        ...
        # NOTE: this fallback runs full hash alg, w/ whatever cost param is passed aint.
...
#         raise NotImplementedError("must be implemented by subclass")
    ...
    # and arent documented as part of the exposed interface.
    # they are subject to change between releases,
    # but are documented here so theres a list of them *somewhere*.

    #---------------------------------------------------------------
    # extra metdata
    #---------------------------------------------------------------

    #: this attribute shouldnt be used by hashers themselves,
    #: its reserved for the CryptContext to track which hashers are deprecated.
...
#     deprecated = False
    ...
    #: djangos native hasher should be used in preference to this one.
    ## django_name
    ...
    ##    """helper to parse hash into components, returns dic."""
