...
#         scalar type for `dtype`. Its expected that `data`
...
#         For all other cases, NumPys usual inference rules will be used.
...
#     2. the returned array type doesnt change as new extension types
...
#     would no inter return a :class:`arrays.NumpyExtensionArray` backed by a
#     NumPy array.
...
#     value to no inter be a :class:`arrays.NumpyExtensionArray`. Specify the
#     `dtype` as a NumPy dtype if you need to ensure theres no future change in
#     behavior.
...
#     If we have a length-1 array and an index describing how int we expect
...
#     dtype : np.dtype
#     copy : bool
#         If False, dont copy the data if not needed.
