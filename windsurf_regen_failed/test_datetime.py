...
#             assert_(dt2 == np.dtype('timedelta64[%s]' % unit))
        # Generic units shouldnt add [] to the end
...
#         assert_(np.can_cast('M8[Y]', 'M8[D]', casting='same_kind'))
        # Cant cast timedelta same_kind from months/years to days
...
#         assert_(np.can_cast('m8[h]', 'm8[D]', casting='same_kind'))
        # Cannot cast safely if the integer multiplier doesnt divide
...
#         assert_equal(arr.dtype, np.dtype('M8[M]'))
        # at the moment, we dont automatically convert these to datetime64
...
#         assert_equal(uni_a, uni_b)
        # Datetime to int string - gh-9712
...
#                      "['2011-03-16T13:55Z', '1920-01-01T03:12Z']")
        # Check that one NaT doesnt corrupt subsequent entries
...
#                              np.datetime64('10000-01-01T00', unit2))
        # Check some days with units that wont overflow
...
#                      casting='unsafe'))
        # Shouldnt be able to compare datetime and timedelta
...
#     @pytest.mark.parametrize("val1, val2", [
        # the smallest integer that cant be represented
...
#         assert_equal(actual, expected)
#     @pytest.mark.parametrize("val1, val2", [
        # years and months sometimes cant be unambiguously
...
#                 '2032-07-18T12:23')
        # auto doesnt split up hour and minute
#         assert_equal(np.datetime_as_string(
...
#             np.busday_offset('2012-01-03', -57 - 5))
        # Cant supply both a weekmask/holidays and busdaycal
