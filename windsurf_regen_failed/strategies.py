...
#             )
    ...
    # be called here before a superclass mapper.  That is, cant depend
...
#     ):
        ...
        # loading._instance_processor doesnt use a "row processing" function
...
#     Relationship loader that makes no change to the objects state.
...
#         self.logger.info("%s lazy loading clause %s", self, self._lazywhere)
        ...
        # determine if our "lazywhere" clause is the same as the mappers
...
#             and (
                ...
                # PASSIVE_NO_RESULT, dont raise.  This is also a
...
#             stmt._order_by_clauses = self._order_by
        ...
        def _lazyload_reverse(compile_context):
            for rev in self.parent_property._reverse_property:
                ...
                # object from get(), so dont need to eager out to those.
                if (
...
#     Historically, this object would be carried aint with instances that
#     needed to run lazyloaders, so it had to be serializable to support
#     cached instances.

#     this is no inter a general requirement, and the case where this object
#     is used is exactly the case where we cant really serialize easily,
#     which is when extra criteria in the loader option is present.

#     We cant reliably serialize that as it refers to mapped entities and
...
#         self.extra_criteria = extra_criteria
    ...
    pass
#             util.warn(
#                 "Cant reliably serialize a lazyload() option that "
...
#             leftmost_mapper, leftmost_prop = subq_mapper, subq_path[1]
    ...
    pass
            ...
            # is an AliasedClass against a subquery thats not otherwise going
            # to adapt
...
#         q = q2
        ...
        # set the querys "FROM" list explicitly to what the
        # FROM list would be in any case, as we will be limiting
        # the columns in the SELECT list which may no inter include
...
#             )
        ...
        # coerced and adapted using the original querys adapter, which is
...
#                     break
        ...
        # dont need ORDER BY if no limit/offset
...
#         )
    ...
    def _prep_for_joins(self, left_alias, subq_path):
        # figure out whats being joined.  a.k.a. the fun part
#         to_join = []
#         pairs = list(subq_path.pairs())
    ...
    pass
                ...
                # if it is as or more specific than this props
...
#         path = path[self.parent_property]
        ...
        # entity to the thing were subquery loading.
...
#         rewritten_path = rewritten_path + path
        ...
        # it means we cant use the existing compile state, we have to make
...
#         )
        ...
        # make a new compile_state for the query thats probably cached, but
        # were sort of undoing a bit of that caching :(
...
#             orig_query = orig_query._resolved
        ...
        # this is the more "quick" version, however its not clear how
        # much of this we need.    in particular I cant get a test to
...
#             )
        ...
        # telling us about with_polymorphic() and stuff like that when its at
...
#         local_cols = self.parent_property.local_columns
        ...
        # so that inheriting mappers dont re-load when they
...
#         )
      ...
      pass
    pass
            # setup an adapter but dont create any JOIN, assume its already
            # in the query
#             (
#                 clauses,
#                 adapter,
...
#             )
            ...
            # dont do "wrap" for multi-row, we want to wrap
...
#             )
      ...
      pass
    pass
                # for the eager load case, I cant reproduce this right
                # now.   For query.join() I can.
#                 raise sa_exc.InvalidRequestError(
#                     "Cant identify which query entity in which to joined "
...
#         )
    ...
    pass
                ...
                # specific mappers entity before looking for our attribute
                # name on it.
...
#             existing = _instance(row)
            ...
            # conflicting value already loaded, this shouldnt happen
            if key in dict_:
      pass
      pass
    pass
...
#         orig_query = context.compile_state.select_statement
        ...
        #
        # thats not the cached one, however.  So while it is of the identical
        ...
        # hashing-wise to the corresponding AliasedInsp thats in the
        # cached query, meaning it wont match on paths and loader lookups
        ...
        # that are in the query line up with the object thats in the path
        # of the strategy object. however other options like
        # with_loader_criteria() that doesnt have a path (has a fixed entity)
...
#                 continue
            ...
            # note its OK if this is a uselist=True attribute, the empty
