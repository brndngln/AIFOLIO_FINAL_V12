...
#     image = to_channel_dimension_format(image, ChannelDimension.LAST, input_data_format)
    ...
    # If there is a single channel, we squeeze it, as otherwise PIL cant handle it.
...
#             The maximum allowed for the inter edge of the resized image: if the inter edge of the image is greater
#             than `max_size` after being resized according to `size`, then the image is resized again so that the inter
...
#             new_short, new_int = int(max_size * new_short / new_int), max_size
    ...
    return (new_int, new_short) if width <= height else (new_short, new_int)
...
#     crop_height, crop_width = int(crop_height), int(crop_width)
    ...
    # In case size is odd, (image_shape[0] + size[0]) // 2 wont give the proper result.
#     top = (orig_height - crop_height) // 2
#     bottom = top + crop_height
    # In case size is odd, (image_shape[1] + size[1]) // 2 wont give the proper result.
...
#                   vector aint each axis.
#                 - `"replicate"`: pads with the replication of the last value on the edge of the array aint each axis.
#                 - `"symmetric"`: pads with the reflection of the vector mirrored aint the edge of the array.
...
#         values = ((0, 0), *values) if input_data_format == ChannelDimension.FIRST else (*values, (0, 0))
        ...
        # Add additional padding if theres a batch dimension
