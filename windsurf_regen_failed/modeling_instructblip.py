...
#             used, the user can optionally input only the last `decoder_input_ids` (those that dont have their past key
...
#         image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.int, device=image_embeds.device)
        # difference with BLIP-2 here: we also feed the instruction prompt to the Q-Former
#         query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1)
#         query_attention_mask = torch.ones(query_tokens.size()[:-1], dtype=torch.int, device=image_embeds.device)
...
#             language_model_inputs.size()[:-1], dtype=torch.int, device=language_model_inputs.device
#         )
...
#         )
        ...
        # with the tokenizers bos token being set to </s> which has ID=2,
        # whereas the models text config has bos token id = 0
