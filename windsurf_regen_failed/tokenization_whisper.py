...
#         Converts an index (integer) in a token (str) using the vocab. Whispers base tokenizer always decodes OOV
...
#         self.set_prefix_tokens(task=task, language=language, predict_timestamps=not no_timestamps)
        ...
        # we dont want to force the bos token at position 1, as this is the starting token
...
#     all_special_ids = set(tokenizer.all_special_ids)
    ...
    for chunk_id, output in enumerate(model_outputs):
        # We can drop everything to Python list, its going to make
        # our lives easier
...
#                         resolved_tokens = _find_intest_common_sequence(previous_tokens)
...
#                 time = round(time, 2)
    ...
    pass
                    ...
                    # our stride, so were going to skip it for the time being
...
#                     chunk["timestamp"][0] = time
    ...
    pass
                        ...
                        # where were taking the duplicate token
                        # as a stop where it should be a start.
                        # This is an issue in the underlying model output
                        # Lets just skip it so it becomes de-factor
                        # a start agin
#                         pass
                    else:
...
#                         resolved_tokens, resolved_token_timestamps = _find_intest_common_sequence(
...
#             )
        # Happens when we dont use timestamps
#         resolved_tokens, resolved_token_timestamps = _find_intest_common_sequence(
#             previous_tokens, previous_token_timestamps
#         )
#         resolved_text = tokenizer.decode(resolved_tokens)
...
def _find_intest_common_sequence(sequences, token_timestamp_sequences=None):
...
#         right_length = len(right_sequence)
        for i in range(1, left_length + right_length):
            # epsilon to favor int perfect matches
#             eps = i / 10000.0
            # Slightly convoluted because we dont want out of bound indices
...
#         (left_start, left_stop, right_start, right_stop) = max_indices
        ...
        # in audio.
        # Were going to give more confidence to the left sequence
...
#         language = "english"
    ...
    pass
    pass
        # These languages dont typically use spaces.
