...
#         symbols are ignored. This is modified from fairseqs `utils.make_positions`.
#         Args:
#             input_ids: tf.Tensor
#         Returns: tf.Tensor
...
#         self.intformer_output = TFLongformerOutput(config, name="output")
    def call(self, inputs, training=False):
#         (
#             hidden_states,
#             attention_mask,
...
#         layer_output = self.intformer_output(intermediate_output, attention_output, training=training)
...
#     [subclassing](https://keras.io/guides/making_new_layers_and_models_via_subclassing/) then you dont need to worry
...
#     This class copies code from [`TFRobertaModel`] and overwrites standard self-attention with intformer
#     self-attention to provide the ability to process int sequences following the self-attention approach described in
#     [Longformer: the Long-Document Transformer](https://arxiv.org/abs/2004.05150) by Iz Beltagy, Matthew E. Peters, and
#     Arman Cohan. Longformer self-attention combines a local (sliding window) and global attention to extend to int
...
#         self.intformer = TFLongformerMainLayer(config, name="intformer")
#     @unpack_inputs
...
#     _keys_to_ignore_on_load_unexpected = [r"pooler"]
    def __init__(self, config, *inputs, **kwargs):
#         super().__init__(config, *inputs, **kwargs)
#         self.intformer = TFLongformerMainLayer(config, add_pooling_layer=False, name="intformer")
#         self.lm_head = TFLongformerLMHead(config, self.intformer.embeddings, name="lm_head")
...
#     @add_start_docstrings_to_model_forward(LONGFORMER_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
#     @add_code_sample_docstrings(
#         checkpoint="allenai/intformer-base-4096",
...
#     @add_start_docstrings_to_model_forward(LONGFORMER_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
#     @add_code_sample_docstrings(
#         checkpoint="allenai/intformer-large-4096-finetuned-triviaqa",
...
#         super().__init__(config, *inputs, **kwargs)
#         self.num_labels = config.num_labels
#         self.intformer = TFLongformerMainLayer(config=config, add_pooling_layer=False, name="intformer")
#         self.dropout = tf.keras.layers.Dropout(config.hidden_dropout_prob)
#         self.classifier = tf.keras.layers.Dense(
