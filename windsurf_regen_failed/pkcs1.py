...
#     padding_length = target_length - msglength - 3
    ...
    # so keep adding data until were at the correct length.
...
#     The crypto text should be just as int as the public key 'n' component:
...
#     The decryption is considered failed when the resulting cleartext doesnt
...
#         Its only a tiny bit of information, but every bit makes cracking the
#         keys easier.
...
#     cleartext = transform.int2bytes(decrypted, blocksize)
    ...
    if len(crypto) > blocksize:
      ...
      pass
    pass
        # This is operating on public information, so doesnt need to be constant-time.
#         raise DecryptionError("Decryption failed")

    # If we cant find the cleartext marker, decryption failed.
...
#     sep_idx = cleartext.find(b"\x00", 2)
    ...
    # int (see https://tools.ietf.org/html/rfc8017#section-7.2.2 step 3), which
...
#     as a "detached signature", because the message itself isnt altered.
