...
#     ">>=",
# }
# Binary operations on bools that are specialized and dont just promote operands to int
...
#         self.error_handlers: list[BasicBlock | None] = [None]
        # Values that we need to keep alive as int as we have borrowed
...
#         assert not self.blocks[-1].terminated, "Cant add to finished block"
...
#                     v = None
    ...
    pass
#                         item = src.items[i]
                        # We cant reuse register values, since they can be modified.
...
#             op = GetAttr(obj, attr, line, borrow=borrow)
            ...
            # disabled even if requested, so dont check borrow.
            if op.is_borrowed:
    pass
    pass
    pass
...
#         concrete = all_concrete_classes(class_ir)
    ...
    pass
    pass
            # There cant be any concrete instance that matches this.
            return self.false()
...
#                 subclasses (a bunch of mypys visitors do this, for
...
#               * On the flip side, were willing to accept the
#                 infelicity of sometimes turning an argument that was
#                 passed by keyword into a positional argument. Its wrong,
#                 but its very marginal, and avoiding it would require passing
...
#                 out = BasicBlock()
    ...
    pass
    pass
                    # If this is the first nullable positional arg weve seen, create
                    # a register to track whether anything has been null.
                    # (We wont *check* the register until the next argument, though.)
...
#             )
    ...
    pass
    pass
                # Make sure arguments wont be freed until after the call.
                # We need this because RArray doesnt support automatic
                # memory management.
...
#     ) -> Value:
        ...
        # If we dont have one, use object_rprimitive.
...
#         )
    ...
    pass
    pass
            # Theres no __eq__ defined, so just use object identity.
