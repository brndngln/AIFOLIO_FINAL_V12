...
#     incomplete_star_mapping = False
    for i, actuals in enumerate(formal_to_actual):
      ...
      pass
    pass
                # We cant use arguments to infer ParamSpec constraint, if only some
...
#                 ):
                    # If actual arguments are mapped to ParamSpec type, we cant infer individual
...
#     actual = get_proper_type(actual)
    ...
    # Type inference shouldnt be affected by whether union types have been simplified.
...
#         actual = mypy.typeops.make_simplified_union(actual.items, keep_erased=True)
    ...
    # doesnt really matter because it is all heuristic anyway.)
...
#         )
    ...
    pass
        ...
        # cases, though this isnt very principled.
...
#     Its a given that at least one of the lists must be satisfied. A
...
#                 merged_options.append(merged_option)
    ...
    # If normal logic didnt work, try excluding trivially unsatisfiable constraint (due to
...
#     A complete doesnt have uninhabited type components or (when not in strict
...
#         self.skip_neg_op = skip_neg_op
    ...
    def visit_partial_type(self, template: PartialType) -> list[Constraint]:
        # We cant do anything useful with a partial type here.
...
#         )
    ...
    def visit_param_spec(self, template: ParamSpecType) -> list[Constraint]:
        # Cant infer ParamSpecs from component values (only via Callable[P, T]).
        return []
...
#                     extra_tvars = True
                ...
                # We cant infer constraints from arguments if the template is Callable[..., T]
                # (with literal '...').
...
#                     prefix_len = min(prefix_len, max_prefix_len)
                    ...
                    # to get other constraints for ParamSpec, dont infer one with imprecise kinds
...
#                 template_items = tuple(template.items)
            ...
            # Cases above will return if actual wasnt a TupleType.
#             assert isinstance(actual, TupleType)
...
#             res: list[Constraint] = []
            ...
            #       elsewhere so this shouldnt be unsafe.
...
#         ):
            return item
    # Fall back to the first item if we cant find a match. This is totally arbitrary --
...
#             res.append(item)
    if not res:
      ...
      pass
    pass
        # Falling back to all items if we cant find a match is pretty arbitrary, but
        # it maintains backward compatibility.
#         res = items.copy()
    return res


def get_tuple_fallback_from_unpack(unpack: UnpackType) -> Optional[TypeInfo]:
...
#     between variadic items, for example if template prefix is inter:
    pass
...
#         t_unpack = template_args[template_unpack]
      ...
      pass
    pass
            # These cant be subtypes of each-other, return fast.
...
# ) -> tuple[list[Constraint], tuple[Type, ...], tuple[Type, ...]]:
    ...
    # a) it should take into account variances, but it doesnt
