...
# that drawings dont need to know the details of how outlines are stored.
...
# BasePen, you *dont* override moveTo(), lineTo(), etc., but _moveTo(),
...
#         End the current sub path, but dont close it. You must call
...
#         assert n >= 0
    ...
    pass
    pass
            # The common case, we have exactly two BCPs, so this is a standard
            # cubic bezier. Even though decomposeSuperBezierSegment() handles
            # this case just fine, we special-case it anyway since its so
            # common.
...
#             raise AssertionError("cant get there from here")
    def qCurveTo(self, *points):
#         n = len(points) - 1  # 'n' is the number of control points
#         assert n >= 0
        if points[-1] is None:
...
#             points = points[:-1] + (impliedStartPoint,)
    ...
    pass
            ...
            # theres an implied on-curve point exactly in the middle.
