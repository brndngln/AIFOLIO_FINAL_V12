...
#             raise ValueError("You have to specify pixel_values")
        # TF 2.0 image layers cant use NCHW format when running on CPU.
...
#         output_depth = int(num_input_channels / block_size_squared)
        # When the number of output channels >= 2, PyTorchs PixelShuffle and
        # TFs depth_to_space differ in their output as the order of channels selected for combining
...
#         reconstructed_pixel_values = self.decoder(sequence_output, training=training)
        # TF 2.0 image layers cant use NCHW format when running on CPU, so intermediate layers use NHWC,
...
#         logits = self.classifier(sequence_output[:, 0, :])
        # we dont use the distillation token
