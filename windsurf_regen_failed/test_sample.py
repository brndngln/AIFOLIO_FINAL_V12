...
#             obj.sample(n=4, weights=bad_weight_series)
    def test_sample_negative_weights(self, obj):
        # Check wont accept negative weights
#         bad_weights = [-0.1] * 10
...
#             df.sample(n=3, weights="not_a_real_column_name")
        # Check that re-normalizes weights that dont sum to one.
...
#         ser = Series([1, 2, 3])
        # Test numpy array weights wont be modified in place
#         ser.sample(weights=result)
#         tm.assert_numpy_array_equal(result, expected)
        # Test DataFrame column wont be modified in place
