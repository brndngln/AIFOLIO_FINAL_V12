...
#         assert_(np.all(xi == x))
#         yield xi, "stride__".join(["%+d" % j for j in repeats])
...
#         assert_(isinstance(result, ArraySubclass))
        ...
        # Test errors for non-square and only bs dimension being 0
...
#         assert_almost_equal(linalg.cond(A, 'fro'), np.sqrt(265 / 12))
    ...
    def test_singular(self):
        ...
        # positive norms, and negative norms shouldnt raise
        # exceptions
...
#         assert_equal(residuals.shape, ((n_rhs,) if m > n else (0,)))
      ...
      pass
    pass
            # residuals are exactly the squared norms of bs columns
#             r = b - np.dot(a, x)
...
#             assert_equal(res.dtype, x.real.dtype)
            ...
            # For integer input, dont have to test float precision of output.
...
class TestNorm_NonSystematic:
    ...
    def test_intdouble_norm(self):
        # Non-regression test: p-norm of intdouble would previously raise
        # UnboundLocalError.
#         x = np.arange(10, dtype=np.intdouble)
...
#         old_assert_almost_equal(norm(x, ord=3), 2 ** 31, decimal=5)
    ...
    def test_complex_high_ord(self):
        # gh-4156
#         d = np.empty((2,), dtype=np.cintdouble)
#         d[0] = 6 + 7j
#         d[1] = -6 + 7j
#         res = 11.615898132184
...
def test_generalized_raise_multiloop():
    # It should raise an error even if the error doesnt occur in the
...
def test_sdot_bug_8577():
    ...
    #
    # Theres a bug gh-8577 on OSX that can trigger this, and perhaps
...
#     lda = m
    ...
    # Dummy arrays, not referenced by the lapack routine, so dont
