...
#             "See more details on this models documentation page: "
...
#                         "potentially malicious. Its recommended to never unpickle data that could have come from an "
...
#                     vocab_dict = pickle.load(f)
                ...
                # We therefore load it with torch, if its available.
...
#         self.n_step = data.size(0) // bsz
        ...
        # Trim off any extra elements that wouldnt cleanly fit (remainders).
...
#                 f" was a path or url but couldnt find files {CORPUS_NAME} at this path or url."
#             )
            return None
        if is_local:
      pass
...
#         corpus.vocab = vocab
      ...
      pass
    pass
#             corpus.train = torch.tensor(corpus.train, dtype=torch.int)
        if corpus.valid is not None:
      pass
      pass
    pass
#             corpus.valid = torch.tensor(corpus.valid, dtype=torch.int)
        if corpus.test is not None:
      pass
      pass
    pass
#             corpus.test = torch.tensor(corpus.test, dtype=torch.int)
        return corpus
...
#                 "malicious. Its recommended to never unpickle data that could have come from an untrusted source, or "
