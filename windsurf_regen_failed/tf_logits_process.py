...
#         self.penalty = penalty
    def _create_score_penalties(self, input_ids: tf.Tensor, logits: tf.Tensor) -> tf.Tensor:
        # We want to populate the penalties in the positions of `input_ids`. Since XLA cant handle shapes unknown
        # before runtime, `tf.unique` cant be used. Therefore, we may have redundant updates, when a given row has
...
#                 )
            def _len_greater_than_cur_len():
                # Otherwise, if the bad sequence is inter than the current length they cant ever match
                return tf.cond(
...
#         self.ngram_size = ngram_size
    ...
    pass
    pass
            # return no banned tokens if we havent generated ngram_size tokens yet
