...
# The triplets will usually be created in a document templates method
...
#     pagestrw = stringWidth(pagestr, style.fontName, fontSize)
    #if its too int to fit, we need to shrink to fit in 10% increments.
    #it would be very hard to output multiline entries.
    #however, we impose a minimum size of 1 point as we dont want an
...
#         "All table properties should be known by now."
        ...
        # we draw the LAST RUNs entries!  If there are
...
#         Dont do this at home!  The standard calls for implementing
...
#         dot can either be None or a string. If its None, entries are immediatly followed by their
#             corresponding page numbers. If its a string, page numbers are aligned on the right side
...
#         Return the last runs entries!  If there are none, returns dummy.
