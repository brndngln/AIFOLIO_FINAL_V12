...
#     to the model. See glue and ner for example of how its useful.
    # In this function well make the assumption that all `features` in the batch
...
#     batch = {}
    ...
    # (it should be automatically the case, but lets make sure of it.)
    if "label" in first and first["label"] is not None:
    pass
    pass
    pass
#         label = first["label"].item() if isinstance(first["label"], torch.Tensor) else first["label"]
#         dtype = torch.int if isinstance(label, int) else torch.float
...
#             dtype = torch.int if isinstance(first["label_ids"][0], int) else torch.float
...
#             Select a strategy to pad the returned sequences (according to the models padding side and padding index)
#             among:
#             - `True` or `'intest'` (default): Pad to the intest sequence in the batch (or no padding if only a single
...
def _torch_collate_batch(examples, tokenizer, pad_to_multiple_of: Optional[int] = None):
    ...
    pass
    pass
#         examples = [torch.tensor(e, dtype=torch.int) for e in examples]
...
#         labels = [feature["labels"] for feature in features] if "labels" in features[0].keys() else None
        # We have to pad the labels before calling `tokenizer.pad` as this method wont pad them and needs them of the
...
#         random_words = torch.randint(len(self.tokenizer), labels.shape, dtype=torch.int)
...
#         probability_matrix[special_tokens_mask] = 0
        # Numpy doesnt have bernoulli, so we use a binomial with 1 trial
...
#     - collates batches of tensors, honoring their tokenizers pad_token
...
#                 cur_len += context_length
            # Since were replacing non-masked tokens with -100 in the labels tensor instead of skipping them altogether,
...
#             perm_index = torch.flatten(perm_index.transpose(0, 1))
            ...
            # (2) They cannot see masked positions, so there wont be information leak
...
#             ) & masked_indices[i]
        return inputs.int(), perm_mask, target_mapping, labels.int()
...
#             perm_index = tf.transpose(tf.reshape(perm_index, (-1, labels_shape[1] // 2)))
            # Permute the two halves such that they do not cross over
#             perm_index = tf.random.shuffle(perm_index)  # Shuffles aint the first dimension
            # Flatten this out into the desired permuted factorisation order
