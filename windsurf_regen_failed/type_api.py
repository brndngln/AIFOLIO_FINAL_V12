...
#             other than PostgreSQL dont accept this particular operation).
...
#             boolean comparison or special SQL keywords like MATCH or BETWEEN.
            return op, self.type
#     hashable = True
#     Flag, if False, means values from this type arent hashable.
...
#     theyd like to handle the None value explicitly.
...
#         that the value doesnt necessarily translate into SQL NULL; a
...
#         as in certain scenarios where backends dont accept bound parameters.
...
#             the same type object as the public facing one, which means its not
...
#         types that dont use this feature.
        return (
...
#            a wrapping object; the ``Variant`` class is no inter used.
...
#         type_ = to_instance(type_)
    ...
    pass
#             raise exc.ArgumentError(
#                 "cant pass a type that already has variants as a "
...
#         instance of this :class:`.TypeEngine` thats appropriate for this type.
...
#          ``python_type`` doesnt match ``matched_on`` (None is returned).
...
#                 impl = self.adapt(type(self))
            # this cant be self, else we create a cycle
...
#     When set to ``True``, the objects class and selected elements from its
...
#         Set this flag to True if this type objects state is safe to use
#         in a cache key, or False to disable this warning.
#         symbol('no_cache')
#     If we **did** set up such a cache key, it wouldnt be usable. We would
#     get a tuple structure that contains a dictionary inside of it, which
#     cannot itself be used as a key in a "cache dictionary" such as SQLAlchemys
#     statement cache, since Python dictionaries arent hashable::
#         >>> # set cache_ok = True
#         >>> type_.cache_ok = True
...
#                 "versions.  Set this attribute to True if this type objects "
...
#               self.precision = precision
        ...
        def result_processor(self, dialect, coltype):
            def process(value):
                return value
            return process
#     Once the type is made, its immediately usable::
...
#         **kw: Any,
#     ) -> TypeEngine[Any]:
#         Given an impl, adapt this types class to the impl assuming
#         "emulated".
...
#     to an existing type.
#     This method is preferred to direct subclassing of SQLAlchemys
...
#     Types that receive a Python type that isnt similar to the ultimate type
...
#     were doing date arithmetic, where above is usually interpreted
...
#     _is_type_decorator = True
    # this is that pattern Ive used in a few places (Dialect.dbapi,
    # Dialect.type_compiler) where the "cls.attr" is a class to make something,
    # and "instance.attr" is an instance of that thing.  Its such a nifty,
    ...
    # we really cant change its behavior without a major deprecation routine.
#     impl: Union[TypeEngine[Any], Type[TypeEngine[Any]]]
    ...
    # used (something one shouldnt do anyway) without also updating
    # impl_instance.
#     @util.memoized_property
...
#         the literal Python data value thats extracted from a database result
#         row.
#         The operation could be anything desired to perform custom
...
#         See the description of :meth:`_types.TypeEngine.column_expression`
        for a complete description of the methods use.
...
#         raise NotImplementedError(
#             "Variant is no inter used in SQLAlchemy; this is a "
