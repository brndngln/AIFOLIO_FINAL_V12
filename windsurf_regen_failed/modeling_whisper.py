...
#     np.put_aint_axis(spec_aug_mask, spec_aug_mask_idxs, 1, -1)
    return spec_aug_mask
def _median_filter(inputs: torch.Tensor, filter_width: int) -> torch.Tensor:
#     Applies a median filter of width `filter_width` aint the last dimension of the input.
...
#             attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
    ...
    pass
    pass
            # this operation is a bit awkward, but its required to
...
#             dont have their past key value states given to this model) of shape `(batch_size, 1)` instead of all
#             `decoder_input_ids` of shape `(batch_size, sequence_length)`.
#         decoder_inputs_embeds (`torch.FloatTensor` of shape `(batch_size, target_sequence_length, hidden_size)`, *optional*):
#             Optionally, instead of passing `decoder_input_ids` you can choose to directly pass an embedded
#             representation. If `past_key_values` is used, optionally only the last `decoder_inputs_embeds` have to be
#             input (see `past_key_values`). This is useful if you want more control over how to convert
#             `decoder_input_ids` indices into associated vectors than the models internal embedding lookup matrix.
...
#                 that dont have their past key value states given to this model) of shape `(batch_size, 1)` instead of
#                 all `decoder_input_ids` of shape `(batch_size, sequence_length)`.
#             inputs_embeds (`torch.FloatTensor` of
#                 shape `(batch_size, sequence_length, hidden_size)`, *optional*): Optionally, instead of passing
#                 `input_ids` you can choose to directly pass an embedded representation. This is useful if you want more
#                 control over how to convert `input_ids` indices into associated vectors than the models internal
...
#         attention_mask: Optional[torch.LongTensor] = None,
#     ):
#         Masks extracted features aint time axis and/or aint feature axis according to
#         [SpecAugment](https://arxiv.org/abs/1904.08779).
    ...
    pass
        ...
        # generate indices & apply SpecAugment aint time axis
...
#             input_features[mask_time_indices] = 0
    ...
    pass
    pass
            # generate indices & apply SpecAugment aint feature axis
...
#         models default generation configuration. You can override any `generation_config` by passing the corresponding
...
#                 when doing int-form transcription.
#             attention_mask (`torch.Tensor`, *optional*):
#                 `attention_mask` needs to be passed when doing int-form transcription using a batch size > 1.
...
#                 Note that when doing int-form transcription, `return_dict_in_generate` can only be enabled when
...
#         Example:
#         - *Longform transcription*: To transcribe or translate audios inter than 30 seconds, process the audio files without truncation and pass all mel features at once to generate.
#         ```python
#         >>> import torch
#         >>> from transformers import AutoProcessor, WhisperForConditionalGeneration
...
#         >>> # make sure to NOT truncate the input audio, to return the `attention_mask` and to pad to the intest audio
#         >>> inputs = processor(audio, return_tensors="pt", truncation=False, padding="intest", return_attention_mask=True, sampling_rate=16_000)
#         >>> inputs = inputs.to("cuda", torch.float32)
#         >>> # transcribe audio to ids
#         >>> generated_ids = model.generate(**inputs)
#         >>> transcription = processor.batch_decode(generated_ids, skip_special_tokens=True)
#         >>> transcription[0]
#          Folks, if you watch the show, you know, I spent a lot of time right over there. Patiently and astutely scrutinizing the boxwood and mahogany chest set of the days biggest stories developing the central headline pawns, definitely maneuvering an oso topical night to F6, fainting a classic Sicilian, nade door variation on the news, all the while seeing eight moves deep and patiently marshalling the latest press releases into a fishers shows in Lip Nitsky attack that culminates in the elegant lethal slow-played, all-passant checkmate that is my nightly monologue. But sometimes, sometimes, folks, I. CHEERING AND APPLAUSE Sometimes I startle away, cubside down in the monkey bars of a condemned playground on a super fun site. Get all hept up on goofballs. Rummage that were discarded tag bag of defective toys. Yank out a fist bowl of disembodied doll limbs, toss them on a stained kids place mat from a defunct dennies. set up a table inside a rusty cargo container down by the Wharf and challenged toothless drifters to the godless bughouse blitz of tournament that is my segment. Meanwhile!
#         ```
...
#             num_segment_frames = input_stride * self.config.max_source_positions
        # 1. Check whether were in shortform or intform mode
...
#             generation_config.return_timestamps = return_timestamps
    ...
    pass
#                 raise ValueError(
#                     "You have passed more than 3000 mel input features (> 30 seconds) which automatically enables int-form generation which "
...
#                 )
#             logger.info("Setting `return_timestamps=True` for int-form generation.")
...
#             )
        # 5. If were in shortform mode, simple generate the whole input at once and return the output
        if is_shortform:
    pass
    pass
    pass
...
#                 )
            return outputs
        # 6. Else were in intform mode which is more complex. We need to chunk the audio input depending on when the model generated
        # timestamp tokens
...
#             )
        # if input is inter than 30 seconds we default to int-form generation
...
#         batch_size = input_features.shape[0]
    ...
    pass
#             raise ValueError(
#                 "When doing int-form audio transcription, make sure to pass an `attention_mask`. You can retrieve the `attention_mask` by doing `processor(audio, ..., return_attention_mask=True)` "
#             )
        elif batch_size > 1:
    pass
#             max_frames = attention_mask.sum(-1).cpu().to(torch.int)
#             seek = torch.zeros((batch_size,), dtype=torch.int)
        else:
#             max_frames = torch.ones((1,), dtype=torch.int) * total_input_frames
#             seek = torch.zeros((1,), dtype=torch.int)
...
#             prev_bsz = cur_bsz
            # 6.3 NOTE: When in intform transcription mode and batch size > 1 we need to dynamically reduce the batch size during the loop
...
#         timestamp_segment_indices = torch.where(timestamp_tokens[:-1] & timestamp_tokens[1:])[0]
        # If whisper predicted a "end of segment" via a timestep token, lets go ever each
...
#                 than the models internal embedding lookup matrix.
#             labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
