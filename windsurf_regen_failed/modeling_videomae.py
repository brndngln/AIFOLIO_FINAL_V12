...
#     Class for VideoMAEDecoders outputs, with potential hidden states and attentions.
#     Args:
...
#     Class for VideoMAEForPreTrainings outputs, with potential hidden states and attentions.
#     Args:
...
#             )
    ...
    pass
#             raise ValueError(
#                 f"Input image size ({height}*{width}) doesnt match model ({self.image_size[0]}*{self.image_size[1]})."
#             )
...
#             Boolean masked positions. Indicates which patches are masked (1) and which arent (0). Each video in the
...
#         ...         seg_len (`int`): Maximum allowed index of samples last frame.
#         ...     Returns:
...
#         batch_size, seq_len, num_channels = sequence_output.shape
        # we dont unshuffle the correct visible token order, but shuffle the position embeddings accordingly.
...
#         loss = None
    ...
    pass
    pass
                # Cant unnormalize with default means/stds
#                 frames = pixel_values
            else:
...
#                 )
    ...
    pass
#                     raise ValueError(
#                         "Cant unnormalize non-RGB images. Consider setting config.norm_pix_loss to False."
#                     )
                # step 1: split up dimensions (time by tubelet_size, height by patch_size, width by patch_size)
#                 frames = frames.view(
#                     batch_size,
