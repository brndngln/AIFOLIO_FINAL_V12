...
#         "9": "₉"
#     })
    # Some fonts dont support full unicode character ranges necessary for
...
#             line_len = len(out.split('\n')[-1]) + len(next_term)
            ...
            # characters inter due to the +/- with the next term
...
#                 "_repr_latex_term(i, arg_str, needs_parens)")
        # since we always add parens, we dont care if the expression needs them
        return f"{{{cls.basis_name}}}_{{{i}}}({arg_str})"
#     @staticmethod
    def _repr_latex_scalar(x, parens=False):
        # TODO: were stuck with disabling math formatting until we handle
