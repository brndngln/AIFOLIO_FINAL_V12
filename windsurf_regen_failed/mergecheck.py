...
#         sym1, sym2 = sym, m[fn]
        ...
        # If the type changed, then it shouldnt have been merged.
...
#                 result += f".{attr}({t}:{obj.name})"
    ...
    pass
                # Omit class name for some classes that arent part of a class
                # hierarchy since there isnt much ambiguity.
#                 result += f".{attr}"
            else:
#                 result += f".{attr}({t})"
    return result
