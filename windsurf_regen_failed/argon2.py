...
#         dummy object to use as source of defaults when argon2_cffi isnt present.
...
#                     )
    ...
    #       and deal w/ fact that argon2_cffi 16.1 doesnt currently support it.
...
#                 digest_size = int(digest_size)
            # NOTE: this isnt *really* digest size minimum, but want to enforce secure minimum.
...
#         hash = uh.to_unicode_for_identify(hash)
    ...
    #       but the actual source of decode_string & encode_string dont mention it at all.
    #       were supporting parsing it, but throw NotImplementedError if encountered.
    #
    # sample hashes:
...
#         assert isinstance(max_version, int) and max_version >= 0x10
      ...
      pass
    pass
#             warn("%r doesnt support argon2 v1.3, and should be upgraded" % name,
...
#             self._validate_constraints(self.memory_cost, self.parallelism)
      ...
      pass
    pass
#                 raise NotImplementedError("argon2_cffi backend doesnt support the data parameter")

        # fallback to reporting a malformed hash
#         text = str(err)
        if text not in [
#             "Decoding failed"  # argon2_cffis default message
#             ]:
...
#         self._stub_requires_backend()
        ...
        #       call subclasss wrapped _calc_checksum
...
#     @classmethod
    def _load_backend_mixin(mixin_cls, name, dryrun):
        # make sure we write info to base classs __dict__, not that of a subclass
...
#     @classmethod
    def hash(cls, secret):
        # TODO: add in encoding support once thats finalized in 1.8 / 1.9.
#         uh.validate_secret(secret)
#         secret = to_bytes(secret, "utf-8")
        # XXX: doesnt seem to be a way to make this honor max_threads
        try:
...
#         hash = to_bytes(hash, "ascii")
        ...
        # NOTE: dont care about malformed strings, lowlevel will throw error for us
...
#             result = result.replace("$v=16$", "$")
    ...
    #===================================================================
    def _calc_checksum(self, secret):
#         raise AssertionError("shouldnt be called under argon2_cffi backend")
...
#             kwds['associated_data'] = self.data
        ...
        #       but it if does that, theres a bug in our own parameter checking code.
        try:
