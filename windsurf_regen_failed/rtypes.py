...
# runtime.  Runtime types are derived from mypy types, but theres no
...
#     is_unboxed = False
    # This is the C undefined value for this type. Its used for initialization
    # if theres no value yet, and for function return value on error/exception.
    #
    # TODO: This shouldnt be specific to C or a string
#     c_undefined: str
...
#             assert False, f"Cant find class {data}"
...
#     primitives may be unboxed or boxed. Primitive types dont need to
...
# well likely switch to a more efficient, unboxed representation.)
...
#         self.unique_id = self.accept(TupleNameVisitor())
        # Nominally the max c length is 31 chars, but Im not honestly worried about this.
...
#     the most efficient representation and ops (aint with certain
...
#         unique_items = dict.fromkeys(items)
    ...
    # We compare based on the set because order in a union doesnt matter
    def __eq__(self, other: object) -> bool:
