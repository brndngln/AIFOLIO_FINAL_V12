...
#         finally:
    ...
    pass
#                 target_file.close()
                # holds an open file descriptor so it cant be deleted on win
...
#         assert_equal(test, control)
    def test_fancy_dtype(self):
        # Check that a nested dtype isnt MIA
#         data = TextIO('1,2,3.0\n4,5,6.0\n')
...
#                 f.write('\N{GREEK PI SYMBOL}'.encode())
            # ResourceWarnings are emitted from a destructor, so wont be
...
#                                  dtype=ndtype, converters=converters)
        # nested but empty fields also arent supported
...
#                       dtype=[('c', '<f8'), ('d', '<f8')])
#         assert_equal(test, control)
        # To continue reading the same "file", dont use skip_header or
...
#                 break_cycles()
#                 break_cycles()
#     @pytest.mark.xfail(IS_WASM, reason="memmap doesnt work correctly")
...
def test_gzip_loadtxt():
    # Thanks to another windows brokenness, we cant use
