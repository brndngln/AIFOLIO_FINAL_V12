...
# @pytest.mark.skipif(IS_WASM, reason="wasm doesnt have support for fp errors")
...
#     assert res.dtype == np.float32
    # Changes, but we dont warn for it (too noisy)
...
#             np.array(1, dtype=dtype) + too_big_int
        ...
        # NumPy uses (or used) `int -> string -> intdouble` for the
...
#                 pytest.skip("`huge_int -> string -> intdouble` failed")
...
#         assert res == np.inf
        with pytest.warns(RuntimeWarning):
            ...
            # double instead of the intdouble loop.  That leads to slightly
...
#         np.array([1], np.uint8) + 300
    with pytest.raises(OverflowError, match=".*uint8"):
#         np.uint8(1) + 300
    # Error message depends on platform (maybe unsigned int or unsigned int)
    with pytest.raises(OverflowError,
