...
#         self.font = font
        # mergeTables populates this from the parents master ttfs
#         self.ttfs = None
...
#     merger.mergeLists(self.Value, padded)
    ...
    # Merge everything else; though, there shouldnt be anything else. :)
#     merger.mergeObjects(
...
#         ret[v].add(k)
    ...
    # Class-0 is special.  Its "everything else".
...
#     self.Coverage.glyphs = glyphs
    ...
    #
    # Note that our canonicalization process merges trailing PairPosFormat2s,
...
#         allClasses = [r.Class for r in glyphRecords if r is not None]
        ...
        # input.  But, since, its unlikely that two marks being both,
        # say, "top" in one master, and one being "top" and other being
        # "top-right" in another master, we shouldnt do that, as any
...
#     an instance.  While theres no "merging" to be done per se,
...
#         self._doneBaseGlyphs = False
    ...
    pass
            ...
            # multiple PaintColrLayers referencing the same layers from LayerList, its
            # a problem because we may risk modifying the same paint more than once, or
            # worse, fail while attempting to do that.
            # We dont know whether the master COLR table was built with layer reuse
...
#         PaintColrLayerss FirstLayerIndex/NumLayers are updated so that no overlap
#         is left. Also, any consecutively nested PaintColrLayers are flattened.
#         The COLR tables LayerList is replaced with the new unique layers.
...
#         referenced by any PaintColrLayers is dropped.
      ...
      pass
    pass
            # if no LayerList, theres nothing to expand
            return
#         uniqueLayerIDs = set()
#         newLayerList = []
...
#                 paint = frontier.pop()
      ...
      pass
    pass
                    # dont traverse these, we treat them as constant for merging
#                     continue
...
#     masters = [{rec.BaseGlyph: rec for rec in m.BaseGlyphPaintRecord} for m in lst]
    ...
    for i, g in enumerate(out.keys()):
        try:
            # missing base glyphs dont participate in the merge
...
#         raise
    ...
    # but I couldnt find a nice way to share the code between the two...
...
#         self.layers.extend(out_layers)
        ...
        # Register our parts for reuse provided we arent a tree
...
#     )
    ...
    pass
      ...
      pass
    pass
            # Some PaintVar* dont have any scalar attributes that can vary,
...
def merge(merger, self, lst):
    ...
    # for some/all glyphs (i.e. they dont participate)
