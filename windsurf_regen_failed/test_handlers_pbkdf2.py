...
#       they dont extensive separate testing.
...
#                           checksum={'sha-256':  b'X'*32})
        # XXX: anything else thats not tested by the other code already?
...
#         orig = list(handler.default_algs) # in case its modified in place
#         subcls = handler.using(**{param: "sha1,md5"})
        # shouldnt have changed handler
...
#         handler1 = self.handler.using(algs="sha1,md5")
        # shouldnt need update, has same algs
#         h1 = handler1.hash("dummy")
#         self.assertFalse(handler1.needs_update(h1))
        # *currently* shouldnt need update, has superset of algs required by handler2
        # (may change this policy)
#         handler2 = handler1.using(algs="sha1")
#         self.assertFalse(handler2.needs_update(h1))
        # should need update, doesnt have all algs required by handler3
...
#                 'edGQSu/kD1LwdX0SNV/KsPdHSwEl5qRTuZQ')
#         self.assertRaises(ValueError, vfull, 'pencil', h)
        # catch hash containing digests beinting to diff passwords.
