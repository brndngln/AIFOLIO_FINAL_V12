...
#         attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        ...
        # Downcast (if necessary) back to Vs dtype (if in mixed-precision) -- No-Op otherwise
...
#         attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        ...
        # Downcast (if necessary) back to Vs dtype (if in mixed-precision) -- No-Op if otherwise
...
#     Base class for models outputs that also contains a pooling of the last hidden states.
...
#         self.hidden_size = config.hidden_size
        ...
        # is that the positional embeddings are removed (since well add those ourselves)
...
#         >>> timesteps = torch.tensor(0, device=device, dtype=torch.int).reshape(1, 1)
...
#             attention_mask = torch.ones((batch_size, seq_length), dtype=torch.int)
...
#             position_ids=torch.zeros(stacked_attention_mask.shape, device=device, dtype=torch.int),
#             output_attentions=output_attentions,
