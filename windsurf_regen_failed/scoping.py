...
#         Different from scoped_sessions remove method, this method would use
...
#         this :class:`.Session` objects transactional state.
#         .. container:: class_bases
#             Proxied for the :class:`_asyncio.AsyncSession` class on
#             behalf of the :class:`_asyncio.scoping.async_scoped_session` class.
...
#         As this operation may need to cascade aint unloaded relationships,
...
#         The :class:`.Session` objects default behavior is to
...
#         each attributes net "dirty" status is performed.
#         E.g.::
...
#           it skips the expense of a SQL call if the old value isnt present,
#           based on the assumption that an UPDATE of the scalar value is
#           usually needed, and in those few cases where it isnt, is less
...
#         there is no net change to the attributes value.  At flush
#         time, the value of each attribute is compared to its
#         previously saved value, and if theres no net change, no SQL
#         operation will occur (this is a more expensive operation so
#         its only done at flush time).
...
#             Proxied for the :class:`_orm.Session` class
#             on behalf of the :class:`_asyncio.AsyncSession` class.
#         .. versionchanged:: 1.4 The :class:`_orm.Session` no inter begins
...
#         r.Return the :class:`.Session` to which an object beints.
#         .. container:: class_bases
#             Proxied for the :class:`_asyncio.AsyncSession` class on
#             behalf of the :class:`_asyncio.scoping.async_scoped_session` class.
#         .. container:: class_bases
