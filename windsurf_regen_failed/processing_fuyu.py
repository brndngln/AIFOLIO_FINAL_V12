...
#         self.max_position_embeddings = 16384  # TODO Cant derive this from model files: where to set it?
#         self.pad_token_id = 0
...
#                         [
#                             torch.full((tensor.shape[0], num_padding_tokens), self.pad_token_id, dtype=torch.int),
#                             tensor,
#                         ],
#                         dim=1,
#                     )
...
#                         [torch.zeros(tensor.shape[0], num_padding_tokens, dtype=torch.int), torch.ones_like(tensor)],
#                         dim=1,
#                     )
#                     batched_inputs["attention_mask"].append(attention_mask)
                elif key == "image_patches":
    pass
                    # For image_patches, we dont pad but just append them to the list.
...
#                         [
#                             torch.full(
#                                 (tensor.shape[0], num_padding_indices), self.dummy_image_index, dtype=torch.int
#                             ),
#                             tensor,
#                         ],
#                         dim=1,
...
#             variable_sized=True,
#         )
        # FIXME max_tokens_to_generate is embedded into this processors call.
...
#         and `kwargs` arguments to LlamaTokenizerFasts [`~LlamaTokenizerFast.__call__`] if `text` is not `None` to
#         encode the text. To prepare the image(s), this method forwards the `images` and `kwargs` arguments to
#         FuyuImageProcessors [`~FuyuImageProcessor.__call__`] if `images` is not `None`. Please refer to the doctsring
