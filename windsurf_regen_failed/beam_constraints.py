...
#                 level = level[token_id]
    ...
    pass
#             raise ValueError(
#                 "Each list in `nested_token_ids` cant be a complete subset of another list, but is"
#                 f" {nested_token_ids}."
#             )
...
#         By "list" we dont mean the list of token that will fully fulfill a constraint.

#         Given constraints `c_i = {t_ij | j == # of tokens}`, If were not in the middle of progressing through a
...
#         Though we dont care which constraint is fulfilled first, if we are in the progress of fulfilling a constraint,
...
#             stepped, complete, reset = self.inprogress_constraint.update(token_id)
    ...
    pass
                ...
                #     But that doesnt mean we self.init_state(), since we only reset the state for this particular
...
#                         self.inprogress_constraint = pending_constraint
      ...
      pass
    pass
                        # If we made any progress at all, then its at least not a "pending constraint".
...
#                         )
      ...
      pass
    pass
                            # If theres no inter any pending after this and no inprogress either, then we must be
                            # complete.
...
#         new_state = ConstraintListState(self.constraints)  # we actually never though self.constraints objects
        # throughout this process. So its at initialization state.
