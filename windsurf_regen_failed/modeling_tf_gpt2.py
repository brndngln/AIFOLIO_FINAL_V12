...
#         1s in the lower triangle, counting from the lower right corner. Same as tf.matrix_band_part(tf.ones([nd, ns]),
#         -1, ns-nd), but doesnt produce garbage on TPUs.
#         i = tf.range(nd)[:, None]
#         j = tf.range(ns)
#         m = i >= j - ns + nd
...
#             If `past` are used, the user can optionally input only the last `decoder_input_ids` (those that dont have
