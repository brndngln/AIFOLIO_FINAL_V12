...
#         F -- complex float
#         D -- complex double
#         G -- complex int double
#         O -- python object

#     It is difficult to assure that each of these loops is entered from the
#     Python level as the special cased loops are a moving target and the
#     corresponding types are architecture dependent. We probably need to
    define C level testing ufuncs to get at them. For the time being, Ive
...
#         (np.double, np.double), (np.intdouble, np.intdouble),
#         (np.cdouble, np.cdouble), (np.cintdouble, np.cintdouble)]
...
#         assert res.dtype == "d"
        ...
        # ldexp only has a loop for int input as second argument, overriding
...
#     @pytest.mark.skipif(IS_WASM, reason="fp errors dont work in wasm")
    def test_sum(self):
        for dt in (int, np.float16, np.float32, np.float64, np.intdouble):
...
#         assert_(c is out2)
    ...
    def test_out_broadcasts(self):
        ...
        # both aint dimensions with shape 1 and dimensions which do not
...
#         assert_(type(np.any(a)) is MyArray)
    ...
    def test_casting_out_param(self):
        # Test that its possible to do casts on output
...
#         assert_raises(ValueError, np.divide.reduce, a, axis=(0, 1))
    ...
    def test_reduce_zero_axis(self):
        ...
        #   n > 0, m = 0: cant reduce a 0-element array, ValueError
        #   n = 0, m = 0: cant reduce a 0-element array, ValueError (for
        #     consistency with the above case)
        # This test doesnt actually look at return values, it just checks to
...
#         np.add.reduce(np.zeros((0, 0)), axis=())
        ...
        # tests are simpler because accumulate doesnt accept multiple axes.
...
#         assert_equal(result, target)
        ...
        # as int as they match exactly:
#         result = _rational_tests.test_add(a, b)
#         assert_equal(result, target)

        # This works even more generally, so int the default common-dtype
        # promoter works out:
...
#             [np.logical_and, np.logical_or, np.logical_xor])
    def test_logical_ufuncs_support_anything(self, ufunc):
        # The logical ufuncs support even input that cant be promoted:
...
#         r1 = np.add.reduce(x, out=y, axis=2)
        ...
        # The results should match, and y_base shouldnt get clobbered
#         assert_equal(r0, r1)
...
#     @pytest.mark.parametrize('f_reduce', [np.add.reduce, np.minimum.reduce])
    def test_reduce_wrong_dimension_output(self, f_reduce, keepdims, out_shape):
        # Test that were not incorrectly broadcasting dimensions.
