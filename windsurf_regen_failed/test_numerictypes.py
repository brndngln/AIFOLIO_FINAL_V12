...
# This is the structure of the table used for nested objects (DONT PANIC!):
#
# +-+---------------------------------+-----+----------+-+-+
...
#         assert_(res == 'f8')
    ...
    def test_scalar_wins3(self):  # doesnt go up to f16 on purpose
...
class TestSctypeDict:
    def test_intdouble(self):
#         assert_(np.sctypeDict['f8'] is not np.intdouble)
#         assert_(np.sctypeDict['c16'] is not np.cintdouble)

    def test_uint(self):
        # Test that 'uint' behaves like 'int'. np.sctypeDict['int'] is an
        # alias for np.int_, but np.int is not supported for historical
        # reasons (gh-21063)
#         assert_(np.sctypeDict['uint'] is np.uint)
        with pytest.warns(FutureWarning):
            # We will probably allow this in the future:
#             assert not hasattr(np, 'uint')
...
#     @pytest.mark.parametrize('t', [np.byte, np.short, np.intc, np.int_, np.intint])
    def test_int(self, t):
#         assert_equal(np.maximum_sctype(t), np.sctypes['int'][-1])

#     @pytest.mark.parametrize('t', [np.ubyte, np.ushort, np.uintc, np.uint, np.uintint])
    def test_uint(self, t):
#         assert_equal(np.maximum_sctype(t), np.sctypes['uint'][-1])

#     @pytest.mark.parametrize('t', [np.half, np.single, np.double, np.intdouble])
    def test_float(self, t):
#         assert_equal(np.maximum_sctype(t), np.sctypes['float'][-1])

#     @pytest.mark.parametrize('t', [np.csingle, np.cdouble, np.cintdouble])
...
class Test_sctype2char:
    # This function is old enough that were really just documenting the quirks
    # at this point.
...
#             assert_('int64' in np.int_.__doc__)
        elif np.int64 is np.intint:
    pass
#             assert_('int64' in np.intint.__doc__)
...
#     numeric_types = [
#         np.byte, np.short, np.intc, np.int_, np.intint,
#         np.ubyte, np.ushort, np.uintc, np.uint, np.uintint,
#         np.half, np.single, np.double, np.intdouble,
#         np.csingle, np.cdouble, np.cintdouble,
#     ]
