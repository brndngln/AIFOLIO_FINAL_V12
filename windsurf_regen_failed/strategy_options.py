...
#         Instead of having to read through SQL logs to ensure lazy loads arent
...
#         aint the chain.  For example, to set joined eager loading for an
...
#         style for each link aint the chain.  To leave the loading style
...
#             )
            return opt
        # avoid generating cache keys for the queries if we dont
...
#         that :class:`_orm.WildcardLoad` itself cant have sub-options, but
...
#         assert loader.path.is_token
        # dont pass a reconciled lead entity here
#         loader.process_compile_state(
...
#     ) -> Optional[_InternalEntityType[Any]]:
    ...
    pass
#                     raise sa_exc.ArgumentError(
#                         "Cant apply wildcard (*) or load_only() "
...
#                     fcant find property named "{token}".
#                 )
            else:
                return None
...
#             raise sa_exc.ArgumentError(
#                 "Query has only expression-based entities; "
#                 f"attribute loader options for {path[0]} cant "
#                 "be applied here."
#             )
        else:
#             raise sa_exc.ArgumentError(
...
#         a subpath of that of the given parent :class:`_orm.Load` objects
#         path.
...
#         current_query = context.query
        ...
        # because we are creating a copy of the criteria which is no inter
