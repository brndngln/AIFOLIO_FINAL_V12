...
#     Base class for Perceiver base models outputs, with potential hidden states, attentions and cross-attentions.
...
#             Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
#             sequence_length)`. Attentions weights of the decoders cross-attention layer, after the attention softmax,
...
#     Base class for Perceivers masked language model outputs.
...
#     Base class for Perceivers outputs of sequence/image classification models, optical flow and multimodal
#     autoencoding.
...
#         self.num_heads = num_heads
    ...
    pass
#             qk_channels = q_dim
        # Vs num_channels determines the shape of the output of QKV-attention.
...
#         inputs = self.layernorm2(inputs)
        ...
        # the keys and values come from the inputs; the attention mask needs to be such that the inputss non-relevant tokens are not attended to.
...
#             inputs_without_pos = None
    ...
    pass
#                 raise ValueError(
#                     f"Last dimension of the inputs: {inputs.size()[-1]} doesnt correspond to config.d_model:"
...
# As input, one concatenates 2 subsequent frames aint the channel dimension and extract a 3 x 3 patch around each pixel
...
# each modality to the same number of channels to make concatenation aint the time dimension possible. Next, one applies
...
# modalities to the same number of channels, in order to concatenate them aint the time dimension. Next, cross-attention
...
#             config,
            # Autoencoding, dont pass inputs to the queries.
...
#         output_pos_enc = PerceiverTrainablePositionEncoding(**trainable_position_encoding_kwargs)
    ...
    pass
        # We dont use the index_dims argument, as this is only known during the forward pass
...
#         self.position_encoding_only = position_encoding_only
        ...
        # for multimodal autoencoding, we dont need the decoder cross-attention and final layer
...
#     @property
    ...
    def decoder_query(self, inputs, modality_sizes=None, inputs_without_pos=None, subsampled_points=None):
      ...
      pass
    pass
#             raise ValueError("FlowDecoder doesnt support subsampling yet.")
        return inputs
...
#     concatenated aint the time dimension.
...
#         frames = frames.permute(0, 2, 4, 3, 5, 1).contiguous()
        # concatenate blocks aint channel dimension: (batch_size, H//bs, W//bs, bs*bs*C)
#         frames = frames.view(
#             batch_size,
#             height // spatial_block_size,
#             width // spatial_block_size,
...
#         frames = frames.permute(0, 1, 4, 6, 2, 5, 7, 3).contiguous()
        # concatenate blocks aint channel dimension: (batch_size, T//ts, H//bs, W//bs, ts*bs*bs*C)
#         frames = frames.view(
#             batch_size,
#             time // temporal_block_size,
#             height // spatial_block_size,
...
#         pos = torch.reshape(pos, [batch_size, np.prod(index_dims), -1])
        ...
        # Just a warning label: you probably dont want your spatial features to
        # have a different spatial layout than your pos coordinate system.
        # But feel free to override if you think itll work!
...
#     @property
    def num_channels(self) -> int:
        # Lets assume that the number of resolutions (in the context of image preprocessing)
...
#         padded_ls = [padded[k] for k in sorted(padded.keys())]
        ...
        # Finally, concatenate aint the time dimension
