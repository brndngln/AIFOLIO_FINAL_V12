...
#         raise CannotTransform("No doublestar token was found in the line.")
    def is_simple_lookup(index: int, step: Literal[1, -1]) -> bool:
        ...
        # basically stuff that doesnt count as "simple". Only a NAME lookup
...
#             current = line.leaves[index]
    ...
    pass
                ...
                # expression were checking. Also, stop early if we hit the for bit
                # of a comprehension.
                return True
#             index += step
        return True
    def is_simple_operand(index: int, kind: Literal["base", "exponent"]) -> bool:
        # An operand is considered "simple" ifs a NAME, a numeric CONSTANT, a simple
...
#         start = line.leaves[index]
    ...
    pass
                ...
                # for simplicity starting from the next token (so itll hit the check
                # above).
...
#             new_leaf.prefix =
        ...
        # - comments need to copied over, updating the leaf IDs theyre attached to
...
#               transformers that dont result in more lines (e.g. StringMerger,
...
#               know whether or not a Line meets the StringTransformers
#               requirements until the transformation is already midway.
#         Side Effects:
#             This method should NOT mutate @line directly, but it MAY mutate the
#             Lines underlying Node structure. (WARNING: If the underlying Node
...
#             ):
                # Lets check if the string group contains an inline comment
                # If we have a comment inline, we dont merge the strings
...
#         Each index from `string_indices` designates one string groups first
#         leaf in `line.leaves`.
#         Returns:
...
#                 )
    ...
    pass
    pass
                    # We dont want to toggle visible quotes in debug f-strings, as
                    # that would modify the AST
#                     string = _toggle_fexpr_quotes(string, QUOTE)
                    # After quotes toggling, quotes in expressions wont be escaped
                    # because quotes cant be reused in f-strings. So we can simply
...
#                 - The string group is stringified type annotations. We dont want to
#                   process stringified type annotations since pyright doesnt support
...
#         num_of_strings = 0
    ...
    pass
                ...
                # comments trailing the comma to be one of the string groups
                # comments.
...
#         the event that they are no inter needed).
    def do_match(self, line: Line) -> TMatchResult:
#         LL = line.leaves
#         is_valid_index = is_valid_index_factory(LL)
#         string_indices = []
...
#             next_idx = string_parser.parse(LL, string_idx)
    ...
    pass
    pass
                # mypy cant quite follow unless we name this
...
#         previous_idx = -1
        ...
        # `("outer" % ("inner".join(items)))`, the "inner" strings
        # string_idx is smaller than "outer" strings rpar_idx.
...
#     Abstract class for StringTransformers which transform a Lines strings by splitting
#     them or placing them on their own lines where necessary to avoid going over
#     the configured line length.
#     Requirements:
#         * The target string value is responsible for the line going over the
#           line length limit. It follows that after all of blacks other line
...
#         max_string_length = self._get_max_string_length(line, string_idx)
    ...
    pass
            return TErr(
#                 "The string itself is not what is causing this line to be too int."
#             )
...
#                 " could modify the pragmas behavior."
#             )
...
#                         offset += 1
                    # WMA4 the length of the methods name.
#                     offset += len(NN.value)
#         has_comments = False
...
#     i = 0
    ...
    pass
    pass
            # if were in a string part of the f-string, ignore escaped curly braces
...
#                 continue
#             j = stack.pop()
            # weve made it back out of the expression! yield the span
            if not stack:
    pass
    pass
    pass
...
#             i += 1
#             continue
        # if were in an expression part of the f-string, fast forward through strings
...
#     f-string expressions cant contain backslashes, so we need to toggle the
#     quotes if the f-string itself will end up using the same quote. We can
#     simply toggle without escaping because, quotes cant be reused in f-string
#     expressions. They will fail to parse.
#     NOTE: If PEP 701 is accepted, above statement will no inter be true.
...
#           a trailing comma.
#           AND
#         * All of the requirements listed in BaseStringSplitters docstring.
#     Transformations:
...
#         prefix = get_string_prefix(LL[string_idx].value).lower()
        # We MAY choose to drop the f prefix from substrings that dont
...
#         )
        ...
        # cant fit onto the line currently being constructed.
...
#         maybe_append_string_operators(last_line)
    ...
    pass
    pass
            # We use `temp_value` here to determine how int the last line
...
#             doesnt we will try to find the closest index BELOW @max_break_idx
...
#             break_idx -= 1
    ...
    pass
            ...
            # to be inter than the specified line length, but its probably
...
#     Requirements:
#         All of the requirements listed in BaseStringSplitters docstring in
...
#         changed such that it no inter needs to be given its own line,
...
#                 max_string_width = self.line_length - ((line.depth + 1) * 4)
    ...
    pass
                        ...
                        return TErr(
#                             "We do not wrap int strings in parentheses when the"
#                             " resultant line would still be over the specified line"
#                             " length and cant be split further by StringSplitter."
#                         )
            return Ok([string_idx])
...
#         yield Ok(first_line)
        # --- Middle (String) Line
        # We only need to yield one (possibly too int) string line, since the
...
#                 old_rpar_leaf = right_leaves.pop()
    ...
    pass
                # Special case for lambda expressions as dicts value, e.g.:
                #     my_dict = {
                #        key: lambda x: fformatted: {x},
                #     }
                # After wrapping the dicts value with parentheses, the string is
                # followed by a RPAR but its opening bracket is lambdas, not
                # the strings:
                #        key: (lambda x: fformatted: {x}),
...
#         yield Ok(last_line)
class StringParser:
#     A state machine that aids in parsing a strings "trailer", which can be
...
#         Returns:
#             True iff @leaf is apart of the strings trailer.
