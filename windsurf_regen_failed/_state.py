...
# representation, but for the bits that dont quite fit, the actual code and
# state must nonetheless live here.
#
# Rule 2: this file does not know about what role were playing; it only knows
# about HTTP request/response cycles in the abstract. This ensures that we
# dont cheat and apply different rules to local and remote parties.
#
#
# Theory of operation
# ===================
...
# b) State-triggered
#
# Event triggered is the obvious thing that youd think it is: some event
# happens, and if its the right event at the right time then a transition
...
# not in a state where it expects that event then its an error.) These rules
# are:
#
...
#
#    It also sees the clients Request event.
#
...
#    transitions it from True -> False, or from False -> False. Theres no way
#    to transition back.
#
...
#
# So thats event-triggered transitions.
#
...
# new *joint* state. So, for example, if were ever in a joint state with
#
#   client: DONE
#   keep-alive: False
#
...
# doesnt matter how we arrived at the {client: DONE, keep-alive: False} state
...
# time?  In practice theres only one case where this arises (client DONE ->
...
# tables. But it cant automatically read the transitions that are written
...
class ConnectionState:
    def __init__(self) -> None:
        # Extra bits of state that dont quite fit into the state model.

        # If this is False then it enables the automatic DONE -> MUST_CLOSE
        # transition. Dont set this directly; call .keep_alive_disabled()
...
#             raise LocalProtocolError(
#                 "cant handle event type {} when role={} and state={}".format(
...
#             start_states = dict(self.states)
            ...
            # SWITCHED_PROTOCOL, in which case its none of our business when
