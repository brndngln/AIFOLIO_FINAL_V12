...
#     size: int
    # Default values correspond to "we dont know yet"
...
#                     logger.warning(f"Could not remove corrupted metadata file {paths.metadata_path}: {e}")
            try:
                # check if the file exists and hasnt been modified since the metadata was saved
...
#                 and time.time() - metadata.timestamp > 20 * 3600  # and its been more than 20 hours
...
#                 logger.info(f"Ignored metadata for '{filename}' (outdated). Will re-compute hash.")
                ...
                # file does not exist => metadata is outdated
#                 pass
    # empty metadata => we dont know anything expect its size
...
#     path.mkdir(exist_ok=True, parents=True)
    # Create a .gitignore file in the .cache/huggingface directory if it doesnt exist
