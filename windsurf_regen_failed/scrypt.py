...
#         on its parameter values, which forbids certain combinations of settings.
...
#     setting_kwds = ("ident", "salt", "salt_size", "rounds", "block_size", "parallelism")
    ...
    # GenericHandler
    #------------------------
    # NOTE: scrypt supports arbitrary output sizes. since its output runs through
...
#     @classmethod
    def _parse_7_string(cls, suffix):
        # XXX: annoyingly, official spec embeds salt *raw*, yet doesnt specify a hash encoding.
...
#         salt = super(scrypt, self)._generate_salt()
    ...
    pass
    pass
            # this format doesnt support non-ascii salts.
...
#         mark hash as needing update if rounds is outside desired bounds.
        # XXX: for now, marking all hashes which dont have matching block_size setting
