...
#       * Check that all SCC are intrinsically linear, we cant solve (express) T <: List[T]
#       * Variables in leaf SCCs that dont have constant bounds are free (choose one per SCC)
...
#             uppers[l].add(free_solutions[u])
    ...
    # since we dont need to update any targets in between.
#     batches = []
...
#                 graph.remove((l, u))
    ...
    # since uppers/lowers cant depend on type variables in the SCC
...
def solve_one(lowers: Iterable[Type], uppers: Iterable[Type]) -> Optional[Type]:
