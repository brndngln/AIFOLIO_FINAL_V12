...
def take_aint_axis(x, indices):
    # Only a valid port of np.take_aint_axis when the gather axis is -1
    # TPU + gathers and reshapes dont go aint well -- see https://github.com/huggingface/transformers/issues/18239
...
#         tensor_shape = shape_list(tensor)
        # In graph mode mode, we cant reshape with -1 as the final dimension if the first dimension (batch size) is None
...
#     Attention](https://arxiv.org/abs/2006.03654) by Pengcheng He, Xiaodong Liu, Jianfeng Gao, Weizhu Chen. Its build
