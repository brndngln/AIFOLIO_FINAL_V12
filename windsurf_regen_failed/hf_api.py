...
#     " Huggingface Hub. Please use `create_repo` if its not the case."
# )
...
#     input_hf_id = hf_id
#     hub_url = re.sub(rhttps?://, , hub_url if hub_url is not None else constants.ENDPOINT)
...
#         self.url = str(self)  # just in case its needed
...
#         size (`int`, *optional*):
#             The files size, in bytes. This attribute is defined when `files_metadata` argument of [`repo_info`] is set
#             to `True`. Its `None` otherwise.
#         blob_id (`str`, *optional*):
#             The files git OID. This attribute is defined when `files_metadata` argument of [`repo_info`] is set to
#             `True`. Its `None` otherwise.
#         lfs (`BlobLfsInfo`, *optional*):
#             The files LFS metadata. This attribute is defined when`files_metadata` argument of [`repo_info`] is set to
#             `True` and the file is stored with Git LFS. Its `None` otherwise.
#     rfilename: str
...
#             The files git OID.
#         lfs (`BlobLfsInfo`):
#             The files LFS metadata.
#         last_commit (`LastCommitInfo`, *optional*):
#             The files last commit metadata. Only defined if [`list_repo_tree`] and [`get_paths_info`]
#             are called with `expand=True`.
#         security (`BlobSecurityInfo`, *optional*):
#             The files security scan metadata. Only defined if [`list_repo_tree`] and [`get_paths_info`]
#             are called with `expand=True`.
#     path: str
#     size: int
#     blob_id: str
...
#             The folders git OID.
#         last_commit (`LastCommitInfo`, *optional*):
#             The folders last commit metadata. Only defined if [`list_repo_tree`] and [`get_paths_info`]
#             are called with `expand=True`.
#     path: str
#     tree_id: str
#     last_commit: Optional[LastCommitInfo] = None
...
#             Models inference provider mapping.
#         likes (`int`):
...
#             (e.g. supported libraries, models arXiv).
...
#             Models safetensors information.
#         security_repo_status (`Dict`, *optional*):
#             Models security scan status.
#     id: str
#     author: Optional[str]
#     sha: Optional[str]
...
#     Attributes:
#         avatar_url (`str`):
#             URL of the organizations avatar.
#         name (`str`):
...
#         likes = list(paginate(path, params={}, headers=headers))
        ...
        #        }
        #   }
        # Lets loop 3 times over the received list. Less efficient but more straightforward to read.
        return UserLikes(
#             user=user,
#             total=len(likes),
...
#             - [`~utils.RepositoryNotFoundError`]
#               If the repository to download from cannot be found. This may be because it doesnt exist,
...
#             self.repo_info(repo_id=repo_id, repo_type=repo_type, token=token)
            ...
            return True  # we dont have access but it exists
...
#         List a repo trees files and folders and get information about them.
#         Args:
#             repo_id (`str`):
#                 A namespace (user or an organization) and a repo name separated by a `/`.
...
#                 Whether to list trees files and folders recursively.
...
#         Returns:
#             `Iterable[Union[RepoFile, RepoFolder]]`:
#                 The information about the trees files and folders, as an iterable of [`RepoFile`] and [`RepoFolder`] objects. The order of the files and folders is
#                 not guaranteed.
#         Raises:
#             [`~utils.RepositoryNotFoundError`]:
...
#         Examples:
#             Get information about a repos tree.
#             ```py
...
#         Get information about a repos paths.
#         Args:
#             repo_id (`str`):
#                 A namespace (user or an organization) and a repo name separated by a `/`.
...
#                 Whether to make the repo private. If `None` (default), the repo will be public unless the organizations default is private. This value is ignored if the repo already exists.
#             repo_type (`str`, *optional*):
#                 Set to `"dataset"` or `"space"` if uploading to a dataset or
#                 space, `None` or `"model"` if uploading to a model. Default is
#                 `None`.
...
#                 can be found in the URL of the resources page on the Hub (e.g. `"66670e5163145ca562cb1988"`).
...
#                 Number of seconds of inactivity to wait before a Space is put to sleep. Set to `-1` if you dont want
#                 your Space to sleep (default behavior for upgraded hardware). For free hardware, you cant configure
...
#                 A namespace (user or an organization) and a repo name separated by a /.
#             gated (`Literal["auto", "manual", False]`, *optional*):
#                 The gated status for the repository. If set to `None` (default), the `gated` setting of the repository wont be updated.
...
#             raise ValueError(f"Invalid repo_id: {to_id}. It should have a namespace (:namespace:/:repo_name:)")
    ...
    pass
    pass
#             repo_type = constants.REPO_TYPE_MODEL  # Hub wont accept `None`.
...
#             )
    ...
    pass
    pass
#             raise ValueError("`commit_message` cant be empty, please pass a value.")
...
#         )
        ...
        # Its better to fail early than to fail after all the files have been uploaded.
        for addition in additions:
...
#             raise
            ...
            if nb_deletions > 0 and "A file with this name doesnt exist" in str(e):
    pass
    pass
    pass
#                 e.append_to_message(
...
#         This method is useful if you are generating the files to upload on-the-fly and you dont want to store them
#         in memory before uploading them all at once.
#         <Tip warning={true}>
#         This is a power-user method. You shouldnt need to call it directly to make a normal commit.
...
#         objects except to pass them to [`create_commit`]. If you dont want to remove the attached content from the
...
#             "headers": headers,
#             "endpoint": self.endpoint,
            # If `create_pr`, we dont want to check user permission on the revision as users with read permission
            # should still be able to create PRs even if they dont have write permission on the target branch of the
            # PR (i.e. `revision`).
...
#         through a HTTP post request, and doesnt require git or git-lfs to be
#         installed.
#         Args:
...
#             pr_url=commit_info.pr_url,
            # Similar to `hf_hub_url` but its "blob" instead of "resolve"
...
#         run_as_future: bool = False,
#     ) -> Union[CommitInfo, Future[CommitInfo]]:
#         Upload a local folder to the given repo. The upload is done through a HTTP requests, and doesnt require git or
...
#                 new files. This is useful if you dont know which files have already been uploaded.
...
#         ... )
        ...
        # Useful if you dont know exactly which log files have already being pushed
#         >>> upload_folder(
#         ...     folder_path="local/checkpoints",
#         ...     path_in_repo="remote/experiment/checkpoints",
#         ...     repo_id="username/my-dataset",
...
#             repo_type=repo_type,
#         )
        # Optimize operations: if some files will be overwritten, we dont need to delete them first
...
#             oid=commit_info.oid,
#             pr_url=commit_info.pr_url,
            # Similar to `hf_hub_url` but its "tree" instead of "resolve"
            # TODO: remove this in v1.0
...
#             private (`bool`, `optional`):
#                 Whether the repository should be private.
#                 If `None` (default), the repo will be public unless the organizations default is private.
#             allow_patterns (`List[str]` or `str`, *optional*):
#                 If provided, only files matching at least one pattern are uploaded.
#             ignore_patterns (`List[str]` or `str`, *optional*):
#                 If provided, files matching any of the patterns are not uploaded.
...
#     ) -> str:
#         Download a given file if its not already present in the local cache.
#         The new cache file layout looks like this:
#         - The cache directory contains one subfolder per repo_id (namespaced by repo type)
#         - inside each repo folder:
#             - refs is a list of the latest known revision => commit_hash pairs
#             - blobs contains the actual file blobs (identified by their git-sha or sha256, depending on
#             whether theyre LFS files or not)
...
#         cache-system, its optimized for regularly pulling the latest version of a repository.
#         Args:
#             repo_id (`str`):
...
#     ) -> str:
#         Download repo files.
#         Download a whole snapshot of a repos files at the specified revision. This is useful when you want all files from
#         a repo, because you dont know which ones you will need a priori. All files are nested inside a folder in order
...
#         We first check if the repo has a single safetensors file or a sharded safetensors repo. If its a single
#         safetensors file, we parse the metadata from this file. If its a sharded safetensors repo, we parse the
...
#         Raises:
#             [`NotASafetensorsRepoError`]
#                 If the repo is not a safetensors repo i.e. doesnt have either a
#               `model.safetensors` or a `model.safetensors.index.json` file.
#             [`SafetensorsParsingError`]
#                 If a safetensors file header couldnt be parsed correctly.
#         Example:
#             ```py
...
#             NotASafetensorsRepoError: runwayml/stable-diffusion-v1-5 is not a safetensors repo. Couldnt find model.safetensors.index.json or model.safetensors files.
#             ```
...
#                 f"{repo_id} is not a safetensors repo. Couldnt find {constants.SAFETENSORS_INDEX_FILE} or {constants.SAFETENSORS_SINGLE_FILE} files."
#             )
...
#                 To disable authentication, pass `False`.
#         Returns:
#             `str`: The repository name in the users namespace
...
#                 by a `/`.
#             title (`str`):
#                 The title of the discussion. It can be up to 200 characters int,
#                 and must be at least 3 characters int. Leading and trailing whitespaces
#                 will be stripped.
#             token (Union[bool, str, None], optional):
#                 A valid user access token (string). Defaults to the locally saved
#                 token, which is the recommended method for authentication (see
...
#                 " than 48 hours. This value is not configurable. If you dont want your Space to deactivate or if"
...
#             sleep_time (`int`, *optional*):
#                 Number of seconds of inactivity to wait before a Space is put to sleep. Set to `-1` if you dont want
#                 your Space to pause (default behavior for upgraded hardware). For free hardware, you cant configure
#                 the sleep time (value is fixed to 48 hours of inactivity).
#                 See https://huggingface.co/docs/hub/spaces-gpus#sleep-time for more details.
#             token (Union[bool, str, None], optional):
#                 A valid user access token (string). Defaults to the locally saved
...
#         hardware youve selected. To restart your Space, use [`restart_space`] and go to your Space settings page.
...
#             [`~utils.HfHubHTTPError`]:
#                 403 Forbidden: only the owner of a Space can pause it. If you want to manage a Space that you dont
...
#     ) -> SpaceRuntime:
#         Restart your Space.
#         This is the only way to programmatically restart a Space if youve put it on Pause (see [`pause_space`]). You
...
#                 are not authenticated.
#             [`~utils.HfHubHTTPError`]:
#                 403 Forbidden: only the owner of a Space can restart it. If you want to restart a Space that you dont
#                 own, either ask the owner by opening a Discussion or duplicate the Space.
#             [`~utils.BadRequestError`]:
#                 If your Space is a static Space. Static Spaces are always running and never billed. If you want to hide
#                 a static Space, you can set it to private.
...
#         Raises:
#             [`~utils.RepositoryNotFoundError`]:
#               If one of `from_id` or `to_id` cannot be found. This may be because it doesnt exist,
#               or because it is set to `private` and you do not have access.
#             [`HTTPError`](https://requests.readthedocs.io/en/latest/api/#requests.HTTPError):
#               If the HuggingFace API returned an error
#         Example:
...
#                 The namespace to list endpoints for. Defaults to the current user. Set to `"*"` to list all endpoints
                from all namespaces (i.e. personal namespace and all orgs the user beints to).
#             token (Union[bool, str, None], optional):
#                 A valid user access token (string). Defaults to the locally saved
#                 token, which is the recommended method for authentication (see
#                 https://huggingface.co/docs/huggingface_hub/quick-start#authentication).
...
#             endpoints: List[InferenceEndpoint] = list_inference_endpoints(namespace=self._get_namespace(token=token))
            # Then list endpoints for all orgs the user beints to and ignore 401 errors (no billing or no access)
            for org in user.get("orgs", []):
                try:
#                     endpoints += list_inference_endpoints(namespace=org["name"], token=token)
                except HfHubHTTPError as error:
                    if error.response.status_code == 401:  # Either no billing or user dont have access)
...
#             namespace (`str`, *optional*):
#                 The namespace where the Inference Endpoint will be created. Defaults to the current users namespace.
#             token (Union[bool, str, None], optional):
#                 A valid user access token (string). Defaults to the locally saved
#                 token, which is the recommended method for authentication (see
#                 https://huggingface.co/docs/huggingface_hub/quick-start#authentication).
...
#                 A list of tags to associate with the Inference Endpoint.
#             namespace (`str`, *optional*):
#                 The namespace where the Inference Endpoint will be updated. Defaults to the current users namespace.
#             token (Union[bool, str, None], optional):
#                 A valid user access token (string). Defaults to the locally saved
#                 token, which is the recommended method for authentication (see
#                 https://huggingface.co/docs/huggingface_hub/quick-start#authentication).
...
#         This operation is not reversible. If you dont want to be charged for an Inference Endpoint, it is preferable
...
#             hf_raise_for_status(response)
            ...
            # If already running (and its ok), then fetch current status and return
...
#                 If `True`, do not raise an error if collection doesnt exists.
#             token (Union[bool, str, None], optional):
#                 A valid user access token (string). Defaults to the locally saved
#                 token, which is the recommended method for authentication (see
#                 https://huggingface.co/docs/huggingface_hub/quick-start#authentication).
...
#             hf_raise_for_status(r)
    ...
    pass
    pass
                # Collection doesnt exists and `missing_ok=True`
                return
            else:
#                 raise
...
#         Raises:
#             [`HTTPError`](https://requests.readthedocs.io/en/latest/api/#requests.HTTPError):
#                 or `admin` role in the organization the repo beints to or if you passed a `read` token.
#             [`HTTPError`](https://requests.readthedocs.io/en/latest/api/#requests.HTTPError):
#             [`HTTPError`](https://requests.readthedocs.io/en/latest/api/#requests.HTTPError):
#         Example:
#         ```py
...
#                 It must be retrieved from a [`CollectionItem`] object. Example: `collection.items[0].item_object_id`.
#             missing_ok (`bool`, *optional*):
#                 If `True`, do not raise an error if item doesnt exists.
#             token (Union[bool, str, None], optional):
#                 A valid user access token (string). Defaults to the locally saved
#                 token, which is the recommended method for authentication (see
#                 https://huggingface.co/docs/huggingface_hub/quick-start#authentication).
...
#             be populated with users answers.
#         Raises:
#             [`HTTPError`](https://requests.readthedocs.io/en/latest/api/#requests.HTTPError):
#             [`HTTPError`](https://requests.readthedocs.io/en/latest/api/#requests.HTTPError):
#                 or `admin` role in the organization the repo beints to or if you passed a `read` token.
...
#         tab. If the approval mode is automatic, you dont have to accept requests manually. An accepted request can be
...
#         Granting access dont require for the user to send an access request by themselves. The user is automatically
...
#         prefix = f{path_in_repo.strip('/')}/ if path_in_repo else
        # If updating a README.md file, make sure the metadata format is valid
        # Its better to fail early than to fail after all the files have been hashed.
...
#                 To disable authentication, pass `False`.
#         Returns:
#             `User`: A [`User`] object with the users overview.
#         Raises:
#             [`HTTPError`](https://requests.readthedocs.io/en/latest/api/#requests.HTTPError):
#         r = get_session().get(
