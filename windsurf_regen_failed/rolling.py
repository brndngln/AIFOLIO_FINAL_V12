...
#         self._as_index = _as_index
        # GH 32262: Its convention to keep the grouping column in
...
#         result.index = result_index
    ...
    def _create_data(self, obj: NDFrameT, numeric_only: bool = False) -> NDFrameT:
#         Split data into blocks & return conformed data.
        # Ensure the object were rolling over is monotonically sorted relative
        # to the groups
        # GH 36197
        if not obj.empty:
    pass
...
#         ).replace(\n, , 1),
#         window_method="rolling",
#         aggregation_description="Fishers definition of kurtosis without bias",
#         agg_method="kurt",
#     )
    def kurt(self, numeric_only: bool = False):
...
#         numpy.corrcoef : NumPy Pearsons correlation calculation.
#         ).replace(\n, , 1),
#         template_see_also,
#         create_section_header("Notes"),
#         dedent(
#         This function uses Pearsons definition of correlation
#         (https://en.wikipedia.org/wiki/Pearson_correlation_coefficient).
#         When `other` is not specified, the output will be self correlation (e.g.
#         all 1s), except for :class:`~pandas.DataFrame` inputs with `pairwise`
#         set to `True`.
...
#             assert isinstance(indexer_kwargs, dict)  # for mypy
            # Well be using the index of each group later
