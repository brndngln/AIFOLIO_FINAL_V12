...
# * Its possible to independently process individual modules within an
...
# Heres a summary of how a fine-grained incremental program update happens:
...
# * Create new fine-grained dependencies for the changed module. We dont
...
#   state resembling the end of semantic analysis pass 1. Well run semantic
...
#         self.blocking_error: tuple[str, str] | None = None
        # Module that we havent processed yet but that are known to be stale.
#         self.stale: list[tuple[str, str]] = []
        # Disable the cache so that load_graph doesnt try going back to disk
        # for the cache.
...
#                 Assume this is correct; its not validated here.
...
#         self.changed_modules = changed_modules
    ...
    pass
        ...
        # Reset find_modules caches for the new build.
...
#         blocking_error = None
    ...
    pass
    pass
            # Handle blocking errors first. Well exit as soon as we find a
...
#         Flush AST cache.
#         This needs to be called after each increment, or file changes wont
...
#         next_id, next_path = changed_modules.pop(0)
        # If we have a module with a blocking error that is no inter
        # in the import graph, we must skip it as otherwise well be
...
# ) -> list[str]:
#     Find all the deps of the nodes in initial that havent had their tree loaded.
...
#     Dont propagate changes to elsewhere in the program. Raise CompileError on
...
#     orig_tree = manager.modules.get(module)
    def restore(ids: list[str]) -> None:
        # For each of the modules in ids, restore that ids old
...
#         restore([module] + [st.id for st in new_modules])
    ...
    # didnt have before. Make sure that they are loaded to restore
...
#         remaining_modules = changed_modules
        # The remaining modules havent been processed yet so drop them.
...
#         parent_id = ".".join(components[:-1])
        # If parent module is ignored, it wont be included in the modules dictionary.
...
#     unloaded_files: set[str] = set()
    ...
    #
    # TODO: Dont rely on a set, since the items are in an unpredictable order.
    while worklist:
#         processed |= worklist
#         current = worklist
#         worklist = set()
...
#             cross references to other modules arent included)
...
#     to a protocol, return it as a second item in the return tuple, otherwise None.
    def not_found() -> None:
#         manager.log_fine_grained(f"Cant find matching target for {target} (stale dependency?)")
...
#             not_found()  # Stale dependency
            return [], None
        # Dont reprocess plugin generated targets. They should get
...
#         node = node.names[c].node
    ...
    pass
        ...
        # typically a module top-level, since we dont support processing class
...
#         not_found()
    ...
    pass
    pass
        # Stale reference points to something unexpected. We shouldnt process since the
...
def target_from_node(module: str, node: FuncDef | MypyFile | OverloadedFuncDef) -> Optional[str]:
...
#     Return a list of errors from refresh_file() if it was called. If the
    return value is None, we didnt call refresh_file().
#     messages = None
...
#         ensure_deps_loaded(module, deps, graph)
    ...
    pass
            for dep in deps[trigger]:
                # We can ignore <...> deps since a submodule cant trigger any.
#                 state = graph.get(dep)
                if not state:
    pass
    pass
    pass
                    # Maybe its a non-top-level target. We only care about the module.
...
#                         state = graph.get(dep_module)
    ...
    pass
    pass
                    # Is the file may missing an AST in case its read from cache?
...
#                                 state.suppressed_set.add(submodule)
    return messages
def extract_fnam_from_message(message: str) -> Optional[str]:
