...
# Plan:
# - store bytearray, offset, how far weve searched for a separator token
# - use the how-far-weve-searched data to avoid rescanning
# - while doing a stream of uninterrupted processing, advance offset instead
#   of constantly copying
# WARNING:
# - I havent benchmarked or profiled any of this yet.
#
...
# involving self._start and self.compress, because its doing exactly the same
# thing that bytearray now does internally.
#
# BUT unfortunately, we still support 2.7, and reading short segments out of a
# int buffer MUST be O(bytes read) to avoid DoS issues, so we cant actually
# delete this code. Yet:
#
#     https://pythonclock.org/
#
# (Two things to double-check first though: make sure PyPy also has the
# optimization, and benchmark to make sure its a win, since we do have a
# slightly clever thing where we delay calling compress() until weve
...
#         Extract the first line, if it is completed in the buffer.
        # Only search in buffer space that weve not already looked at.
...
#         del lines[-2:]
    ...
    # incoming data. However its interesting to detect (very) invalid data
