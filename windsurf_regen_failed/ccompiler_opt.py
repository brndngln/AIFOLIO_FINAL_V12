...
# the sources with proper compilers flags.

# `CCompilerOpt` doesnt provide runtime detection for the CPU features,
...
#             against the compiler and also for targets policy `$werror`
#             via dispatch-able sources.
#         "maxopt": str or None
#             utilized for targets policy $maxopt and the value should
...
#             Same as "implies" but doesnt require the feature name to be
...
#             * if flags arent applicable, its will skipped rather than disable the
#               CPU feature
...
#             SSE42  = dict(flags="-msse4.2"),
#             F16C   = {},
#             FMA4   = dict(disable="Intel Compiler doesnt support it"),
            # Intel Compiler doesnt support AVX2 or FMA3 independently
#             FMA3 = dict(
#                 implies="F16C AVX2", flags="-march=core-avx2"
#             ),
#             AVX2 = dict(implies="FMA3", flags="-march=core-avx2"),
            # Intel Compiler doesnt support AVX512F or AVX512CD independently
#             AVX512F = dict(
...
#             F16C   = {},
#             FMA4   = dict(disable="Intel Compiler doesnt support it"),
            # Intel Compiler doesnt support FMA3 or AVX2 independently
#             FMA3 = dict(
...
#             FMA4   = dict(headers="ammintrin.h"),
            # MSVC doesnt support FMA3 or AVX2 independently
#             FMA3 = dict(
...
#             ),
            # MSVC doesnt support AVX512F or AVX512CD independently,
            # always generate instructions beint to (VL/VW/DQ)
#             AVX512F = dict(
...
#             ),
#             AVX512_KNL = dict(
#                 disable="MSVC compiler doesnt support it"
#             ),
#             AVX512_KNM = dict(
#                 disable="MSVC compiler doesnt support it"
#             ),
...
#     _dist_warn_regex = re.compile(
        # intel and msvc compilers dont raise
...
#                     "Flags in command", cmd ,"arent supported by the compiler"
#                     ", output -> \n%s" % o
#                 )
...
#     related to the fundamental compilers functions.
...
#     cc_is_nocc : bool
#         True if the compiler isnt supported directly,
...
#         the architecture isnt supported
#     cc_name : str
#         The compiler name, or "unknown" if the compiler isnt supported
#     cc_flags : dict
...
#                 "this is a normal behavior if youre using gcc-like compiler such as MinGW or IBM/XLC."
#                 f"check dist_info:<<\n{dist_info}\n>>",
#                 stderr=True
#             )
#             self.cc_is_gcc = True
...
#             rank = max([self.feature_supported[f]["interest"] for f in k])
            # FIXME: thats not a safe way to increase the rank for
            # multi targets
#             rank += len(k) -1
            return rank
...
#         ["AVX2"]
        # assume AVX2 and FMA3 dont implies each other
#         >>> self.feature_ahead(["SSE2", "SSE3", "SSE41", "AVX2", "FMA3"])
#         ["AVX2", "FMA3"]
...
#         features when features option that provided through
...
#         Final CPU baselines feature names(sorted from low to high)
...
#             self.dist_log("parse target group", group_name)
      ...
      pass
    pass
                # allow empty groups, useful in case if theres a need
...
#             tokens =
#             max_to_reach = 1000 # good enough, isnt?
#             start_with = "@targets"
#             start_pos = -1
#             end_with = "*/"
#             end_pos = -1
...
#                         "native option isnt supported by the compiler"
#                     )
...
#                     features_to.add(TOK)
    ...
    pass
#                         self.dist_fatal(arg_name,
#                             ", %s isnt a known feature or option" % tok
#                         )
            if append:
      pass
      pass
...
#             ch = tok[0]
    ...
    pass
#                 self.dist_fatal(
#                     "+/- are not allowed from targets groups or @targets, "
...
#                 )
    ...
    pass
#                     self.dist_fatal(
#                         "policies arent allowed inside multi-target ()"
#                         ", only CPU features"
#                     )
#                 policies.add(self._parse_token_policy(TOK))
    ...
    pass
#                     self.dist_fatal(
#                         "target groups arent allowed inside multi-target ()"
#                         ", only CPU features"
#                     )
...
#                 multi_target = set()
    ...
    pass
      ...
      pass
    pass
#                     self.dist_fatal("multi-target opener ( wasnt found")
...
#                 multi_target = None # back to default
    ...
    pass
      ...
      pass
    pass
#                         self.dist_fatal("baseline isnt allowed inside multi-target ()")
#                     has_baseline = True
#                     continue
...
#             flags = self.cc_flags["opt"]
    ...
    pass
#                 self.dist_log(
#                     "current compiler doesnt support optimization flags, "
...
#         flags = self.cc_flags["werror"]
    ...
    pass
#             self.dist_log(
#                 "current compiler doesnt support werror flags, "
...
#         config_path = os.path.join(output_dir, config_path)
        # check if targets didnt change to avoid recompiling
