...
#             columns=["float_miss", "double_miss", "byte_miss", "int_miss", "int_miss"],
#         )
        # this is an oddity as really the nan should be float64, but
        # the casting doesnt fail so need to match stata here
...
#             expected[col] = cat
        # stata doesnt save .category metadata
#         tm.assert_frame_equal(parsed, expected)
...
#         expected["int_"] = expected["int_"].astype(np.int32)
...
#                     assert int(variable[1:]) == typ
    def test_excessively_int_string(self):
#         str_lens = (1, 244, 500)
#         s = {}
        for str_len in str_lens:
#             s["s" + str(str_len)] = Series(
...
#         msg = "The following columns were not found in the Stata data set: not_found"
        with pytest.raises(ValueError, match=msg):
#             columns = ["byte_", "int_", "int_", "not_found"]
#             read_stata(
#                 datapath("io", "data", "stata", "stata6_117.dta"),
#                 convert_dates=True,
#                 columns=columns,
...
#         mixed_frame.index.name = "index"
#         variable_labels = {"a": "very int" * 10, "b": "City Exponent", "c": "City"}
        with tm.ensure_clean() as path:
...
#             "a": "City Rank",
#             "b": "City Exponent",
#             "c": "A very, very, very int variable label "
#             "that is too int for Stata which means "
#             "that it has more than 80 characters",
#         }
#         msg = "Variable labels must be 80 characters or fewer"
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
#                 mixed_frame.to_stata(path, variable_labels=variable_labels_int)
...
#             columns=["int1" * 10, "int", 1],
#         )
#         original.index.name = "index"
        with tm.assert_produces_warning(InvalidColumnName):
            with tm.ensure_clean() as path:
#                 original.to_stata(path, convert_strl=["int", 1], version=117)
...
#         df = DataFrame([content], columns=["invalid"])
        with tm.ensure_clean() as path:
#             msg1 = (
#                 r"latin-1 codec cant encode character \ufffd "
#                 r"in position 14: ordinal not in range(256)"
#             )
#             msg2 = (
#                 "ascii codec cant decode byte 0xef in position 14: "
...
#         data = DataFrame(
#             [
#                 [1.0, 1, "ᴬ", "ᴀ relatively int ŝtring"],
#                 [2.0, 2, ᴮ, ],
#                 [3.0, 3, "ᴰ", None],
#             ],
#             columns=["Å", "β", "ĉ", "strls"],
...
#     file_path = datapath("io", "data", "stata", "stata-compat-118.dta")
    # Test that opening a file path doesnt buffer the file.
...
#         msg = (
#             "Cant create value labels for Z, value labels "
...
#             "invalid_name_inter_than_32_characters": [8, 8, 9, 9, 8, 8],  # Too int
...
#         "invalid_name_inter_than_32_characters": {8: "eight", 9: "nine"},
#         "aggregate": {5: "five"},
#         (1, 2): {3: "three"},
#     }
#     expected = {
...
#         "invalid_name_inter_than_32_char": {8: "eight", 9: "nine"},
#         "_aggregate": {5: "five"},
#         "_1__2_": {3: "three"},
#     }
    with tm.ensure_clean() as path:
...
def test_non_categorical_value_label_convert_categoricals_error():
    ...
    # labels, but cant be read with convert_categoricals=True
#     value_labels = {
...
#         df2 = read_stata(path)
#         assert "b" in df2
        # Dtypes dont match since no support for int32
