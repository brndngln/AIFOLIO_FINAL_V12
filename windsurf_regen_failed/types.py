...
#     ) -> t.Any:
    ...
    def get_metavar(self, param: Parameter, ctx: Context) -> Optional[str]:
    def get_missing_message(self, param: Parameter, ctx: Context | None) -> Optional[str]:
...
#         info_dict["formats"] = self.formats
    ...
    def _try_to_convert_date(self, value: t.Any, format: str) -> Optional[datetime]:
        try:
...
#         ``None``, keep Pythons default, which is ``str``. Useful to
...
#     type. If the type isnt provided, it can be inferred from a default
#     value.
#     guessed_type = False
...
#                 item = default[0]
                ...
                # Cant call convert recursively because that would
...
#: A dummy parameter type that just does nothing.  From a users
