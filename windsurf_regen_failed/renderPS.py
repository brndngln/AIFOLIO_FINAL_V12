...
# __version__='3.3.0'
# __doc__."""Render drawing objects in Postscrip."""
import math
...
# come out in Adobes old StandardEncoding, which NOBODY uses.
# PS_WinAnsiEncoding.
# /RE { %def
#   findfont begin
...
#         self.code_append('[%s] concat' % fp_str(a,b,c,d,e,f))
    def _drawTimeResize(self,w,h):
        if this is used were probably in the wrong world
#         self.width, self.height = w, h
    def _drawImageLevel1(self, image, x1, y1, width=None, height=None):
        # Postscript Level1 version available for fallback mode when Level2 doesnt work
        # For now lets start with 24 bit RGB images (following piddlePDF again)
#         component_depth = 8
...
#             height = imgheight
        ...
        # first lets map the user coordinates start at offset x1,y1 on page
#         self.code.extend([
#             'gsave',
...
#         At present were handling only PIL
...
#             self._canvas.polyLine(_pointsFromList(p.points))
    def drawEllipse(self, ellipse):
        #need to convert to pdfgens bounding box representation
