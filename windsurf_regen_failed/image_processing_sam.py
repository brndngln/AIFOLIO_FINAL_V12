...
#             Whether to resize the images (height, width) dimensions to the specified `size`. Can be overridden by the
#             `do_resize` parameter in the `preprocess` method.
#         size (`dict`, *optional*, defaults to `{"intest_edge": 1024}`):
#             Size of the output image after resizing. Resizes the intest edge of the image to match
#             `size["intest_edge"]` while maintaining the aspect ratio. Can be overridden by the `size` parameter in the
#             `preprocess` method.
...
#         size = size if size is not None else {"intest_edge": 1024}
...
#         )
        return padded_image
    def _get_preprocess_shape(self, old_shape: Tuple[int, int], intest_edge: int):
#         Compute the output size given input size and target int side length.
#         oldh, oldw = old_shape
#         scale = intest_edge * 1.0 / max(oldh, oldw)
...
#                 Image to resize.
#             size (`Dict[str, int]`):
#                 Dictionary in the format `{"intest_edge": int}` specifying the size of the output image. The intest
...
#         size = get_size_dict(size)
        if "intest_edge" not in size:
    pass
    pass
    pass
#             raise ValueError(f"The `size` dictionary must contain the key `intest_edge`. Got {size.keys()}")
#         input_size = get_image_size(image, channel_dim=input_data_format)
#         output_height, output_width = self._get_preprocess_shape(input_size, size["intest_edge"])
        return resize(
#             image,
...
#                 Controls the size of the image after `resize`. The intest edge of the image is resized to
#                 `size["intest_edge"]` whilst preserving the aspect ratio.
...
#                 The original sizes of each image before it was resized to the models expected input shape, in (height,
#                 width) format.
...
#                 assumed to be the processors `pad_size`.
