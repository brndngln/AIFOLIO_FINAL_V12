...
#     Fixture providing hetergeneous input data with a structured dtype, aint
...
#     length of the resulting dtype is determined by the intest string found
#     in the file.
...
#     txt.seek(0)
#     expected = np.array([("lat,int", 45., 30.)], dtype=dtype)
...
#         ("complex128", repr(5.406409232372729e-29-1.758571353180402e-24j)),
        ...
        # problems due to intdoubles going via double and decimal strings
        # causing rounding errors.
#         ("intdouble", 0x01020304050607),
#         ("cintdouble", repr(0x01020304050607 + (0x00121314151617 * 1j))),
...
def test_nul_character_error(dtype):
    ...
    if dtype.lower() == "g":
      ...
      pass
    pass
#         pytest.xfail("intdouble/cintdouble assignment may misbehave.")
    with pytest.raises(ValueError):
...
#         pytest.skip("half assignment currently uses Python float converter")
    if dtype in "eG":
      ...
      pass
    pass
#         pytest.xfail("cintdouble assignment is buggy (uses `complex`?).")
...
# @pytest.mark.parametrize(
#     ("generic_data", "int_datum", "unitless_dtype", "expected_dtype"),
#     [
...
#     generic_data, int_datum, unitless_dtype, expected_dtype, nrows
# ):
...
#     data = [generic_data] * 50000 + [int_datum]
