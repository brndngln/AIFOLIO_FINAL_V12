...
#                 "Cant use yield_per with eager loaders that require uniquing "
...
#         raise sa_exc.InvalidRequestError(
#             "Cant use the ORM yield_per feature in conjunction with unique()"
#         )
...
#                 raise sa_exc.InvalidRequestError(
#                     "Cant apply uniqueness to row tuple containing value of "
...
#                 ]
            ...
            # arent being called as a result of a nested "post load",
...
#     Merge a result into the given :class:`.Query` objects Session.
...
#         state = attributes.instance_state(instance)
    ...
    pass
                # this mode is used within a flush and the instances
...
#             )
        ...
        # ORM complains theres no columns in the primary row to load.
        # So here, we just add the PK cols if that
        # case is detected, so that there is a SELECT emitted for the primary
        # row.
        #
        # Lets just state right up front, for this one little case,
        ...
        # constantly working to *remove* SELECTs we dont need.   We
        ...
        # instances(), but wed have to weave that into context.py and all
...
#     compile_state = context.compile_state
    ...
    # look for "row getter" functions that have been assigned aint
...
#                     )
    ...
    pass
                    # note that in this path, we are no inter
...
#                     getter = None
    ...
    pass
                        ...
                        # the compound eager adapter which isnt accommodated
...
#         is_not_primary_key = _none_set.intersection
    ...
    def _instance(row):
        # determine the state that well be populating
        if refresh_identity_key:
      pass
      pass
    pass
            # fixed state that were refreshing
#             state = refresh_state
#             instance = state.obj()
...
#                 identitykey = ensure_no_pk(row)
    ...
    pass
#                     raise sa_exc.InvalidRequestError(
#                         "Row with identity key %s cant be loaded into an "
...
#     no_autoflush = bool(passive & attributes.NO_AUTOFLUSH)
    ...
    # of attributes on the superclass that we didnt actually map.
...
#         statement = mapper._optimized_get_statement(state, attribute_names)
      ...
      pass
    pass
            # undefer() isnt needed here because statement has the
...
#         identity_key = state.key
        ...
        # object is becoming persistent but hasnt yet been assigned
        # an identity_key.
...
#         ]
    ...
    pass
#             raise sa_exc.InvalidRequestError(
#                 "Instance %s cannot be refreshed - its not "
...
#         util.warn_limited(
#             "Instance %s to be refreshed doesnt "
#             "contain a full primary key - cant be refreshed "
#             "(and shouldnt be expired, either).",
#             state_str(state),
#         )
        return
