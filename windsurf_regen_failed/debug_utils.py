...
#     around 62.7K, which is very close to fp16s top limit of 64K. In the next frame we have `Dropout` which
#     renormalizes the weights, after it zeroed some of the elements, which pushes the absolute max value to more than
#     64K, and we get an overlow.

#     As you can see its the previous frames that we need to look into when the numbers start going into very large for
#     fp16 numbers.
...
#         Lets say you want to watch the absolute min and max values for all the ingredients of each `forward` call of a
...
#     As this module measures absolute `min`/``max` of each weight of the model on every forward itll slow the training
...
#         self.module_names = {m: name for name, m in self.model.named_modules()}
        # self.intest_module_name = max(len(v) for v in self.module_names.values())
...
#             self.dump_saved_frames()
            ...
            # now we can abort, as its pointless to continue running
#             raise ValueError(
