...
#             Note: for better compatibility with OpenAIs client, `model` has been aliased as `base_url`. Those 2
...
#             Defaults to "auto" i.e. the first of the providers available for the model, sorted by the users order in https://hf.co/settings/inference-providers.
#             If model is a URL or `base_url` is passed, then `provider` is not used.
#         token (`str`, *optional*):
#             Hugging Face token. Will default to the locally saved token if not provided.
#             Note: for better compatibility with OpenAIs client, `token` has been aliased as `api_key`. Those 2
...
#             The billing account to use for the requests. By default the requests are billed on the users account.
...
#                 " `base_url` is an alias for `model` to make the API compatible with OpenAIs client."
...
#                 " `api_key` is an alias for `token` to make the API compatible with OpenAIs client."
...
#             request_parameters.headers["Accept"] = "image/png"
        ...
        with _open_as_binary(request_parameters.data) as data_as_binary:
            # Do not use context manager as we dont want to close the connection immediately when returning
            # a stream
...
#                 "This can happen if youve stopped streaming data from the server before the stream was complete. "
...
#                 Additional provider-specific parameters to pass to the model. Refer to the providers documentation
...
#         The `client.chat_completion` method is aliased as `client.chat.completions.create` for compatibility with OpenAIs client.
...
#         Check out the [Inference guide](https://huggingface.co/docs/huggingface_hub/guides/inference#openai-compatibility)
        for more details about OpenAIs compatibility.
...
#                 text so far, increasing the models likelihood to talk about new topics.
...
#         ...     {
#         ...         "role": "system",
#         ...         "content": "Dont make assumptions about what values to plug into functions. Ask for clarification if a user request is ambiguous.",
#         ...     },
#         ...     {
#         ...         "role": "user",
#         ...         "content": "Whats the weather like the next 3 days in San Francisco, CA?",
#         ...     },
#         ... ]
#         >>> tools = [
#         ...     {
...
#                 If the words in the document are too int to fit with the question for the model, it will be split in
...
#         (OCR), Pix2Struct, etc). Please have a look to the model card to learn more about a models specificities.
...
#             doc_stride (`int`, *optional*):
#                 If the context is too int to fit with the question for the model, it will be split in several chunks
...
#         >>> from huggingface_hub import AsyncInferenceClient
#         >>> client = AsyncInferenceClient()
#         >>> await client.question_answering(question="Whats my name?", context="My name is Clara and I live in Berkeley.")
...
#         ...         "I cant believe how much I struggled with this.",
#         ...     ],
#         ... )
...
#                 the text so far, decreasing the models likelihood to repeat the same line verbatim.
...
#         TranslationOutput(translation_text=Je mappelle Wolfgang et je vis à Berlin.)
#         ```

#         Specifying languages:
#         ```py
#         >>> client.translation("My name is Sarah Jessica Parker but you can call me Jessica", model="facebook/mbart-large-50-many-to-many-mmt", src_lang="en_XX", tgt_lang="fr_XX")
#         "Mon nom est Sarah Jessica Parker mais vous pouvez mappeler Jessica"
#         ```
...
#         >>> client = AsyncInferenceClient()
#         >>> text = (
#         ...     "A new model offers an explanation for how the Galilean satellites formed around the solar systems"
#         ...     "largest world. Konstantin Batygin did not set out to solve one of the solar systems most puzzling"
