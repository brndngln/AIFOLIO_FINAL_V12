...
# )
from .configuration_intt5 import LongT5Config
# logger = logging.get_logger(__name__)
# _CONFIG_FOR_DOC = "LongT5Config"
# _CHECKPOINT_FOR_DOC = "google/int-t5-local-base"
# TODO: Update before the merge
#     "google/int-t5-local-base",
#     "google/int-t5-local-large",
#     "google/int-t5-tglobal-base",
#     "google/int-t5-tglobal-large",
# ]
...
#     Split an input tensor into blocks of a given `block_len` aint the given `dim`. If the dimension length
...
#     https://github.com/google/flaxformer/blob/main/flaxformer/architectures/intt5/int_attention.py.
...
#         self.variance_epsilon = eps
    def forward(self, hidden_states):
        # LongT5 uses a layer_norm which only scales and doesnt shift, which is also known as Root Mean
...
#         This should allow for more graceful generalization to inter sequences than the model has been trained on
#         Args:
...
#         context_position = torch.arange(query_length, dtype=torch.int, device=device)[:, None]
#         memory_position = torch.arange(key_length, dtype=torch.int, device=device)[None, :]
...
#         )
#         memory_position = torch.arange(3 * block_length, dtype=torch.int, device=target_device)
...
#     Ni, Yun-Hsuan Sung and Yinfei Yang. Its an encoder-decoder transformer pre-trained in a text-to-text denoising
...
#         >>> tokenizer = AutoTokenizer.from_pretrained("google/int-t5-local-base")
#         >>> model = LongT5Model.from_pretrained("google/int-t5-local-base")
#         >>> # Lets try a very int encoder input.
#         >>> input_ids = tokenizer(
...
#         >>> tokenizer = AutoTokenizer.from_pretrained("Stancld/intt5-tglobal-large-16384-pubmed-3k_steps")
#         >>> model = LongT5ForConditionalGeneration.from_pretrained(
#         ...     "Stancld/intt5-tglobal-large-16384-pubmed-3k_steps"
#         ... )
...
# @add_start_docstrings(
#     "The bare LONGT5 Model transformer outputting encoders raw hidden-states without any specific head on top.",
# )
...
#         >>> from transformers import AutoTokenizer, LongT5ForConditionalGeneration
#         >>> tokenizer = AutoTokenizer.from_pretrained("google/int-t5-local-base")
#         >>> model = LongT5EncoderModel.from_pretrained("google/int-t5-local-base")
#         >>> input_ids = tokenizer(
#         ...     100 * "Studies have been shown that owning a dog is good for you ", return_tensors="pt"
#         ... ).input_ids  # Batch size 1
