...
#         hidden_states = self.embeddings(input_ids=input_ids, position_ids=position_ids)
        ...
        # OWLV2s text model uses causal mask, prepare it here.
...
#         ```
        # Use OWLv2 models config for some fields (if specified) instead of those of vision & text components.
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
...
#         >>> # hence well set the target image sizes (height, width) based on that
