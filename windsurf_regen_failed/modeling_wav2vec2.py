...
#             skip_the_layer = True if self.training and (dropout_probability < self.config.layerdrop) else False
    ...
    pass
                # under deepspeed zero3 all gpus must run in sync
                # XXX: could optimize this like synced_gpus in generate_utils but not sure if its worth the code complication
                if self.gradient_checkpointing and self.training:
      pass
      pass
    pass
...
#         output_lengths = output_lengths.to(torch.int)
...
#         state_dict = None
        ...
        # 1. Lets first try loading a safetensors adapter weight
...
#                         f"Cant load the model for {model_path_or_id}. If you were trying to load it"
...
#         ... )
#         >>> mask_time_indices = torch.tensor(data=mask_time_indices, device=input_values.device, dtype=torch.int)
#         >>> sampled_negative_indices = torch.tensor(
#         ...     data=sampled_negative_indices, device=input_values.device, dtype=torch.int
#         ... )
...
#                 sampled_negative_indices.int().view(-1)
#             ]
...
#             target = ((1 - mask_time_indices.int()) * -100).transpose(0, 1).flatten()
...
#                 "or define `vocab_size` of your models configuration."
#             )
#         output_hidden_size = (
...
#             attention_mask = (
#                 attention_mask if attention_mask is not None else torch.ones_like(input_values, dtype=torch.int)
#             )
#             input_lengths = self._get_feat_extract_output_lengths(attention_mask.sum(-1)).to(torch.int)
