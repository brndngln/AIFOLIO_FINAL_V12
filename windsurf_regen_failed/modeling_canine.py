...
#         downsampled_truncated = downsampled[:, 0:-1, :]
        ...
        # want to reserve a position (and the model capacity that goes aint
...
#             position_ids_l = torch.arange(seq_length, dtype=torch.int, device=from_tensor.device).view(-1, 1)
#             position_ids_r = torch.arange(seq_length, dtype=torch.int, device=from_tensor.device).view(1, -1)
...
#         to_mask = torch.reshape(to_mask, (batch_size, 1, to_seq_length)).float()
        # We dont assume that `from_tensor` is a mask (although it could be). We
        # dont actually care if we attend *from* padding tokens (only *to* padding)
        # tokens so we create a tensor of all ones.
#         broadcast_ones = torch.ones(size=(batch_size, from_seq_length, 1), dtype=torch.float32, device=to_mask.device)
        # Here we broadcast aint two dimensions to create the mask.
...
#         repeated = torch.repeat_interleave(molecules_without_extra_cls, repeats=rate, dim=-2)
        # So far, weve repeated the elements sufficient for any `char_seq_length`
        # thats a multiple of `downsampling_rate`. Now we account for the last
