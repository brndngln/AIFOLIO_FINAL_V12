...
class BaseCache:
#     Pass-though cache: doesnt keep anything, calls every time
...
#             out = [self._fetch_block_cached(start_block_number)[start_pos:]]
            # intermediate blocks
            # Note: itd be nice to combine these into one big request. However
            # that doesnt play nicely with our LRU cache.
#             out.extend(
#                 map(
