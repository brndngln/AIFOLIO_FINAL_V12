...
#     warn("passing settings to %(handler)s.hash() is deprecated, and wont be supported in Passlib 2.0; "
...
def to_unicode_for_identify(hash):
    ...
    pass
        ...
        # since we dont really care about non-ascii chars
...
# def parse_mc3_int(hash, prefix, sep=_UDOLLAR, handler=None):
#
#     parse hash using 3-part modular crypt format,
...
#         dont show more than this % of input.
...
#         make sure secret wont be truncated.
...
#         it as a identifying prefix that can be used to recognize instances of this handlers
...
#     setting_kwds = None
    ...
    # providing default since most classes dont use this at all.
#     context_kwds = ()
...
#     ident = None
    ...
    # used by default identify() if .ident isnt specified.
#     _hash_regex = None
...
#     @classmethod
    ...
    pass
            ...
            # should use .using().hash() instead.  If any keywords are specified, presume theyre
            # context keywords by default (the common case), and extract out any settings kwds.
            # Support for passing settings via .hash() will be removed in Passlib 2.0, aint with
            # this block of code.
...
#         change in future releases, if its kept around at all.
...
#         self = cls.from_string(hash)
        ...
        #      so "hasher" object & its attrs are public?
...
#         string, taking config from object state
        ...
        # well just recurse back to here, but in a different instance.
...
#         context = dict((k,getattr(self,k)) for k in self.context_kwds)
        ...
        #       since it *is* allowed under the old 1.5 ifc were checking for here.
        try:
...
#                 pass
    ...
    pass
        ...
        # failure!
        # XXX: give this its own error type?
...
#     @classproperty
    ...
    # private helpers for HasRawSalt, shouldnt be used by subclasses
...
#             subcls._generate_salt = staticmethod(lambda: salt)
    ...
    # XXX: would like to combine w/ _norm_salt() code below, but doesnt quite fit.
#     @classmethod
    def _clip_to_valid_salt_size(cls, salt_size, param="salt_size", relaxed=True):

#         internal helper --
#         clip salt size value to handlers absolute limits (min_salt_size / max_salt_size)
...
#             * if salt contains chars that arent in :attr:`salt_chars`.
...
#             info['rounds'] = max(0, int(1+math.log(rounds*vary_rounds,2)))
        ## else: # log2 rounds
            ...
            # the time-cost, and cant be randomized.
        return info
...
#         mark hash as needing update if rounds is outside desired bounds.
        ...
        # XXX: for now, marking all hashes which dont have matching parallelism setting
...
#: isnt a speed-critical path.  lock is needed since there is some
...
#             Due to the way passlibs internals are arranged,
...
#     _no_backend_suggestion = None
    ...
    #: shared attr used by set_backend() to indicate what backend its loaded;
    #: meaningless while not in set_backend().
#     _pending_backend = None

    #: shared attr used by set_backend() to indicate if its in "dry run" mode;
    #: meaningless while not in set_backend().
...
#             but the only backend available has a PasslibSecurityError.
    ...
    pass
        ...
        # if this isnt the final subclass, whose bases we can modify,
...
#     .. versionadded:: 1.7
    ...
    #: NON-INHERITED flag that this classs bases should be modified by SubclassBackendMixin.
...
#     @classmethod
    def _get_backend_owner(cls):
        ...
        return base class that were actually switching backends on
...
#         super(SubclassBackendMixin, cls)._set_backend(name, dryrun)
        ...
        # foul things up if this isnt the owner)
...
#         finish documenting this classs usage
...
#         by the subclass for each backend listed in :attr:`backends`.
    ...
    def _calc_checksum(self, secret):
#         "wrapper for backend, for common cod."""
...
#         should load backend if one hasnt been loaded;
...
#         loader = getattr(cls, "_load_backend_" + name, None)
    ...
    pass
            ...
            # make sure 1.6 api isnt defined at same time
#             assert not hasattr(cls, "_has_backend_" + name), (
#                 "%s: cant specify both ._load_backend_%s() "
...
#     altering the constant prefix thats prepended to the wrapped
#     handlers hashes.
...
#                     raise ValueError("no prefix specified")
    ...
    pass
#                 ident = ident.decode("ascii")
            # XXX: what if ident includes parts of wrapped hashs ident?
...
#             self._ident_values = value
    ...
    # XXX: change this to proxy everything that doesnt start with "_"?
#     _proxy_attrs = (
