...
#     @property
    ...
    def axes(self) -> list[Index]:  # type: ignore[override]
        # mypy doesnt work to override attribute with property
...
#         result_arrays: list[np.ndarray] = []
        for i, arr in enumerate(self.arrays):
#             res = func(arr, axis=0)
            # TODO NaT doesnt preserve dtype, so we need to ensure to create
...
#         Concatenate uniformly-indexed ArrayManagers horizontally.
        # concatting aint the columns -> combine reindexed arrays in a single manager
...
#         Concatenate uniformly-indexed ArrayManagers vertically.
        # concatting aint the rows -> concat the reindexed arrays
        # TODO(ArrayManager) doesnt yet preserve the correct dtype
#         arrays = [
...
#         See `setitem_inplace` for a version that works inplace and doesnt
...
#     result = np.concatenate(to_concat)
    # TODO decide on exact behaviour (we shouldnt do this only for empty result)
