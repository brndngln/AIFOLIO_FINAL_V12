...
#         missing_decoder_tokens = self.get_missing_alphabet_tokens(decoder, tokenizer)
    ...
    pass
#             raise ValueError(
#                 f"The tokens {missing_decoder_tokens} are defined in the tokenizers "
#                 "vocabulary, but not in the decoders alphabet. "
#                 f"Make sure to include {missing_decoder_tokens} in the decoders alphabet."
#             )
...
#         <Tip>
#         This class method is simply calling Wav2Vec2FeatureExtractors
#         [`~feature_extraction_utils.FeatureExtractionMixin.from_pretrained`], Wav2Vec2CTCTokenizers
...
#             **kwargs
#                 Additional keyword arguments passed aint to both [`SequenceFeatureExtractor`] and
#                 [`PreTrainedTokenizer`]
...
#     @staticmethod
    def get_missing_alphabet_tokens(decoder, tokenizer):
        ...
        # we need to make sure that all of the tokenizers except the special tokens
        # are present in the decoders alphabet. Retrieve missing alphabet token
        # from decoder
...
#         When used in normal mode, this method forwards all its arguments to Wav2Vec2FeatureExtractors
#         [`~Wav2Vec2FeatureExtractor.__call__`] and returns its output. If used in the context
#         [`~Wav2Vec2ProcessorWithLM.as_target_processor`] this method forwards all its arguments to
#         Wav2Vec2CTCTokenizers [`~Wav2Vec2CTCTokenizer.__call__`]. Please refer to the docstring of the above two
...
#         [`~Wav2Vec2ProcessorWithLM.as_target_processor`] this method forwards all its arguments to
#         Wav2Vec2CTCTokenizers [`~Wav2Vec2CTCTokenizer.pad`]. Please refer to the docstring of the above two methods
...
#         <Tip>
#         This function makes use of Pythons multiprocessing. Currently, multiprocessing is available only on Unix
...
#                 should be instantiated *after* `Wav2Vec2ProcessorWithLM`. Otherwise, the LM wont be available to the
#                 pools sub-processes.
#                 <Tip>
...
#                 Maximum number of beams at each step in decoding. Defaults to pyctcdecodes DEFAULT_BEAM_WIDTH.
#             beam_prune_logp (`int`, *optional*):
#                 Beams that are much worse than best beam will be pruned Defaults to pyctcdecodes DEFAULT_PRUNE_LOGP.
#             token_min_logp (`int`, *optional*):
#                 Tokens below this logp are skipped unless they are argmax of frame Defaults to pyctcdecodes
#             hotwords (`List[str]`, *optional*):
#                 List of words with extra importance, can be OOV for LM
#             hotword_weight (`int`, *optional*):
#                 Weight factor for hotword importance Defaults to pyctcdecodes DEFAULT_HOTWORD_WEIGHT.
...
#         hotword_weight = hotword_weight if hotword_weight is not None else DEFAULT_HOTWORD_WEIGHT
        # reset params at every forward call. Its just a `set` method in pyctcdecode
...
#         logits_list = [array[(array != -100.0).all(axis=-1)] for array in logits]
    ...
    pass
            ...
            # multiprocessings docs (https://docs.python.org/3/library/multiprocessing.html#contexts-and-start-methods)
...
#                 cm = nullcontext()
            ...
            # pool is managed by the user, so we dont need to close it
#             cm = nullcontext()
...
#                 be <= 0. Defaults to pyctcdecodes DEFAULT_PRUNE_LOGP.
#             token_min_logp (`int`, *optional*):
#                 Tokens with log-probs below token_min_logp are skipped unless they are have the maximum log-prob for an
#                 utterance. Defaults to pyctcdecodes DEFAULT_MIN_TOKEN_LOGP.
#             hotwords (`List[str]`, *optional*):
#                 List of words with extra importance which can be missing from the LMs vocabulary, e.g. ["huggingface"]
#             hotword_weight (`int`, *optional*):
#                 Weight multiplier that boosts hotword scores. Defaults to pyctcdecodes DEFAULT_HOTWORD_WEIGHT.
#             alpha (`float`, *optional*):
#                 Weight for language model during shallow fusion
#             beta (`float`, *optional*):
#                 Weight for length score adjustment of during scoring
...
#         Example:
#         ```python
#         >>> # Lets see how to retrieve time steps for a model
