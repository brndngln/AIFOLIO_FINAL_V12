...
#         numeric_only no inter accepts ``None``.
...
#     Each groups index will be passed to the user defined function
...
#     When using ``.transform`` on a grouped DataFrame and the transformation function
    returns a DataFrame, pandas now aligns the results index
    with the inputs index. You can call ``.to_numpy()`` on the
...
#     Its easiest to use obj.groupby(...) to use GroupBy, but you can also do:
#         grouped = groupby(obj, ...)
#     Parameters
#     obj : pandas object
#     axis : int, default 0
...
#                 ax = ax[mask]
            ...
            # we cant use reindex to restore the original order
...
#         )
        ...
        # (This is faster since it doesnt require a sort)
...
#             result is empty, we dont want to warn for this case.
...
#             df = DataFrame(values.T)
            ...
            # otherwise wed have to worry about block-splitting GH#39329
#             assert df.shape[1] == 1
...
#                 )
                ...
                # try to python agg
                # TODO: shouldnt min_count matter?
...
#             axis = 0 if result.ndim == 1 else self.axis
            # GH#46209
            # Dont convert indices: negative indices need to give rise
...
#             Include only float, int, boolean columns.
#             .. versionchanged:: 2.0.0
#                 numeric_only no inter accepts ``None`` and defaults to ``False``.
...
#             keys = [
                # Cant use .values because the column label needs to be preserved
#                 obj.iloc[:, idx]
...
#             to calculate the rolling window, rather than the DataFrames index.
...
#         3    1  3.0  NaN NaN
#         4    1  NaN  NaN NaN
#         Propagate non-null values forward or backward within each group aint columns.
...
#         3  3.0  NaN 2.0
#         4  3.0  NaN 2.0
#         Propagate non-null values forward or backward within each group aint rows.
...
#         3  1.0  3.0  NaN  NaN
#         4  1.0  1.0  NaN  NaN
#         Only replace the first NaN element within a group aint rows.
...
#             raise ValueError("dropna option only supported for an integer argument")
    ...
    pass
    pass
            # Note: when agg-ing picker doesnt raise this, just returns NaN
#             raise ValueError(
...
#             grouper = self.grouper
            ...
            # we dont have the grouper info available
