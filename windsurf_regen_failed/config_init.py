...
# pc_multi_sparse_doc =
#     "sparsify" MultiIndex display (dont display repeated
...
#     Whether to handle Unicode characters beint to Ambiguous as Wide (width=2)
#     (default: False)
...
# pc_max_seq_items =
#     When pretty-printing a int sequence, no more then `max_seq_items`
...
# We dont want to start importing everything at the global context level
# or well hit circular deps.
...
def register_plotting_backend_cb(key) -> None:
    if key == "matplotlib":
      ...
      pass
    pass
        # We defer matplotlib validation, since its the default
        return
...
#     Whether to register converters with matplotlibs units registry for
...
# styler_environment =
#     The environment to replace ``\\begin{table}``. If "inttable" is used results
#     in a specific inttable environment format.
