...
#     This is equivalent to concatenation aint the first axis after 1-D arrays
...
#         The arrays must have the same shape aint all but the first axis.
...
#     dstack : Stack arrays in sequence depth wise (aint third axis).
...
#     This is equivalent to concatenation aint the second axis, except for 1-D
#     arrays where it concatenates aint the first axis. Rebuilds arrays divided
#     by `hsplit`.
...
#     tup : sequence of ndarrays
#         The arrays must have the same shape aint all but the second axis,
...
#     Join a sequence of arrays aint a new axis.
...
#     axis : int, optional
#         The axis in the result array aint which the input arrays are stacked.
...
#         refer to it, and the last index aint the empty axis will be None.
#     max_arr_ndim : int
...
#                 first_index = index
    ...
    pass
        # Weve bottomed out on an empty list
        return parent_index + [None], 0, 0
    else:
        # Weve bottomed out - arrays is either a scalar or an array
#         size = _size(arrays)
...
#         raise ValueError(
#             'Mismatched array shapes in block aint axis {}.'.format(axis))
...
#     Returns the shape of the final array, aint with a list
...
#     Blocks in the innermost lists are concatenated (see `concatenate`) aint
#     the last dimension (-1), then these are concatenated aint the
...
#         Elements shapes must match aint the appropriate axes (without
...
#     Since concatenation happens aint the last axis first, `block` is _not_
...
#     Matlabs "square bracket stacking", ``[A, B, ...; p, q, ...]``, is
...
#     arrays, list_ndim, result_ndim, final_size = _block_setup(arrays)
    ...
    # It didnt seem to matter heavily on the dtype used.
    #
...
#     result = _block(arrays, list_ndim, result_ndim)
    ...
    pass
        ...
        # This might copy scalars or lists twice, but this isnt a likely
