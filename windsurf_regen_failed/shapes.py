...
#     'strokeLineCap': 0,
#     'strokeLineJoin': 0,
#     strokeMiterLimit : 10,    # dont know yet so let bomb here
...
#     rendered - they dont do anything active.  They provide
...
#             raise ValueError("Cant add, need name")
...
#             P[i] = P[i]+d
    ...
    #if theres a bounding box given we constrain so our points lie in it
...
#                         P[i] = c + fac*(P[i]-a)
                    ...
                    #theres no range in the bbox so fixed as average
                    for i in range(j,len(P),2):
#                         P[i] = (c + d)*0.5
            else:
                #if   theres a  lower bound shift so min is lower bound
                #else theres an upper bound shift so max is upper bound
