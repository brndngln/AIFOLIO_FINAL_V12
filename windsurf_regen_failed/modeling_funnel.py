...
#         min_dist = pooled_pos[0] - pos[-1]
        ...
        return torch.arange(max_dist, min_dist - 1, -stride, dtype=torch.int, device=pos.device)
...
#         Perform pooling by stride slicing the tensor aint the given axis.
...
#     batch_size, n_head, seq_len, max_rel_len = positional_attn.shape
    ...
    # Whats next is the same as doing the following gather, which might be clearer code but less efficient.
...
#     ) -> Union[Tuple, BaseModelOutput]:
        # The pooling is not implemented on int tensors, so we convert this mask.
