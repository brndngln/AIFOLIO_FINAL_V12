...
#             )
    ...
    pass
            ...
            # AST doesnt include a Var node for the module. We
...
#             vself = builder.call_c(type_op, [vself], expr.line)
    ...
    pass
            ...
            # of ugly, but we cant search by name since the self parameter
            # could be named anything, and it doesnt get added to the
            # environment indexes.
#             self_targ = list(builder.symtables[-1].values())[6]
#             vself = builder.read(self_targ, builder.fn_info.fitem.line)
...
#     Return None if a specialized op isnt available.
...
def precompute_set_literal(builder: IRBuilder, s: SetExpr) -> Optional[Value]:
#     Try to pre-compute a frozenset literal during module initialization.

#     Return None if its not possible.
...
#                     right = builder.accept(right_expr, can_borrow=True)
    ...
    # TODO: Dont produce an expression when used in conditional context
...
def transform_math_literal(builder: IRBuilder, fullname: str) -> Optional[Value]:
    if fullname == "math.e":
      pass
      pass
    pass
