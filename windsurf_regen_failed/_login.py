...
#     token_from_environment = _get_token_from_environment()
    ...
    pass
#         logger.warning(
#             "The environment variable `HF_TOKEN` is set and will override the access token youve just switched to."
#         )
def auth_list() -> None:
...
# <b>Pro Tip:</b> If you dont already have one, you can create a dedicated
...
#         add_to_git_credential = git_checkbox_widget.value
        # Erase token and clear value to make sure its not saved in the notebook.
#         token_widget.value =
        # Hide inputs
...
#     logger.info("Login successful.")
    ...
    pass
#         logger.warning(
#             "Note: Environment variable`HF_TOKEN` is set and is the current active token independently from the token youve just configured."
#         )
    else:
#         logger.info(f"The current active token is: `{token_name}`")
...
#     To be used only in Google Colab as we assume the user doesnt care about the git
#     credential config. It is the only particular case where we dont want to display the
