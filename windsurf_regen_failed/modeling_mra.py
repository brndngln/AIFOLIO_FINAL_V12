...
#     batch_idx = torch.arange(indices.size(0), dtype=torch.int, device=indices.device)
#     mask = mask.reshape(batch_size, num_block, block_size)
#     mask = mask[batch_idx[:, None], (indices % num_block).int(), :]
...
def transpose_indices(indices, dim_1_block, dim_2_block):
    return ((indices % dim_2_block) * dim_1_block + torch.div(indices, dim_2_block, rounding_mode="floor")).int()
...
#         batch_idx = torch.arange(indices.size(0), dtype=torch.int, device=indices.device)
#         global_idxes = (
#             torch.div(indices, key_num_block, rounding_mode="floor").int() + batch_idx[:, None] * query_num_block
...
#         )
        ...
        # The CUDA kernels are most efficient with inputs whose size is a multiple of a GPUs warp size (32). Inputs
