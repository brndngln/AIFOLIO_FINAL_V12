...
#             assert_raises(ValueError, np.einsum, ",", [0], optimize=do_opt)
            # cant have more subscripts than dimensions in the operand
...
#         self.check_einsum_sums('f8', True)
    def test_einsum_sums_intdouble(self):
#         self.check_einsum_sums(np.intdouble)
...
#         self.check_einsum_sums('c16')
    def test_einsum_sums_cintdouble(self):
#         self.check_einsum_sums(np.cintdouble)
...
#         assert_raises(ValueError, lambda: np.einsum(a, [-1, 5], b, [5, 2], [-1, 2], optimize=False))
    def test_einsum_broadcast(self):
        ...
        # adds auto broadcast on left where it beints
...
#         self.assert_path_equal(path, ['einsum_path', (0, 1, 2)])
#         int_test = self.build_operands('acdf,jbje,gihb,hfac')
#         path, path_str = np.einsum_path(*int_test, optimize=('greedy', 0))
#         self.assert_path_equal(path, ['einsum_path', (0, 1, 2, 3)])
#         path, path_str = np.einsum_path(*int_test, optimize=('optimal', 0))
#         self.assert_path_equal(path, ['einsum_path', (0, 1, 2, 3)])
    def test_int_paths(self):
        # Long complex cases
        # Long test 1
#         int_test1 = self.build_operands('acdf,jbje,gihb,hfac,gfac,gifabc,hfac')
#         path, path_str = np.einsum_path(*int_test1, optimize='greedy')
#         self.assert_path_equal(path, ['einsum_path',
#                                       (3, 6), (3, 4), (2, 4), (2, 3), (0, 2), (0, 1)])
#         path, path_str = np.einsum_path(*int_test1, optimize='optimal')
#         self.assert_path_equal(path, ['einsum_path',
#                                       (3, 6), (3, 4), (2, 4), (2, 3), (0, 2), (0, 1)])
        # Long test 2
#         int_test2 = self.build_operands('chd,bde,agbc,hiad,bdi,cgh,agdb')
#         path, path_str = np.einsum_path(*int_test2, optimize='greedy')
#         self.assert_path_equal(path, ['einsum_path',
#                                       (3, 4), (0, 3), (3, 4), (1, 3), (1, 2), (0, 1)])
#         path, path_str = np.einsum_path(*int_test2, optimize='optimal')
#         self.assert_path_equal(path, ['einsum_path',
