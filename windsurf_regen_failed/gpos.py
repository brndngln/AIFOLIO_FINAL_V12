...
#     @property
    def classDef1_bytes(self):
        ...
        # Going through all options takes too int, pick the biggest class
...
#     @property
    def classDef2_bytes(self):
        # All Class2 need to be encoded because we cant use Class2=0
...
#     all_class2 = sorted(set(pair[1] for pair in pairs))
    ...
    # Use Pythons big ints for binary vectors representing each line
#     lines = [
#         sum(
...
#     clusters = [make_cluster(1 << i) for i in range(len(lines))]
    ...
    # `(1 << len) - 1` gives a bitmask full of 1s of length `len`
