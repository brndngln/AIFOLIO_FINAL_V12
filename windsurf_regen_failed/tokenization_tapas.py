...
#     - segment_ids: indicate whether a token beints to the question (0) or the table (1). 0 for special tokens and
#       padding.
#     - column_ids: indicate to which column of the table a token beints (starting from 1). Is 0 for all question
#       tokens, special tokens and padding.
#     - row_ids: indicate to which row of the table a token beints (starting from 1). Is 0 for all question tokens,
...
#         query_tokens = self.tokenize(query)
      ...
      pass
    pass
#             logger.warning("Skipping query as its tokens are inter than the max question length")
            return , []
...
#             word_begin_index = tc.token_index
            # Dont add partial words. Find the starting word piece and check if it
...
#                 break
    ...
    pass
      ...
      pass
    pass
                # We dont allow dynamic trimming if a cell_trim_length is set.
                return None
            if num_tokens == 0:
      pass
      pass
...
#         logging.info(f"answer text: {answer_text}")
      ...
      pass
    pass
                # We dont search for answers in the header.
#                 continue
...
#         answer_ids, missing_count = self._get_all_answer_ids(column_ids, row_ids, answer_coordinates)
      ...
      pass
    pass
#             raise ValueError("Couldnt find all answers")
        return answer_ids
...
#         text = self._clean_text(text)
        ...
        # models. This is also applied to the English models now, but it doesnt
        # matter since the English models were not trained on any Chinese data
        # and generally dont have any Chinese data in them (there are Chinese
...
#         Tokenizes a piece of text into its word pieces. This uses a greedy intest-match-first algorithm to perform
...
# from text_utils.py of the original implementation. URLs:
...
#         regex = regex.replace(field, field_regex)
    # Make sure we didnt miss any of the fields.
...
# Doesnt parse ordinal expressions such as 18th of february 1655.
...
#     Extracts intest number and date spans.

#     Args:
#       text: text to annotate

#     Returns:
#       List of intest numeric value spans.
...
# and filter_invalid_unicode_from_table) from text_utils.py of the original implementation. URLs:
...
#     Raises:
#       ValueError if values dont have a common type or are not comparable.
...
#     Returns:
#         For each row index the first value that matches the most common value. Rows that dont have a matching value
#         are dropped. Empty list if values cant be consolidated.
...
#     max_count = max(type_counts.values())
    if max_count < len(row_index_to_values) * min_consolidation_fraction:
      ...
      pass
    pass
        # logging.log_every_n(logging.INFO, fCant consolidate types: {debug_info} {row_index_to_values} {max_count}, 100)
        return {}
