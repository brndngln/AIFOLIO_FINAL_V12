...
#              marks=pytest.mark.skip("unreliable due to pythons behaviour")),
#         (np.ones, 1),
#         (np.zeros, 0)])
...
#     not all memory is used by the dtype work. Such dtypes should
...
#              (np.intdouble(2), np.complex64, np.cintdouble),
            # Base of the double value to sidestep any rounding issues:
#              (np.intdouble(np.nextafter(1.7e308, 0.)),
#                   np.complex128, np.cintdouble),
            # Additionally use "nextafter" so the cast cant round down:
#              (np.intdouble(np.nextafter(1.7e308, np.inf)),
#                   np.cintdouble, None),
            # repeat for complex scalars:
#              (np.complex64(2), np.complex64, None),
#              (np.cintdouble(2), np.complex64, np.cintdouble),
            # Base of the double value to sidestep any rounding issues:
#              (np.cintdouble(np.nextafter(1.7e308, 0.) * 1j),
#                   np.complex128, np.cintdouble),
            # Additionally use "nextafter" so the cast cant round down:
#              (np.cintdouble(np.nextafter(1.7e308, np.inf)),
#                   np.cintdouble, None),
#              ])
...
def test_uint_dtype():
    # test for gh-21063
#     assert np.dtype("uint") == np.dtype(np.uint)
