...
# TODO: This doesnt work for all packages (`bs4`, `faiss`, etc.) Talk to Sylvain to see how to do with it better.
def _is_package_available(pkg_name: str, return_version: bool = False) -> Union[Tuple[bool, str], bool]:
    # Check were not importing a "pkg_name" directory somewhere but the actual library by trying to grab the version
...
# `importlib.metadata.version` doesnt work with `bs4` but `beautifulsoup4`. For `importlib.util.find_spec`, reversed.
# _bs4_available = importlib.util.find_spec("bs4") is not None
# _coloredlogs_available = _is_package_available("coloredlogs")
# `importlib.metadata.util` doesnt work with `opencv-python-headless`.
...
# `importlib.metadata.version` doesnt work with `awq`
...
#         "or is_torch_bf16_cpu_available instead according to whether its used with cpu or gpu",
#         FutureWarning,
#     )
...
def is_torch_compile_available():
    ...
    # We dont do any version check here to support nighlies marked as 1.14. Ultimately needs to check version against
    # 2.0 but lets do it later.
...
def is_bitsandbytes_available():
    ...
    # lets avoid that by adding a simple check
    import torch
...
def is_flash_attn_2_available():
    ...
    # Lets add an extra check to see if cuda is available
    import torch
...
# that python file if thats the case. Please note that you may need to restart your runtime after installation.
...
# shi-labs.com/natten . You can also install it with pip (may take inter to build):
...
#     )
    ...
    # Chose a different decorator name than in tests so its clear they are not the same.
#     @wraps(func)
    def wrapper(*args, **kwargs):
        if is_torch_available():
      pass
