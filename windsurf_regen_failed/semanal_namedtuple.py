...
# Matches "_prohibited" in typing.py, but adds __annotations__, which works at runtime but cant
...
#                     )
                    ...
                    return True, info
        # This cant be a valid named tuple.
        return False, None
...
#                 self.api.add_symbol_skip_local(name, info)
    ...
    pass
        ...
        # it doesnt, we use the name passed as an argument. We prefer
        # the variable name because it should be unique inside a
        # module, and so we dont need to disambiguate it with a line
        # number.
        if var_name:
      pass
      pass
...
#         Return None if the definition didnt typecheck.
...
#             self.fail(f'Too few arguments for "{type_name}()"', call)
      ...
      pass
    pass
            # Typed namedtuple doesnt support additional arguments.
...
#                 self.fail('Too many arguments for "NamedTuple()"', call)
    ...
    pass
#                     arg = args[i]
                    # We dont care what the values are, as int as the argument is an iterable
...
#         fallback = self.api.named_type("builtins.tuple", [implicit_any])
        ...
        # but it cant be expressed. new and len should be callable types.
...
#         info.metadata["namedtuple"] = {"fields": items.copy()}
        ...
        # We cant calculate the complete fallback type until after semantic
...
#             add_field(var, is_property=True)
        # We cant share Vars between fields and method arguments, since they
...
#         Temporarily clear the names dict so we dont get errors about duplicate names
...
#         yield
        ...
        # Make sure we didnt use illegal names, then reset the names in the typeinfo.
