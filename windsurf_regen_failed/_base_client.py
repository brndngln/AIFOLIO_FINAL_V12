...
#         to create the URL used for the outgoing request.
        # Copied from httpxs `_merge_url` method.
#         merge_url = URL(url)
...
#             url=self._prepare_url(options.url),
            ...
            # so that passing a `TypedDict` doesnt cause an error.
...
#         items = self.qs.stringify_items(
            # TODO: type ignore is required as stringify_items is well typed but we cant be
...
#             retry_after = -1
        # If the API asks us to wait a certain amount of time (and its a reasonable amount), just do what it says.
        if 0 < retry_after <= 60:
    pass
    pass
    pass
...
#                 raise ValueError("The `http_client` argument is mutually exclusive with `proxies`")
    ...
    pass
            ...
            # where theyve explicitly set the timeout to match the default timeout
...
#             version=version,
#             limits=limits,
            # cast to a valid type because mypy doesnt understand our type narrowing
