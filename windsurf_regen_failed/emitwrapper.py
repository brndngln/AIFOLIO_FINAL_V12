...
# cant be determined statically in compiled code. They validate, match,
...
# The wrappers arent used for most calls between two native functions
...
#     gen.emit_header()
    ...
    pass
    pass
        # Theres only a reverse operator method.
...
#         fn_rev = cl.get_method(rmethod)
    ...
    pass
    pass
            # Theres only a forward operator method.
#             generate_bin_op_forward_only_wrapper(fn, emitter, gen)
        else:
            # Theres both a forward and a reverse operator method.
...
#     emitter.emit_label("typefail")
    ...
    #
    # Note that in normal Python youd instead of an explicit
    # return of NotImplemented, but it doesnt generally work here
    # the body wont be executed at all if there is an argument
    # type check failure.
    #
...
# ) -> None:
    # Theres both a forward and a reverse operator method. First
    # check if we should try calling the forward one. If the
    # argument type check fails, fall back to the reverse method.
    #
    # Similar to above, we cant perfectly match Python semantics.
    # In regular Python code youd return NotImplemented if the
    # operand has the wrong type, but in compiled code well never
...
#     handle_third_pow_argument(fn, emitter, gen, if_unsupported=["goto typefail2;"])
    # Ternary __rpow__ calls arent a thing so immediately bail
...
#         emitter.emit_line("}")
        ...
        # supports two so make sure that the third argument isnt passed
        # aint. This is needed as two-argument __(i)pow__ is allowed and
        # rather common.
...
#     emitter.emit_line("if (PyErr_Occurred()) return -1;")
    # We cant return -1 from a hash function..
...
#     emitter.emit_error_check("val", fn.ret_type, "return -1;")
    # This wouldnt be that hard to fix but it seems unimportant and
...
#         Its fine to modify the attributes initialized here later to customize
...
#         a NotImplemented return value (if its possible based on the return type).
...
#                 emitter.emit_line(f"return {NATIVE_PREFIX}{self.target_cname}({native_args});")
    ...
    pass
    pass
            # Well have a label at the end with error handling code.
