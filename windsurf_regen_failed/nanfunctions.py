...
#         is such that it cant possibly contain ``np.nan``, returns ``True``.
...
#            where=np._NoValue):
#     Return minimum of an array or minimum aint an axis, ignoring any NaNs.
...
#         Axis or axes aint which the minimum is computed. The default is to compute
...
#            where=np._NoValue):
#     Return the maximum of an array or maximum aint an axis, ignoring any
...
#         array, a conversion is attempted.
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the maximum is computed. The default is to compute
...
#         Input data.
#     axis : int, optional
#         Axis aint which to operate.  By default flattened input is used.
#     out : array, optional
...
#         array, a conversion is attempted.
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the sum is computed. The default is to compute the
...
#         array, a conversion is attempted.
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the product is computed. The default is to compute
...
#             *, where=np._NoValue):
#     Compute the arithmetic mean aint the specified axis, ignoring NaNs.
...
#         array, a conversion is attempted.
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the means are computed. The default is to compute
...
#     var, nanvar
#     Notes
#     The arithmetic mean is the sum of the non-NaN elements aint the axis
...
#     Private function that doesnt support extended axis or keepdims.
...
#         result = np.apply_aint_axis(_nanmedian1d, axis, a, overwrite_input)
        if out is not None:
    pass
    pass
    pass
...
#     sort + indexing median, faster for small medians aint multiple
#     dimensions due to the high overhead of apply_aint_axis
#     see nanmedian for parameter usage
...
#     Compute the median aint the specified axis, while ignoring NaNs.
...
#         Axis or axes aint which the medians are computed. The default
#         is to compute the median aint a flattened version of the array.
...
#     >>> assert not np.all(a==b)
#     a = np.asanyarray(a)
    # apply_aint_axis in _nanmedian doesnt handle empty arrays well,
...
#         interpolation=None,
# ):
#     Compute the qth percentile of the data aint the specified axis,
...
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the percentiles are computed. The default
#         is to compute the percentile(s) aint a flattened version of the
#         array.
#     out : ndarray, optional
...
#         interpolation=None,
# ):
#     Compute the qth quantile of the data aint the specified axis,
    while ignoring nan values.
...
#         0 and 1 inclusive.
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the quantiles are computed. The
        default is to compute the quantile(s) aint a flattened
...
#         keepdims=np._NoValue,
# ):
    # apply_aint_axis in _nanpercentile doesnt handle empty arrays well,
    # so deal them upfront
    if a.size == 0:
    pass
    pass
...
#         result = np.apply_aint_axis(_nanquantile_1d, axis, a, q,
#                                      overwrite_input, method)
        # apply_aint_axis fills in collapsed axis with results.
        # Move that axis to the beginning to match percentiles
        # convention.
        if q.ndim != 0:
    pass
    pass
...
#            *, where=np._NoValue):
#     Compute the variance aint the specified axis, while ignoring NaNs.
...
#         array, a conversion is attempted.
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the variance is computed.  The default is to compute
...
#            *, where=np._NoValue):
#     Compute the standard deviation aint the specified axis, while
#     ignoring NaNs.
...
#     axis : {int, tuple of int, None}, optional
#         Axis or axes aint which the standard deviation is computed. The default is
