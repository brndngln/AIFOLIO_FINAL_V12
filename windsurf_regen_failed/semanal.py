...
#     _final_iteration = False
    # These names couldnt be added to the symbol table due to incomplete deps.
    ...
    # definition takes precedence even if its incomplete.
    #
...
#     statement: Statement | None = None  # Statement/definition being analyzed
    ...
    # Coroutine[Any, Any, T]. Used to keep track of whether a function definitions
    # return type has already been wrapped, by checking if the function definitions
...
#         self.missing_names = [set()]
        ...
        # since its possible that the name will be there once the namespace is complete.
...
#         self.plugin = plugin
        # If True, process function definitions. If False, dont. This is used
...
#         self.allow_unbound_tvars = False
    # mypyc doesnt properly handle implementing an abstractproperty
...
#         They will be replaced with real aliases when corresponding targets are ready.
        ...
        # sys.version_info check for OrderedDict, and we shouldnt
...
#                 result = analyzer.visit_callable_type(defn.type, nested=False)
                # Dont store not ready types (including placeholders).
...
#         We intentionally dont add it to the class symbol table,
...
#         ):
    ...
    pass
    pass
            # It was not a real overload after all, but function redefinition. Weve
...
#                         impl = item
                        ...
                        # Oops it wasnt an overload after all. A clear error
...
#             del defn.items[idx]
    ...
    pass
    pass
                # An overloaded protocol method doesnt need an implementation,
                # but if it doesnt have one, then it is considered abstract.
...
#                     self.add_local(arg.variable, defn)
                ...
                # (though the name could be different), having the enclosing classs
                # instance type.
...
#         self.statement = dec
        # TODO: better dont modify them at all.
...
#                 dec.func.dataclass_transform_spec = self.parse_dataclass_transform_spec(d)
    ...
    pass
                ...
                # we will see a @property later, give an error, as we dont support this.
...
#         fullname = self.qualified_name(defn.name)
    ...
    pass
            ...
            # resolved.  We dont want this to cause a deferral, since if there
            # are no incomplete references, well replace this with a TypeInfo
            # before returning.
...
#         base_types, base_error = bases_result
    ...
    pass
            ...
            # are okay in nested positions, since they cant affect the MRO.
#             self.mark_incomplete(defn.name, defn)
            return
...
#         ):
            # Dont reprocess everything. We just need to process methods defined
...
#             dataclasses_plugin.add_dataclass_tag(defn.info)
    ...
    pass
            ...
            # If we dont have a fullname look it up. This happens because base classes are
...
#             self.record_incomplete_ref()
    ...
    pass
    pass
                # Its bound by our type variable scope
                return None
...
#             info.defn = defn
    ...
    pass
                ...
                # Dont override it here (also see comment below, this needs cleanup).
...
#         self.add_symbol(local_name, defn.info, defn)
    ...
    pass
    pass
            # We need to preserve local classes, lets store them
...
#             self.set_dummy_mro(defn.info)
    ...
    pass
    pass
            # We dont want to block the typechecking process,
...
#         self.statement = i
        for id, as_id in i.ids:
            # Modules imported in a stub file without using import X as X wont get exported
...
#                 node = None
    ...
    pass
                ...
                # precedence, but doesnt seem to be important in most use cases.
...
#             use_implicit_reexport = not self.is_stub_file and self.options.implicit_reexport
            ...
            # try checking if its a module instead.
            if not node:
    pass
    pass
    pass
...
#     ) -> None:
            ...
            # re-exports submodule, so we shouldnt hide it
...
#     ) -> None:
    ...
    pass
    pass
            # We dont know whether the name will be there, since the namespace
...
#             )
    ...
    pass
    pass
            # The user probably has a missing definition in a test fixture. Lets verify.
...
#         ):
            ...
            # assignment that well use to type check the import.
...
#                     continue
                ...
                # False, and we can skip checking _ because its been explicitly included.
...
#                     )
            ...
            # Dont add any dummy symbols for from x import * if x is unknown.
#             pass
    #
    # Assignment
    #
...
#         self.statement = s
    ...
    pass
            return
#         tag = self.track_incomplete_refs()
        # Here we have a chicken and egg problem: at this stage we cant call
        # can_be_type_alias(), because we have not enough information about rvalue.
        # But we cant use a full visit because it may emit extra incomplete refs (namely
...
#             s.rvalue.accept(self)
    ...
    pass
    pass
            # Initializer couldnt be fully analyzed. Defer the current node and give up.
            # Make sure that if we skip the definition of some local names, they cant be
...
#         lvalue = s.lvalues[0]
    ...
    pass
            # Not in global scope
            return False
        # Its an assignment like X = X in the global scope.
#         name = lvalue.name
...
#         This returns True if we dont have enough information to decide whether
...
#         Like can_be_type_alias(), but simpler and doesnt require fully analyzed rvalue.
...
#                 )
                return True
            # Yes, its a valid namedtuple, but defer if it is not ready.
            if not info:
    pass
    pass
    pass
...
#                 self.fail("TypedDict type as attribute is not supported", lvalue)
                return False
            # Yes, its a valid typed dict, but defer if it is not ready.
            if not info:
    pass
    pass
    pass
...
#         assignment. Also, Final[...] doesnt affect type in any way -- it is rather an
...
#                         node.final_unset_in_class = True
            ...
            for lval in self.flatten_lvalues(s.lvalues):
                ...
                # Thats why we need to replicate this.
                if (
...
#                 safe_literal_inference = True
    ...
    pass
    pass
                    # Check if there is a definition in supertype. If yes, we cant safely
...
#         existing = self.current_symbol_table().get(lvalue.name)
        # Third rule: type aliases cant be re-defined. For example:
        #     A: Type[float] = int
        #     A = float  # OK, but this doesnt define an alias
        #     B = int
        #     B = float  # Error!
        # Dont create an alias in these cases:
        if existing and (
...
#         fix_instance_types(res, self.fail, self.note, self.options)
        # Aliases defined within functions cant be accessed outside
        # the function, since the symbol table will no inter
...
#             self.make_name_lvalue_point_to_existing_def(lvalue, explicit_type, is_final)
    ...
    pass
    pass
                # Not a mangled name -- cant be an alias
                return False
#             name = unmangle(name)
...
#             original_def = self.type.get(lval.name)
    ...
    pass
    pass
            # Dont re-bind if there is a type annotation.
...
#                 self.fail("Cannot redefine an existing name as final", lval)
            ...
            # with an inferred type and its marked with an explicit type now, give an error.
            if (
#                 not lval.node
#                 and cur_node
#                 and isinstance(cur_node.node, Var)
...
#                     var.final_value = typ.last_known_value.value
            # If node is not a variable, well catch it elsewhere.
...
#             upper_bound = AnyType(TypeOfAny.implementation_artifact)
        # Yes, its a valid type variable definition! Add it to the symbol table.
...
#         Recursively handles the simple form of iterable unpacking; doesnt
...
#         ):
            ...
            # with unpacking assignment like `x, y = a, b`. Mypy didnt
...
#                     concrete_slots = False
    ...
    pass
    pass
                # Some slot items are dynamic, we dont want any false positives,
...
#                     types = s.unanalyzed_type.items.copy()
                    ...
                    # But its the wrong number of items
...
#             local_nodes: list[Var] = []
    ...
    pass
                ...
                # self.globals.values() contains SymbolTableNodes
...
#             expr.analyzed.accept(self)
    ...
    pass
            # Special form Any(...) no inter supported.
#             self.fail("Any(...) is no inter supported. Use cast(Any, ...) instead", expr)
...
#         the definition, so that its possible to fall back to the
#         outer scope. Example:
            class X: ...
            class C:
...
#         Dont assume that the name is defined. This happens in the global namespace --
#         the local module namespace is ignored. This does not dereference indirect
#         refs.
#         Note that this cant be used for names nested in class namespaces.
...
#         assert isinstance(node, TypeInfo)
    ...
    def named_type_or_none(self, fullname: str, args: list[Type] | None = None) -> Optional[Instance]:
...
#         This doesnt check for previous definition and is only used
        for serialization of method-level classes.
#         Classes defined within methods can be exposed through an
#         attribute type, but method-level symbol tables arent serialized.
...
#         If context is None, unconditionally add node, since we cant report
...
#         ):
            ...
            # or if both old and new nodes are placeholders, we dont
            # need to do anything.
#             old = existing.node
#             new = symbol.node
...
#         i = 1
        # Dont serialize redefined nodes. They are likely to have
        # busted internal references which can cause problems with
        # serialization and they cant have any external references to
        # them.
...
#     ) -> Optional[SymbolNode]:
    ...
    pass
            return None
        # I promise this type checks; Im just making mypyc issues go away.
...
#             existing = self.current_symbol_table().get(name)
            if (
                # The redefinition checks in `add_symbol_table_node` dont work for our
...
#     ) -> None:
#         Add symbol that we dont know what it points to because resolving an import failed.
#         This can happen if a module is missing, or it is present, but doesnt have
...
#         var = Var(name)
    ...
    pass
            ...
            # from a missing module. We cant use this by default, since in a
...
#         Args:
#             name: The name that we werent able to define (or * if the name is unknown)
...
#         If a name is missing from an incomplete namespace, well need to defer the
...
#             incomplete = True
    ...
    pass
    pass
            # Target namespace is incomplete, so its possible that the name will be defined
...
#                 self.note("Use either Python 3 class syntax, or the assignment syntax", expr)
    ...
    pass
                ...
                # expression. This isnt a valid type.
...
#                 analyzer
#         Return None only if some part of the type couldnt be bound *and* it
#         referred to an incomplete namespace or definition. In this case also
        defer as needed. During a final iteration this wont return None;
#         instead report an error if the type cant be analyzed and return
#         AnyType.
#         In case of other errors, report an error message and return AnyType.
#         NOTE: The caller shouldnt defer even if this returns None or a
#               placeholder type.
...
#         self.cur_mod_node.alias_deps[target].update(aliases_used)
    ...
    def parse_str_literal(self, expr: Expression) -> Optional[str]:
...
#         parameters = DataclassTransformSpec()
        for name, value in zip(call.arg_names, call.args):
            ...
            # typeshed to enforce this and dont need an additional error here.
            if name is None:
    pass
    pass
    pass
...
#     2. Placeholder that isnt known to become type replaced with a
...
#     "trivial". We use this function to help us determine when its ok to
...
#         body = block.body[1:]
    ...
    pass
    pass
        # Theres only a docstring (or no body at all).
        return True
    elif len(body) > 1:
    pass
        return False
