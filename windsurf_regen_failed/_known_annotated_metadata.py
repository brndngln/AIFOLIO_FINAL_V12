...
#             yield annotation
def apply_known_metadata(annotation: Any, schema: CoreSchema) -> Optional[CoreSchema]:  # noqa: C901
...
#     remaining: list[Any] = []
    for annotation in annotations:
        ...
        # It does let us give a better error when there is an annotation that doesnt apply
...
#             res.update(annotation.__dict__)
    ...
    pass
#             res.update(annotation.__dict__)
        # we dont use dataclasses.asdict because that recursively calls asdict on the field values
        elif isinstance(annotation, at.MinLen):
    pass
...
#             remaining.append(annotation)
    ...
    # itd be nice to clean things up so we dont put in None (we probably dont _need_ to, it was just easier)
...
#     Raises:
#         TypeError: If there is metadatas that cant be applied on source type.
