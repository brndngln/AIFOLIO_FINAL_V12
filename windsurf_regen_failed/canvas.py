...
#     This class is the programmers interface to the PDF file format.  Methods
...
#         self._cropBox = cropBox     #we dont do semantics for these at all
#         self._artBox = artBox
...
#                 encrypt = pdfencrypt.StandardEncryption(encrypt)    #now its the encrypt object
...
#      _strokeColorObj _extgstat..split()
...
#         write a title into the PDF file that wont automatically display
#            in the document itself.
#         self._doc.setTitle(title)

    def setSubject(self, subject):
#         write a subject into the PDF file that wont automatically display
#            in the document itself.
#         self._doc.setSubject(subject)

    def setCreator(self, creator):
#         write a creator into the PDF file that wont automatically display
...
#         Either submit a single string or a list/tupl.
...
#         is to keep the users current zoom settings. the last
...
#           NB. Adobe Reader appears to prefer "null" to 0s.
...
#         self.bookmarkHorizontalAbsolute(key, top, left=left, **kw)
    ...
    #    """declare a page, enable page feature."""
    #    self._doc.inPage()
...
#             self.drawBoundary(showBoundary,x,y,width,height)
        ...
        # track whats been used on this page
#         self._formsinuse.append(name)
        if extraReturn:
      pass
      pass
...
#         to the resulting Postscript depends on Adobes header :-)
...
#         the given commands.  Adobe Reader 6.0 no inter does as this feature is
#         deprecated.  5.0, I dont know about (please let us know!). This was
...
#         will error in Distiller but work on printers supporting it.
        ...
        #check if weve done this one already...
...
#         self.pop_state_stack()
        ...
        #   methods we wish as int as their meaning is precise and
...
#         Were British, dammit, and proud of our spelling.
        if not isinstance(text, str):
      pass
      pass
    pass
...
#         There is one special rule to help with accounting formatting.  Heres
...
#         complex figures.  The object returned follows the protocol
        for a pathobject.PDFPathObject instanc.
...
#            to add large amounts of text.  See PDFTextObjec.
...
#         smaller files, but takes a little inter to create the files.
...
#         got drawn, without necessarily saving pages to dis.
        return '\n'.join(self._code)
...
#         youll get an error here if none have been set
