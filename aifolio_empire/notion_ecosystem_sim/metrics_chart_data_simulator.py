...
It's stateless, rule-based, and includes randomization, simulated trends, and imperfections.
"""
...
class MetricsChartDataSimulator:
    """Simulates time-series data for Notion metrics charts with anti-sentience safeguards."""
    ...
    def _generate_metric_series(
        ...
        current_value = random.uniform(
            metric_props["base_min"], metric_props["base_max"]
        )
        ...
        for i in range(num_points):
            ...
            if random.random() < config.SIM_METRICS_SPIKE_CHANCE:
                spike = value * random.uniform(
                    config.SIM_METRICS_SPIKE_MULTIPLIER_MIN,
                    config.SIM_METRICS_SPIKE_MULTIPLIER_MAX,
                )
                logger.info(
                    f"Simulated spike in metric '{metric_name}' at {current_date.strftime('%Y-%m-%d')}: {spike}"
                )
                value = spike
            if random.random() < config.SIM_METRICS_ANOMALY_CHANCE:
                anomaly = random.choice([0, value * 0.1, value * 2])
                logger.info(
                    f"Simulated anomaly in metric '{metric_name}' at {current_date.strftime('%Y-%m-%d')}: {anomaly}"
                )
                value = anomaly
    ...
    def get_simulated_metrics_data(
        self,
        ...
        if num_data_points is None:
            num_points = random.randint(
                config.SIM_METRICS_NUM_DATA_POINTS_MIN,
        ...
        target_metrics = (
            ...
            else list(SIMULATED_METRIC_DEFINITIONS.keys())
        )
        ...
        for metric_key in target_metrics:
            if metric_key in SIMULATED_METRIC_DEFINITIONS:
                ...
                series = self._generate_metric_series(
                    metric_key, metric_props, num_points, current_time
                )
                ...
                logger.warning(
                    f"Metric key '{metric_key}' not found in definitions. Skipping."
                )
...
if __name__ == "__main__":
    ...
    # Print one metric's data to keep output manageable
    if metrics_data_all["simulated_metrics_charts_data"]:
        first_metric_key = list(
            metrics_data_all["simulated_metrics_charts_data"].keys()
        )[0]
        print(f"Data for '{first_metric_key}':")
        print(
            json.dumps(
                metrics_data_all["simulated_metrics_charts_data"][first_metric_key],
                indent=2,
            )
        )
    else:
    ...
    print(
        f"Generated data for {len(metrics_data_all['simulated_metrics_charts_data'])} metrics."
    )

    print("\n📊 Simulated Metrics Chart Data (Specific Metrics, 15 Points): 📊")
    specific_metrics = ["daily_sales_sim", "email_open_rate_sim"]
    metrics_data_specific = simulator.get_simulated_metrics_data(
        metrics_to_generate=specific_metrics, num_data_points=15
    )
    print(json.dumps(metrics_data_specific, indent=2))
    print(
        f"Generated data for {len(metrics_data_specific['simulated_metrics_charts_data'])} metrics with 15 data points each."
    )
