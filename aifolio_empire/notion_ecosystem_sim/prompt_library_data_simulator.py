...
try:
    ...
    class MockConfig:
        ...
        SIM_PROMPT_LIB_TEXT_PLACEHOLDER_NOTE_CHANCE = (
            0.1  # Chance to add a note about placeholder text
        )
...
SIMULATED_PROMPT_CATEGORIES = [
    ...
    "Email Sequence Crafting",
    "Social Media Posts",
    "Blog Post Ideas",
    "Product Descriptions_sim",
    "Vault Title Brainstorming",
    "Image Generation Concepts_sim",
    "Ad Copy_sim",
    "Video Script Ideas_sim",
]
...
SIMULATED_PROMPT_TEMPLATES = [
    (
        "Generate a {length} {type} for a product about {topic}",
        ["length", "type", "topic"],
    ),
    (
        "Create 5 compelling {hook_type} hooks for a {platform} post on {niche}",
        ["hook_type", "platform", "niche"],
    ),
    (
        "Write a persuasive {email_type} email to encourage sign-ups for a {offering}",
        ["email_type", "offering"],
    ),
    (
        "Outline a 7-section PDF vault titled '{vault_title_core}' focusing on {key_benefit}",
        ["vault_title_core", "key_benefit"],
    ),
    (
        "Suggest 3 creative concepts for a cover image for a vault about {subject_matter}",
        ["subject_matter"],
    ),
]
...
class PromptLibraryDataSimulator:
    ...
    def _generate_simulated_prompt_text(self) -> Dict[str, Any]:
        ...
        prompt_obj = {"prompt_text_sim": filled_template, "params_sim": sim_params}
        logger.info(f"Generated simulated prompt: {prompt_obj}")
        return prompt_obj

    def get_simulated_prompt_library_data(
        self, num_prompts: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        if num_prompts is None:
            num_to_simulate = random.randint(
                config.SIM_PROMPT_LIB_NUM_PROMPTS_MIN,
        ...
        for i in range(num_to_simulate):
            ...
            if random.random() < config.SIM_PROMPT_LIB_RATING_GLITCH_CHANCE:
                rating = random.choice(
                    [round(random.uniform(0.5, 2.0), 1), None]
                )  # Simulate a bad or missing rating
                if prompt_notes:
            ...
            entry = {
                ...
                "tags_sim": tags,
                "usage_count_sim": random.randint(
                    0, config.SIM_PROMPT_LIB_MAX_USAGE_COUNT
                ),
                "rating_sim": rating,  # Scale of 1-5, or None
                "created_at_sim": (
                    current_time - timedelta(days=random.randint(10, 365))
                ).isoformat(),
                "last_updated_sim": (
                    current_time - timedelta(days=random.randint(0, 9))
                ).isoformat(),
...
if __name__ == "__main__":
    ...
    print(
        f"Generated {len(library_data['prompt_library_entries_sim'])} prompt entries."
    )
    ...
    print(
        f"Generated {len(library_data_specific['prompt_library_entries_sim'])} prompt entries."
    )
