...
class PDFOutlineTemplateSimulator:
    ...
    def _generate_simulated_sections(
        self, max_depth: int, current_depth: int = 0, topic_sim: str = "Generic Topic"
    ) -> List[Dict[str, Any]]:
        if current_depth >= max_depth:
            return []
        logger.info(
            f"Generating simulated PDF outline sections at depth {current_depth} for topic '{topic_sim}'"
        )
        num_sections = random.randint(
            1, config.SIM_PDF_OUTLINE_MAX_SECTIONS - current_depth * 2
        )  # Fewer sections deeper
        sections = []
        for i in range(num_sections):
            title_template = random.choice(SIMULATED_SECTION_TITLES)
            # Basic placeholder replacement for simulated dynamic titles
            section_title = (
                title_template.replace("{Topic}", topic_sim)
                .replace("{N}", str(i + 1))
            ...
            section = {
                ...
                "content_placeholder_sim": random.choice(
                    SIMULATED_SECTION_PLACEHOLDER_CONTENT
                ),
                "estimated_pages_sim": random.randint(1, 3)
                if current_depth < 2
                else 1,  # Sub-sections are shorter
                "note_sim": section_note,
                "sub_sections_sim": [],
            }
            if (
                current_depth < max_depth - 1
            ):  # Only add subsections if not at max depth for subsections
                if random.random() < 0.3 / (
                    current_depth + 1
                ):  # Higher chance for subsections at shallower depths
                    section["sub_sections_sim"] = self._generate_simulated_sections(
                        max_depth, current_depth + 1, topic_sim
                    )
            sections.append(section)
        return sections

    def get_simulated_outline_templates(
        self, num_templates: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        if num_templates is None:
            num_to_simulate = random.randint(
                config.SIM_PDF_OUTLINE_NUM_TEMPLATES_MIN,
        ...
        for i in range(num_to_simulate):
            ...
            max_nesting_depth = random.randint(
                1, config.SIM_PDF_OUTLINE_MAX_SUBSECTIONS + 1
            )
            sections = self._generate_simulated_sections(
                max_depth=max_nesting_depth, topic_sim=sim_topic
            )
            ...
            entry = {
                ...
                "tags_sim": tags,
                "complexity_sim": random.choice(
                    ["Basic", "Intermediate", "Advanced_sim"]
                ),
                "created_at_sim": (
                    current_time - timedelta(days=random.randint(30, 400))
                ).isoformat(),
                "last_updated_sim": (
                    current_time - timedelta(days=random.randint(0, 29))
                ).isoformat(),
...
if __name__ == "__main__":
    ...
    print(
        f"Generated {len(template_data['pdf_outline_templates_sim'])} template entries."
    )
    ...
    if template_data_specific["pdf_outline_templates_sim"]:
        ...
        print(
            json.dumps(
                template_data_specific["pdf_outline_templates_sim"][0],
                indent=2,
                ensure_ascii=False,
            )
        )
    print(
        f"Generated {len(template_data_specific['pdf_outline_templates_sim'])} template entries."
    )
