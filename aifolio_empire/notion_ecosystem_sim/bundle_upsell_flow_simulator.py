...
SIMULATED_VAULT_THEMES = [
    "Productivity Hacks_sim",
    "AI Writing Secrets_sim",
    "Marketing Mastery_sim",
    "Wellness Guide_sim",
    "Financial Freedom Blueprint_sim",
    "Creative Toolkit_sim",
    "Startup Essentials_sim",
]
...
class BundleUpsellFlowSimulator:
    ...
    def _generate_simulated_bundles(
        self, vaults: List[Dict[str, Any]], num_bundles: int
    ) -> List[Dict[str, Any]]:
        bundles = []
        if not vaults:
            return []
        ...
        for i in range(num_bundles):
            ...
            if (
                num_vaults_in_bundle == 0 and len(vaults) > 0
            ):  # Ensure at least one vault if possible
            ...
            constituent_vault_ids = [
                v["vault_id_sim"] for v in selected_vaults_for_bundle
            ]
            ...
            if random.random() < config.SIM_BUNDLE_DISCOUNT_CHANCE:
                discount_percent = random.randint(
                    config.SIM_BUNDLE_DISCOUNT_PERCENT_MIN,
                    config.SIM_BUNDLE_DISCOUNT_PERCENT_MAX,
                )
            ...
            if selected_vaults_for_bundle:
                bundle_theme = (
                    selected_vaults_for_bundle[0]["category_sim"] + " Collection_sim"
                )
            ...
            bundles.append(
                {
                    ...
                    "description_sim": f"A simulated collection of {num_vaults_in_bundle} vaults focusing on {bundle_theme.split(' ')[0]}.".replace(
                        "_sim", ""
                    ),
    ...
    def _generate_simulated_upsell_flows(
        self, vaults: List[Dict[str, Any]], bundles: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        flows = []
        if not vaults and not bundles:
            return []
                ...
                flows.append(
                    {
                        ...
                        "trigger_item_name_sim": item.get("vault_name_sim")
                        or item.get("bundle_name_sim"),
                        "suggested_item_id_sim": suggested_item["bundle_id_sim"],
                        "suggested_item_name_sim": suggested_item["bundle_name_sim"],
                        "suggestion_type_sim": "Upsell_sim"
                        if item_type == "vault"
                        or (
                            item_type == "bundle"
                            and suggested_item["sim_bundle_price_usd"]
                            > item["sim_bundle_price_usd"]
                        )
                        else "Cross_sell_sim",
    ...
    def get_simulated_bundle_upsell_data(self) -> Dict[str, Any]:
        ...
        num_vaults = random.randint(
            config.SIM_BUNDLE_NUM_VAULTS_MIN, config.SIM_BUNDLE_NUM_VAULTS_MAX
        )
        sim_vaults = self._generate_simulated_vaults(num_vaults)

        num_bundles = random.randint(
            config.SIM_BUNDLE_NUM_BUNDLES_MIN, config.SIM_BUNDLE_NUM_BUNDLES_MAX
        )
