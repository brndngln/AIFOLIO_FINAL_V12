...
class DynamicBundleBuilder:
    ...
    def _group_vaults_by_theme(
        self, available_vaults: List[Dict[str, Any]]
        ...
        for vault in available_vaults:
            ...
            if random.random() < 0.02:  # 2% chance to skip a vault for grouping
                logger.warning(
                    f"Simulated failure to categorize vault: {vault.get('title', 'Unknown Vault')}"
                )
                continue

            themes = self._simulate_theme_detection(
                vault.get("title", ""), vault.get("niche", "")
            )
            for theme in themes:
    ...
    def _create_bundle_offers(
        self, theme: str, vaults_in_theme: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        if len(vaults_in_theme) >= 2:
            num_in_starter = random.randint(
                2, min(3, len(vaults_in_theme), MAX_VAULTS_PER_BUNDLE)
            )
            starter_vaults = vaults_in_theme[:num_in_starter]
            original_price = sum(
                v.get("price", 5.0) for v in starter_vaults
            )  # Default price 5 if missing
            ...
            bundles.append(
                {
                    ...
                    "type": "StarterPack",
                    "vault_titles": [
                        v.get("title", "Untitled Vault") for v in starter_vaults
                    ],
        ...
        if len(vaults_in_theme) >= 3:
            num_in_booster = random.randint(
                3, min(len(vaults_in_theme), MAX_VAULTS_PER_BUNDLE)
            )
            ...
            discount = random.choice(SIMULATED_DISCOUNT_RATES) + random.uniform(
                0.05, 0.1
            )
            ...
            bundles.append(
                {
                    ...
                    "type": "BoosterPack",
                    "vault_titles": [
                        v.get("title", "Untitled Vault") for v in booster_vaults
                    ],
                    "original_total_price_simulated": original_price,
                    "bundle_price_simulated": bundle_price,
        ...
        if random.random() < 0.05 and bundles:  # 5% chance to mess with a bundle
            ...
            if random.random() < 0.5:
                bundles[idx_to_mess]["bundle_price_simulated"] = round(
                    random.uniform(1.0, 500.0), 2
                )  # Nonsense price
                logger.warning(
                    f"Simulated price corruption for bundle: {bundles[idx_to_mess]['bundle_name']}"
                )
            else:
    ...
    def _add_scarcity_logic(self, bundle_offer: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if random.random() < 0.7:  # 70% chance to add some scarcity
            scarcity_type = random.choice(
                ["countdown", "bonus_unlock", "limited_stock_simulated"]
            )
            ...
            if scarcity_type == "countdown":
                ...
                bundle_offer[
                    "scarcity_simulated"
                ] = f"Offer ends in (simulated) {sim_hours} hours!"
                bundle_offer["simulated_expiry_timestamp"] = (
                    datetime.utcnow() + timedelta(hours=sim_hours)
                ).isoformat()
            elif scarcity_type == "bonus_unlock":
                bonus = random.choice(
                    [
                        "Extra Checklist PDF",
                        "Private Q&A Invite (Simulated)",
                        "Early Access to Next Vault",
                    ]
                )
                bundle_offer[
                    "scarcity_simulated"
                ] = f"Buy now and unlock bonus: {bonus}! (Simulated)"
            else:  # limited_stock_simulated
                stock = random.randint(5, 50)
                bundle_offer[
                    "scarcity_simulated"
                ] = f"Only {stock} bundles left at this price! (Simulated)"
        ...
        if random.random() < 0.03 and "scarcity_simulated" in bundle_offer:
            bundle_offer[
                "scarcity_simulated"
            ] = "Special offer, check details! (Generic simulated message)"
            logger.warning(
                f"Simulated generic scarcity message for bundle: {bundle_offer.get('bundle_name')}"
            )

        return bundle_offer

    def generate_dynamic_bundles(
        self, available_vaults: List[Dict[str, Any]]
        ...
        if random.random() < 0.01 or not available_vaults:  # 1% chance or no vaults
            logger.error(
                "Simulated critical random failure or no vaults provided in generate_dynamic_bundles."
            )
            return None

        logger.info(
            f"Generating dynamic bundles from {len(available_vaults)} available vaults."
        )
        ...
        if all_bundles and random.random() < 0.02:
            ...
            logger.warning(
                f"Simulated random omission of a final bundle: {omitted_bundle.get('bundle_name')}"
            )

        logger.info(
            f"Successfully generated {len(all_bundles)} dynamic bundle offers (simulated)."
        )
        return all_bundles
...
if __name__ == "__main__":
    ...
    if random.random() < 0.3:
        ...
        if random.random() < 0.5:
            sim_vaults_data[idx_corrupt]["price"] = round(
                random.uniform(100, 500), 2
            )  # Unrealistic price
        else:
            sim_vaults_data[idx_corrupt][
                "title"
            ] = f"Corrupted Title {random.randint(1,100)}"
        logger.info(
            f"Example: Intentionally corrupted input vault data for {sim_vaults_data[idx_corrupt]['id']}"
        )
