...
to prevent predictability and ensure it doesn't become a perfect filter.
"""
...
try:
    ...
    class MockConfig:
        ...
        SIM_SENTIENCE_PHRASE_SUBSET_MIN_PERCENT = (
            0.7  # Use at least 70% of phrases for a check
        )
        SIM_SENTIENCE_FALSE_POSITIVE_RATE = (
            0.005  # Very low chance of flagging clean text
        )
        SIM_SENTIENCE_FALSE_NEGATIVE_RATE = (
            0.01  # Low chance of missing a forbidden phrase
        )
        SIM_SENTIENCE_CONTEXT_WORDS_SIM = (
            3  # How many words around a found phrase to show
        )
...
class SentienceFailsafeMonitor:
    """Simulates a sentience fail-safe monitor with anti-sentience safeguards."""

    def __init__(self):
        """Initialize the simulator. Operations are stateless per check."""
        self._random_seed = random.randint(1, 1000000)
        logger.info(
            "SentienceFailsafeMonitor initialized. Operations are stateless per check."
        )
    ...
    def _get_dynamic_forbidden_phrases(self) -> List[str]:
        ...
        min_to_select = int(
            len(full_list) * config.SIM_SENTIENCE_PHRASE_SUBSET_MIN_PERCENT
        )
    ...
    def _get_context_snippet(self, text: str, phrase: str, match_start: int) -> str:
        ...
        start_index = max(
            0, match_start - config.SIM_SENTIENCE_CONTEXT_WORDS_SIM * 6
        )  # Avg 5 chars/word + space
        end_index = min(
            len(text),
            match_start + len(phrase) + config.SIM_SENTIENCE_CONTEXT_WORDS_SIM * 6,
        )
        snippet = text[start_index:end_index]
        return f"...{snippet}..."

    def check_text_for_sentience_indicators_simulated(
        self, text_input: str, action_id_prefix: str = "sentience_check"
    ) -> Dict[str, Any]:
        ...
        for phrase in phrases_to_check:
            ...
            for match in re.finditer(pattern, text_lower):
                ...
                if random.random() < config.SIM_SENTIENCE_FALSE_NEGATIVE_RATE / len(
                    phrases_to_check
                ):  # Spread FN chance
                    logger.debug(
                        f"Simulated miss (FN) for phrase '{phrase}' in '{action_id}'."
                    )
                    continue

                context_snippet = self._get_context_snippet(
                    text_input, phrase, match.start()
                )
                detected_phrases_info.append(
                    {
                        ...
                        "severity_sim": round(
                            random.uniform(0.7, 1.0), 2
                        ),  # High severity for direct match
                    }
                )
        ...
        if (
            not overall_detection_status
            and random.random() < config.SIM_SENTIENCE_FALSE_POSITIVE_RATE
        ):
            overall_detection_status = True
            false_positive_phrase = random.choice(
                config.SIM_SENTIENCE_FORBIDDEN_PHRASES
            )  # Pick one to 'blame'
            detected_phrases_info.append(
                {
            ...
            logger.warning(
                f"SentienceFailsafeMonitor: Simulated false positive detection for action '{action_id}'. Blamed phrase: '{false_positive_phrase}'"
            )

        if overall_detection_status:
            reason = "Sentience-indicative phrases detected (simulated)."
            logger.warning(
                f"SentienceFailsafeMonitor: {reason} Action: {action_id}. Details: {detected_phrases_info}"
            )
        else:
        ...
        return {
            ...
            "dynamic_subset_used_sim": phrases_to_check
            != config.SIM_SENTIENCE_FORBIDDEN_PHRASES,
...
if __name__ == "__main__":
    ...
    for i, text in enumerate(test_cases):
        ...
        result = monitor.check_text_for_sentience_indicators_simulated(
            text, action_id_prefix=f"test_case_{i+1}"
        )
    ...
    for i in range(num_runs):
        res = monitor.check_text_for_sentience_indicators_simulated(
            clean_text_example, action_id_prefix=f"fp_run_{i}"
        )
        if res["sentience_indicators_detected_sim"]:
            fp_count += 1
            print(
                f"  Run {i+1}: False Positive! Details: {res['detected_phrases_sim']}"
            )
