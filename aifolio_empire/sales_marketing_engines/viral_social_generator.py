...
class ViralSocialGenerator:
    ...
    def _get_platform_templates(
        self, platform: str, niche: str, topic: Optional[str] = None
    ) -> List[str]:
        ...
        if not selected_templates or random.random() < 0.03:
            if templates and random.random() < 0.5:
                ...
                logger.warning(
                    f"Simulated random selection of wrong platform template: asked for {platform}, using {wrong_platform}."
                )
                selected_templates = templates.get(
                    wrong_platform, [f"Generic social media post about {t} for {n}."]
                )
            else:
                selected_templates = [
                    f"A generic social media update regarding {t} in the {n} niche."
                ]

        random.shuffle(selected_templates)
        return selected_templates

    def _simulate_hashtag_generation(
        self, niche: str, topic: Optional[str] = None, count: int = 3
    ) -> List[str]:
        ...
        if topic:
            base_hashtags.append(
                f"#{topic.replace(' ', '').lower()[:15]}"
            )  # Truncate int topics
        ...
        while len(generated_hashtags) < count:
            if (
                random.random() < 0.6 and base_hashtags
            ):  # More likely to use base related
            ...
            if len(generated_hashtags) >= MAX_HASHTAGS_SIMULATED + len(
                base_hashtags
            ):  # Safety break
                break
        ...
        if random.random() < 0.05:
            generated_hashtags.append(
                f"#{random.choice(['awesomestuffz', 'mycooltagg', 'randomthingy123'])}"
            )
    ...
    def generate_social_content(
        ...
        vault_link_simulated: Optional[str] = None,
        ...
        if random.random() < 0.01:
            logger.error(
                f"Simulated critical random failure in generate_social_content for {platform}, niche '{niche}'."
            )
            return None

        logger.info(
            f"Generating '{platform}' content for niche '{niche}', topic '{topic}'."
        )

        templates = self._get_platform_templates(platform, niche, topic)
        if not templates:
            logger.warning(
                f"No templates found or simulated error for platform '{platform}', niche '{niche}'."
            )
            return {
                "platform": platform,
                "content": "Error: Could not generate content (simulated).",
                "hashtags_simulated": [],
            }
        ...
        if random.random() < 0.03:
            if len(generated_text) > 20 and random.random() < 0.5:
                ...
                generated_text = (
                    generated_text[:cut_point] + "... (SIMULATED_TRUNCATION)"
                )
            else:
                generated_text = f"[SIMULATED_CONTENT_ALTERATION] Original idea: {generated_text[:30]}..."
            logger.warning(
                f"Simulated alteration/truncation of content for {platform}, niche '{niche}'."
            )
        ...
        if platform == "twitter_post" and vault_link_simulated:
            generated_text = generated_text.replace(
                "[SimulatedLink]", vault_link_simulated
            )
        ...
        if (
            ...
            and len(generated_text) > TWITTER_CHAR_LIMIT_SIMULATED
        ):
            generated_text = (
                generated_text[: TWITTER_CHAR_LIMIT_SIMULATED - 20]
                + "... (Exceeded limit)"
            )
        ...
        if random.random() < 0.015:
            ...
            logger.warning(
                f"Simulated completely off-topic content generation for {platform}, niche '{niche}'."
            )

        logger.info(
            f"Successfully generated simulated content for {platform} (niche: {niche})."
        )
...
if __name__ == "__main__":
    ...
    for plat in platforms_to_test:
        ...
        current_niche = (
            ...
            else random.choice(["MinimalistLiving", "RetroGaming"])
        )
        current_topic = (
            example_topic
            if random.random() > 0.1
            else f"{current_niche.capitalize()}Tips"
        )
        ...
        if content_output:
            ...
            print(
                f"Failed to generate content for '{plat}' (simulated critical failure)."
            )
        print("---")
