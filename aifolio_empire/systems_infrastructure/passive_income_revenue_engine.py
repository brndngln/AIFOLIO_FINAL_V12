...
class PassiveIncomeRevenueEngine:
    ...
    def __init__(self, simulated_current_date: Optional[datetime] = None):
        ...
        logger.info(
            f"PassiveIncomeRevenueEngine initialized. Operations stateless. Sim Date: {self.simulated_current_date.strftime('%Y-%m-%d')}"
        )
    ...
    def _get_simulated_price(self, base_price: Optional[float] = None) -> float:
        ...
        return round(
            random.uniform(
                BASE_VAULT_PRICE_RANGE_SIM[0], BASE_VAULT_PRICE_RANGE_SIM[1]
            ),
            2,
        )

    def define_pricing_tiers_simulated(
        self, base_product_name: str
    ) -> List[Dict[str, Any]]:
        ...
        tier_structures = [
            {
                "name": "Basic",
                "multiplier": 1.0,
                "features_sim": ["Core Content Access"],
            },
            {
                "name": "Premium",
                "multiplier": 1.5,
                "features_sim": ["Core Content", "Bonus Mini-Vault", "Checklist"],
            },
            {
                "name": "Deluxe",
                "multiplier": 2.2,
            ...
            tiers.append(
                {
                    ...
                    "features_sim": [
                        "Unknown benefits (Sim)",
                        "May include glitter (Sim)",
                    ],
    ...
    def create_launch_bundle_simulated(
        self, vault_ids: List[str], bundle_name: str
        ...
        discount_percentage = random.uniform(
            BUNDLE_DISCOUNT_RANGE_SIM[0], BUNDLE_DISCOUNT_RANGE_SIM[1]
        )
        ...
        if random.random() < 0.6:  # 60% chance of a launch bonus
            bonuses = [
                "Exclusive Q&A Session (Sim)",
                "Early Access to Next Vault (Sim)",
                "Bonus Mini-Guide (Sim)",
            ]
        ...
        bundle = {
            ...
            "offer_valid_until_sim": (
                self.simulated_current_date + timedelta(days=random.randint(3, 14))
            ).strftime("%Y-%m-%d"),
        ...
        if random.random() < 0.03:
            if random.random() < 0.5:
                bundle["bundle_price_usd_sim"] = bundle[
                    "total_individual_price_usd_sim"
                ]  # No discount applied
                bundle["discount_applied_sim_percent"] = 0.0
                logger.warning(
                    f"Simulated error: Launch bundle '{bundle_name}' discount not applied."
                )
            else:
                if len(bundle["included_vault_ids_sim"]) > 1:
                    bundle["included_vault_ids_sim"].pop()
                    logger.warning(
                        f"Simulated error: Launch bundle '{bundle_name}' missing an item."
                    )

        logger.info(f"Simulated launch bundle '{bundle_name}' created.")
        return bundle

    def create_future_drop_pass_simulated(
        self, pass_name: str, num_future_drops_sim: int
    ) -> Dict[str, Any]:
        ...
        pass_price_multiplier = random.uniform(
            FUTURE_DROP_PASS_PRICE_MULTIPLIER_SIM[0],
            FUTURE_DROP_PASS_PRICE_MULTIPLIER_SIM[1],
        )
        ...
        pass_price = min(
            pass_price, round(total_future_value_sim * 0.8, 2)
        )  # Max 80% of total future value
        pass_price = max(
            pass_price, round(avg_vault_price_sim * 1.2, 2)
        )  # Min 120% of single vault price
        if num_future_drops_sim <= 1:
            pass_price = round(avg_vault_price_sim * 0.9, 2)  # Special case for 1 drop
        ...
        if random.random() < 0.04:
            if random.random() < 0.5:
                pass_data["price_usd_sim"] = round(
                    pass_data["price_usd_sim"] * random.uniform(0.5, 1.5), 2
                )  # Mess up price
            else:
                pass_data["grants_access_to_next_n_drops_sim"] = max(
                    1,
                    num_future_drops_sim + random.randint(-num_future_drops_sim + 1, 5),
                )
            logger.warning(
                f"Simulated miscalculation for future drop pass '{pass_name}'."
            )
    ...
    def get_active_promotions_simulated(self) -> List[Dict[str, Any]]:
        ...
        if (
            1 <= day <= 7 and random.random() < 0.3
        ):  # 30% chance for a monthly promo in first week
            promos.append(
                {
        ...
        if (month == 5 and day >= 15) or (
            month == 6 and day <= 20
        ):  # Late May to Mid June - Summer Sale Sim
            if random.random() < 0.5:
                promos.append(
                    {
        ...
        elif (month == 11 and day >= 20) or (
            month == 12 and day <= 25
        ):  # Late Nov to Christmas - Holiday Sale Sim
        ...
        logger.info(
            f"Simulated {len(promos)} active promotions for date {self.simulated_current_date.strftime('%Y-%m-%d')}."
        )
        return promos
...
if __name__ == "__main__":
    ...
    test_date_for_seasonal = datetime(
        datetime.now().year, 6, 1
    )  # June 1st for Summer Sale
    ...
    launch_bundle = engine.create_launch_bundle_simulated(
        vault_ids_for_bundle, bundle_name
    )
    ...
    engine_december = PassiveIncomeRevenueEngine(
        simulated_current_date=datetime(datetime.now().year, 12, 1)
    )
