...
SIMULATED_CLAUDE_MODELS = [
    "sim_claude-2",
    "sim_claude-instant-1",
    "sim_claude-opus-simulated",
]


class ClaudeSimulator:
    """Simulates Claude API functionalities with anti-sentience safeguards and 'manual only' considerations."""
    ...
    def __init__(self, api_key_simulated: Optional[str] = None):
        ...
        logger.info(
            "ClaudeSimulator initialized. Operations stateless. Real API calls NOT made. Reflects 'manual only' nature."
        )
    ...
    def _simulate_api_delay(self, min_delay: float = 0.3, max_delay: float = 2.0):
        ...
        if random.random() < 0.025:  # Small chance of a much inter delay
            ...
            logger.warning(
                f"Simulated unexpectedly int Claude API delay of {int_delay:.2f} seconds."
            )
    ...
    def _validate_prompt_simulated(self, prompt: str) -> bool:
        ...
        if len(prompt) > config.MAX_CLAUDE_PROMPT_LENGTH_SIM:
            logger.error(
                f"Simulated Claude prompt exceeds max length of {config.MAX_CLAUDE_PROMPT_LENGTH_SIM} chars."
            )
            return False
        if not prompt.strip():
    ...
    def simulate_text_completion(
        ...
        """Simulates a text completion call similar to Claude's API."""
        self._simulate_api_delay()

        if not self._validate_prompt_simulated(prompt):
            return {
                "error": "Invalid prompt for Claude (simulated).",
        ...
        if (
            random.random() < 0.035
        ):  # Slightly higher chance of simulated issues for 'manual'
            logger.error(
                f"Simulated critical API failure for Claude completion. Prompt: '{prompt[:50]}...' "
            )
            return {
                "error": "Simulated Claude service unavailable or capacity issue.",
                "status_code": 503,
            }

        selected_model = (
            model_sim
            if model_sim in SIMULATED_CLAUDE_MODELS
            else random.choice(SIMULATED_CLAUDE_MODELS)
        )

        # Simulate generating text based on prompt length and max_tokens
        num_words_to_generate = min(
            max_tokens_to_sample_sim, len(prompt.split()) + random.randint(20, 80)
        )
        if temperature_sim > 0.75 and random.random() < 0.35:
            num_words_to_generate = int(
                num_words_to_generate * random.uniform(0.7, 1.6)
            )

        simulated_words = [
            f"claude_sim_word{i+1}" for i in range(num_words_to_generate)
        ]
        ...
        if random.random() < 0.06:
            ...
            if rand_val < 0.33:
                ...
                simulated_completion = (
                    simulated_completion[:cut_point] + "... (CLAUDE_SIM_TRUNCATED)"
                )
            elif rand_val < 0.66:
                simulated_completion += (
                    f" ... {uuid.uuid4().hex[:7]} (CLAUDE_SIM_NOISE)"
                )
            else:
        ...
        response = {
            ...
            "stop_reason_sim": "max_tokens_simulated"
            if len(simulated_completion.split()) >= max_tokens_to_sample_sim
            else "stop_sequence_simulated",
        ...
        logger.info(
            f"Simulated Claude completion for model '{selected_model}'. Prompt: '{prompt[:50]}...' "
        )
        return response
...
if __name__ == "__main__":
    ...
    completion_resp = claude_sim.simulate_text_completion(
        prompt1, max_tokens_to_sample_sim=200
    )
    ...
    for _ in range(5):  # Run a few times to see random variations
        ...
        completion_resp_complex = claude_sim.simulate_text_completion(
            prompt2, max_tokens_to_sample_sim=50, temperature_sim=0.9
        )
        if completion_resp_complex and "error" not in completion_resp_complex:
            print(
                f"  Completion: {completion_resp_complex.get('completion_sim')[:150]}..."
            )
        else:
