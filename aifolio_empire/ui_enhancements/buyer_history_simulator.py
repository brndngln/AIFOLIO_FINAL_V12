...
This module simulates fetching a buyer's purchase history and suggesting upgrade paths.
...
SIMULATED_PRODUCT_CATALOG = {
    "vault_alpha": {
        "name": "Alpha Vault - The Basics",
        "type": "vault",
        "price_sim": 9.99,
    },
    "vault_beta": {
        "name": "Beta Vault - Advanced Techniques",
        "type": "vault",
        "price_sim": 19.99,
    },
    "vault_gamma": {
        "name": "Gamma Vault - Niche Secrets",
        "type": "vault",
        "price_sim": 14.99,
    },
    "vault_delta": {
        "name": "Delta Vault - Marketing Mastery",
        "type": "vault",
        "price_sim": 24.99,
    },
    "bundle_starter": {
        "name": "Starter Bundle (Alpha + Gamma)",
        "type": "bundle",
        "items": ["vault_alpha", "vault_gamma"],
        "price_sim": 19.99,
    },
    "bundle_pro": {
        "name": "Pro Bundle (Beta + Delta)",
        "type": "bundle",
        "items": ["vault_beta", "vault_delta"],
        "price_sim": 39.99,
    },
    "bundle_all_access": {
        "name": "All Access Pass (All Vaults)",
        "type": "bundle",
        "items": ["vault_alpha", "vault_beta", "vault_gamma", "vault_delta"],
        "price_sim": 59.99,
    },
}
...
class BuyerHistorySimulator:
    ...
    def _generate_simulated_purchase_history(
        self, user_id_sim: str, product_catalog: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        num_purchases = random.randint(
            0, min(config.SIM_HISTORY_MAX_PURCHASES, len(product_catalog))
        )
        available_product_ids = [
            pid
            for pid, pdata in product_catalog.items()
            if pdata.get("type") == "vault"
        ]  # prefer individual vaults for history
        if (
            not available_product_ids and product_catalog
        ):  # fallback to any product if no vaults
            available_product_ids = list(product_catalog.keys())

        if not available_product_ids:
            return []

        purchased_ids = random.sample(
            available_product_ids, min(num_purchases, len(available_product_ids))
        )
            ...
            history.append(
                {
                    ...
                    "product_name_sim": product_data.get(
                        "name", f"Unknown Product {product_id}"
                    ),
                    "purchase_date_sim": (
                        base_purchase_time
                        - timedelta(days=random.randint(1, 365) + i * 10)
                    ).isoformat(),
                    "download_link_sim": f"https://simulated.aifolio.com/download/{product_id}/{uuid.uuid4().hex[:12]}",
                    "price_paid_sim": product_data.get(
                        "price_sim", round(random.uniform(5, 30), 2)
                    ),
                }
            )
    ...
    def get_simulated_history_and_upgrades(
        ...
        catalog = (
            current_product_catalog_sim
            if current_product_catalog_sim is not None
            else SIMULATED_PRODUCT_CATALOG
        )

        purchase_history = self._generate_simulated_purchase_history(
            user_id_sim, catalog
        )
        purchased_product_ids: Set[str] = {
            item["product_id_sim"] for item in purchase_history
        }
        ...
        available_bundles = {
            ...
            if pdata.get("type") == "bundle"
        }

        potential_bundle_upgrades = []
        for bundle_id, bundle_data in available_bundles.items():
            if (
                bundle_id not in purchased_product_ids
            ):  # Don't suggest if bundle itself already bought
                bundle_item_ids = set(bundle_data.get("items", []))
                owned_items_in_bundle = all_owned_vault_ids.intersection(
                    bundle_item_ids
                )
                    ...
                    potential_bundle_upgrades.append(
                        {
                            ...
                            "product_name_sim": bundle_data.get(
                            ...
                            "price_sim": bundle_data.get(
                                "price_sim", round(random.uniform(20, 100), 2)
                            ),
        ...
        potential_bundle_upgrades.sort(
            key=lambda x: (x["new_items_count_sim"], -x["price_sim"]), reverse=True
        )
        ...
        if (
            suggested_upgrades
            and random.random() < config.SIM_HISTORY_MISS_OBVIOUS_SUGGESTION_CHANCE
        ):
            removed_suggestion = suggested_upgrades.pop(
                random.randrange(len(suggested_upgrades))
            )
            logger.info(
                f"Simulated missing an obvious upgrade: {removed_suggestion['product_name_sim']} for {action_id}"
            )

        # Anti-sentience: Potentially add a random, less relevant suggestion (e.g., a single vault they don't own)
        if random.random() < config.SIM_HISTORY_RANDOM_IRRELEVANT_SUGGESTION_CHANCE:
            all_vault_ids = [
                pid for pid, pdata in catalog.items() if pdata.get("type") == "vault"
            ]
            unowned_vaults = [
                pid
                for pid in all_vault_ids
                if pid not in all_owned_vault_ids and pid not in purchased_product_ids
            ]
            if unowned_vaults:
                ...
                suggested_upgrades.append(
                    {
                        ...
                        "product_name_sim": vault_data.get(
                            "name", f"Vault {random_vault_id}"
                        ),
                        "type_sim": "individual_vault",
                        "reason_sim": "Consider this popular vault to expand your library (simulated random suggestion).",
                        "price_sim": vault_data.get(
                            "price_sim", round(random.uniform(5, 30), 2)
                        ),
                    }
                )
                logger.info(
                    f"Simulated adding a random irrelevant upgrade: {random_vault_id} for {action_id}"
                )

        # Limit number of suggestions
        final_suggestions = suggested_upgrades[
            : config.SIM_HISTORY_UPGRADE_SUGGESTION_COUNT
        ]
...
if __name__ == "__main__":
    ...
    history_custom_data = simulator.get_simulated_history_and_upgrades(
        test_user, current_product_catalog_sim=custom_catalog
    )
    print(json.dumps(history_custom_data, indent=2))

    # Simulate a user who might have bought a bundle already to see if it's re-suggested
    # For this, we'd need to control the generated history, which is internal to the method.
