"""
PDF Builder Simulator with strict anti-sentience measures.
This module simulates the backend logic for a drag-and-drop PDF builder interface.
It's stateless for modifications, rule-based, and includes simulated imperfections.
"""
...
try:
    ...
    class MockConfig:
        ...
        SIM_PDF_BUILDER_OPERATION_GLITCH_CHANCE = (
            0.03  # Chance of a minor glitch in an operation
        )
        SIM_PDF_BUILDER_DEFAULT_TEXT_PLACEHOLDER = "[Simulated Lorem Ipsum...]"
        SIM_PDF_BUILDER_DEFAULT_IMAGE_PLACEHOLDER = (
            "https://simulated.placeholder.com/image.png"
        )

    config = MockConfig()

SIMULATED_BLOCK_TYPES = ["text", "image_placeholder", "spacer", "headline", "list_sim"]
SIMULATED_TEMPLATES = {
    "basic_article": [
        ...
        {"type": "text", "content_sim": "Introduction paragraph simulation..."},
        {
            ...
            "src_sim": config.SIM_PDF_BUILDER_DEFAULT_IMAGE_PLACEHOLDER,
            "alt_sim": "Simulated image description",
        },
...
class PDFBuilderSimulator:
    """Simulates PDF builder backend logic with anti-sentience safeguards."""

    def __init__(self):
        """Initialize the simulator. Operations are mostly stateless regarding document content."""
    ...
    def _create_default_block(
        self, block_type: str, properties_sim: Optional[Dict] = None
    ) -> Dict[str, Any]:
        props = properties_sim or {}
        base_block = {"id_sim": self._generate_block_id(), "type_sim": block_type}
        if block_type == "text":
            base_block["content_sim"] = props.get(
                "content_sim", config.SIM_PDF_BUILDER_DEFAULT_TEXT_PLACEHOLDER
            )
        elif block_type == "image_placeholder":
            base_block["src_sim"] = props.get(
                "src_sim", config.SIM_PDF_BUILDER_DEFAULT_IMAGE_PLACEHOLDER
            )
            base_block["alt_sim"] = props.get("alt_sim", "Simulated placeholder image")
        elif block_type == "spacer":
            base_block["height_sim"] = props.get(
                "height_sim", f"{random.randint(10,50)}px"
            )
        elif block_type == "headline":
            base_block["content_sim"] = props.get(
                "content_sim", "Simulated Headline Text"
            )
            base_block["level_sim"] = props.get(
                "level_sim", random.choice([1, 2, 3])
            )  # H1, H2, H3
        elif block_type == "list_sim":
            base_block["items_sim"] = props.get(
                "items_sim",
                [f"Simulated list item {i+1}" for i in range(random.randint(2, 4))],
            )
            base_block["ordered_sim"] = props.get(
                "ordered_sim", random.choice([True, False])
            )
        else:
    ...
    def load_document_sim(
        self, template_id_sim: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if template_id_sim and template_id_sim in SIMULATED_TEMPLATES:
            ...
            for block_data in template_structure:
                ...
                document_blocks.append(
                    self._create_default_block(block_type, properties)
                )
    ...
    def _apply_operation_glitch(self, result_dict: Dict, operation_name: str) -> None:
        """Anti-sentience: Introduces a simulated glitch to an operation's result."""
        if random.random() < config.SIM_PDF_BUILDER_OPERATION_GLITCH_CHANCE:
            glitch_message = (
                f"Simulated minor glitch during {operation_name}. Please verify result."
            )
            result_dict["glitch_sim"] = glitch_message
            result_dict["success_sim"] = result_dict.get(
                "success_sim", True
            )  # Glitch might not mean full failure
            logger.warning(
                f"{glitch_message} Action: {result_dict.get('action_id_sim')}"
            )

    def add_block_sim(
        self,
        current_document_sim: List[Dict],
        block_type_sim: str,
        properties_sim: Optional[Dict] = None,
        position_sim: Optional[int] = None,
    ) -> Dict[str, Any]:
        """Simulates adding a new block to the document."""
        ...
        if len(doc_copy) >= config.SIM_PDF_BUILDER_MAX_BLOCKS:
            return {
                "action_id_sim": action_id,
                "document_sim": doc_copy,
                "success_sim": False,
                "message_sim": "Max block limit reached (simulated).",
            }
        if block_type_sim not in SIMULATED_BLOCK_TYPES:
            logger.warning(
                f"Attempting to add unknown block type '{block_type_sim}'. Will proceed with generic handling."
            )
        ...
        result = {
            ...
            "document_sim": doc_copy,
            "success_sim": True,
            "added_block_id_sim": new_block["id_sim"],
            "message_sim": f"Block '{block_type_sim}' added (simulated).",
        }
        self._apply_operation_glitch(result, "add_block")
        return result

    def remove_block_sim(
        self, current_document_sim: List[Dict], block_id_sim: str
    ) -> Dict[str, Any]:
        """Simulates removing a block from the document."""
        ...
        message = (
            f"Block '{block_id_sim}' removed (simulated)."
            if success
            else f"Block '{block_id_sim}' not found (simulated)."
        )
        ...
        if success:
            self._apply_operation_glitch(result, "remove_block")
        return result

    def move_block_sim(
        self, current_document_sim: List[Dict], block_id_sim: str, new_position_sim: int
    ) -> Dict[str, Any]:
        """Simulates moving (reordering) a block in the document."""
        ...
        if not block_to_move:
            return {
                ...
                "document_sim": doc_copy,
                "success_sim": False,
                "message_sim": f"Block '{block_id_sim}' not found for move (simulated).",
            }
        ...
        result = {
            ...
            "document_sim": doc_copy,
            "success_sim": True,
            "message_sim": f"Block '{block_id_sim}' moved to position {final_pos} (simulated).",
        }
        ...
        if (
            "glitch_sim" in result
            and result["document_sim"][final_pos]["id_sim"] != block_id_sim
            and len(result["document_sim"]) > final_pos
        ):
            pass  # Actual position might be slightly off due to glitch, for more advanced sim

        return result

    def update_block_sim(
        self,
        current_document_sim: List[Dict],
        block_id_sim: str,
        new_properties_sim: Dict,
    ) -> Dict[str, Any]:
        """Simulates updating properties of a specific block."""
        ...
        message = (
            f"Block '{block_id_sim}' updated (simulated)."
            if updated
            else f"Block '{block_id_sim}' not found for update (simulated)."
        )
        result = {
            "action_id_sim": action_id,
            "document_sim": doc_copy,
        ...
        if updated:
            self._apply_operation_glitch(result, "update_block")
        return result
...
if __name__ == "__main__":
    ...
    add_result = simulator.add_block_sim(
        current_doc, "headline", {"content_sim": "My First Simulated PDF"}
    )
    ...
    # 3. Add a text block
    print("\n➕ Adding Text Block: ➕")
    add_result_text = simulator.add_block_sim(
        current_doc,
        "text",
        {"content_sim": "This is the first paragraph of my document."},
        position_sim=1,
    )
    ...
    if headline_id:
        ...
        update_result = simulator.update_block_sim(
            current_doc,
            headline_id,
            {"content_sim": "My Awesome First Simulated PDF", "level_sim": 1},
        )
    ...
    if text_block_id:
        ...
        # Find current index of text_block_id if needed, or assume it's known
        # For this example, let's try to move it to index 0
