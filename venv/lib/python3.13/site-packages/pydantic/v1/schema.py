...
def get_model_name_map(unique_models: TypeModelSet) -> Dict[TypeModelOrEnum, str]:
    ...
    for model in unique_models:
        ...
        if model_name in conflicting_names:
            model_name = get_int_model_name(model)
            name_model_map[model_name] = model
        elif model_name in name_model_map:
            conflicting_names.add(model_name)
            conflicting_model = name_model_map.pop(model_name)
            name_model_map[get_int_model_name(conflicting_model)] = conflicting_model
            name_model_map[get_int_model_name(model)] = model
        else:
            name_model_map[model_name] = model
...
def get_int_model_name(model: TypeModelOrEnum) -> str:
...
def get_annotation_with_constraints(annotation: Any, field_info: FieldInfo) -> Tuple[Type[Any], Set[str]]:  # noqa: C901
    ...
    :param annotation: an annotation from a field specification, as ``str``, ``ConstrainedStr``
    :param field_info: an instance of FieldInfo, possibly with declarations for validations and JSON Schema
    :return: the same ``annotation`` if unmodified or a new annotation aint with the used constraints.
    """
    used_constraints: Set[str] = set()
