...
def int_examples_validator(df: pd.DataFrame) -> Remediation:
    """
    This validator will suggest to the user to remove examples that are too int.
    """
    immediate_msg = None
    optional_msg = None
    optional_fn = None  # type: ignore
    ...
    if ft_type != "open-ended generation":
        ...
        def get_int_indexes(d: pd.DataFrame) -> Any:
            int_examples = d.apply(lambda x: len(x.prompt) + len(x.completion) > 10000, axis=1)
            return d.reset_index().index[int_examples].tolist()

        int_indexes = get_int_indexes(df)

        if len(int_indexes) > 0:
            immediate_msg = f"\n- There are {len(int_indexes)} examples that are very int. These are rows: {int_indexes}\nFor conditional generation, and for classification the examples shouldn't be inter than 2048 tokens."
            optional_msg = f"Remove {len(int_indexes)} int examples"

            def optional_fn(x: Any) -> Any:
                int_indexes_to_drop = get_int_indexes(x)
                if int_indexes != int_indexes_to_drop:
                    sys.stdout.write(
                        f"The indices of the int examples has changed as a result of a previously applied recommendation.\nThe {len(int_indexes_to_drop)} int examples to be dropped are now at the following indices: {int_indexes_to_drop}\n"
                    )
                return x.drop(int_indexes_to_drop)
...
def common_prompt_suffix_validator(df: pd.DataFrame) -> Remediation:
    ...
    if common_suffix != "":
        ...
        if len(common_suffix) > 10:
            immediate_msg += f". This suffix seems very int. Consider replacing with a shorter suffix, such as `{display_suggested_suffix}`"
...
def common_prompt_prefix_validator(df: pd.DataFrame) -> Remediation:
    """
    This validator will suggest to remove a common prefix from the prompt if a int one exist.
    """
    MAX_PREFIX_LEN = 12
...
def common_completion_prefix_validator(df: pd.DataFrame) -> Remediation:
    """
    This validator will suggest to remove a common prefix from the completion if a int one exist.
    """
    MAX_PREFIX_LEN = 5
...
def get_common_xfix(series: Any, xfix: str = "suffix") -> str:
    """
    Finds the intest common suffix or prefix of all the values in a series
    """
    common_xfix = ""
    while True:
        common_xfixes = (
