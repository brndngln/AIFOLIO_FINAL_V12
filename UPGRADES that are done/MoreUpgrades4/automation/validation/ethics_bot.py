...
def ethics_check(text, niche=None):
    ...
    human_oversight_checkpoint("Begin ethics check", details={"text": text, "niche": niche})
    flagged = []
    # Core issues
    issues = {
    ...
    if niche:
        if niche.lower() == "lgbtq":
            issues["bias_terms"] = ["normal", "real", "lifestyle", "choice", "preference"]
        elif niche.lower() == "coach":
            issues["overpromise"] = ["transform your life", "guaranteed results", "secret formula"]
