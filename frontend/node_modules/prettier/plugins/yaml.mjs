...
`),n}xs.parse=Io});var qe=te(k=>{"use strict";var p=le();function Po(t,e,n){return n?`#${n.replace(/[\s\S]^/gm,`$&${e}#`)}
...
`)&&(h=!0),m&&M<this.items.length-1&&(_+=","),_=Fe(_,i,A),y&&(A||m)&&(y=!1),C.push({type:"item",str:_}),C},[]),w;if(g.length===0)w=r.start+r.end;else if(m){let{start:C,end:L}=r,M=g.map(A=>A.str);if(h||M.reduce((A,_)=>A+_.length+2,2)>t.maxFlowStringSingleLineLength){w=C;for(let A of M)w+=A?`
${l}${c}${A}`:`
`;w+=`
...
${e}${t.slice(A+1,_)}`)}return L}var Un=({indentAtStart:t})=>t?Object.assign({indentAtStart:t},Ne.fold):Ne.fold,Wt=t=>/^(%|---|\.\.\.)/m.test(t);function $o(t,e,n){if(!e||e<0)return!1;let r=e-n,s=t.length;if(s<=r)return!1;for(let i=0,o=0;i<s;++i)if(t[i]===`
...
`,l+=2;a+=o,i[l+2]===" "&&(a+="\\"),l+=1,c=l+1}break;default:l+=1}return a=c?a+i.slice(c):i,n?a:Vt(a,o,qt,Un(e))}function Us(t,e){if(e.implicitKey){if(/\n/.test(t))return we(t,e)}else if(/[ \t]\n|\n[ \t]/.test(t))return we(t,e);let n=e.indent||(Wt(t)?"  ":""),r="'"+t.replace(/'/g,"''").replace(/\n+/g,`$&
${n}`)+"'";return e.implicitKey?r:Vt(r,n,qs,Un(e))}function Ut({comment:t,type:e,value:n},r,s,i){if(/\n[\t ]+$/.test(n)||/^\s*$/.test(n))return we(n,r);let o=r.indent||(r.forceBlockIndent||Wt(n)?"  ":""),a=o?"2":"1",c=e===p.Type.BLOCK_FOLDED?!1:e===p.Type.BLOCK_LITERAL?!0:!$o(n,Ne.fold.lineWidth,o.length),l=c?"|":">";if(!n)return l+`
=======
...
${o}${f}${n}${m}`;n=n.replace(/\n+/g,`
<<<<<<< HEAD
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${o}`);let d=Vt(`${f}${n}${m}`,o,Fn,Ne.fold);return`${l}
...
`)===-1?o.indexOf('"')!==-1&&o.indexOf("'")===-1?Us(o,e):we(o,e):Ut(t,e,n,r);if(!c&&!f&&i!==p.Type.PLAIN&&o.indexOf(`
`)!==-1)return Ut(t,e,n,r);if(l===""&&Wt(o))return e.forceBlockIndent=!0,Ut(t,e,n,r);let m=o.replace(/\n+/g,`$&
${l}`);if(a){let{tags:y}=e.doc.schema;if(typeof qn(m,y,y.scalarFallback).value!="string")return we(o,e)}let d=c?m:Vt(m,l,qs,Un(e));return s&&!f&&(d.indexOf(`
`)!==-1||s.indexOf(`
...
`&&n!=="	"&&n!==" "){let r="Comments must be separated from other tokens by white space characters";t.push(new p.YAMLSemanticError(e,r))}}function Ws(t,e){let n=String(e),r=n.substr(0,8)+"..."+n.substr(-8);return new p.YAMLSemanticError(t,`The "${r}" key is too long`)}function js(t,e){for(let{afterKey:n,before:r,comment:s}of e){let i=t.items[r];i?(n&&i.value&&(i=i.value),s===void 0?(n||!i.commentBefore)&&(i.spaceBefore=!0):i.commentBefore?i.commentBefore+=`
...
`&&n++,n===1&&/\S/u.test(i))return!1;if(n===2)return!0}return!1}function Ot(t){let{node:e}=t;switch(e.type){case"tag":case"anchor":case"comment":return!1}let n=t.stack.length;for(let r=1;r<n;r++){let s=t.stack[r],i=t.stack[r-1];if(Array.isArray(i)&&typeof s=="number"&&s!==i.length-1)return!1}return!0}function Lt(t){return _e(t.children)?Lt(x(!1,t.children,-1)):t}function br(t){return t.value.trim()==="prettier-ignore"}function Or(t){let{node:e}=t;if(e.type==="documentBody"){let n=t.parent.head;return R(n)&&br(x(!1,n.endComments,-1))}return ee(e)&&br(x(!1,e.leadingComments,-1))}function Re(t){return!_e(t.children)&&!Ji(t)}function Ji(t){return ee(t)||ie(t)||hn(t)||K(t)||R(t)}function ee(t){return _e(t==null?void 0:t.leadingComments)}function ie(t){return _e(t==null?void 0:t.middleComments)}function hn(t){return t==null?void 0:t.indicatorComment}function K(t){return t==null?void 0:t.trailingComment}function R(t){return _e(t==null?void 0:t.endComments)}function Lr(t){let e=[],n;for(let r of t.split(/( +)/u))r!==" "?n===" "?e.push(r):e.push((e.pop()||"")+r):n===void 0&&e.unshift(""),n=r;return n===" "&&e.push((e.pop()||"")+" "),e[0]===""&&(e.shift(),e.unshift(" "+(e.shift()||""))),e}function Ar(t,e,n){let r=e.split(`
