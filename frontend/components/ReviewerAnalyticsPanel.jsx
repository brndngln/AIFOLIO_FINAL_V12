...
const ReviewerAnalyticsPanel = () => {
  ...
  useEffect(() => {
    axios.get('/api/reviewer/analytics').then(res => setStats(res.data)).finally(() => setLoading(false));
  }, []);

  return (
    <Box sx={{ p: 3, bgcolor: '#f8f8f8', borderRadius: 2, mb: 2 }}>
      <Typography variant="h6" mb={2}>Reviewer Performance Analytics</Typography>
            ...
            <TableBody>
              {stats.map((s, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell><Chip label={s.approvals} color="success" /></TableCell>
                  <TableCell><Chip label={s.rejections} color="error" /></TableCell>
                  <TableCell><Chip label={s.workflows} color="info" /></TableCell>
