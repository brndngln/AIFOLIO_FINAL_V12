...
import {
  Box, Typography, Button, Paper, Tabs, Tab, Badge, Chip, Tooltip, IconButton, CircularProgress, Dialog, DialogTitle, DialogContent, DialogActions, TextField
} from '@mui/material';
  ...
  return (
    <Paper elevation={1} sx={{ p: 2, mb: 1, display: 'flex', alignItems: 'center', gap: 2 }}>
      <Badge
        ...
        badgeContent={event.status === 'error' ? <ErrorIcon color="error" /> : <CheckCircleIcon color="success" />}
      ...
      <Tooltip title="Retrigger">
        <IconButton onClick={() => onRetrigger(event)}><ReplayIcon /></IconButton>
      </Tooltip>
      <Tooltip title="Edit Payload">
        <IconButton onClick={() => onEdit(event)}><EditIcon /></IconButton>
      </Tooltip>
      {event.status === 'error' && (
        <Tooltip title="Retry via SMS">
          <IconButton color="secondary"><SmsIcon /></IconButton>
        </Tooltip>
      )}
    </Paper>
  );
...
export default function EliteEventsDashboard() {
  ...
  const handleRetrigger = async (event) => {
    await fetch(`${API_URL}/retrigger`, {
      ...
      body: JSON.stringify({ event_type: event.event_type, payload: event.payload })
    });
    fetchEvents();
  };
  ...
  const handleEditSave = async () => {
    await fetch(`${API_URL}/edit`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ event_type: editEvent.event_type, payload: JSON.parse(editPayload) })
    });
    setEditEvent(null);
    fetchEvents();
  };
  ...
  return (
    <Box>
      <Typography variant="h5" sx={{ mb: 2, fontWeight: 'bold' }}>Elite Events Dashboard</Typography>
      ...
      <Dialog open={!!editEvent} onClose={() => setEditEvent(null)} maxWidth="md" fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={handleEditSave} variant="contained">Save & Retrigger</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
