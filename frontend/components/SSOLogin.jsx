...
const SSOLogin = ({ onLogin }) => {
  ...
  return (
    <Box sx={{ p: 3, bgcolor: '#fff', borderRadius: 2, maxWidth: 400, mx: 'auto', mt: 8 }}>
      <Typography variant="h6" mb={2}>Single Sign-On (SSO) Login</Typography>
      <TextField label="Email" value={email} onChange={e => setEmail(e.target.value)} fullWidth margin="normal" />
      {error && <Alert severity="error">{error}</Alert>}
      <Button variant="contained" color="primary" onClick={handleSSO} fullWidth disabled={loading || !email} sx={{ mt: 2 }}>
