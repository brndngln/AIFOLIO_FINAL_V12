...
const LiveComplianceDashboard = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(
        ([usageRes, anomalyRes, rootCauseRes, rotationRes, overrideRes]) => {
    ...
    const interval = setInterval(() => {
      axios
        ...
        .then(() => setLastExport(new Date().toLocaleString()));
    }, 60000);
        ...
        <Grid container spacing={3}>
            ...
            <Paper sx={{ p: 2 }}>
              ...
              <Doughnut
                data={{
                  ...
                  datasets: [
                    {
                      data: rootCauses.map(() => 1),
                      backgroundColor: rootCauses.map(
                        (_, i) => `hsla(${i * 60},70%,50%,0.6)`,
                      ),
                    },
                  ],
                }}
          ...
          <Grid item xs={12}>
            <Stack direction="row" spacing={2} sx={{ mt: 2 }}>
              ...
              <Chip
                label={`Last Export: ${lastExport || "N/A"}`}
                color="info"
              />
            </Stack>
          </Grid>
          <Grid item xs={12}>
            <Alert severity="info">
              All compliance data is live and exportable for audit at any time.
            </Alert>
          </Grid>
        </Grid>
      )}
