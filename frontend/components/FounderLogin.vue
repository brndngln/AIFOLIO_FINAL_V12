<template>
  <div class="founder-login flex flex-col items-center justify-center min-h-screen bg-gray-50">
    ...
    <form @submit.prevent="submit" class="w-full max-w-sm">
      ...
      <div class="mb-4">
        ...
        <button type="button" @click="biometricAuth" class="btn">Verify Biometric</button>
        <span v-if="biometricVerified" class="text-green-600 ml-2">Verified</span>
      </div>
      <div class="mb-4">
      ...
      <div class="mb-4">
        ...
        <input v-model="signature" class="input" placeholder="Paste digital signature" />
      </div>
...
if (window && window.navigator) {
  device_id.value = window.navigator.userAgent + '-' + window.navigator.platform
}
...
.btn { @apply bg-blue-100 text-blue-700 px-4 py-2 rounded; }
