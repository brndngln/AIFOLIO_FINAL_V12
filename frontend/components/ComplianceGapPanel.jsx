...
const ComplianceGapPanel = () => {
  ...
  useEffect(() => {
    axios.get('/api/compliance/gaps').then(res => setGaps(res.data)).finally(() => setLoading(false));
  }, []);

  return (
    <Box sx={{ p: 3, bgcolor: '#f8f8f8', borderRadius: 2, mb: 2 }}>
      <Typography variant="h6" mb={2}>Dynamic Compliance Gap Analysis</Typography>
      {loading ? <CircularProgress /> : gaps.length === 0 ? <Alert severity="success">No gaps found. All mapped!</Alert> : (
        <Paper>
          <Table size="small">
            <TableHead>
              <TableRow>
