...
import { Box, Typography, Paper, Button, Divider, Select, MenuItem } from '@mui/material';
  ...
  return (
    <Paper sx={{ p: 3, mb: 2 }}>
      ...
      <Box mb={2}>
        ...
        <Select value={reviewer} onChange={e=>setReviewer(e.target.value)} sx={{ mr: 2 }}>
          {reviewers.map(r => <MenuItem key={r} value={r}>{r}</MenuItem>)}
        </Select>
        <Typography variant="subtitle2" sx={{ ml: 4 }}>For Vault:</Typography>
        <Select value={vault} onChange={e=>setVault(e.target.value)}>
          {vaults.map(v => <MenuItem key={v} value={v}>{v}</MenuItem>)}
        </Select>
      </Box>
      <Button variant="contained" color="primary" onClick={()=>onSimulate && onSimulate(reviewer, vault)}>
