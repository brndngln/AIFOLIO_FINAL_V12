...
const RuleManagerPanel = () => {
  ...
  const handleSave = () => {
    ...
    axios
      .post("/api/rules/update", payload)
      .then(() => {
        setOpen(false);
        setSnackbar("Rule saved!");
        fetchRules();
      })
  ...
  const handleDelete = (p, r) => {
    axios
      .post("/api/rules/delete", { platform: p, pattern: r.pattern })
      .then(() => {
        setSnackbar("Rule deleted!");
        fetchRules();
      })
    ...
    <Box sx={{ p: 3, bgcolor: "#f8f8f8", borderRadius: 2, mb: 2 }}>
      ...
      <Paper sx={{ mt: 2 }}>
        <Table size="small">
          ...
          <TableBody>
            {Object.entries(rules).flatMap(([p, arr]) =>
              arr.map((r, i) => (
                <TableRow key={p + i}>
                  ...
                  <TableCell>
                    <Button size="small" onClick={() => handleOpen(p, r)}>
                      Edit
                    </Button>
                    <IconButton size="small" onClick={() => handleDelete(p, r)}>
                      <DeleteIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
