...
const integrations = [
  {
    label: "Notion",
    endpoint: "/api/integrate/notion",
    fields: ["title", "content"],
  },
  {
    label: "Airtable",
    endpoint: "/api/integrate/airtable",
    fields: ["table", "record"],
  },
  ...
  {
    label: "Webhook",
    endpoint: "/api/integrate/webhook",
    fields: ["payload", "endpoint"],
  },
  {
    label: "SendGrid",
    endpoint: "/api/integrate/sendgrid",
    fields: ["to_email", "subject", "message"],
  },
  { label: "Discord", endpoint: "/api/integrate/discord", fields: ["message"] },
  {
    label: "Google Sheets",
    endpoint: "/api/integrate/googlesheets",
    fields: ["sheet_id", "values"],
  },
  ...
  const handleSend = async (integration) => {
    try {
      ...
      integration.fields.forEach((f) => {
        payload[f] = form[`${integration.label}_${f}`] || "";
      });
            ...
            <Paper sx={{ p: 2, mb: 2 }}>
              ...
              {integration.fields.map((f) => (
                <TextField
                  ...
                  onChange={(e) =>
                    handleChange(`${integration.label}_${f}`, e.target.value)
                  }
                  fullWidth
                  sx={{ mb: 1 }}
                />
