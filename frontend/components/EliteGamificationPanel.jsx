...
const mockGamification = [
  { name: 'Reviewer Alpha', streak: 12, badges: ['Accuracy Pro', 'Speedster'], avatar: 'A' },
  { name: 'Reviewer Beta', streak: 8, badges: ['Compliance Hero'], avatar: 'B' },
    ...
    <Paper sx={{ p: 3, mb: 2 }}>
      ...
      {mockGamification.map((r, idx) => (
        <Box key={idx} display="flex" alignItems="center" mb={2}>
          ...
          <Typography sx={{ ml: 2, fontWeight: 'bold', flex: 1 }}>{r.name}</Typography>
          <Chip icon={<WhatshotIcon />} label={`Streak: ${r.streak}`} color="warning" sx={{ mr: 2 }} />
          {r.badges.map((b, j) => (
