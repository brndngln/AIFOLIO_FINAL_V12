...
const SecretRotationHeatmap = () => {
  ...
  const fetchData = () => {
    ...
    axios
      .get("/api/logs/secret_rotation", { params })
      .then((res) => {
        setData(res.data);
        ...
        if (last && last.status !== "SUCCESS") {
          setAlert(
            `ALERT: Last rotation failed at ${last.timestamp} (${last.key}: ${last.status})`,
          );
        } else {
          setAlert(null);
        }
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginBottom: 8, fontSize: 13 }}>
        Rotations: {total} | Failures: {failures} | Last Failure:{" "}
        {lastFail
          ? `${lastFail.timestamp} (${lastFail.key}: ${lastFail.status})`
          : "None"}
      </div>
          ...
          >
            <div
              style={{
                ...
                height: 36,
                background: getColor(
                  byDay[day].some((e) => e.status !== "SUCCESS")
                    ? "FAIL"
                    : "SUCCESS",
                ),
                borderRadius: 4,
      ...
      <div style={{ marginTop: 8, fontSize: 12 }}>
        <span
          style={{ background: "#4CAF50", padding: "0 8px", marginRight: 4 }}
        />{" "}
        Success
        <span
          style={{ background: "#FFC107", padding: "0 8px", margin: "0 4px" }}
        />{" "}
        Verify Fail
        <span
          style={{ background: "#F44336", padding: "0 8px", margin: "0 4px" }}
        />{" "}
        Fail/Fallback
      </div>
    </div>
