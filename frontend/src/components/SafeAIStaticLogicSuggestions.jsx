...
const suggestions = [
  {
    label: "Deterministic logic only",
    desc: "Use only deterministic, non-adaptive logic in all business flows",
  },
  {
    label: "OWNER approval required",
    desc: "OWNER must approve all logic changes before deploy",
  },
  {
    label: "Document and audit",
    desc: "Document all static logic and audit changes",
  },
  {
    label: "No adaptive/agentic AI",
    desc: "Never allow adaptive or agentic AI in SAFE AI mode",
  },
  {
    label: "OWNER-controlled outputs",
    desc: "All exports and outputs must be OWNER-controlled",
  },
];
...
export default function SafeAIStaticLogicSuggestions() {
  return (
    <section
      aria-labelledby="static-logic-suggestions-heading"
      style={{
        marginTop: 32,
        background: "#fff",
        borderRadius: 8,
    ...
    >
      ...
      >
        <h3
          id="static-logic-suggestions-heading"
          style={{ color: "#0ea5e9", fontWeight: 700, margin: 0 }}
        >
          SAFE AI — Static Logic Suggestions
        </h3>
        <span
          style={{
            background: "#0ea5e9",
        ...
        <span
          tabIndex={0}
          aria-label="Help: What are Static Logic Suggestions?"
          title="These are OWNER rules for SAFE AI compliance. All logic must be static, deterministic, and OWNER-controlled."
          style={{
            marginLeft: 6,
            color: "#64748b",
            cursor: "help",
      ...
      <ul
        ...
        aria-label="SAFE AI static logic suggestions list"
      >
        {suggestions.map((s, i) => (
          <li
            key={i}
          ...
          >
            ...
            <span tabIndex={0} aria-label={s.label} title={s.desc}>
              {s.label}: {s.desc}
            </span>
            <span
              tabIndex={0}
      ...
      <div style={{ marginTop: 18, color: "#64748b", fontSize: 13 }}>
        <b>OWNER CONTROLLED:</b> Follow all above logic rules for SAFE AI
        compliance.
      </div>
    </section>
  );
