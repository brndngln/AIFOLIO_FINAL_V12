  ...
  return (
    <div className="gumroad-panel">
      ...
      <div className="vault-summary mb-4 p-4 border rounded bg-gray-50">
        ...
        <div>
          <strong>Niche:</strong> {vault.metadata.niche}
        </div>
        <div>
          <strong>Price:</strong> ${vault.metadata.price}
        </div>
        <div>
          <strong>Bundle Size:</strong> {vault.metadata.bundle_size}
        </div>
        <div>
          <strong>Value Score:</strong> {vault.preview.value_score}
        </div>
        <div>
          <strong>Outline:</strong>{" "}
          <ul>
            {vault.preview.outline.map((o, i) => (
              <li key={i}>{o}</li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul>
            {vault.preview.testimonials.map((t, i) => (
              <li key={i}>
                {t.text} <em>({t.persona})</em>
              </li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul>
            {vault.preview.benefits.map((b, i) => (
              <li key={i}>{b}</li>
            ))}
          </ul>
        </div>
        <div>
          <strong>Average Rating:</strong> {vault.preview.avg_rating} (
          {vault.preview.total_reviews} reviews)
        </div>
        <div>
          <strong>Featured Review:</strong> {vault.preview.featured_review}
        </div>
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              alert(
                result.success
                  ? `Gumroad delivery successful! Product ID: ${result.productId}`
