...
const AILogVisualizerPanel = () => {
  ...
  if (!logs.length)
    return <div className="theme-card">No recent AI activity.</div>;
        ...
        {logs.map((entry, i) => (
          <div
            ...
            style={{
              ...
              color:
                entry.status === "WARNING_SIM"
                  ? "var(--warning)"
                  : entry.status === "ERROR_SIM"
                    ? "var(--error)"
                    : "var(--text)",
            }}
          >
            <div style={{ fontWeight: 600 }}>
              {entry.message || entry.log_id_sim}
            </div>
            <div style={{ fontSize: "0.85rem", color: "var(--cta)" }}>
              {entry.timestamp_sim || entry.timestamp}
            </div>
            {entry.status && (
