...
export default function InnovationPipeline() {
  return (
    ...
    >
      <h3 style={{ color: "#fbbf24", fontWeight: 700, marginBottom: 12 }}>
        SAFE AI Innovation Pipeline
      </h3>
      <ul style={{ listStyle: "none", padding: 0, fontSize: 15 }}>
        {PIPELINE.map((item, i) => (
          <li key={i} style={{ marginBottom: 8 }}>
            <span style={{ color: "#fbbf24", fontWeight: 600 }}>★</span> {item}
          </li>
        ))}
      </ul>
      <div style={{ marginTop: 18, color: "#64748b", fontSize: 13 }}>
        <b>OWNER CONTROLLED:</b> All pipeline actions are static, audit-logged,
