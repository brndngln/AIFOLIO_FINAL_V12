...
export default function OwnerOnboardingPanel() {
  const [progress, setProgress] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : Array(ONBOARDING_STEPS.length).fill(false);
  });
  ...
  return (
    <section
      aria-labelledby="owner-onboarding-heading"
      style={{
        background: "#f9fafb",
        borderRadius: 12,
        padding: 32,
    ...
    >
      ...
      >
        ...
        <span
          tabIndex={0}
          aria-label="Help: Onboarding steps"
          title="Follow these steps to complete initial setup and compliance."
          style={{
            marginLeft: 6,
            color: "#64748b",
            cursor: "help",
      ...
      <div style={{ marginBottom: 18 }} aria-label="Onboarding progress">
        ...
        >
          <div
            style={{ width: `${percent}%`, background: "#0ea5e9", height: 8 }}
          ></div>
        </div>
        <span style={{ fontSize: 13, color: "#64748b" }}>
          {completeCount} of {ONBOARDING_STEPS.length} steps complete
        </span>
      </div>
      <ul
      ...
      >
        {ONBOARDING_STEPS.map((step, i) => (
          ...
          >
            ...
            />
            <span tabIndex={0} aria-label={`Onboarding step: ${step}`}>
              {step}
            </span>
          </li>
        ))}
      ...
      <div style={{ marginTop: 18, color: "#64748b", fontSize: 13 }}>
        <b>Tip:</b> Progress is saved in your browser. Complete all steps for
