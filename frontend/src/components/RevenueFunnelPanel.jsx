...
export default function RevenueFunnelPanel({ onAction }) {
  ...
  if (loading)
    return <div className="theme-card">Loading revenue funnel...</div>;
  if (error) return <div className="theme-card text-error">{error}</div>;

  return (
    <section
      aria-labelledby="revenue-funnel-heading"
      style={{
        background: "#fef9c3",
        borderRadius: 14,
        padding: 32,
    ...
    >
      ...
      >
        ...
        <span
          tabIndex={0}
          aria-label="Help: What is the Revenue Funnel?"
          title="Static, deterministic revenue funnel. OWNER controls all actions and exports."
          style={{
            marginLeft: 6,
            color: "#64748b",
            cursor: "help",
      ...
      <div
        ...
        aria-label="Revenue funnel visualization"
      >
        {funnel.stages.map((stage, i) => (
          <div key={stage.name} style={{ flex: 1, textAlign: "center" }}>
            <div
              style={{
                height: 40 + stage.count * 2,
                width: 60,
                background: "#fde68a",
                borderRadius: 10,
                margin: "0 auto",
            ...
            <div style={{ marginTop: 8, fontWeight: 600, color: "#b45309" }}>
              {stage.name}
            </div>
