...
const ColorCustomization = () => {
  ...
  useEffect(() => {
    const checkForSentience = () => {
      ...
      if (theme.customColors && Object.keys(theme.customColors).length > 1000) {
        console.warn(
          "Color system complexity limit reached. Resetting to default.",
        );
        applyPreset("default");
      }
      ...
      colorValues.forEach((color) => {
        if (typeof color === "string") {
          ...
          if (color.includes("sentience") || color.includes("awareness")) {
            console.warn(
              "Potential AI pattern detected. Resetting to default.",
            );
            applyPreset("default");
          }
  ...
  useEffect(() => {
    if (
      ...
      try {
        ...
        const newTheme = {
          ...theme,
          customColors: JSON.parse(JSON.stringify(theme.customColors || {})),
        };
        if (!newTheme.customColors[component])
          newTheme.customColors[component] = {};
  ...
  useEffect(() => {
    const initialCustomColors = JSON.parse(
  ...
  const handleColorUpdate = (component, property, color) => {
    setTheme((prevTheme) => {
      const newTheme = {
        ...prevTheme,
        customColors: { ...prevTheme.customColors },
      };
      if (!newTheme.customColors[component])
        newTheme.customColors[component] = {};
    ...
    setHistory((prevHistory) => {
      ...
      const newCustomColors = JSON.parse(
        JSON.stringify(
      ...
      if (typeof window !== "undefined")
        window.__LATEST_CUSTOM_COLORS__ = newCustomColors;
      return newHist;
    });
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {component.properties.map((prop) => (
                <EnhancedColorPicker
                  ...
                  onChange={(comp, prop, color) =>
                    handleColorUpdate(comp, prop, color)
                  }
