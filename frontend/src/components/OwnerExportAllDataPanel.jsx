...
const FILES = [
  { label: "Sample Vaults", file: "sample_vaults.json", url: "/data/sample_vaults.json" },
  { label: "Pricing Logs", file: "pricing_log.json", url: "/analytics/pricing_log.json" },
...
>>>>>>> omni_repair_backup_20250704_1335
  ...
  return (
    <section aria-labelledby="export-all-data-heading" style={{background:'#f9fafb',borderRadius:12,padding:32,marginBottom:32,boxShadow:'0 2px 8px #e0e7ef'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
        <h2 id="export-all-data-heading" style={{color:'#0ea5e9',fontWeight:800,fontSize:22,margin:0}}>Export All Data</h2>
        ...
        <span tabIndex={0} aria-label="Help: Export all data" title="Download all business data, logs, and policies as a ZIP. OWNER controls all exports." style={{marginLeft:6, color:'#64748b', cursor:'help', fontSize:18, fontWeight:800}}>?</span>
      </div>
      <ul style={{listStyle:'none',padding:0,fontSize:15}} aria-label="Export file selection">
        {FILES.map((f, i) => (
          <li key={i} style={{marginBottom:10,display:'flex',alignItems:'center',gap:8}}>
            ...
            />
            <span tabIndex={0} aria-label={`Export file: ${f.label}`}>{f.label}</span>
          </li>
        ))}
      </ul>
      <button
      ...
      <div style={{marginTop:18,color:'#64748b',fontSize:13}} aria-live="polite">{status}</div>
      <div style={{marginTop:10,color:'#64748b',fontSize:13}}>
        <b>OWNER CONTROLLED:</b> Only OWNER can export data. All actions are static and audit-logged.
      </div>
    </section>
  );
}
