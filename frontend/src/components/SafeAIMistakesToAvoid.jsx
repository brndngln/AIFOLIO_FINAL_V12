...
export default function SafeAIMistakesToAvoid() {
  return (
    <section
      aria-labelledby="safe-ai-mistakes-heading"
      style={{
        marginTop: 32,
        background: "#fff",
        borderRadius: 8,
    ...
    >
      ...
      >
        <h3
          id="safe-ai-mistakes-heading"
          style={{ color: "#ef4444", fontWeight: 700, margin: 0 }}
        >
        ...
        <span
          tabIndex={0}
          aria-label="Help: Why these mistakes matter"
          title="Each mistake can break SAFE AI compliance, audit, or trust. OWNER must avoid all for deterministic, compliant business mode."
          style={{
            marginLeft: 6,
            color: "#64748b",
            cursor: "help",
      ...
      <ul
        ...
        aria-label="SAFE AI mistakes list"
      >
        {MISTAKES.map((item, i) => (
          <li
            key={i}
          ...
          >
            ...
            <span
              tabIndex={0}
              aria-label={`Mistake: ${item}`}
              title={`Why avoid: ${item}`}
            >
              {item}
            </span>
            <span
