...
const policies = [
  {
    ...
    summary: "Static, business-focused terms for all users.",
      ...
      <ul style={{fontSize:15, margin:0, padding:0, listStyle:'none'}}>
        {policies.map((policy, i) => (
          <li key={policy.title} style={{marginBottom:16}}>
            <button
              style={{background:'none',border:'none',color:'#0ea5e9',fontWeight:600,fontSize:16,cursor:'pointer',padding:0,textAlign:'left'}}
              onClick={() => setOpen(open => open.map((o, idx) => idx === i ? !o : o))}
            ...
            <div style={{marginLeft:24,marginTop:4}}>
              {!open[i] && <span style={{color:'#64748b',fontSize:14}}>{policy.summary}</span>}
              {open[i] && (
                <pre id={`policy-content-${i}`} style={{whiteSpace:'pre-wrap',background:'#f3f4f6',padding:12,borderRadius:6,marginTop:8,fontSize:14}}>{policy.content}</pre>
              )}
            </div>
          </li>
        ))}
