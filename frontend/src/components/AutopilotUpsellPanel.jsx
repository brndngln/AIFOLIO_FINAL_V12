...
export default function AutopilotUpsellPanel({ onAction }) {
  ...
  if (loading) return <div className="theme-card">Loading upsell offers...</div>;
  if (error) return <div className="theme-card text-error">{error}</div>;

  return (
    <section aria-labelledby="autopilot-upsell-heading" style={{background:'#f0fdfa',borderRadius:14,padding:32,marginBottom:32,boxShadow:'0 2px 8px #bae6fd'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
        <h3 id="autopilot-upsell-heading" style={{color:'#0ea5e9',fontWeight:700,margin:0}}>Autopilot Upsell Configurator</h3>
        ...
        <span tabIndex={0} aria-label="Help: What is Autopilot Upsell?" title="Static, deterministic upsell configuration. OWNER controls all actions and exports." style={{marginLeft:6, color:'#64748b', cursor:'help', fontSize:18, fontWeight:800}}>?</span>
      </div>
      <table aria-label="Upsell offers list" style={{width:'100%',marginBottom:18,background:'#fff',borderRadius:8,boxShadow:'0 1px 2px #e5e7eb'}}>
        <thead>
            ...
            <tr key={i}>
              ...
              <td>
                <button
                  ...
                  onClick={() => onAction && onAction('auditUpsell', { offer: o })}
                >Audit</button>
                <button
