...
export default function ReferralManagementPanel({ onAction }) {
  ...
  function handleExportCSV() {
    ...
    const csv = [
      ...
      ...referrals.map(r => `${r.referrer},${r.referred},${r.status},${r.revenue}`)
    ].join('\n');
  ...
  return (
    <section aria-labelledby="referral-management-heading" style={{background:'#f3f4f6',borderRadius:14,padding:32,marginBottom:32,boxShadow:'0 2px 8px #cbd5e1'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
        <h3 id="referral-management-heading" style={{color:'#0ea5e9',fontWeight:700,margin:0}}>Referral Management</h3>
        <span style={{background:'#0ea5e9',color:'#fff',padding:'2px 10px',borderRadius:6,fontWeight:700,fontSize:13}} aria-label="SAFE AI badge">SAFE AI</span>
        <span tabIndex={0} aria-label="Help: What is Referral Management?" title="Static, deterministic referral management. OWNER controls all actions and exports." style={{marginLeft:6, color:'#64748b', cursor:'help', fontSize:18, fontWeight:800}}>?</span>
      </div>
      <table aria-label="Referral list" style={{width:'100%',marginBottom:18,background:'#fff',borderRadius:8,boxShadow:'0 1px 2px #e5e7eb'}}>
        <thead>
            ...
            <tr key={i}>
              ...
              <td>
                <button
                  ...
                  onClick={() => onAction && onAction('auditReferral', { referral: r })}
                >Audit</button>
                <button
      ...
      <div style={{marginTop:18,display:'flex',gap:16,alignItems:'center'}}>
        ...
        <button
          ...
          onClick={() => onAction && onAction('auditAllReferrals', { referrals })}
        >Audit All</button>
      </div>
