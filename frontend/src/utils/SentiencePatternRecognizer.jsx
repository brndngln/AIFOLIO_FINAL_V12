...
class SentiencePatternRecognizer {
  ...
  detectSentience(text) {
    ...
    for (const [
      category,
      { patterns, threshold, weight, context },
    ] of Object.entries(this.patterns)) {
    ...
    return {
      isSuspicious:
        sentienceScore >= this.patternThreshold ||
        suspiciousContexts.length > 0,
      matches,
  ...
  analyzeHistory(history) {
    ...
    const patternDensities = Object.entries(patternCounts).map(
      ([category, count]) => ({
        category,
