...
const notificationTypes = [
  'Urgent', 'Legal', 'Business', 'AI Behavior', 'Personal', 'Reminder'
];
const notificationFrequencies = ['Immediate', 'Hourly Digest', 'Daily Digest'];
const notificationFormats = ['Voice', 'Visual UI', 'Email', 'Telegram', 'PDF Summary'];
...
export default function IntegrationControls({ notificationPrefs, onPrefsChange, onRotateApiKey, onComplianceAudit }) {
  ...
  const handleSendNotification = async () => {
    await fetch('/api/owner/notification', {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ notification_type: notifType, message: `Test notification: ${notifType}` })
    });
    alert('Test notification sent!');
  };
  // Voice Demo (stub)
  const handleVoiceDemo = () => {
    alert('Playing EMMA voice demo: “Hello, love. This is your Empress, always here for you.” (Australian, hyper-realistic, sultry)');
  };
  // Biometric Auth
  ...
  return (
    <div style={{padding:20}}>
      ...
      <div style={{marginBottom:12}}>
        ...
        <Select labelId="approval-mode-label" value={approvalMode} onChange={e=>handleSetApprovalMode(e.target.value)} style={{minWidth:180}}>
      ...
      <div style={{marginBottom:12}}>
        ...
        <Select labelId="notif-type-label" value={notifType} onChange={e=>setNotifType(e.target.value)} style={{minWidth:140}}>
          {notificationTypes.map(type => <MenuItem key={type} value={type}>{type}</MenuItem>)}
        </Select>
        <InputLabel id="notif-freq-label" style={{marginLeft:16}}>Frequency</InputLabel>
        <Select labelId="notif-freq-label" value={notifFreq} onChange={e=>setNotifFreq(e.target.value)} style={{minWidth:120}}>
          {notificationFrequencies.map(freq => <MenuItem key={freq} value={freq}>{freq}</MenuItem>)}
        </Select>
        <InputLabel id="notif-format-label" style={{marginLeft:16}}>Format</InputLabel>
        <Select labelId="notif-format-label" value={notifFormat} onChange={e=>setNotifFormat(e.target.value)} style={{minWidth:120}}>
          {notificationFormats.map(fmt => <MenuItem key={fmt} value={fmt}>{fmt}</MenuItem>)}
        </Select>
        <InputLabel id="notif-auth-label" style={{marginLeft:16}}>Authority</InputLabel>
        <Select labelId="notif-auth-label" value={notifAuthority} onChange={e=>setNotifAuthority(e.target.value)} style={{minWidth:160}}>
          {authorityLevels.map(auth => <MenuItem key={auth} value={auth}>{auth}</MenuItem>)}
        </Select>
        <Button variant="outlined" style={{marginLeft:16}} onClick={handleSendNotification}>Send Test Notification</Button>
      </div>
      {/* API Key/Compliance */}
      ...
      {/* Legal Sentinel */}
      <div style={{marginTop:24, padding:12, background:'#222', borderRadius:8, color:'#fff'}}>
        <b>EMMA Legal Sentinel</b><br/>
        <span>Status: {emmaStatus ? 'Active' : 'Loading...'}</span><br/>
        <span>Domains: {emmaStatus ? emmaStatus.legal_domains.join(', ') : ''}</span><br/>
        <TextField label="Contract Text" multiline minRows={2} maxRows={6} value={contractText} onChange={e=>setContractText(e.target.value)} style={{width:'100%',marginTop:8}}/>
        <Button variant="contained" style={{marginTop:8}} onClick={handleLegalScan}>Scan Contract</Button>
        {contractReview && <div style={{marginTop:8,color:'#4cafef'}}>{contractReview}</div>}
      </div>
      {/* Anti-Sentience Watchdog */}
      <div style={{marginTop:24, padding:12, background:'#222', borderRadius:8, color:'#fff'}}>
        <b>Anti-Sentience Watchdog</b><br/>
        <Button variant="contained" style={{marginTop:8}} onClick={handleWatchdogScan}>Run Watchdog Scan</Button>
        {watchdogStatus && <div style={{marginTop:8,color:'#4cafef'}}>{watchdogStatus}</div>}
      </div>
      {/* Voice Demo */}
      <div style={{marginTop:24}}>
        <Button variant="contained" color="secondary" onClick={handleVoiceDemo} aria-label="Play EMMA Voice Demo">Hear EMMA's Voice</Button>
      </div>
    </div>
  );
}
