...
def test_payments_routes(client):
    rv = client.post('/pay/stripe', data={'csrf_token': get_csrf(client) or 'test', 'email': 'test@example.com'})
    assert rv.status_code in (200, 400)
    rv = client.post('/pay/gumroad', data={'csrf_token': get_csrf(client) or 'test', 'email': 'test@example.com'})
    assert rv.status_code in (200, 400)
