...
IP_REGISTRY_PATH = os.path.join(os.path.dirname(__file__), '../audit/exports/ip_registry.json')
...
class IPGuardian:
    ...
    def __init__(self):
        self.registry_path = IP_REGISTRY_PATH
        ...
        with IP_LOCK:
            ...
            for asset in reg:
                if asset.get('id') == asset_id:
                    ...
                    self._write_registry(reg)
                    emma.log_event('ip_transfer', {'asset_id': asset_id, 'new_owner': new_owner, 'reason': reason}, critical=True)
                    return True
            return False
    ...
    def enforce_protection(self, asset_id: str, action: str, actor: str) -> bool:
        asset = self.get_ip(asset_id)
        if not asset:
            return False
        # Example: Block non-owner destructive actions
        if action in ['delete', 'export', 'transfer'] and actor != asset.get('owner'):
            emma.log_event('ip_protection_blocked', {'asset_id': asset_id, 'action': action, 'actor': actor}, critical=True)
            return False
        emma.log_event('ip_action', {'asset_id': asset_id, 'action': action, 'actor': actor}, critical=False)
        return True
...
ip_guardian = IPGuardian.instance()
